...
- 只有一个可靠 canonical 目标、且没有其他正式专业名与该说法竞争的简称，例如 `计科`、`电力`：允许进入唯一候选，但结果页必须保留“搜索识别说明”；
- 专业类简称，例如 `机械`、`电气`、`材料`、`计算机`：必须进入候选消歧，不能因为别名库里存在一个默认 target 就静默选择；
- 宽泛家长词，即使旧别名库有默认 target，只要现行本科目录还有其他正式专业名称竞争，也必须消歧。例如 `测控` 同时涉及 `测控技术与仪器 080301` 与 `智能测控工程 080720T`；`软件`、`食品`、`汽车`、`临床`、`口腔` 也按同一规则处理；
...
- `tools/verify-major-path-v001.mjs`：883 本科专业全量状态、184 个研究生国家目录实体、代码唯一性、所有关系必须指向 canonical 研究生目录，以及 `机械/电气/材料/计算机/计科/测控/食品/汽车/临床/软件/口腔` 等语义搜索合同。
