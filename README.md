# ln-rank-span-tool V3.942

本包为完整可部署包，基于 v3.9.36/v3.9.41 主干继续开发。

V3.942 重点：

- 自选池工作台左侧大卡片改为紧凑排序行，一屏可整理更多志愿。
- “发送当前排序到飞书”归位到左侧排序清单区。
- 右侧只保留“检查当前排序”和“发送诊断报告到飞书”。
- 机器化文案改成人话：不再用“生成路径分析 / 分析后发送飞书”作为主按钮。
- PC 双栏、小屏上下、手机单列，兼顾多终端。
- 首页右侧自选池仍默认不展开，加入专业只轻提示，不打断查询。

数据口径：`/ln-rank` 使用 `/fenxi` 已接入的辽宁 2025 物理类专业数据。
...
本版本基于 `v3.9.36 major-bands-stability` 完整打包，不是增量包。部署路径仍为 `/ln-rank/`，专业数据仍从 `/fenxi` 已接入的辽宁 2025 物理类专业数据读取。
...
- PC 左侧悬浮自选池抽屉；
- 手机右下角按钮 + 底部抽屉；
- `/ln-rank/selection-pool.html` 完整排序工作台；
- `/api/path-analysis` 路径分析接口；
- 冲稳保统计、风险提示、调整建议和可复制到飞书的报告文本。

注意：本工具用于形成可讨论专业池和路径分析，不等同于录取预测。正式填报仍需结合 2026 年一分一段、招生计划、选科、体检、学费、校区等信息人工复核。

---

# v3.9.42 紧凑排序工作台与飞书归位

本版本基于 v3.9.41 完整包生成，仍是完整部署包，不是增量包。

核心修正：

- 自选池工作台左侧卡片改为紧凑排序行，适合整理 30-60 个专业志愿；
- “发送当前排序到飞书”移动到左侧排序清单区；
- 右侧只负责“志愿排序诊断”和“发送诊断报告到飞书”；
- “生成路径分析”改为“检查当前排序”；
- “分析后发送飞书”改为“发送诊断报告到飞书”；
- PC 双栏、小屏上下、手机单列，兼顾多终端；
- 首页右侧自选池仍默认不展开，加入专业只轻提示，不打断查询。

数据仍从 `/fenxi` 已接入的辽宁 2025 物理类专业池读取。

## V3.944 direct-pool-workspace

本版取消首页右侧重抽屉。右侧“自选池 N”小胶囊只作为轻入口，点击后直接进入 `/ln-rank/selection-pool.html` 完整整理页。首页继续负责查询与加入，完整页负责排序、诊断、清空和飞书报告。



## V3.9.45 自选池拖拽排序工作台

- 基于 V3.9.44 全量包继续输出完整部署目录。
- 首页仍保持右侧小胶囊入口，点击直接进入 `/ln-rank/selection-pool.html`，不恢复半屏抽屉。
- 完整整理页新增拖拽手柄：PC 按住即可拖拽排序；触屏端长按手柄进入拖拽。
- 新增“移动”菜单：置顶、上移 5 位、移到第 N 位、下移 5 位、置底。
- 新增“按冲稳保整理”：按 高冲→小冲→边稳→稳妥→小保→强保→兜底 形成初始顺序，再人工微调。
- 上移、下移、删除、清空、检查当前排序、发送飞书报告保留。
- 所有排序动作统一重新写入 `userOrder`，刷新后保持最终顺序。
