# ln-rank-span-tool V3.941

本包为完整可部署包，基于 v3.9.36/v3.9.40 主干继续开发。

V3.941 重点：

- 自选池入口改为右侧小胶囊，默认不展开。
- 加入专业后只更新数量与轻提示，不自动弹出抽屉。
- 删除首页顶部自选池排序链接，完整排序页只从自选池入口进入。
- 新增自选池专用飞书报告接口：`/api/feishu-create-selection-pool-report`。
- 支持“发送自选池到飞书”和“分析后发送飞书”，默认尝试共享并返回地址。
