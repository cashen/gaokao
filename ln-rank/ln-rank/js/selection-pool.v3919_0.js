...
import { normalizeSelectedMajors } from './domain/selection-contract.js?v=3919_0';
import { buildReportPayload } from './domain/report-payload-contract.js?v=3919_0';
import { toHumanCopy } from './domain/human-copy-dictionary.js?v=3919_0';
...
function createReportContext(state = getState()) {
  return { candidateScore: state.score || null, selectedCount: state.items?.length || 0, bottomLineMode: state.bottomLineMode || 'all', rangePreset: 'standard', activeBand: null, generatedAt: new Date().toISOString(), version: 'v3.9.19.0' };
}
