...
async function runResultUiChecks(){
  ...
  try{
    ...
    const cssResult=await text('./css/components/result-section-contract.css?v=3927');
    const cssCard=await text('./css/components/major-card-contract.css?v=3927');
    const cssBand=await text('./css/components/band-selector-contract.css?v=3927');
    const cssColor=await text('./css/core/color-system.css?v=3927');
    const cssTheme=await text('./css/components/post-exam-calm-theme.css?v=3927');
    const cssBreath=await text('./css/components/band-card-breathing-link.css?v=3927');
    const renderMajor=await text('./js/feature/major-pool/render.js?v=3927');
    const bandRender=await text('./js/feature/score-bands/render.js?v=3927');
    ...
    checks.push({name:'RESULT-UI-003：结果说明区承载当前查看',ok:/result-context-bar/.test(await text('./css/components/result-context-bar.css?v=3927').catch(e=>''))&&/renderResultContextBar/.test(renderMajor),detail:'当前分段范围/数量进入紧凑结果说明区，不再重复占位'});
    checks.push({name:'RESULT-UI-004：特殊项目提示进入结果说明区',ok:/result-context-special/.test(await text('./css/components/result-context-bar.css?v=3927').catch(e=>''))&&/data-context-special-toggle/.test(renderMajor),detail:'特殊项目数量与显示入口收纳进紧凑说明条'});
...
const mainDistCss=await text('./css/dist/ln-rank-main.v3927.css').catch(e=>'');
checks.push({name:'SPECIAL-004：active dist CSS 包含特殊项目样式',ok:(manifest.cssEntry||[]).includes('css/dist/ln-rank-main.v3927.css')&&mainDistCss.includes('SOURCE: css/components/special-project-filter.css'),detail:mainDistCss.includes('SOURCE: css/components/special-project-filter.css')?'特殊项目样式已纳入 main dist CSS':'main dist CSS 缺少 special-project-filter.css source marker'});
...
document.getElementById('runSelfCheck')?.addEventListener('click',run);run();console.info('v3.9.27 Android 条件区收口自测已启用。');
