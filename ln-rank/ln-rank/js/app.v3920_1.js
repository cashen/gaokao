import { state } from './state/app-state.js?v=3920_1';
import { toInt } from './core/number-utils.js?v=3920_1';
import { getScoreGuard } from './core/score-guard.js?v=3920_1';
import { REGION_OPTIONS } from './config/region-options.js?v=3920_1';
import { fetchMajorBands } from './feature/major-pool/index.js?v=3920_1';
import { renderBandLegend, renderResultBandSwitcher } from './feature/score-bands/index.js?v=3920_1';
import { renderMajorResults } from './feature/major-pool/index.js?v=3920_1';
import { buildKeywordQuery } from './feature/major-pool/index.js?v=3920_1';
import { mountKeywordPresetPanel } from './feature/major-pool/index.js?v=3920_1';
import { initFeishuReport, renderFeishuReport, clearFeishuReport } from './feature/feishu/index.js?v=3920_1';
import { initSelectionPool, refreshSelectionPool, createSelectionPoolAdapter } from './feature/selection-pool/index.js?v=3920_1';
import { renderSearchTrendHint } from './feature/trend/index.js?v=3920_1';
import { getQueryButtonLabel, getQueryButtonClass, initBottomLineSheet, bottomLineLabel as uiBottomLineLabel, initRankBandLegend } from './feature/ui/index.js?v=3920_1';
...
import { setRangePreset, setBandFocus, syncRangeState } from './state/range-state.js?v=3920_1';
import { syncControlConsoleState } from './ui/control-console.js?v=3920_1';
import { normalizeMajorBandsResponse } from './domain/score-band-contract.js?v=3920_1';
...
function syncMobileDirtyBar(g) {
  ...
  if (text) text.textContent = `条件已变化${keyword}`;
}
...
function setActionButton() {
  ...
  if (state.bands.loading) {
    ...
    if (guide) guide.textContent = '正在读取专业数据，请稍候。';
    return;
  }
  ...
  if (dirty) {
    ...
    button.className = getQueryButtonClass({ ready: true, level: g.level, primary: true });
    if (guide) guide.textContent = '筛选条件已变化，点击更新下方结果。';
    return;
  }

  button.textContent = getQueryButtonLabel({ hasQueried, dirty: false, topRange: g.key === 'topRange' });
  button.className = getQueryButtonClass({ ready: true, level: g.level, primary: true });
  if (guide) guide.textContent = '可继续调整条件后更新结果。';
}
...
function renderBottomLinePanel() {
  ...
  if (summary) {
    summary.textContent = visible
      ? `当前分数处于本科线—特控线之间，建议先看清办学性质和费用；已选“${bottomLineLabel(state.filters.bottomLineMode)}”。`
      : score ? '当前分数不在本科线—特控线区间，前端不启用公办底线。' : '输入分数后，本科线—特控线区间会显示办学性质提醒。';
  }
}
