...
function buildInterfaceError({ path, status, raw }) {
  ...
  if (looksLikeHtml(raw)) {
    return new Error(`报告暂时生成失败。可以先复制文字版报告，稍后再试。技术详情：请求路径：${path}；HTTP状态：${status}；返回内容像 HTML。通常表示该 Functions 路由没有部署到当前生产环境，或请求路径被 Pages 回退到了 HTML 页面。返回片段：${preview}`);
  }
  return new Error(`报告暂时生成失败。可以先复制文字版报告，稍后再试。技术详情：请求路径：${path}；HTTP状态：${status}；返回片段：${preview}`);
}
