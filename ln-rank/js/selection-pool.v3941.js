import {
  ...
  removePoolItem,
  classifyPoolItem
} from './feature/selection-pool/selection-pool-store.v3941.js';
import { requestPathAnalysis } from './feature/selection-pool/path-analysis-api.v3941.js';
import { createSelectionPoolFeishuReport } from './feature/selection-pool/feishu-selection-report-api.v3941.js';

const SCORE_KEY = 'lnRank.selectionPool.candidateScore.v3941';
const LEGACY_SCORE_KEY = 'lnRank.selectionPool.candidateScore.v3940';
...
function loadScore() {
  const raw = localStorage.getItem(SCORE_KEY) || localStorage.getItem(LEGACY_SCORE_KEY) || '';
...
function renderAnalysis() {
  ...
  if (!root) return;
  if (!currentAnalysis) {
    root.innerHTML = '<div class="pool-empty">点击“生成路径分析”后，这里会输出整体判断、冲稳保风险和调整建议。需要报告时，直接点击“分析后发送飞书”。</div>';
    return;
  }
...
async function runAnalysis() {
  ...
  try {
    ...
    if (button) { button.disabled = !getPoolItems().length; button.textContent = '生成路径分析'; }
    renderAll();
  }
}
...
async function sendFeishu({ withAnalysis = false } = {}) {
  ...
  if (btn) { btn.disabled = true; btn.textContent = withAnalysis ? '分析并发送中…' : '发送中…'; }
  ...
  try {
    ...
    if (btn) { btn.disabled = !getPoolItems().length; btn.textContent = withAnalysis ? '分析后发送飞书' : '发送自选池到飞书'; }
    renderAll();
  }
}
