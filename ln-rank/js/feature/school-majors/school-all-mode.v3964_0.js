...
async function fetchSchoolData({ append = false } = {}) {
  ...
  try {
    ...
    state.schoolSelection.input = state.schoolSelection.displayName || school;
    state.schoolSelection.entityId = payload.meta?.schoolEntity?.entityId || state.schoolSelection.entityId;
    const schoolInput = byId('schoolKeyword');
    if (schoolInput && state.schoolSelection.displayName) schoolInput.value = state.schoolSelection.displayName;
    state.filters.schoolKeyword = state.schoolSelection.displayName || school;
...
function bindEvents() {
  ...
  document.addEventListener('gaokao:school-search-submit', () => fetchSchoolData());
...
export function mountSchoolAllMode() {
  ...
  globalThis.__GAOKAO_SCHOOL_ALL_MODE__ = Object.freeze({
    version: 'school-all-mode-v3964_0',
    mountPolicy: 'static-result-owner',
    sharedControlOwner: 'selection-workspace-orchestration-v3964_0',
    getState: () => ({
