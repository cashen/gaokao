...
} from '../../../../shared/resources/schools/school-resource-center.js?v=3962_2';
import { UI_ACTION_COPY } from '../../../../shared/ui/contracts/action-contract.v3959_0.js?v=3962_2';
import {
...
const ACTION_COPY = Object.freeze({
  scoreView: UI_ACTION_COPY.viewScoreNearby?.label || '按我的分数附近看',
  schoolView: UI_ACTION_COPY.viewSchoolAllMajors?.label || '看该校全部招生专业',
...
function assertStaticStructure() {
  ...
  const mount = byId('schoolViewModeMount');
  const filterGrid = document.querySelector('.search-grid-top');
  if (!filterGrid?.contains(mount)) throw new Error('school view mode must be mounted inside the shared filter grid');
}
...
function currentSchoolInput() {
  return String(byId('schoolKeyword')?.value || '').trim();
}

function currentMajorKeyword() {
  return String(byId('majorKeyword')?.value || '').trim();
}

function currentScore() {
  const raw = String(byId('candidateScore')?.value || '').replace(/[^0-9.]/g, '');
  if (!raw) return null;
...
function renderRecord(record) {
  ...
  return `<article class="ui-card school-major-row ${positionTone(record)} ${expanded ? 'is-expanded' : ''}" data-school-record="${escapeHtml(key)}">
    ...
    <div class="school-major-score"><b>${fmt(record.score2026 ?? record.score)}分</b><span>约第 ${fmt(record.rank2026 ?? record.rank)} 位</span></div>
    <div class="school-major-position"><b>${escapeHtml(positionText(record))}</b><span>${escapeHtml(record.statusLabel || record.position || '历史位置参考')}</span></div>
...
function renderCandidates(payload) {
  ...
  content.innerHTML = `<section class="ui-state ui-state--pending school-all-message"><b>${escapeHtml(payload?.message || '没有精确找到这所学校')}</b><p>学校本部、分校和招生校区不能混在一起，请选择准确名称。</p>${candidates.length ? `<div class="school-candidate-list">${candidates.map(item => `<button class="ui-button ui-button--compact ui-button--secondary" type="button" data-school-candidate="${escapeHtml(item.school)}"><span>${escapeHtml(item.school)}</span><em>${fmt(item.count)}条记录</em></button>`).join('')}</div>` : '<p>请检查学校名称，或先切回分数附近模式。</p>'}</section>`;
}
...
function renderSchoolData() {
  ...
  content.innerHTML = `<section class="ui-state school-all-boundary">${escapeHtml(data.meta?.dataBoundary || '')}</section>
    <section class="ui-card school-all-summary" aria-label="学校全部专业概览">
      <div><span>全部记录</span><b>${fmt(total)}</b></div><div><span>普通记录</span><b>${fmt(summary.regularCount)}</b></div><div><span>特殊项目</span><b>${fmt(summary.specialCount)}</b></div><div><span>${data.meta?.candidateScore ? `参考分数 ${fmt(data.meta.candidateScore)}` : '参考分数'}</span><b>${data.meta?.candidateScore ? `${fmt(summary.nearCount)} 条接近` : '未填写'}</b></div>
    </section>
...
function captureScoreModeUi() {
  if (scoreModeUiSnapshot) return;
  const button = byId('queryButton');
  const guide = byId('queryGuide');
  scoreModeUiSnapshot = {
...
function restoreScoreModeUi() {
  const button = byId('queryButton');
  const guide = byId('queryGuide');
  if (scoreModeUiSnapshot && button) {
    button.textContent = scoreModeUiSnapshot.buttonText;
    button.className = scoreModeUiSnapshot.buttonClass;
    button.disabled = scoreModeUiSnapshot.buttonDisabled;
  }
  if (scoreModeUiSnapshot && guide) guide.textContent = scoreModeUiSnapshot.guideText;
  scoreModeUiSnapshot = null;
  const dirtyBar = byId('mobileDirtyBar');
  if (dirtyBar) {
...
function syncModeUi() {
  ...
  byId('schoolViewModeMount').hidden = !selection.input;
  byId('schoolAllResultsPanel').hidden = !schoolMode;
  ...
  const modeButtons = document.querySelectorAll('[data-school-view-mode]');
  if (modeButtons[0]) modeButtons[0].textContent = ACTION_COPY.scoreView;
  if (modeButtons[1]) modeButtons[1].textContent = ACTION_COPY.schoolView;
  byId('schoolResolveStatus').textContent = selection.input
  ...
  if (button) {
    ...
    button.textContent = state.schoolAll.loading ? '正在读取全部招生专业…' : state.schoolAll.data && state.schoolAll.dirty ? '按新条件更新学校专业' : `查看${label}全部招生专业`;
    button.className = `query-button ${button.disabled ? 'is-waiting' : 'is-ready is-primary'}`;
  }
  if (guide) guide.textContent = selection.input ? '参考分数可以不填；填写后只增加历史位置关系，不会减少该校专业数量。' : '请先输入学校，例如东北大学。';
  const dirtyBar = byId('mobileDirtyBar');
  if (dirtyBar) {
...
function setMode(mode, { updateHistory = true } = {}) {
  ...
  document.dispatchEvent(new CustomEvent('gaokao:result-mode-change', { detail: { mode: state.resultMode } }));
}

function markSchoolDirty() {
  resolveSchoolSelection();
  if (state.resultMode === MODE_SCHOOL) state.schoolAll.dirty = Boolean(state.schoolAll.data);
  syncModeUi();
  updateUrl();
}
...
async function fetchSchoolData({ append = false } = {}) {
  ...
  syncModeUi();
  try {
    const response = await fetch(`${API_PATH}?${buildParams(offset).toString()}`, { signal: activeController.signal, headers: { accept: 'application/json' } });
    ...
    state.schoolAll.data = append ? { ...payload, records: mergeRecords(previous?.records, payload.records), meta: { ...payload.meta, filteredTotal: payload.meta?.filteredTotal ?? previous?.meta?.filteredTotal } } : payload;
...
function bindEvents() {
  document.addEventListener('click', event => {
    ...
    if (event.target.closest('#schoolAllBack')) {
      event.preventDefault();
      setMode(MODE_SCORE);
      return;
    }
    ...
    if (candidate) {
      ...
      input.value = candidate.dataset.schoolCandidate || '';
    ...
    if (action) {
      ...
      if (!record) return;
      const result = action.dataset.schoolSelectionAction === 'remove' ? selectionPool.remove(recordKey(record)) : selectionPool.add(record);
      if (result?.ok === false) {
  ...
  byId('schoolKeyword')?.addEventListener('input', markSchoolDirty);
  byId('majorKeyword')?.addEventListener('input', markSchoolDirty);
  byId('candidateScore')?.addEventListener('input', markSchoolDirty);
...
function restoreFromUrl() {
  ...
  if (school) {
    byId('schoolKeyword').value = school;
    state.filters.schoolKeyword = school;
  }
  if (score && !byId('candidateScore')?.value) byId('candidateScore').value = score;
  resolveSchoolSelection();
...
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', mountSchoolAllMode, { once: true });
else mountSchoolAllMode();
