...
function renderSchoolResolveLoading() {
  byId('schoolAllContent').innerHTML = '<div class="ui-state ui-state--loading school-all-message"><b>正在确认学校名称…</b><p>确认本部、分校或校区后，才会读取这所学校的历史专业记录。</p></div>';
}
...
async function preflightSchoolSelection({ submit = true } = {}) {
  ...
  try {
    ...
    byId('schoolAllContent').innerHTML = `<div class="ui-state ui-state--error school-all-message"><b>这次没有确认成功</b><p>${escapeHtml(state.schoolAll.error)}</p><div class="ui-state-actions"><button class="ui-button ui-button--compact ui-button--secondary" type="button" data-school-retry>${ACTION_COPY.retry}</button></div></div>`;
    return false;
  } finally {
...
export function mountSchoolAllMode() {
  ...
  globalThis.__GAOKAO_SCHOOL_ALL_MODE__ = Object.freeze({
    version: 'school-all-mode-v3969_1',
