import { classifyPoolItem, getPoolStats, majorFamily } from './selection-pool-store.v3940.js';
...
export function buildPathAnalysis({ items = [], candidateScore = null } = {}) {
  ...
  if (!total) {
    return {
      ok: true,
      level: 'empty',
      summary: '自选池暂无专业志愿。请先从查询结果中加入若干“专业+学校”卡片，再进行路径分析。',
      stats,
      risks: ['自选池为空，无法判断冲稳保路径。'],
  ...
  if (stats.rushCount > Math.ceil(total * 0.38)) {
    risks.push('冲刺区占比偏高，容易形成“前段好看、后段发虚”的路径。');
