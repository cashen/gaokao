...
export function buildClientHealthLights(state = {}) {
  ...
  return {
    ...
    structure: light(total && stats.stableCount >= Math.ceil(total * 0.25) && stats.safeCount >= Math.max(3, Math.ceil(total * 0.18)) ? 'ok' : 'warn', total ? '冲稳保结构' : '自选池为空', total ? `冲刺 ${stats.rushCount || 0}｜稳妥 ${stats.stableCount || 0}｜保底 ${stats.safeCount || 0}` : '先加入候选专业。'),
    bottomDepth: light((stats.safeCount || 0) >= Math.max(3, Math.ceil(total * 0.18)) ? 'info' : 'warn', '保底深度', (stats.safeCount || 0) >= Math.max(3, Math.ceil(total * 0.18)) ? '保底数量不算少，仍需核验接受度。' : '保底数量或深度需复核。'),
    majorDiversity: light(total >= 8 && familyCount / total >= 0.55 ? 'warn' : 'ok', '专业集中度', family ? `${family} 占比较高：${Math.round(familyCount / Math.max(1,total)*100)}%` : '专业方向待核验。'),
    locationDiversity: light(total >= 8 && cityCount / total >= 0.45 ? 'warn' : 'ok', '地域集中度', city ? `${city} 占比约 ${Math.round(cityCount / Math.max(1,total)*100)}%` : '地域待核验。'),
