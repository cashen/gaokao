...
export function buildClientHealthLights(state = {}) {
  ...
  return {
    ...
    structure: light(total && stats.stableCount >= Math.ceil(total * 0.25) && stats.safeCount >= Math.max(3, Math.ceil(total * 0.18)) ? 'ok' : 'warn', total ? '前中后段结构' : '还没有选择专业', total ? `稍高目标 ${stats.rushCount || 0}｜稳妥 ${stats.stableCount || 0}｜稳妥补充 ${stats.safeCount || 0}` : '先加入候选专业。'),
    bottomDepth: light((stats.safeCount || 0) >= Math.max(3, Math.ceil(total * 0.18)) ? 'info' : 'warn', '后段是否够稳', (stats.safeCount || 0) >= Math.max(3, Math.ceil(total * 0.18)) ? '稳妥补充数量不算少，仍需核验接受度。' : '稳妥补充数量或深度需复核。'),
