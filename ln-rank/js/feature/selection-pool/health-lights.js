...
export function buildClientHealthLights(state = {}) {
  ...
  return {
    ...
    structure: light(total && stats.stableCount >= Math.ceil(total * 0.25) && stats.safeCount >= Math.max(3, Math.ceil(total * 0.18)) ? 'ok' : 'warn', total ? '前中后段结构' : '自选专业为空', total ? `稍高目标 ${stats.rushCount || 0}｜稳妥 ${stats.stableCount || 0}｜稳妥补充 ${stats.safeCount || 0}` : '先加入候选专业。'),
