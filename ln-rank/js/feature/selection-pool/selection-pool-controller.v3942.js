import {
  ...
  movePoolItem,
  classifyPoolItem
} from './selection-pool-store.v3942.js';
import { requestPathAnalysis } from './path-analysis-api.v3942.js';
import { createSelectionPoolFeishuReport } from './feishu-selection-report-api.v3942.js';
...
function analysisHtml(result) {
  if (!result) return '<div class="pool-analysis-empty">需要时点击“检查当前排序”，系统会帮你看冲稳保是否接得住，完整报告可一键发送飞书。</div>';
  ...
  root.innerHTML = `<button id="selectionPoolFab" class="selection-pool-fab ${isBumped ? 'is-bumped' : ''}" type="button" aria-label="打开自选池">
    ...
    <aside class="selection-pool-drawer" aria-label="自选池抽屉">
      ...
      <div class="pool-drawer-actions">
        ...
        <button id="poolQuickAnalysis" class="pool-primary" type="button" ${items.length ? '' : 'disabled'}>检查当前排序</button>
        <button id="poolSendOnly" class="pool-secondary" type="button" ${items.length ? '' : 'disabled'}>发送当前排序到飞书</button>
        <button id="poolSendAnalyzed" class="pool-primary" type="button" ${items.length ? '' : 'disabled'}>发送诊断报告到飞书</button>
      </div>
      ${feishuStatusHtml()}
...
function getCandidateScore() {
  return latestState?.candidateScore || Number(localStorage.getItem('lnRank.selectionPool.candidateScore.v3942')) || Number(localStorage.getItem('lnRank.selectionPool.candidateScore.v3941')) || Number(localStorage.getItem('lnRank.selectionPool.candidateScore.v3940')) || null;
}
