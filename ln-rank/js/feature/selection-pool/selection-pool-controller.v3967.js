...
function renderPoolEntry({ items, isBumped }) {
  return `<a id="selectionPoolFab" class="pool-entry-direct ${isBumped ? 'is-bumped' : ''}" href="${getPoolHref()}" aria-label="进入自选池完整整理页">
  ...
  return {
    ...
    add(record) {
      ...
      if (result.ok) {
        ...
        window.dispatchEvent(new CustomEvent('lnrank:pool-updated'));
        window.dispatchEvent(new CustomEvent('lnrank-selection-pool-updated'));
      }
      onChanged();
      render();
      return result;
