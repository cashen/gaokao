...
function renderPoolEntry({ items, isBumped, variant = 'desktop' }) {
  ...
  return `<a${idAttr} class="${cls} ${isBumped ? 'is-bumped' : ''}" href="${getPoolHref()}" aria-label="进入自选池完整整理页">
...
function renderStickyBar(items) {
  if (!items.length) return '';
  return `<div class="pool-result-sticky-bar" role="status">
...
function render() {
  ...
  if (stickyMount) {
    stickyMount.innerHTML = renderStickyBar(items);
  }
}

function showToast(message) {
  toastText = message || `已加入自选池 · 共 ${getPoolItems().length} 个`;
  ...
  return {
    ...
    add(record) {
      ...
      if (result.ok) {
        ...
        showToast(`已加入自选池 · 共 ${countLabel(count)} 个`);
