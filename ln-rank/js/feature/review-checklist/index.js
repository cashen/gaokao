import { getCampusForRecord } from '../campus/index.js?v=3933_13';
...
function outProvince(item = {}) {
  const t = `${item.displayLocation || ''} ${(item.schoolTags || []).join(' ')} ${item.school || ''}`;
  if (!t) return false;
...
function rankVolatility(item = {}) {
  const r25 = Number(item.rank2025 ?? item.rank ?? item.minRank ?? item.lowestRank);
  const r24 = Number(item.rank2024);
  if (!Number.isFinite(r25) || !Number.isFinite(r24)) return null;
  return Math.abs(Math.round(r25 - r24));
}
function push(cat, item, reason, action) {
  if (!item) return;
  cat.items.push({
    school: clean(item.school || '学校待核验', 80),
    major: clean(item.major || '专业待核验', 120),
...
export function buildReviewChecklist(items = []) {
  const cats = [
  ...
  (Array.isArray(items) ? items : []).forEach(item => {
    ...
    if (campus?.reviewSummary || /校区待核验|校区需核验/.test(t)) {
      push(by.campus, item, campus?.reviewSummary || '该专业办学地点/校区需要以招生计划备注核验。', '核验招生计划中的办学地点、校区和住宿通勤。');
    }
    if (/中外|合作办学|高收费|较高收费|收费较高|国际|联合培养/.test(t)) {
      push(by.fee, item, '该条目涉及中外合作、高收费或项目属性，费用和培养模式需要确认。', '核验学费、培养模式、是否必须出国、毕业证/学位证、校区和转专业政策。');
    }
    const vol = rankVolatility(item);
    if (vol == null) {
      push(by.rank, item, '历史位次证据不完整，不能只按单一年份最低位次判断。', '结合 2026 招生计划、当年位次和同类专业分布人工复核。');
    } else if (vol >= 12000) {
      push(by.rank, item, `2024/2025 最低位次波动约 ${vol.toLocaleString('zh-CN')} 位，波动较大。`, '降低位次余量的稳定判断权重，结合 2026 招生计划复核。');
    } else if (vol >= 6000) {
      push(by.rank, item, `2024/2025 最低位次波动约 ${vol.toLocaleString('zh-CN')} 位，需结合当年计划复核。`, '不要只按 2025 最低位次判断。');
    }
    if (outProvince(item) || /计划人数待核验|省外/.test(t)) {
      push(by.plan, item, '省外高校在辽宁招生计划人数需要重点确认。', '核验 2026 年辽宁物理类招生计划人数，避免裸读位次余量。');
    }
    if (/医学|药学|生物|食品|农学|园艺|动物医学|兽医|交通运输|油气储运|化工|材料|护理|检验|影像|康复/.test(t)) {
      push(by.exam, item, '该方向可能涉及体检限制或培养要求。', '如孩子存在色弱、色盲、视力等情况，结合招生章程和体检指导意见人工核验。');
    }
    if (/英语|日语|俄语|翻译|商务英语|外语|中外|国际|双语|口试|语种/.test(t)) {
      push(by.language, item, '该方向可能涉及外语语种、口试、单科成绩或授课语言要求。', '核验招生章程中的外语语种、口试、单科成绩和授课语言。');
    }
    if (/定向|公费师范|优师|订单|免费医学|培养协议|委托培养/.test(t)) {
      push(by.service, item, '该条目可能涉及定向、公费师范、优师或履约项目。', '核验培养协议、服务地区、履约年限、违约责任和招生条件。');
    }
  });
  ...
  return {
    summary: {
      ...
      totalItems,
      headline: categories.length ? `本方案有 ${categories.length} 类事项建议人工复核` : '暂未汇总出明显复核事项，但仍需以 2026 招生计划和章程为准。'
    },
    categories
  };
}
