...
import { buildSchoolAllHref, buildTongxueSchoolHref } from '../../../../shared/resources/schools/school-resource-center.js?v=3990_2&r=r051-major-scope-clarity';
...
function currentReturnTarget(){ const url=new URL(location.href); if(!url.hash) url.hash='majorAllResultsPanel'; return `${url.pathname}${url.search}${url.hash}`; }
function renderRecord(r){ const key=recordKey(r), on=selectionPool.has(r), project=r.projectLabel||(r.isSino?'中外合作/高收费（需核验）':'普通项目'), canonicalCode=r.standardMajorCode||r.majorCode2026, school=String(r.school||'学校名称待核验').trim(), schoolHref=buildSchoolAllHref({school:r.school,score:scoreValue()||'',focus:'school-all'}), schoolVoiceHref=buildTongxueSchoolHref({school:r.school,returnTo:currentReturnTarget(),resultMode:'major-all',returnAnchor:'majorAllResultsPanel',sourceSurface:'ln-rank-school'}); return `<article class="major-all-record" data-major-record-key="${esc(key)}" data-major-path-record data-workspace-record-key="${esc(key)}"><header class="major-all-record-head"><div><a class="major-all-school-link" href="${esc(schoolHref)}" aria-label="查看${esc(school)}的在辽专业"><h3>${esc(school)}</h3></a><p>${esc(r.displayLocation||'地域待核验')} · ${esc(r.major||'专业名称待核验')}</p></div><span class="major-all-project ${r.isSino?'is-sino':''}">${esc(project)}</span></header><div class="major-all-record-meta"><span>专业代码 ${esc(canonicalCode||'待核验')}</span><span>${esc(positionText(r))}</span><span>2026历史投档记录</span></div><div class="major-all-years" aria-label="三年同口径历史数据"><div><b>2026</b><span>${esc(yearText(r,2026))}</span></div><div><b>2025</b><span>${esc(yearText(r,2025))}</span></div><div><b>2024</b><span>${esc(yearText(r,2024))}</span></div></div><p class="major-all-trend"><b>趋势：</b>${esc(trendText(r))}。仅对当前学校、专业、项目记录做历史对照。</p><div class="major-all-record-actions"><button type="button" class="ui-button ui-button--compact ui-button--secondary" data-major-selection-action="${on?'remove':'add'}" data-major-record-key="${esc(key)}" aria-pressed="${on}">${on?'移出家庭方案':'加入家庭方案'}</button>${schoolVoiceHref?`<a class="ui-button ui-button--compact ui-button--tertiary" href="${esc(schoolVoiceHref)}">学生谈这所学校</a>`:''}</div><p class="major-all-boundary">这是历史投档数据，不是录取承诺；“学生谈这所学校”只代表学校整体留言，不是这个专业的专属评价；学费、校区、合作办学条件以当年招生计划和院校章程为准。</p></article>`; }
...
function renderResults(){ const root=byId('majorAllContent'), panel=byId('majorAllResultsPanel'); if(!root||!panel)return; const active=state.resultMode===MODE; panel.hidden=!active; byId('resultsPanel')?.toggleAttribute('hidden',active); byId('schoolAllResultsPanel')?.toggleAttribute('hidden',active||state.resultMode!=='school-all'); renderCandidates(); if(!active)return; const names=selectedNames(); if(view.loading&&!view.data){root.innerHTML='<div class="ui-state ui-state--loading major-all-message"><b>正在读取专业最低分记录…</b><p>先保留你已经输入的条件。</p></div>';return;} if(view.error&&!view.data){root.innerHTML=`<div class="ui-state ui-state--error major-all-message"><b>这次没有读取成功</b><p>${esc(view.error)}</p><button type="button" class="ui-button ui-button--compact ui-button--secondary" data-major-retry>重新读取</button></div>`;return;} if(!view.data){root.innerHTML=`<div class="ui-state ui-state--pending major-all-message"><b>${names.length?'确认后点击查看专业最低分':'先输入一个或多个专业方向'}</b><p>${names.length?'可选填写分数、地区、学校关键词和普通/中外项目条件。':'例如：电气、机械/测控/材料；模糊输入必须先确认规范名称。'}</p></div>`;return;} const d=view.data,s=d.summary||{},records=Array.isArray(d.records)?d.records:[]; const hasCandidateScore=d.candidateScore!==null&&d.candidateScore!==undefined&&Number.isFinite(Number(d.candidateScore)); if(!records.length){root.innerHTML=`<div class="major-all-summary"><b>${esc(names.join('、'))}</b><span>当前条件下没有找到辽宁最低分记录</span><small>这可能是 2026 年辽宁物理类没有该专业的投档记录，也可能是地区、学校或项目条件过窄。可以放宽条件后再查。</small></div><div class="ui-state ui-state--pending major-all-message"><b>没有查到可展示的记录</b><p>专业本身不一定不存在，只是当前辽宁和筛选口径下没有可核验的最低投档分。</p></div>`;return;} root.innerHTML=`<div class="major-all-summary"><b>${esc(names.join('、'))}</b><span>当前条件下 ${Number(s.schoolCount||0).toLocaleString('zh-CN')} 所学校 · ${Number(s.total||d.total||0).toLocaleString('zh-CN')} 条最低分记录 · ${hasCandidateScore?`参考${Number(d.candidateScore)}分，按历史位次排序`:'未填写参考分数'} · ${d.projectMode==='sino-only'?'仅中外/高收费':d.projectMode==='ordinary-only'?'仅普通项目':'普通与中外分开显示'}</span><small>2026为主口径，2025/2024只在同一学校、专业和项目可以对应时展示。</small></div><div class="major-all-record-list">${records.map(renderRecord).join('')}</div>${d.nextOffset!=null?'<button type="button" class="ui-button ui-button--compact ui-button--secondary major-all-load-more" data-major-load-more>继续加载专业结果</button>':'<p class="major-all-complete">当前条件下的专业最低分记录已全部加载完成。</p>'}`; syncSelectionButtons();document.dispatchEvent(new CustomEvent('gaokao:major-result-render')); }
function syncAction(){ if(state.resultMode!==MODE){ renderCandidates(); document.dispatchEvent(new CustomEvent('gaokao:major-input-state',{detail:inputState()})); return; } const btn=byId('queryButton'), guide=byId('queryGuide'),ready=queryReady(),names=selectedNames(),invalid=rawScorePresent()&&scoreValue()===null; if(!btn)return; btn.disabled=Boolean(view.loading||!ready||invalid); if(view.loading){btn.textContent='正在读取专业最低分…';btn.className='query-button is-loading';if(guide)guide.textContent='正在按已确认的专业、地区和项目条件读取数据。';}else if(!draftMajorText()){btn.textContent='先输入专业方向';btn.className='query-button is-waiting';if(guide)guide.textContent='先输入一个专业或方向；确认后可以继续添加。';}else if(!ready){btn.textContent='先确认具体本科专业';btn.className='query-button is-waiting';if(guide)guide.textContent='模糊输入会先出现候选，确认后才查询，专业类不会静默替换。';}else if(invalid){btn.textContent='分数需为150—750';btn.className='query-button is-waiting';if(guide)guide.textContent='分数可不填；填写时请输入150—750之间的整数。';}else if(view.error){btn.textContent='重新读取专业最低分';btn.className='query-button is-error';if(guide)guide.textContent='读取异常，可以检查网络或稍后重试。';}else if(view.data&&view.dirty){btn.textContent='按新条件更新专业最低分';btn.className='query-button is-ready';if(guide)guide.textContent='条件已变化，下面保留上一轮结果，确认后再更新。';}else{btn.textContent=`查询 ${names.length} 个已确认专业`;btn.className='query-button is-ready';if(guide)guide.textContent='可以继续添加专业；点击后一次查询全部已确认专业。';}}
function syncMode(){const active=state.resultMode===MODE,inputActive=INPUT_MODES.has(state.resultMode);restoreConfirmedFromUrl();byId('majorCandidatePanel')?.toggleAttribute('hidden',!inputActive);byId('majorAllResultsPanel')?.toggleAttribute('hidden',!active);byId('resultsPanel')?.toggleAttribute('hidden',active);byId('schoolAllResultsPanel')?.toggleAttribute('hidden',active||state.resultMode!=='school-all');byId('majorProjectMode')?.closest('.major-all-project-filter')?.toggleAttribute('hidden',!active);if(active){const params=new URLSearchParams(location.search),savedRegion=params.get('region'),savedProject=params.get('projectMode');if(savedRegion&&byId('region')){byId('region').value=savedRegion;state.filters.region=savedRegion;}if(savedProject&&byId('majorProjectMode'))byId('majorProjectMode').value=savedProject;setQueryUrl();document.dispatchEvent(new CustomEvent('gaokao:major-filter-restored'));}renderCandidates();if(active)renderResults();else document.dispatchEvent(new CustomEvent('gaokao:major-input-state',{detail:inputState()}));if(active&&!autoQueryConsumed&&new URLSearchParams(location.search).get('autoQuery')==='1'&&queryReady()){autoQueryConsumed=true;queueMicrotask(()=>load(false));}}
