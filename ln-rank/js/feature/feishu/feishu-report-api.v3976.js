...
function buildInterfaceError({ path, status, raw }) {
  ...
  if (looksLikeHtml(raw)) {
    return new Error(`飞书报告接口没有返回 JSON。请求路径：${path}；HTTP状态：${status}；返回内容像 HTML。通常表示该 Functions 路由没有部署到当前生产环境，或请求路径被 Pages 回退到了 HTML 页面。返回片段：${preview}`);
  }
  return new Error(`飞书报告接口返回内容不是 JSON。请求路径：${path}；HTTP状态：${status}；返回片段：${preview}`);
}
