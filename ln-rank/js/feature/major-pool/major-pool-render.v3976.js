...
function renderKeywordSummary(data) {
  ...
  return `<div class="keyword-summary"><span>当前关键词：</span>${chips}<span class="muted">匹配逻辑：按关系远近排序</span>${matchSummary}${project}${industry}${warnings}</div>`;
}
...
export function renderMajorResults(state, { onMore, selectionPool, onSelectionChange } = {}) {
  ...
  const emptyReason = bottomLineMode !== 'all'
    ? `<div class="empty">当前筛选条件下暂无记录。可能是办学性质底线、查询范围、地域或专业关键词共同限制导致。可以切换为“公办优先”、选择“多看一些”，或放宽专业/地域。</div>`
  ...
  root.querySelectorAll('[data-search-advice-action="switch_bottomline"]').forEach(button => {
    button.addEventListener('click', () => {
      ...
      if (!target) return;
      try { localStorage.setItem('lnRank.bottomLineMode.v3976', target); localStorage.setItem('lnRank.bottomLineMode.v3962', target); } catch {}
