...
import { buildReviewPointsForRecord } from '../../review-point-builder.v3912.js';
...
function tags(record, campus = null) {
  const arr = [];
  ...
  if (campus?.displayTag) arr.push(campus.displayTag);
  return [...new Set(arr.filter(Boolean))].slice(0, 7);
}
...
function reviewSummary(record, points, campus = null) {
  const text = [record.major, record.school, record.matchReason, ...(Array.isArray(record.flags) ? record.flags : []), ...points].join(' ');
  const labels = [];
  if (campus?.displayTag && /校区/.test(campus.displayTag)) labels.push(campus.displayTag);
...
function renderReviewPoints(record, campus = null) {
  const points = buildReviewPointsForRecord(record, { limit: 5 });
  if (campus?.reviewSummary) points.unshift(campus.reviewSummary);
  if (!points.length) return '';
  const summary = reviewSummary(record, points, campus);
...
function card(record, index = 0, selectionPool = null) {
  ...
  const campus = getCampusForRecord(record);
  const tagHtml = tags(record, campus).map(t => `<span class="school-tag ${tagClass(t)} ${campus?.displayTag === t ? `campus-risk-${escapeHtml(campus.risk || 'medium')}` : ''}">${escapeHtml(t)}</span>`).join('');
  return `<article class="major-card status-${statusKey}">
    ...
    ${matchReason(record)}
    ${renderReviewPoints(record, campus)}
