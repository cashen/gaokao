...
async function runResultUiChecks(){
  ...
  try{
    ...
    const cssResult=await text('./css/components/result-section-contract.css?v=3928_1');
    const cssCard=await text('./css/components/major-card-contract.css?v=3928_1');
    const cssBand=await text('./css/components/band-selector-contract.css?v=3928_1');
    const cssColor=await text('./css/core/color-system.css?v=3928_1');
    const cssTheme=await text('./css/components/post-exam-calm-theme.css?v=3928_1');
    const cssBreath=await text('./css/components/band-card-breathing-link.css?v=3928_1');
    const renderMajor=await text('./js/feature/major-pool/render.js?v=3928_1');
    const bandRender=await text('./js/feature/score-bands/render.js?v=3928_1');
    ...
    checks.push({name:'RESULT-UI-003：结果说明区承载当前查看',ok:/result-context-bar/.test(await text('./css/components/result-context-bar.css?v=3928_1').catch(e=>''))&&/renderResultContextBar/.test(renderMajor),detail:'当前分段范围/数量进入紧凑结果说明区，不再重复占位'});
    checks.push({name:'RESULT-UI-004：特殊项目提示进入结果说明区',ok:/result-context-special/.test(await text('./css/components/result-context-bar.css?v=3928_1').catch(e=>''))&&/data-context-special-toggle/.test(renderMajor),detail:'特殊项目数量与显示入口收纳进紧凑说明条'});
...
const textContractCss=await text('./css/components/text-resilience-contract.css?v=3928_1').catch(e=>'');
const selectionActiveJs=await text('./js/selection-pool.v3928.js?v=3928_1').catch(e=>'');
...
checks.push({name:'REPORT-FLOW-007：确认页存在已选专业分布组件',ok:(selectionText.includes('已选专业分布')|| (await text('./js/selection-pool.v3928.js').catch(e=>'')).includes('已选专业分布')) && (await text('./css/components/report-content-confirm.css').catch(e=>'')).includes('report-distribution-panel'),detail:'确认页分布组件用于家长快速判断稍高/主要/稳妥搭配'});
checks.push({name:'REPORT-FLOW-008：确认页存在生成前看一眼组件',ok:(selectionText.includes('生成前看一眼')|| (await text('./js/selection-pool.v3928.js').catch(e=>'')).includes('生成前看一眼')) && (await text('./css/components/report-content-confirm.css').catch(e=>'')).includes('before-report-check-panel'),detail:'生成前看一眼用于温和提示方向、城市、费用和特殊项目'});
checks.push({name:'REPORT-FLOW-009：飞书失败态有人话备用方案',ok:(await text('./js/selection-pool.v3928.js').catch(e=>'')).includes('飞书报告暂时没生成成功，可以先复制文字版保存。'),detail:'失败态提供复制文字版，不用冰冷报错'});
...
const trendJs=await text('./js/major-trend-render.v3928.js').catch(e=>'');
...
document.getElementById('runSelfCheck')?.addEventListener('click',run);run();console.info('v3.9.28.1 Android 条件区收口自测已启用。');
