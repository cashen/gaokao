import {
  ...
  savePoolItems
} from './feature/selection-pool/index.js?v=3944_0';
import { requestPathAnalysis } from './feature/selection-pool/index.js?v=3944_0';
import { buildCandidateContext, buildComputedSignature } from './feature/selection-pool/index.js?v=3944_0';
import { getComputedStats, recomputeSelectionPool, sortComputedByBand, stripComputedForStorage } from './feature/selection-pool/index.js?v=3944_0';
import { renderHealthLights } from './feature/selection-pool/index.js?v=3944_0';
import { createSelectionPoolFeishuReport } from './feature/selection-pool/index.js?v=3944_0';
import { buildTrendSummaryForSelection, renderSelectionTrendBox } from './feature/trend/index.js?v=3944_0';
import { compactHistoryScoreText, historyScoreText } from './feature/major-pool/history-score-render.js?v=3944_0';
import { renderParentCoach } from './feature/decision-coach/index.js?v=3944_0';
import { getCampusForRecord } from './feature/campus/index.js?v=3944_0';
import { buildReviewChecklist, renderReviewChecklist } from './feature/review-checklist/index.js?v=3944_0';
import { normalizeSelectedMajors } from './domain/selection-contract.js?v=3944_0';
import { buildReportPayload } from './domain/report-payload-contract.js?v=3944_0';
import { toHumanCopy, REPORT_COPY } from './domain/human-copy-dictionary.js?v=3944_0';
import { renderDirectionExplorerReportHtml, buildDirectionExplorerReportText, getDirectionExplorerReportContext } from './feature/direction-explorer/direction-explorer-report.js?v=3944_0';
import { buildKnowledgeReviewForRecord, buildKnowledgePortfolioSummary, buildSchoolIndustryTags, KNOWLEDGE_DATA_BOUNDARY, safeGetLocalContextPresentation, buildLocalContextSummary } from './knowledge/index.js?v=3944_0';
import { resolvePlanFlowStep, markFlowAction, clearFlowAction } from './domain/flow-step-contract.js?v=3944_0';
import { renderPlanFlowStepper } from './feature/flow-stepper/flow-stepper-render.js?v=3944_0';

const SCORE_KEY = 'lnRank.selectionPool.candidateScore';
const SCORE_VERSION_KEY = 'lnRank.selectionPool.candidateScore.v3934_1';
  ...
  return `<article class="workspace-item band-${bandClass(item)}" data-id="${escapeHtml(item.id)}" data-index="${index}">
    ...
    <div class="workspace-item-main">
      ...
      <div class="workspace-item-meta">
        <span>2025最低分 ${score}</span><span>${rank}</span><span>${delta}</span>${itemHistoryText(item) ? `<span class="workspace-history-chip">${escapeHtml(itemHistoryText(item))}</span>` : ''}<span class="is-band">${escapeHtml(band.detail || '')}</span>${location}${campusTag}${itemCodeText(item)}${itemLocalContextChip(item)}
      </div>
...
function renderAnalysis() {
  ...
  root.innerHTML = toHumanCopy(`<div class="analysis-box">${staleNotice}${rankZoneCard}${trendBox}${narrativeCard}${renderLocalContextSummaryPanel(state)}${renderKnowledgeSummaryPanel(state)}${dataReview}</div>`);
}
...
function renderSelectionFilterRelation(state = currentComputedState) {
  ...
  const items = state.items || [];
  const hasCost = items.some(item => item.isSinoForeign || item.isHighFee || /中外|合作办学|高收费|较高收费/.test(`${item.major || ''} ${item.tuition || ''} ${item.feeType || ''}`));
  const hasSpecial = items.some(item => item.specialProject?.hasSpecialProject || item.hasSpecialProject || /定向|专项|公费师范|优师|预科|民族班|公安|司法|航海|轮机/.test(`${item.major || ''} ${item.flags || ''}`));
  const notes = [];
  if (state.bottomLineMode === 'public_regular_only' && hasCost) notes.push('当前筛选是“只看公办普通”，但已选清单中仍有中外/高收费项目。已选清单不会自动删除，报告会单独列入费用、培养模式和证书口径核验。');
  if (hasSpecial) notes.push('已选清单中包含或疑似包含专项、定向、公费师范、预科等特殊项目，报告会提示资格、协议、服务年限、批次和体检/政审核验。');
...
function createReportContext(state = getState()) {
  return { candidateScore: state.score || null, selectedCount: state.items?.length || 0, bottomLineMode: state.bottomLineMode || 'all', rangePreset: 'standard', activeBand: null, directionExplorer: getDirectionExplorerReportContext(), generatedAt: new Date().toISOString(), version: 'v3.9.44' };
}
