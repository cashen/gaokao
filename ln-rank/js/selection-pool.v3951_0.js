import {
  ...
  savePoolItems
} from './feature/selection-pool/index.js?v=3951_0';
import { requestPathAnalysis } from './feature/selection-pool/index.js?v=3951_0';
import { buildCandidateContext, buildComputedSignature } from './feature/selection-pool/index.js?v=3951_0';
import { getComputedStats, recomputeSelectionPool, sortComputedByBand, stripComputedForStorage } from './feature/selection-pool/index.js?v=3951_0';
import { renderHealthLights } from './feature/selection-pool/index.js?v=3951_0';
import { createSelectionPoolFeishuReport } from './feature/selection-pool/index.js?v=3951_0';
...
import { normalizeSelectedMajors } from './domain/selection-contract.js?v=3951_0';
import { buildReportPayload } from './domain/report-payload-contract.js?v=3951_0';
...
import { resolvePlanFlowStep, markFlowAction, clearFlowAction } from './domain/flow-step-contract.js?v=3951_0';
...
function plainTextReport(state = getState()) {
  ...
  state.items.forEach((item, index) => {
    ...
    lines.push(`   ${code}`);
    lines.push(`   2026投档最低分：${fmt(item.score2025 ?? item.score)}｜2025最低位次：${fmt(item.rank2025 ?? item.rank)}｜相对孩子：${item.scoreDelta == null ? '待核验' : (Number(item.scoreDelta) >= 0 ? '+' : '') + fmt(item.scoreDelta)} 分`);
