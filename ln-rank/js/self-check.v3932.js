...
async function runResultUiChecks(){
  ...
  try{
    ...
    const cssResult=await text('./css/components/result-section-contract.css?v=3933_2');
    const cssCard=await text('./css/components/major-card-contract.css?v=3933_2');
    const cssBand=await text('./css/components/band-selector-contract.css?v=3933_2');
    const cssColor=await text('./css/core/color-system.css?v=3933_2');
    const cssTheme=await text('./css/components/post-exam-calm-theme.css?v=3933_2');
    const cssBreath=await text('./css/components/band-card-breathing-link.css?v=3933_2');
    const renderMajor=await text('./js/feature/major-pool/render.js?v=3933_2');
    const bandRender=await text('./js/feature/score-bands/render.js?v=3933_2');
    ...
    checks.push({name:'RESULT-UI-003：结果说明区承载当前查看',ok:/result-context-bar/.test(await text('./css/components/result-context-bar.css?v=3933_2').catch(e=>''))&&/renderResultContextBar/.test(renderMajor),detail:'当前分段范围/数量进入紧凑结果说明区，不再重复占位'});
    checks.push({name:'RESULT-UI-004：特殊项目提示进入结果说明区',ok:/result-context-special/.test(await text('./css/components/result-context-bar.css?v=3933_2').catch(e=>''))&&/data-context-special-toggle/.test(renderMajor),detail:'特殊项目数量与显示入口收纳进紧凑说明条'});
...
const textContractCss=await text('./css/components/text-resilience-contract.css?v=3933_2').catch(e=>'');
const selectionActiveJs=await text('./js/selection-pool.v3928.js?v=3933_2').catch(e=>'');
...
document.getElementById('runSelfCheck')?.addEventListener('click',run);run();console.info('v3.9.32 Android 条件区收口自测已启用。');
