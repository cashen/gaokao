...
import { requestPathAnalysis } from './feature/selection-pool/path-analysis-api.v3955.js';
import { createSelectionPoolFeishuReport } from './feature/selection-pool/feishu-selection-report-api.v3955.js';

const SCORE_KEY = 'lnRank.selectionPool.candidateScore.v3955';
...
function renderAnalysis() {
  ...
  if (!root) return;
  if (!currentAnalysis) {
    root.innerHTML = '<div class="pool-empty diagnose-empty">点击“检查当前排序与AI解读”，系统会按位次、特控线锚点和自选池结构检查冲稳保、保底深度与专业路径。</div>';
    return;
  }
  ...
  const ai = currentAnalysis.aiNarrative || null;
  const density = rz.density || {};
  const aiSource = currentAnalysis.source === 'workers-ai'
    ? 'Cloudflare AI 解读'
    : currentAnalysis.source ? '规则兜底解读' : '规则诊断';
  const rankZoneCard = `<div class="analysis-rankzone">
    <div class="rankzone-main">
      <span class="rankzone-label">位次功能区</span>
      <strong>${escapeHtml(rz.zoneName || '待判断')}</strong>
      <p>${escapeHtml(rz.note || '分数只是展示口径，系统会优先按当年一分一段位次和自选池结构判断。')}</p>
    </div>
    <div class="rankzone-grid">
      <div><span>考生位次</span><b>${escapeHtml(rz.candidateRankLabel || '待核验')}</b></div>
  ...
  const aiCard = ai ? `<div class="analysis-ai-card">
    <div class="analysis-ai-head"><span>${escapeHtml(aiSource)}</span>${currentAnalysis.model ? `<em>${escapeHtml(currentAnalysis.model)}</em>` : ''}</div>
    ${ai.overall ? `<p class="analysis-ai-overall">${escapeHtml(ai.overall)}</p>` : ''}
    <div class="analysis-ai-grid">
      ${ai.rankZoneExplain ? `<div><h3>位次定位</h3><p>${escapeHtml(ai.rankZoneExplain)}</p></div>` : ''}
      ${ai.structureDiagnosis ? `<div><h3>结构诊断</h3><p>${escapeHtml(ai.structureDiagnosis)}</p></div>` : ''}
      ${ai.majorPathDiagnosis ? `<div><h3>专业路径</h3><p>${escapeHtml(ai.majorPathDiagnosis)}</p></div>` : ''}
      ${ai.bottomLineRisk ? `<div><h3>保底底线</h3><p>${escapeHtml(ai.bottomLineRisk)}</p></div>` : ''}
    </div>
    ${Array.isArray(ai.actions) && ai.actions.length ? `<div class="analysis-section"><h3>AI调整动作</h3><ol class="analysis-list">${ai.actions.map(x => `<li>${escapeHtml(x)}</li>`).join('')}</ol></div>` : ''}
    ${ai.disclaimer ? `<div class="analysis-ai-disclaimer">${escapeHtml(ai.disclaimer)}</div>` : ''}
  </div>` : '';
