...
import { renderMajorResults } from './feature/major-pool/major-pool-render.v3960.js';
import { initFeishuReport, renderFeishuReport, clearFeishuReport } from './feature/feishu/feishu-report-controller.v3916.js';
import { initSelectionPool, refreshSelectionPool, createSelectionPoolAdapter } from './feature/selection-pool/selection-pool-controller.v3949.js';
...
function shouldShowBottomLinePanel(score) {
  ...
  return Number.isFinite(n) && n > 0 && n <= SPECIAL_CONTROL_SCORE;
}
...
function renderBottomLinePanel() {
  ...
  if (summary) {
    summary.textContent = visible
      ? `当前为特控线以下/边缘分段，可按“${bottomLineLabel(state.filters.bottomLineMode)}”筛选。600分左右的公办中外上探不在这里做按钮，由AI诊断和报告解释。`
      : score ? '当前分数已明显离开公办底线提示区，办学性质底线已收起；需要中外上探时由AI诊断说明。' : '输入分数后，特控线以下/边缘分段会显示办学性质底线。';
  }
}
