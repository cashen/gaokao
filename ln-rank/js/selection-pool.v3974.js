import {
  ...
  savePoolItems
} from './feature/selection-pool/selection-pool-store.v3955.js';
import { requestPathAnalysis } from './feature/selection-pool/path-analysis-api.v3959.js';
import { buildCandidateContext, buildComputedSignature } from './feature/selection-pool/candidate-context.v3959.js';
...
import { createSelectionPoolFeishuReport } from './feature/selection-pool/feishu-selection-report-api.v3955.js';
...
const BOTTOMLINE_STORAGE_KEY = 'lnRank.bottomLineMode.v3960';
...
function bottomLineMode() {
  try { return localStorage.getItem(BOTTOMLINE_STORAGE_KEY) || 'all'; } catch { return 'all'; }
}
...
function buildCurrentState({ persistScore = true } = {}) {
  ...
  const score = persistScore ? (readScoreFromInput() || loadScore()) : (readScoreFromInput() || loadScore());
  const candidateContext = buildCandidateContext(score);
  const computedItems = recomputeSelectionPool(candidateContext, rawItems);
  ...
  currentComputedState = { rawItems, items: computedItems, stats, candidateContext: { ...candidateContext, bottomLineMode: bottomLineMode() }, orderSignature, signature, score, bottomLineMode: bottomLineMode() };
...
function contextStatusHtml(state) {
  ...
  const text = score
    ? `当前计算口径：2025 辽宁物理类 · 考生 ${score} 分 · 已按当前成绩重算 ${total} 个专业。排序、AI诊断和飞书报告都会先使用这个口径。`
    : '当前计算口径：考生分数待填写。请先填写分数，再进行排序、AI诊断或飞书报告。';
  const stale = score && currentAnalysis && !fresh ? '<b>成绩或排序已变化，旧诊断已过期。</b>' : '';
...
function itemCodeText(item = {}) {
  const parts = [];
  if (item.codes?.majorCode) parts.push(`专业代码 ${item.codes.majorCode}`);
  if (item.standardMajor?.code && item.standardMajor?.name) parts.push(`本科专业代码 ${item.standardMajor.code}｜${item.standardMajor.name}`);
  else if (item.standardMajor?.categoryCode && item.standardMajor?.categoryName) parts.push(`本科专业类 ${item.standardMajor.categoryCode}｜${item.standardMajor.categoryName}`);
  return parts.length ? `<span class="workspace-code-line">${escapeHtml(parts.join('　'))}</span>` : '';
}
  ...
  return `<article class="workspace-item band-${bandClass(item)}" data-id="${escapeHtml(item.id)}" data-index="${index}">
    ...
    <div class="workspace-item-main">
      ...
      <div class="workspace-item-meta">
        <span class="is-band">${band.detail}</span><span>${score}</span><span>${rank}</span><span>${delta}</span>${location}${itemCodeText(item)}
      </div>
    </div>
...
function renderAnalysis() {
  ...
  if (!root) return;
  if (!currentAnalysis) {
    root.innerHTML = '<div class="pool-empty diagnose-empty">点击“检查当前排序与AI解读”，系统会按位次、控制线锚点和自选池结构判断位次功能区、主要矛盾与调整方向。</div>';
    return;
  }
  const state = getState();
  const fresh = analysisIsFresh(state);
  const staleNotice = fresh ? '' : '<div class="analysis-stale">排序或分数已经变化，请重新点击“检查当前排序与AI解读”后再发送诊断报告。</div>';
  ...
  const sourceText = currentAnalysis.source === 'workers-ai'
    ? 'Cloudflare AI 高报师解读'
    : currentAnalysis.source ? '规则兜底高报师解读' : '规则诊断';
  ...
  const rankZoneCard = `<div class="analysis-rankzone">
    <div class="rankzone-main">
      <span class="rankzone-label">AI位次功能区判断</span>
      <strong>${escapeHtml(finalZone.zoneName || rz.zoneName || '待判断')}</strong>
      <p>${escapeHtml(narrative?.zoneJudgement || rz.note || '分数只是展示口径，系统会优先按当年一分一段位次、控制线锚点和自选池结构判断。')}</p>
  ...
  const narrativeCard = narrative ? `<div class="analysis-ai-card">
    <div class="analysis-ai-head"><span>${escapeHtml(sourceText)}</span>${currentAnalysis.model ? `<em>${escapeHtml(currentAnalysis.model)}</em>` : ''}</div>
    ${narrative.overall ? `<section class="analysis-ai-section"><h3>整体判断</h3><p>${escapeHtml(narrative.overall)}</p></section>` : ''}
    ${narrative.reasoning ? `<section class="analysis-ai-section"><h3>为什么这样判断</h3><p>${escapeHtml(narrative.reasoning)}</p></section>` : ''}
    <div class="analysis-ai-grid">
      ${narrative.structureDiagnosis ? `<div><h3>结构诊断</h3><p>${escapeHtml(narrative.structureDiagnosis)}</p></div>` : ''}
      ${narrative.majorPathDiagnosis ? `<div><h3>专业与地域路径</h3><p>${escapeHtml(narrative.majorPathDiagnosis)}</p></div>` : ''}
      ${narrative.pushRateDiagnosis ? `<div><h3>升学与推免参考</h3><p>${escapeHtml(narrative.pushRateDiagnosis)}</p></div>` : ''}
      ${narrative.bottomLineDiagnosis ? `<div><h3>保底底线</h3><p>${escapeHtml(narrative.bottomLineDiagnosis)}</p></div>` : ''}
    </div>
    ${Array.isArray(narrative.riskDiagnosis) && narrative.riskDiagnosis.length ? `<div class="analysis-section"><h3>主要风险</h3>${listHtml(narrative.riskDiagnosis)}</div>` : ''}
    ${Array.isArray(narrative.actions) && narrative.actions.length ? `<div class="analysis-section"><h3>调整建议</h3>${listHtml(narrative.actions)}</div>` : ''}
  ...
  const dataReview = `<details class="analysis-raw-details"><summary>展开查看冲稳保快速复核</summary>
    <div class="analysis-mini-grid">
      <div><span>冲刺</span><b>${escapeHtml(fmt(stats.rushCount))}</b></div>
      <div><span>稳妥</span><b>${escapeHtml(fmt(stats.stableCount))}</b></div>
      <div><span>保底</span><b>${escapeHtml(fmt(stats.safeCount))}</b></div>
      <div><span>高冲</span><b>${escapeHtml(fmt(stats.highRushCount))}</b></div>
    ...
    ${(currentAnalysis.risks || []).length ? `<h3>规则风险底稿</h3>${listHtml(currentAnalysis.risks)}` : ''}
    ${(currentAnalysis.actions || []).length ? `<h3>规则动作底稿</h3>${listHtml(currentAnalysis.actions)}` : ''}
  </details>`;
  root.innerHTML = `<div class="analysis-box">${staleNotice}${rankZoneCard}${renderParentCoach(currentAnalysis.parentCoach)}${narrativeCard}${dataReview}</div>`;
}
...
async function runAnalysis() {
  ...
  const state = getState();
  if (!state.items.length) { alert('请先加入专业志愿。'); return; }
  ...
  try {
    ...
    const latest = getState();
    if (button) { button.disabled = !latest.items.length || !latest.candidateContext.score; button.textContent = '检查当前排序与AI解读'; }
    renderAll();
  }
}
...
async function sendFeishu({ withAnalysis = false } = {}) {
  ...
  feishuStatus = { ok: true, message: withAnalysis ? '正在按当前成绩口径重新计算并生成飞书诊断报告…' : '正在按当前成绩口径把排序清单发送到飞书…' };
  renderFeishuStatus();
  try {
    ...
    const latest = getState();
    if (btn) { btn.disabled = !latest.items.length || !latest.candidateContext.score; btn.textContent = withAnalysis ? '发送AI诊断报告到飞书' : '发送当前排序到飞书'; }
    renderAll();
  }
}
...
function bind() {
  ...
  $('sortByBand')?.addEventListener('click', () => {
    ...
    if (!state.candidateContext.score) { alert('请先填写考生分数，再按当前成绩整理冲稳保。'); return; }
    if (!confirm(`将按当前 ${state.candidateContext.score} 分口径，按 高冲→小冲→边稳→稳妥→小保→强保→兜底 重新整理当前自选池。确认执行吗？`)) return;
