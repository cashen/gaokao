import {
  ...
  savePoolItems
} from './feature/selection-pool/selection-pool-store.v3981.js';
...
import { createSelectionPoolFeishuReport } from './feature/selection-pool/feishu-selection-report-api.v3981.js';
...
function renderAnalysis() {
  ...
  const sourceText = currentAnalysis.source === 'workers-ai'
    ? '方案解读（AI辅助）'
    : currentAnalysis.source ? '方案解读（规则兜底）' : '规则解读';
  ...
  const rankZoneCard = `<div class="analysis-rankzone">
    <div class="rankzone-main">
      ...
      <p>${escapeHtml(narrative?.zoneJudgement || rz.note || '分数只是展示口径，系统会优先按当年一分一段位次、控制线锚点和自选专业结构判断。')}</p>
  ...
  const narrativeCard = narrative ? `<div class="analysis-ai-card">
    ...
    ${narrative.overall ? `<section class="analysis-ai-section"><h3>一句话结论</h3><p>${escapeHtml(narrative.overall)}</p></section>` : ''}
    ...
    <div class="analysis-ai-grid">
      ${narrative.structureDiagnosis ? `<div><h3>这套方案的结构</h3><p>${escapeHtml(narrative.structureDiagnosis)}</p></div>` : ''}
      ${narrative.majorPathDiagnosis ? `<div><h3>专业和城市是否集中</h3><p>${escapeHtml(narrative.majorPathDiagnosis)}</p></div>` : ''}
      ${narrative.pushRateDiagnosis ? `<div><h3>升学与推免参考</h3><p>${escapeHtml(narrative.pushRateDiagnosis)}</p></div>` : ''}
      ${narrative.bottomLineDiagnosis ? `<div><h3>后段是否够稳</h3><p>${escapeHtml(narrative.bottomLineDiagnosis)}</p></div>` : ''}
    </div>
