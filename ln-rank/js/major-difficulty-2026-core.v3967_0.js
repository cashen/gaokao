import { LIAONING_MAJOR_TREND_RESOURCE, trendSegmentOrder } from '../../shared/resources/trends/liaoning-major-trend.v3967_0.js?v=3967_0';
import { resolveTrendInterpretation } from '../../shared/algorithms/trend/trend-interpretation.v3967_0.js?v=3967_0';

const DATA_URL = LIAONING_MAJOR_TREND_RESOURCE.dataUrl;
...
function scoreBandOrder(band) {
  return trendSegmentOrder(band?.label);
}

function directionState(row, threshold) {
  return resolveTrendInterpretation(row, { neutralThresholdPctPoint: threshold });
}
...
function renderBandButtons(data) {
  ...
  const bands = [...(data?.segments || [])].sort((a, b) => scoreBandOrder(a) - scoreBandOrder(b));
