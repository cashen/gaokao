...
function statHtml(stats, items, state) {
  ...
  return `${contextStatusHtml(state)}<div class="pool-stats workspace-stats">
    ...
    <div><strong>${stats.safeCount}</strong><span>主要参考补充</span></div>
...
function renderAnalysis() {
  ...
  if (!root) return;
  if (!currentAnalysis) {
    root.innerHTML = '<div class="pool-empty diagnose-empty">点击“检查这套方案”，下方会显示完整方案解读结果；报告按钮会固定在右侧/下方操作卡内，不会被诊断内容挤走。</div>';
    return;
  }
  const state = getState();
  ...
  const structureText = narrative?.structureDiagnosis
    || (stats.total ? `当前自选专业共 ${fmt(stats.total)} 个：稍高目标 ${fmt(stats.rushCount)} 个，主要参考 ${fmt(stats.stableCount)} 个，主要参考补充 ${fmt(stats.safeCount)} 个。建议重点看中段是否接得住、后段是否够稳。` : '当前自选专业数量和前中后段结构待核验。');
  ...
  const narrativeCard = `<div class="analysis-ai-card">
    ...
    <section class="analysis-section"><h3>下一步调整建议</h3>${actions.length ? listHtml(actions) : '<p>建议先补齐考生位次、确认可接受城市和专业方向，再按主要参考与主要参考补充顺序逐条核验。</p>'}</section>
  ...
  const dataReview = `<details class="analysis-raw-details"><summary>展开查看前中后段快速复核</summary>
    <div class="analysis-mini-grid">
      ...
      <div><span>主要参考补充</span><b>${escapeHtml(fmt(stats.safeCount))}</b></div>
...
function renderActionPanelState() {
  ...
  root.innerHTML = '<div class="ln-action-hint">建议先点击“检查这套方案”。也可以先生成清单版或复制文字版。</div>';
}
...
function createReportContext(state = getState()) {
  return { candidateScore: state.score || null, selectedCount: state.items?.length || 0, bottomLineMode: state.bottomLineMode || 'all', generatedAt: new Date().toISOString() };
}
...
function plainTextReport(state = getState()) {
  ...
  lines.push(`结构概览：稍高目标 ${state.stats.rushCount || 0} 个｜主要参考 ${state.stats.stableCount || 0} 个｜主要参考补充 ${state.stats.safeCount || 0} 个`);
  lines.push('');
  lines.push('自选专业清单：');
...
function bind() {
  ...
  $('sortByBand')?.addEventListener('click', () => {
    ...
    if (!confirm(`将按当前 ${state.candidateContext.score} 分口径，把当前自选专业按“稍高目标→主要参考→主要参考补充”的顺序重新整理。确认执行吗？`)) return;
