...
import { requestPathAnalysis } from './feature/selection-pool/index.v3967_0.js?v=3967_0';
import { buildCandidateContext, buildComputedSignature } from './feature/selection-pool/index.v3967_0.js?v=3967_0';
import { getComputedStats, recomputeSelectionPool, sortComputedByBand, stripComputedForStorage } from './feature/selection-pool/index.v3967_0.js?v=3967_0';
import { renderHealthLights } from './feature/selection-pool/index.v3967_0.js?v=3967_0';
import { createSelectionPoolFeishuReport } from './feature/selection-pool/index.v3967_0.js?v=3967_0';
import { buildTrendSummaryForSelection, renderSelectionTrendBox } from './feature/trend/index.js?v=3951_0';
import { compactHistoryScoreText, historyScoreText } from './feature/major-pool/history-score-render.v3967_0.js?v=3967_0';
import { renderParentCoach } from './feature/decision-coach/index.js?v=3951_0';
import { getCampusForRecord } from './feature/campus/index.js?v=3951_0';
import { buildReviewChecklist, renderReviewChecklist } from './feature/review-checklist/index.js?v=3951_0';
import { normalizeSelectedMajors } from './domain/selection-contract.v3963_1.js?v=3963_1';
import { buildReportPayload } from './domain/report-payload-contract.v3963_1.js?v=3963_1';
import { toHumanCopy, REPORT_COPY } from './domain/human-copy-dictionary.v3964_0.js?v=3964_0';
import { renderDirectionExplorerReportHtml, buildDirectionExplorerReportText, getDirectionExplorerReportContext } from './feature/direction-explorer/direction-explorer-report.js?v=3951_0';
import { buildKnowledgeReviewForRecord, buildKnowledgePortfolioSummary, buildSchoolIndustryTags, KNOWLEDGE_DATA_BOUNDARY, safeGetLocalContextPresentation, buildLocalContextSummary } from './knowledge/index.js?v=3951_0';
import { resolveLocalStrengthMark, buildLocalStrengthSummary } from './feature/major-pool/local-strength-view.js?v=3951_0';
import { resolveMajorUnderstanding, majorUnderstandingReportLines } from './knowledge/major-understanding-resolver.js?v=3951_0';
import { resolvePlanFlowStep, markFlowAction, clearFlowAction } from './domain/flow-step-contract.v3963_1.js?v=3963_1';
import { buildSelectionSignature, markReportGenerating, markReportFresh, markReportFailed, expireReport, clearReportFreshness, readReportFreshness } from './domain/report-freshness-contract.js?v=3951_0';
import { buildSelectionConsistencyNotes } from './domain/selection-consistency-contract.js?v=3951_0';
import { renderPlanFlowStepper } from './feature/flow-stepper/flow-stepper-render.v3963_1.js?v=3963_1';
import { FEISHU_REPORT_CONTRACT, FEISHU_YEAR_CALIBER } from '../../shared/resources/reports/feishu-report-contract.v3966_0.js?v=3966_0';
...
function loadRankContext() {
  try {
    ...
    return { year: FEISHU_REPORT_CONTRACT.dataYear, audienceYear: FEISHU_REPORT_CONTRACT.audienceYear, rangePreset: 'standard' };
  }
}
...
function contextStatusHtml(state) {
  ...
  const text = score
    ? `当前按 2026 辽宁物理类 · 孩子 ${score} 分计算。报告里的相对分差和分段提醒都会按这个分数重新整理。`
...
function itemHtml(item, index, total) {
  ...
  const rank = Number.isFinite(Number(item.rank2026 ?? item.rank)) ? `位次 ${fmt(item.rank2026 ?? item.rank)}` : '位次待核验';
  ...
  return `<article class="workspace-item band-${bandClass(item)}" data-id="${escapeHtml(item.id)}" data-index="${index}" data-only-item="${total === 1}">
    ...
    <div class="workspace-item-main">
      ...
      <div class="workspace-item-meta">
        <span>2026投档最低分 ${score}</span><span>${rank}</span><span>${delta}</span><span class="is-band">${escapeHtml(band.detail || '')}</span>${itemSchoolProfileHtml(item)}${campusTag}${itemCodeText(item)}${itemLocalContextChip(item)}${itemLocalStrengthChip(item)}
      </div>
      ${campusReview ? `<div class="workspace-item-review">${campusReview}</div>` : ''}
      ${itemHistoryText(item) ? `<details class="workspace-item-history"><summary>历史对照（不参与2026当前分组）</summary><p>${escapeHtml(itemHistoryText(item))}</p></details>` : ''}
      ${itemMajorUnderstandingHtml(item)}
      ${itemKnowledgeHtml(item)}
    </div>
    <div class="workspace-item-actions">
      ${total > 1 ? `<button class="workspace-mini-button icon" title="上移一位" data-up="${escapeHtml(item.id)}" ${index === 0 ? 'disabled' : ''}>上移</button>
...
function renderAnalysis() {
  ...
  if (!root) return;
  if (!currentAnalysis) {
    root.innerHTML = '<section class="analysis-not-run" data-check-state="not-run"><h3>还没有检查当前方案</h3><p>点击“检查当前方案”后，这里才会根据当前分数、已选顺序、城市、费用、校区和特殊项目生成检查结果。</p></section>';
    return;
  }
  const state = getState();
  ...
  const staleNotice = fresh ? '' : '<div class="analysis-stale">分数或顺序已经变化，建议重新点“检查当前方案”后再生成报告。</div>';
  ...
  const rankZoneCard = `<div class="analysis-rankzone human-rankzone">
    <div class="rankzone-main">
      ...
      <p>${escapeHtml(narrative?.rankZoneExplain || narrative?.zoneJudgement || rz.note || FEISHU_YEAR_CALIBER.unpublishedBoundary)}</p>
  ...
  const rankText = narrative?.rankZoneExplain || narrative?.zoneJudgement || rz.note || FEISHU_YEAR_CALIBER.unpublishedBoundary;
  ...
  const narrativeCard = `<div class="analysis-ai-card">
    <div class="analysis-ai-head"><span>检查当前方案</span></div>
...
function renderActionPanelState() {
  ...
  root.innerHTML = '<div class="ln-action-hint">建议先点“检查当前方案”，再生成飞书报告。</div>';
}
...
function createPathAnalysisRequest(state) {
  return {
    items: state.items,
...
async function runAnalysis() {
  ...
  try {
    const analysis = await requestPathAnalysis(createPathAnalysisRequest(state));
    ...
    if (latest.signature === state.signature) {
      ...
      currentAnalysisSignature = '';
      currentAnalysisScore = '';
      feishuStatus = { ok: false, message: '检查过程中顺序或分数发生了变化，请重新点“检查当前方案”。' };
    }
  } catch (error) {
    feishuStatus = { ok: false, message: '暂时没能完成检查。可以先确认已选专业和分数，稍后再试。', detail: error?.message || String(error) };
  } finally {
    const latest = getState();
    if (button) { button.disabled = !latest.items.length || !latest.candidateContext.score; button.textContent = '检查当前方案'; }
    renderAll();
  }
}
...
function createReportContext(state = getState()) {
  return {
    candidateScore: state.score || null,
    selectedCount: state.items?.length || 0,
    bottomLineMode: state.bottomLineMode || 'all',
    rangePreset: 'standard',
    activeBand: null,
    directionExplorer: getDirectionExplorerReportContext(),
    dataYear: FEISHU_REPORT_CONTRACT.dataYear,
    rankYear: FEISHU_REPORT_CONTRACT.rankYear,
    audienceYear: FEISHU_REPORT_CONTRACT.audienceYear,
    yearCaliberVersion: FEISHU_REPORT_CONTRACT.yearCaliberVersion,
...
function plainTextReport(state = getState()) {
  ...
  state.items.forEach((item, index) => {
    ...
    lines.push(`   ${code}`);
    lines.push(`   2026投档最低分：${fmt(item.score2026 ?? item.score)}｜2026最低投档位次：${fmt(item.rank2026 ?? item.rank)}｜相对孩子：${item.scoreDelta2026 == null && item.scoreDelta == null ? '待核验' : (Number(item.scoreDelta2026 ?? item.scoreDelta) >= 0 ? '+' : '') + fmt(item.scoreDelta2026 ?? item.scoreDelta)} 分`);
  ...
  state.items.forEach((item, index) => {
    lines.push(`${index + 1}. ${item.school || '学校待核验'} · ${item.major || '专业待核验'}｜${reportHistoryLine(item)}`);
  });
  lines.push('说明：2025、2024只用于同校、同专业、同项目属性的历史对照，不改变2026当前分组。');
  lines.push('');
...
async function sendFeishu({ withAnalysis = false } = {}) {
  ...
  try {
    ...
    if (withAnalysis && !analysisIsFresh(reportState)) {
      analysis = await requestPathAnalysis(createPathAnalysisRequest(reportState));
...
bind();
renderAll();
const runtimeScoreInput = $('pathCandidateScore');
if (runtimeScoreInput) runtimeScoreInput.disabled = false;
document.querySelectorAll('[data-selection-runtime-control]').forEach(control => control.removeAttribute('aria-disabled'));
