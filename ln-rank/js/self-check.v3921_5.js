...
async function runResultUiChecks(){
  ...
  try{
    ...
    const cssResult=await text('./css/components/result-section-contract.css?v=3921_6');
    const cssCard=await text('./css/components/major-card-contract.css?v=3921_6');
    const cssBand=await text('./css/components/band-selector-contract.css?v=3921_6');
    const cssColor=await text('./css/core/color-system.css?v=3921_6');
    const cssTheme=await text('./css/components/post-exam-calm-theme.css?v=3921_6');
    const cssBreath=await text('./css/components/band-card-breathing-link.css?v=3921_6');
    const renderMajor=await text('./js/feature/major-pool/render.js?v=3921_6');
    const bandRender=await text('./js/feature/score-bands/render.js?v=3921_6');
    ...
    checks.push({name:'RESULT-UI-003：结果说明区承载当前查看',ok:/result-context-bar/.test(await text('./css/components/result-context-bar.css?v=3921_6').catch(e=>''))&&/renderResultContextBar/.test(renderMajor),detail:'当前分段范围/数量进入紧凑结果说明区，不再重复占位'});
    checks.push({name:'RESULT-UI-004：特殊项目提示进入结果说明区',ok:/result-context-special/.test(await text('./css/components/result-context-bar.css?v=3921_6').catch(e=>''))&&/data-context-special-toggle/.test(renderMajor),detail:'特殊项目数量与显示入口收纳进紧凑说明条'});
...
checks.push({name:'REPORT-FLOW-009：飞书失败态有人话备用方案',ok:(await text('./js/selection-pool.v3921_6.js').catch(e=>'')).includes('飞书报告暂时没生成成功，可以先复制文字版保存。'),detail:'失败态提供复制文字版，不用冰冷报错'});
...
const trendJs=await text('./js/major-trend-render.v3921_6.js').catch(e=>'');
...
checks.push({name:'TREND-006：趋势 import 版本统一',ok:trendIndex.includes('v=3921_6')&&trendRules.includes('v=3921_6'),detail:'trend/index.js 与 rules.js 内部 import 使用 v3921_6'});
...
document.getElementById('runSelfCheck')?.addEventListener('click',run);run();console.info('v3.9.21.6 家长报告 UI 精修自测已启用。');
