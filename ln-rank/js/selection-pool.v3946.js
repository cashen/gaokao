...
} from './feature/selection-pool/selection-pool-store.v3946.js';
import { requestPathAnalysis } from './feature/selection-pool/path-analysis-api.v3946.js';
import { createSelectionPoolFeishuReport } from './feature/selection-pool/feishu-selection-report-api.v3946.js';

const SCORE_KEY = 'lnRank.selectionPool.candidateScore.v3946';
const LEGACY_SCORE_KEYS = ['lnRank.selectionPool.candidateScore.v3945', 'lnRank.selectionPool.candidateScore.v3944', 'lnRank.selectionPool.candidateScore.v3943', 'lnRank.selectionPool.candidateScore.v3942', 'lnRank.selectionPool.candidateScore.v3941', 'lnRank.selectionPool.candidateScore.v3940'];
  ...
  try {
    ...
    feishuStatus = {
      ...
      message: data.partial
        ...
        : withAnalysis
          ? `诊断报告已发送到飞书，${data.sharePublic ? '已设置为获得链接的人可阅读。' : '共享权限需要人工确认。'}`
          : `当前排序清单已发送到飞书，${data.sharePublic ? '已设置为获得链接的人可阅读。' : '共享权限需要人工确认。'}`,
      url: data.url || ''
    };
  } catch (error) {
