...
export function markFlowAction(action) {
  try {
...
export function resolveMainFlowStep(context = {}) {
  ...
  const reportActive = Boolean(context.reportActive) || recentAction('report');
  const reportFailed = Boolean(context.reportFailed);
  if (reportActive || reportFailed) return { current: 'report', completed: ['input','filter','select'], status: reportFailed ? 'failed' : 'active', note: reportFailed ? '报告暂时没生成成功，可以复制文字版或稍后重试。' : '正在整理家庭复核报告，最后仍需人工核验。' };
  if (selectionCount > 0) return { current: 'select', completed: ['input','filter'], status: 'active', note: `已加入 ${selectionCount} 个可讨论专业，建议去自选池整理结构。` };
  if (hasValidScore || hasFilters || hasResults) return { current: 'filter', completed: hasValidScore ? ['input'] : [], status: 'active', note: hasResults ? '已有查询结果，下一步把能接受的专业加入自选池。' : '现在可以按方向、地区、学校和项目条件缩小范围。' };
...
export function resolvePlanFlowStep(context = {}) {
  ...
  const reportGenerating = Boolean(context.reportGenerating) || recentAction('report');
  const reportFailed = Boolean(context.reportFailed);
