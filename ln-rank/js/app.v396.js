...
import { fetchMajorBands } from './feature/major-pool/major-bands-api.v396.js';
import { renderBandTabs } from './feature/score-bands/score-bands-render.js';
...
async function loadData(){
  state.bands.loading = true; state.bands.error = null; renderAll();
  try { state.bands.data = await fetchMajorBands({ candidateScore: state.candidateScore, rangePreset: state.rangePreset, filters: state.filters }); }
...
function bind(){
  document.getElementById('candidateScore').addEventListener('input', (e)=>{ state.candidateScore = toInt(e.target.value, state.candidateScore); resetVisible(); debouncedLoad(); renderAll(); });
  document.getElementById('rangeButtons').addEventListener('click', (e)=>{ const btn=e.target.closest('[data-preset]'); if(!btn) return; state.rangePreset=btn.dataset.preset; resetVisible(); document.querySelectorAll('[data-preset]').forEach(el=>el.classList.toggle('is-active',el===btn)); loadData(); });
...
renderRegionOptions(); bind(); renderAll(); loadData();
