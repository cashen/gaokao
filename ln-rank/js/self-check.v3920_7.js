...
async function runResultUiChecks(){
  ...
  try{
    ...
    const css1=await text('./css/components/result-section-contract.css?v=3920_7');
    const css2=await text('./css/components/major-card-contract.css?v=3920_7');
    checks.push({name:'RESULT-UI-001：结果区视觉 CSS 存在',ok:css1.length>1200 && css2.length>1200,detail:`${css1.length}/${css2.length}`});
    checks.push({name:'RESULT-UI-002：结果区使用统一外壳',ok:/ln-result-section/.test(html),detail:'resultsPanel 已接入 ln-result-section'});
    checks.push({name:'RESULT-UI-003：不再主动输出大号当前查看卡片',ok:!/result-band-current[\s\S]*当前查看[\s\S]*<strong>/.test(css1),detail:'当前说明改为轻提示行'});
    checks.push({name:'RESULT-UI-004：分段查看使用 segment tabs',ok:/result-band-options/.test(css1)&&/grid-template-columns: repeat\(3/.test(css1),detail:'PC 三段横向切换'});
    checks.push({name:'RESULT-UI-005：Android 分段支持横向滚动',ok:/overflow-x: auto/.test(css1)&&/scroll-snap/.test(css1),detail:'移动端横滑'});
    checks.push({name:'RESULT-UI-006：特殊项目提示为轻提示条',ok:/results-special-project-note/.test(css1)&&/result-assist-line/.test(css1),detail:'轻提示条样式存在'});
    checks.push({name:'RESULT-UI-007：专业卡片存在快速判断区',ok:/meta-pills/.test(css2)&&/grid-template-columns: repeat\(4/.test(css2),detail:'PC 关键数据四列'});
  }catch(e){checks.push({name:'RESULT-UI：结果区视觉检查',ok:false,detail:String(e)})}
  return checks;
}
...
checks.push({name:'SPECIAL-004：active CSS 包含特殊项目样式',ok:(manifest.cssEntry||[]).includes('css/components/special-project-filter.css'),detail:(manifest.cssEntry||[]).includes('css/components/special-project-filter.css')?'特殊项目样式已纳入 active CSS':'active CSS 缺少 special-project-filter.css'});
...
checks.push({name:'POOL-MOBILE-001：Android 不显示右下固定自选入口',ok:poolMobileCss.includes('.selection-pool-shell.pool-entry-direct-shell') && poolMobileCss.includes('> .pool-entry-direct'),detail:'手机端保留 toast shell、隐藏桌面固定入口'});
...
checks.push({name:'POOL-MOBILE-003：气泡包含“去整理”链接',ok:poolControllerText.includes('>去整理</a>'),detail:poolControllerText.includes('>去整理</a>')?'气泡包含跳转入口':'缺少去整理入口'});
...
checks.push({name:'POOL-MOBILE-005：结果区自选状态行不是 sticky',ok:poolMobileCss.includes('position: static !important') && !poolMobileCss.includes('position: sticky'),detail:poolMobileCss.includes('position: static !important')?'自选状态行已降为静态轻提示':'仍可能 sticky 遮挡结果'});
...
document.getElementById('runSelfCheck')?.addEventListener('click',run);run();console.info('v3.9.20.7 结果区视觉契约自测：active manifest、import、结果区视觉、CSS、文案坏值检查已启用。');
