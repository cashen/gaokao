...
async function runResultUiChecks(){
  ...
  try{
    ...
    const cssResult=await text('./css/components/result-section-contract.css?v=3920_8');
    const cssCard=await text('./css/components/major-card-contract.css?v=3920_8');
    const cssBand=await text('./css/components/band-selector-contract.css?v=3920_8');
    const cssColor=await text('./css/core/color-system.css?v=3920_8');
    const cssTheme=await text('./css/components/post-exam-calm-theme.css?v=3920_8');
    const bandRender=await text('./js/feature/score-bands/render.js?v=3920_8');
...
document.getElementById('runSelfCheck')?.addEventListener('click',run);run();console.info('v3.9.20.8 考后松弛色系与 Android 分段契约自测已启用。');
