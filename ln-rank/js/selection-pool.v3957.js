...
import { requestPathAnalysis } from './feature/selection-pool/path-analysis-api.v3957.js';
import { createSelectionPoolFeishuReport } from './feature/selection-pool/feishu-selection-report-api.v3955.js';

const SCORE_KEY = 'lnRank.selectionPool.candidateScore.v3956';
...
async function runAnalysis() {
  ...
  try {
    ...
    if (button) { button.disabled = !getPoolItems().length; button.textContent = '检查当前排序与AI解读'; }
    renderAll();
  }
}
...
async function sendFeishu({ withAnalysis = false } = {}) {
  ...
  try {
    ...
    if (btn) { btn.disabled = !getPoolItems().length; btn.textContent = withAnalysis ? '发送AI诊断报告到飞书' : '发送当前排序到飞书'; }
    renderAll();
  }
}
