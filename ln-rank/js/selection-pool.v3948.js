...
} from './feature/selection-pool/selection-pool-store.v3948.js';
import { requestPathAnalysis } from './feature/selection-pool/path-analysis-api.v3948.js';
import { createSelectionPoolFeishuReport } from './feature/selection-pool/feishu-selection-report-api.v3948.js';

const SCORE_KEY = 'lnRank.selectionPool.candidateScore.v3948';
const RANK_KEY = 'lnRank.selectionPool.candidateRank.v3948';
...
const LEGACY_RANK_KEYS = ['lnRank.selectionPool.candidateRank.v3947', 'lnRank.selectionPool.candidateRank.v3946', 'lnRank.selectionPool.candidateRank.v3945', 'lnRank.selectionPool.candidateRank'];
const LONG_PRESS_MS = 220;
const DRAG_MOVE_TOLERANCE = 7;
let currentAnalysis = null;
let currentAnalysisSignature = '';
let currentAnalysisScore = '';
let currentAnalysisRank = '';
...
function loadRank() {
  const raw = localStorage.getItem(RANK_KEY) || LEGACY_RANK_KEYS.map(k => localStorage.getItem(k)).find(Boolean) || '';
  const input = $('pathCandidateRank');
  if (input && raw) input.value = raw;
  return parseNumText(raw);
}

function readRankFromInput() {
  const input = $('pathCandidateRank');
  const raw = input ? input.value.trim() : '';
  const rank = parseNumText(raw);
  if (rank) localStorage.setItem(RANK_KEY, String(rank));
  if (!rank && input && raw === '') localStorage.removeItem(RANK_KEY);
  return rank;
}
...
function analysisIsFresh(items = getPoolItems(), score = readScoreFromInput() || loadScore(), rank = readRankFromInput() || loadRank()) {
  return Boolean(currentAnalysis)
    ...
    && String(currentAnalysisRank || '') === String(rank || '');
}

function attachAnalysisFreshness(analysis, items, score, rank = readRankFromInput() || loadRank()) {
  currentAnalysis = analysis;
  currentAnalysisSignature = getPoolOrderSignature(items);
  currentAnalysisScore = String(score || '');
  currentAnalysisRank = String(rank || '');
  if (currentAnalysis && typeof currentAnalysis === 'object') {
    ...
    currentAnalysis.candidateRank = rank || null;
  }
}
...
async function runAnalysis() {
  ...
  const rank = readRankFromInput() || loadRank();
  const signature = getPoolOrderSignature(items);
  if (button) { button.disabled = true; button.textContent = '检查中…'; }
  try {
    const analysis = await requestPathAnalysis({ items, candidateScore: score, candidateRank: rank, orderSignature: signature });
    if (getPoolOrderSignature(getPoolItems()) === signature && String(readScoreFromInput() || loadScore() || '') === String(score || '') && String(readRankFromInput() || loadRank() || '') === String(rank || '')) {
      attachAnalysisFreshness(analysis, items, score, rank);
    } else {
      currentAnalysis = null;
      currentAnalysisSignature = '';
      currentAnalysisScore = '';
...
async function sendFeishu({ withAnalysis = false } = {}) {
  ...
  try {
    ...
    if (withAnalysis && !analysisIsFresh(itemsForReport, score, rank)) {
      const signature = getPoolOrderSignature(itemsForReport);
      analysis = await requestPathAnalysis({ items: itemsForReport, candidateScore: score, candidateRank: rank, orderSignature: signature });
      attachAnalysisFreshness(analysis, itemsForReport, score, rank);
    }
...
function bind() {
  ...
  $('pathCandidateRank')?.addEventListener('input', () => {
    readRankFromInput();
    if (currentAnalysis) {
      currentAnalysis = null;
      currentAnalysisSignature = '';
