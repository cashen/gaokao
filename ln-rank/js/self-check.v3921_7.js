...
checks.push({name:'REPORT-FLOW-009：飞书失败态有人话备用方案',ok:(await text('./js/selection-pool.v3921_7.js').catch(e=>'')).includes('飞书报告暂时没生成成功，可以先复制文字版保存。'),detail:'失败态提供复制文字版，不用冰冷报错'});
...
const trendJs=await text('./js/major-trend-render.v3921_7.js').catch(e=>'');
...
checks.push({name:'TREND-006：趋势 import 版本统一',ok:trendIndex.includes('v=3921_6')&&trendRules.includes('v=3921_6'),detail:'trend/index.js 与 rules.js 内部 import 使用 v3921_7'});
...
document.getElementById('runSelfCheck')?.addEventListener('click',run);run();console.info('v3.9.21.7 家长报告 UI 精修自测已启用。');
