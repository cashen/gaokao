/* v3.9.49.0 active-assets driven self check: version + report contract + global color token + responsive contract + scoped CSS gate */
const VERSION = 'v3.9.49.0';
const ASSET = 'v3949_0';
const ASSET_QUERY = '3949_0';
...
function hasWrongActiveQuery(text){return /\?v=(?!3949_0\b)[0-9A-Za-z_.-]+/.test(text)}
...
async function checkSelfCheckApi(){
  const data = await fetchJson('/api/ln-rank-self-check?v=3949_0');
  const checks=[];
...
async function main(){
 const checks=[];
 try{
  const assets=await fetchJson('/ln-rank/active-assets.json?v=3949_0');
  const meta=await fetchJson('/ln-rank/release-meta.json?v=3949_0');
  const manifest=await fetchJson('/ln-rank/module-manifest.json?v=3949_0');
  ...
  for(const rel of [...assets.jsEntry,...assets.cssEntry]){
    ...
    if(rel.endsWith('.js')) checks.push({name:`JS query 统一：${rel}`,ok:!hasWrongActiveQuery(text),detail:'active 文件内 import/export query 应统一 ?v=3949_0'});
    if(rel.endsWith('.css')){
  ...
  const contract=await fetchText('/ln-rank/js/domain/version-contract.js?v=3949_0');
  checks.push({name:'前端 version-contract 同步',ok:contract.includes(VERSION)&&contract.includes('3949_0')&&contract.includes(meta.release),detail:'display / asset / release 应与 release-meta 一致'});
  const intent=await fetchText('/ln-rank/docs/product-intent-map.md?v=3949_0');
