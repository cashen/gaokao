...
export const TRAJECTORY_COPY = {
  ...
  boundary: '该提示不代表录取优势，也不代表就业保证，只帮助理解专业在该校可能面对的学习、实习和行业场景。'
};

export const LIAONING_MAJOR_TRAJECTORY_CHAINS = [
  {
    school: '沈阳农业大学', schoolAliases: ['沈农', '沈阳农大'], tier: 'A', trajectoryName: '农机与智能农业装备', cardShort: '农机装备', selectionShort: '农机装备｜农机 / 智能装备',
  ...
  {
    school: '大连交通大学', schoolAliases: ['大连交大'], tier: 'S', trajectoryName: '轨道交通装备', cardShort: '轨道交通装备', selectionShort: '轨道交通装备｜中车 / 轨道装备',
  ...
  {
    school: '沈阳化工大学', schoolAliases: ['沈化'], tier: 'A', trajectoryName: '化工过程装备', cardShort: '化工过程装备', selectionShort: '化工装备｜装置 / 过程控制',
  ...
  {
    school: '沈阳工业大学', schoolAliases: ['沈工大'], tier: 'S', trajectoryName: '电机电器与装备制造', cardShort: '装备制造', selectionShort: '装备制造｜电机 / 工业控制',
  ...
  {
    school: '辽宁科技大学', schoolAliases: ['辽科大'], tier: 'S', trajectoryName: '冶金材料与工业装备', cardShort: '冶金工业装备', selectionShort: '冶金工业装备｜钢铁 / 自动化',
  ...
  {
    school: '沈阳建筑大学', schoolAliases: ['沈建大'], tier: 'S', trajectoryName: '建筑土木市政', cardShort: '建筑土木市政', selectionShort: '建筑土木市政｜工程 / 市政',
  ...
  {
    school: '辽宁石油化工大学', schoolAliases: ['辽石化', '辽宁石化'], tier: 'S', trajectoryName: '石油化工储运与过程控制', cardShort: '石油化工储运', selectionShort: '石油化工储运｜储运 / 过程控制',
  ...
  {
    school: '沈阳药科大学', schoolAliases: ['沈药'], tier: 'S', trajectoryName: '药学制药与质量注册', cardShort: '药学制药', selectionShort: '药学制药｜质量 / 注册',
...
function matchMajorEntry(entry, baseMajorName, catalogCode) {
  const name = normalizeMajorName(entry.name);
  const code = normalizeCatalogCode(entry.catalogCode);
  const nameHit = name && (baseMajorName === name || baseMajorName.includes(name) || name.includes(baseMajorName));
  const codeHit = code && catalogCode && code === catalogCode;
  ...
  if (codeHit) return { ...entry, matchBy: 'catalogCode', confidence: 'catalogCode' };
  return null;
}
function matchMajorList(list = [], baseMajorName, catalogCode) {
  for (const entry of list) {
    const hit = matchMajorEntry(entry, baseMajorName, catalogCode);
    if (hit) return hit;
  }
  return null;
}
...
export function matchLiaoningMajorTrajectory(record = {}) {
  ...
  for (const rule of LIAONING_MAJOR_TRAJECTORY_CHAINS) {
    ...
    const hit = matchMajorList(rule.matchMajors, baseMajorName, catalogCode);
