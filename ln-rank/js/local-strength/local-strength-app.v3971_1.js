import { CURRENT_RELEASE } from '../../../shared/resources/release/current-release.js?v=3971_1';
...
function renderStats() {
  ...
  if (compact) compact.innerHTML = `<b>${fmt(meta.localAdmissionSchoolCount)}</b>所省内院校 · <b>${fmt(meta.matchedSchoolCount)}</b>所有背景专业 · <b>${fmt(meta.matchedRecordCount)}</b>条记录`;
  const coverageHtml = meta.completeEvaluation
  ...
  const mobileCoverage = $('[data-coverage-note-mobile]');
  if (mobileCoverage) mobileCoverage.innerHTML = coverageHtml;
}
...
function renderOptions() {
  ...
  $('#scoreBandButtons').innerHTML = state.scoreBands.map(item => `<button type="button" class="ls-chip" data-score-band="${esc(item.key)}" data-min="${item.min}" data-max="${item.max}" aria-pressed="false">${esc(item.label)}</button>`).join('');
}
...
function syncControls() {
  ...
  const scoreBandDisclosure = $('#scoreBandDisclosure');
  if (scoreBandDisclosure && !matchMedia('(max-width: 767px)').matches) scoreBandDisclosure.open = true;
  const scoreContext = $('#scoreContext');
  if (scoreContext) { scoreContext.hidden = !band; scoreContext.textContent = band ? `当前查看：${band.label}` : ''; }
  const scoreError = $('#scoreError');
  if (scoreError) { scoreError.hidden = !state.scoreValidationMessage; scoreError.textContent = state.scoreValidationMessage; }
  const filterShell = $('[data-filter-shell]');
  const canFilter = state.view === 'list_all' || state.hasLoadedResults;
  filterShell.hidden = !canFilter;
  const count = filterCountForCurrentView();
  const summary = $('[data-filter-summary]');
  if (summary) summary.textContent = count ? `筛选 · ${count}` : '更多筛选';
...
function evidenceClass(label) { if (label === '学校主线') return 'is-primary'; if (label === '学校相关') return 'is-secondary'; return 'is-clue'; }
...
function recordHtml(record) {
  ...
  const project = Array.isArray(record.projectTags) && record.projectTags.length ? `<div class="ls-project-tags">${record.projectTags.map(tag => `<span>${esc(tag)}</span>`).join('')}</div>` : '';
  ...
  return `<article class="ls-record" data-record-id="${esc(record.id)}">
    <div class="ls-record-main">
      <div class="ls-record-identity"><p class="ls-school">${esc(record.school)}</p><h3>${esc(record.major)}</h3><p class="ls-location">${esc(record.displayLocation || record.city || '辽宁')} ${record.natureLabel ? `· ${esc(record.natureLabel)}` : ''}</p></div>
      <div class="ls-position"><span><small>2026最低投档分</small><b>${fmt(record.score2026)}分</b></span><span><small>对应累计位次</small><b>${fmt(record.rank2026)}</b></span></div>
      <div class="ls-background"><span class="ls-evidence ${evidenceClass(background.evidenceLabel)}">${esc(background.evidenceLabel || '背景提示')}</span><b>${esc(background.direction || '学校专业背景')}</b><p>${esc(background.note || '该专业与学校办学或学科背景存在可复核连接，建议继续查看培养方案和招生章程。')}</p></div>
    </div>
    ${project}
    <div class="ls-record-actions">
      <details class="ls-evidence-details"><summary>查看依据与核验项</summary><div class="ls-detail-body">
...
function emptyHtml(data) {
  if (state.view === 'school' && data.schoolStatus) {
    ...
    if (school.recordCount2026 > 0 && school.matchedRecordCount === 0) return `<div class="ls-empty"><h3>${esc(school.officialName)}当前没有可公开的背景专业记录</h3><p>已检查该校 ${fmt(school.recordCount2026)} 条辽宁2026物理类投档专业，但当前背景证据不足以触发前台提示。这不代表学校没有优势专业。</p><a class="ui-button ui-button--secondary" href="/ln-rank/?school=${encodeURIComponent(school.officialName)}">查看该校全部在辽专业</a></div>`;
  }
...
function bindPaginationButtons(root) {
  root.querySelectorAll('[data-page]').forEach(button => button.addEventListener('click', () => { state.page = Number(button.dataset.page); writeUrl(); loadResults({ scroll:true }); }));
}
function renderPagination(page) {
  ...
  if (matchMedia('(max-width: 767px)').matches) {
    ...
    bindPaginationButtons(root); return;
  }
  const start = Math.max(1,page.page - 2); const end = Math.min(page.pageCount,page.page + 2); const numbers = [];
  for (let number = start; number <= end; number += 1) numbers.push(number);
  root.innerHTML = `<button type="button" data-page="${page.page - 1}" ${page.hasPrevious ? '' : 'disabled'}>上一页</button>${start > 1 ? '<span>…</span>' : ''}${numbers.map(number => `<button type="button" data-page="${number}" class="${number === page.page ? 'is-active' : ''}" aria-current="${number === page.page ? 'page' : 'false'}">${number}</button>`).join('')}${end < page.pageCount ? '<span>…</span>' : ''}<button type="button" data-page="${page.page + 1}" ${page.hasNext ? '' : 'disabled'}>下一页</button>`;
...
function resultParams() {
  ...
  if (state.view === 'school') { if (state.schoolView.school) params.set('school',state.schoolView.school); }
  else if (state.view === 'score') { const band = activeScoreBand(); if (band) { params.set('minScore',String(band.min)); params.set('maxScore',String(band.max)); } }
  else { if (state.listView.q) params.set('q',state.listView.q); if (state.listView.minScore) params.set('minScore',state.listView.minScore); if (state.listView.maxScore) params.set('maxScore',state.listView.maxScore); }
  if (state.commonFilters.direction) params.set('direction',state.commonFilters.direction);
  if (state.commonFilters.evidence) params.set('evidence',state.commonFilters.evidence);
  if (state.commonFilters.city) params.set('city',state.commonFilters.city);
  if (state.commonFilters.projectMode !== 'all') params.set('projectMode',state.commonFilters.projectMode);
...
function renderResultContext(data) {
  ...
  let title = '全部背景专业'; let desc = '按2026最低投档分从高到低排列。';
  if (state.view === 'school') { title = state.schoolView.school || '按学校查询'; desc = data.schoolStatus ? `该校2026物理类投档专业 ${fmt(data.schoolStatus.recordCount2026)} 条，背景专业 ${fmt(data.schoolStatus.matchedRecordCount)} 条。` : '输入学校后查看该校背景专业。'; }
  else if (state.view === 'score') { const band = activeScoreBand(); title = band ? `${band.label}的省内背景专业` : '按分数位置查看'; desc = band ? `显示2026历史最低投档分位于${band.label}的记录。` : '输入分数或选择一个分数段。'; }
...
function currentViewIsRunnable() { if (state.view === 'list_all') return true; if (state.view === 'school') return Boolean(state.schoolView.school); return Boolean(activeScoreBand()); }
...
async function loadResults({ scroll = false } = {}) {
  ...
  state.loading = true; state.scoreValidationMessage = ''; $('#resultsPanel').classList.remove('is-idle'); $('#records').innerHTML = '<div class="ls-loading">正在整理符合条件的背景专业…</div>';
  try {
    const data = await getJson(`${API}?${resultParams().toString()}`);
    state.hasLoadedResults = true; renderResultContext(data); $('#records').innerHTML = data.records?.length ? data.records.map(recordHtml).join('') : emptyHtml(data); renderPagination(data.page); syncControls();
    if (scroll) $('#resultsPanel').scrollIntoView({ behavior:'smooth', block:'start' });
  } catch (error) {
    state.hasLoadedResults = false; $('#records').innerHTML = `<div class="ls-empty is-error"><h3>目录暂时没有读取成功</h3><p>${esc(error.message)}</p><button type="button" class="ui-button ui-button--secondary" data-retry>重新加载</button></div>`; $('[data-retry]')?.addEventListener('click',() => loadResults());
  } finally { state.loading = false; }
}

function resetFilters() {
  state.listView.q = ''; state.listView.minScore = ''; state.listView.maxScore = ''; state.commonFilters.direction = ''; state.commonFilters.evidence = ''; state.commonFilters.city = ''; state.commonFilters.projectMode = 'all'; state.page = 1; syncControls(); writeUrl(); loadResults();
}
function applyCurrentFilters() {
  state.listView.q = $('#queryInput').value.trim(); state.commonFilters.direction = $('#directionFilter').value; state.commonFilters.evidence = $('#evidenceFilter').value; state.commonFilters.city = $('#cityFilter').value.trim(); state.commonFilters.projectMode = $('#projectFilter').value; state.listView.minScore = $('#minScoreFilter').value.trim(); state.listView.maxScore = $('#maxScoreFilter').value.trim(); state.page = 1; writeUrl(); loadResults({ scroll:true });
}
function removeFilter(key) {
  if (key === 'q') state.listView.q = ''; if (key === 'direction') state.commonFilters.direction = ''; if (key === 'evidence') state.commonFilters.evidence = ''; if (key === 'city') state.commonFilters.city = ''; if (key === 'projectMode') state.commonFilters.projectMode = 'all'; if (key === 'scoreRange') { state.listView.minScore = ''; state.listView.maxScore = ''; }
  state.page = 1; syncControls(); writeUrl(); loadResults();
}
function submitScore() {
  const value = $('#scoreInput').value.trim(); const score = Number(value);
  if (!Number.isFinite(score) || score < 344 || score > 750) { state.scoreValidationMessage = '请输入344—750之间的参考分数。'; syncControls(); $('#scoreInput').focus(); return; }
  state.scoreView.score = value; state.scoreView.band = ''; state.scoreValidationMessage = ''; state.page = 1; writeUrl(); syncControls(); loadResults({ scroll:true });
}
function selectScoreBand(button) {
  state.scoreView.score = ''; state.scoreView.band = button.dataset.scoreBand; state.scoreValidationMessage = ''; state.page = 1; const disclosure = $('#scoreBandDisclosure'); if (matchMedia('(max-width: 767px)').matches && disclosure) disclosure.open = false; writeUrl(); syncControls(); loadResults({ scroll:true });
}
function switchView(view) { state.view = view; state.page = 1; state.hasLoadedResults = false; state.scoreValidationMessage = ''; syncControls(); writeUrl(); loadResults(); }
...
function bind() {
  $$('.ls-view-tab').forEach(button => button.addEventListener('click',() => switchView(button.dataset.view))); bindTabsKeyboard();
  ...
  $('#querySubmit').addEventListener('click',applyCurrentFilters); $('#applyFilters').addEventListener('click',applyCurrentFilters); $('#resetFilters').addEventListener('click',resetFilters); $('#queryInput').addEventListener('keydown',event => { if (event.key === 'Enter') $('#querySubmit').click(); });
  ...
  let resizeTimer = 0;
  window.addEventListener('resize',() => { clearTimeout(resizeTimer); resizeTimer = setTimeout(() => { const nextPageSize = pageSizeForViewport(); if (nextPageSize === state.pageSize) return; const firstRecordIndex = Math.max(0,(state.page - 1) * state.pageSize); state.pageSize = nextPageSize; state.page = Math.floor(firstRecordIndex / nextPageSize) + 1; writeUrl(true); if (currentViewIsRunnable()) loadResults(); },180); });
}

async function boot() {
  readUrl(); setRuntime('loading'); bind();
  try {
    const data = await getJson(`${API}?mode=meta`); state.meta = data.meta; state.schools = data.schools || []; state.directions = data.directions || []; state.scoreBands = data.scoreBands || []; renderStats(); renderOptions(); syncControls(); setRuntime('ready'); await loadResults(); document.body.dataset.release = CURRENT_RELEASE.display;
  } catch (error) { setRuntime('error',error.message); $('#resultsPanel').classList.remove('is-idle'); $('#records').innerHTML = `<div class="ls-empty is-error"><h3>全量目录初始化失败</h3><p>${esc(error.message)}</p></div>`; }
}
