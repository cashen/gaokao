...
function loadingHtml(text = '正在读取 2025 历史数据…') { return `<div class="lm-empty is-loading"><b>${esc(text)}</b><p>如果网络较慢，可以稍后重试。</p></div>`; }
...
async function queryScore() {
  ...
  if (!Number.isFinite(score) || score <= 0) { root.innerHTML = emptyHtml('请先输入孩子分数', '例如 666。'); return; }
