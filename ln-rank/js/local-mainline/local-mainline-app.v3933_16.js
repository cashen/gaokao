import { fetchApiJson, formatApiErrorForHuman, apiErrorDiagnosticHtml } from '../shared/api-client.js?v=3933_16';
...
async function queryScore() {
  ...
  try {
    ...
    root.innerHTML = `<div class="lm-detail-head"><h2>按孩子当前位置查看${esc(label)}</h2><p>${esc(data.boundary || '这里不是录取概率，只是按 2025 历史最低分和省内专业背景整理，方便家庭先讨论。')}</p></div>
