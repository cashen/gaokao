import { formatHistoricalEvidenceText } from '../../../shared/resources/exam/historical-score-rank-contract.js?v=3967_0';
...
function scoreNum(record) { const n = Number(record?.score2026); return Number.isFinite(n) ? n : -1; }
function rankNum(record) { const n = Number(record?.rank2026); return Number.isFinite(n) && n > 0 ? n : Number.MAX_SAFE_INTEGER; }
...
function historyLine(r) {
  const text = formatHistoricalEvidenceText(r, { years: [2025, 2024], prefix: false, empty: '' });
  return text ? `<div class="lm-history">历史同口径参考：${esc(text)}</div>` : '';
}
...
function miniRecordRow(r, score = '') {
  const ev = r.localMainline || {};
  const level = r.localMainlineRaw?.level || (ev.label === '本校方向' ? 'primary' : ev.label === '本校相关' ? 'secondary' : 'trajectory');
  const scorePart = Number.isFinite(Number(r.score2026)) ? `｜2025历史参考 ${fmt(r.score2026)}分 / ${fmt(r.rank2026)}位` : '';
  const scoreParam = score ? `score=${encodeURIComponent(score)}&` : '';
  const link = `/ln-rank/?${scoreParam}school=${encodeURIComponent(r.school)}&major=${encodeURIComponent(r.major)}&source=local-mainline`;
...
function fallbackScoreGroups(records = [], score) {
  const candidate = Number(score);
  const near = records.filter(r => Number(r.score2026) >= candidate - 10 && Number(r.score2026) <= candidate);
  const upper = records.filter(r => Number(r.score2026) > candidate && Number(r.score2026) <= candidate + 10);
  const lower = records.filter(r => Number(r.score2026) >= candidate - 25 && Number(r.score2026) < candidate - 10);
