import { fetchApiJson, formatApiErrorForHuman, apiErrorDiagnosticHtml } from '../shared/api-client.js?v=3933_12';
...
function renderScoreSection(title, desc, records, score, empty) {
  return `<section class="lm-score-section"><h3>${esc(title)}</h3><p class="lm-score-section-desc">${esc(desc)}</p>${records.length ? sortRecordsByCloseness(records, score).slice(0, 40).map(r => recordCard(r, score)).join('') : emptyHtml(empty, '可以调整查看方式，或回到专业初选工具扩大范围。')}</section>`;
}
async function queryScore() {
  ...
  try {
    ...
    root.innerHTML = `<div class="lm-detail-head"><h2>2025 历史数据中，接近 ${fmt(score)} 分的${esc(label)}</h2><p>${esc(data.boundary || '这里不是录取概率，只是按 2025 历史最低分和省内专业背景整理，方便家庭先讨论。')}</p></div>
      ${renderScoreSection('接近孩子分数', `${fmt(score - 10)}–${fmt(score)} 分之间，优先看离孩子分数近、同时有省内背景的专业。`, grouped.near || [], score, '暂无接近孩子分数的明确条目')}
