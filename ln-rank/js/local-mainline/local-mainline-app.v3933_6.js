import { fetchApiJson, formatApiErrorForHuman, apiErrorDiagnosticHtml } from '../shared/api-client.js?v=3933_6';
...
async function loadMeta() {
  try {
    const data = await fetchApiJson('/api/local-mainline?mode=meta', { userMessage: '学校主线索引暂时没有读取成功，正在改用本地摘要。' });
...
async function showSchoolDetail(school) {
  ...
  try {
    const data = await fetchApiJson(`/api/local-mainline?mode=school&school=${encodeURIComponent(school)}&max=240`, { userMessage: '该校专业主线数据暂时没有读取成功。可以稍后重试。' });
