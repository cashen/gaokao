...
async function loadMeta() {
  try {
    const data = await fetchJson('/api/local-mainline?mode=meta');
    state.meta = data.index || null;
    state.schools = data.schools || state.meta?.allSchools || [];
    state.majors = data.majors || state.meta?.majors || [];
  } catch (error) {
    const fallback = await fetchJson('/ln-rank/data/local-mainline/local-mainline-index.generated.json');
    state.meta = fallback;
...
function schoolCard(school) {
  ...
  return `<article class="lm-card ${school.hasMainline ? '' : 'is-muted'}">
    <div class="lm-card-top"><div><h3>${esc(school.name)}</h3><p>${esc(school.city || '辽宁')}</p></div>${school.hasMainline ? '<span class="lm-pill primary">有主线证据</span>' : '<span class="lm-pill muted">待补证据</span>'}</div>
...
function majorCard(major) {
  return `<article class="lm-card">
    <div class="lm-card-top"><div><h3>${esc(major.major)}</h3><p>省内 ${fmt(major.schoolCount)} 所学校有主线线索</p></div><span class="lm-pill primary">专业入口</span></div>
...
function recordCard(r, score = '') {
  ...
  return `<article class="lm-record-card">
    ...
    <div class="lm-evidence-row">${(ev.evidence || []).map(x => `<span class="lm-evidence-chip">${esc(x)}</span>`).join('') || '<span class="lm-evidence-chip">学校主线证据</span>'}</div>
...
async function showSchoolDetail(school) {
  ...
  try {
    const data = await fetchJson(`/api/local-mainline?mode=school&school=${encodeURIComponent(school)}&max=240`);
    root.innerHTML = `<div class="lm-detail-head"><h2>${esc(school)}：2025 历史数据中可讨论的主线专业</h2><p>按本校方向、本校相关和方向提醒查看。这里不是录取判断。</p></div>${data.records?.length ? data.records.map(r => recordCard(r)).join('') : emptyHtml('暂无 2025 分数条目', '当前知识库有学校主线，但 2025 专业数据里暂未匹配到明确专业。')}`;
    root.scrollIntoView({ behavior: 'smooth', block: 'start' });
  } catch (error) { root.innerHTML = emptyHtml('数据暂时没有读取成功', error.message || '请稍后重试。'); }
}
async function showMajorDetail(major) {
  ...
  root.innerHTML = loadingHtml();
  try {
    const data = await fetchJson(`/api/local-mainline?mode=major&major=${encodeURIComponent(major)}&max=240`);
    root.innerHTML = `<div class="lm-detail-head"><h2>${esc(major)}：省内学校差异</h2><p>同名专业在不同学校可能对应不同培养方向，先看硬数据，再看证据。</p></div>${data.records?.length ? data.records.map(r => recordCard(r)).join('') : emptyHtml('暂无明确匹配', '没有找到同时具备 2025 分数和学校主线证据的条目。')}`;
    root.scrollIntoView({ behavior: 'smooth', block: 'start' });
  } catch (error) { root.innerHTML = emptyHtml('数据暂时没有读取成功', error.message || '请稍后重试。'); }
}
async function queryScore() {
  ...
  try {
    const data = await fetchJson(`/api/local-mainline?mode=score&score=${encodeURIComponent(score)}&level=${encodeURIComponent(level)}&natureMode=${encodeURIComponent(natureMode)}&max=300`);
    ...
    root.innerHTML = `<div class="lm-detail-head"><h2>2025 年 ${fmt(score)} 分及以下，${esc(label)}专业</h2><p>${esc(data.boundary || '')}</p></div>
      ...
      <section class="lm-score-section"><h3>稍高一点可少量了解</h3>${upper.length ? upper.slice(0,40).map(r => recordCard(r, score)).join('') : '<p class="lm-muted">暂无 +10 分内的主线条目。</p>'}</section>`;
  } catch (error) { root.innerHTML = emptyHtml('数据暂时没有读取成功', error.message || '请稍后重试，或返回 /ln-rank/。'); }
}
function bind() {
