/* v3.9.33.3 active-assets driven self check */
...
function hasOldVersionQuery(text){return /\?v=(?!3933_1\b)[0-9A-Za-z_.-]+/.test(text)}
async function main(){
 const checks=[];
 try{
  const assets=await fetchJson('/ln-rank/active-assets.json?v=3933_3');
  checks.push({name:'active-assets 版本',ok:assets.version==='v3.9.33.3'&&assets.assetVersion==='v3933_1',detail:`${assets.version} / ${assets.assetVersion}`});
  for(const rel of [...assets.jsEntry,...assets.cssEntry]){
    const text=await fetchText(`/ln-rank/${rel}?v=3933_3`);
    checks.push({name:`资源存在：${rel}`,ok:text.length>80,detail:`${text.length} bytes`});
    if(rel.endsWith('.js')) checks.push({name:`JS query 统一：${rel}`,ok:!hasOldVersionQuery(text),detail:'active 文件内 import/export query 应统一 ?v=3933_3'});
  ...
  const indexHtml=await fetchText('/ln-rank/index.html?v=3933_3');
  const selectionHtml=await fetchText('/ln-rank/selection-pool.html?v=3933_3');
  checks.push({name:'页面底部版本一致',ok:/版本：v3\.9\.33/.test(indexHtml)&&/版本：v3\.9\.33/.test(selectionHtml),detail:'首页和自选页 footer 都应显示 v3.9.33.3'});
  const activeJs=await fetchText('/ln-rank/js/app.v3933_1.js?v=3933_3') + '\n' + await fetchText('/ln-rank/js/selection-pool.v3933_1.js?v=3933_3');
