...
const RUNTIME_VERSION = 'selection-runtime-v3966_0';
...
try {
  await import('./selection-pool-runtime.v3966_0.js?v=3966_0');
  ...
  document.dispatchEvent(new CustomEvent('gaokao:runtime-state', {
    detail: Object.freeze({ state: 'ready', version: RUNTIME_VERSION })
  }));
} catch (error) {
  ...
  document.dispatchEvent(new CustomEvent('gaokao:runtime-state', {
    detail: Object.freeze({ state: 'error', version: RUNTIME_VERSION })
  }));
  console.error('[selection-runtime-v3966_0] initialization failed', error);
}
