...
const BOTTOMLINE_STORAGE_KEY = 'lnRank.bottomLineMode.v3961';
...
function renderBottomLinePanel() {
  ...
  if (summary) {
    summary.textContent = visible
      ? `当前分数段建议先确认公办底线；已选“${bottomLineLabel(state.filters.bottomLineMode)}”。筛选只缩小结果，不改变查看范围。`
      : score ? '当前分数已离开公办底线提示区；600分左右的中外上探由AI诊断和报告解释。' : '输入分数后，特控线以下/边缘分段会显示公办底线。';
  }
}
