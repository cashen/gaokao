...
const SCORE_BAND_ORDER = new Map([
  ['344—449 分', 0],
  ['450—499 分', 1],
  ['500—549 分', 2],
  ['550—589 分', 3],
...
function scoreBandOrder(band) {
  const label = String(band?.label || '').replace(/\s+/g, ' ').trim();
  if (SCORE_BAND_ORDER.has(label)) return SCORE_BAND_ORDER.get(label);
  const firstScore = Number((label.match(/\d{3}/) || [])[0]);
  return Number.isFinite(firstScore) ? firstScore : Number.MAX_SAFE_INTEGER;
}
