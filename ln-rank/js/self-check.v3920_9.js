...
async function runResultUiChecks(){
  ...
  try{
    ...
    const cssResult=await text('./css/components/result-section-contract.css?v=3920_9');
    const cssCard=await text('./css/components/major-card-contract.css?v=3920_9');
    const cssBand=await text('./css/components/band-selector-contract.css?v=3920_9');
    const cssColor=await text('./css/core/color-system.css?v=3920_9');
    const cssTheme=await text('./css/components/post-exam-calm-theme.css?v=3920_9');
    const cssBreath=await text('./css/components/band-card-breathing-link.css?v=3920_9');
    const renderMajor=await text('./js/feature/major-pool/render.js?v=3920_9');
    const bandRender=await text('./js/feature/score-bands/render.js?v=3920_9');
    ...
    checks.push({name:'RESULT-UI-003：存在当前查看轻提示条',ok:/result-current-line/.test(cssResult)&&/result-current-detail/.test(bandRender),detail:'当前分段范围/数量进入说明条，不再靠移动端大卡承载'});
    checks.push({name:'RESULT-UI-004：特殊项目提示为轻提示条',ok:/results-special-project-note/.test(cssResult)&&/result-assist-line/.test(cssResult),detail:'特殊项目提示保持轻提示条'});
...
document.getElementById('runSelfCheck')?.addEventListener('click',run);run();console.info('v3.9.20.9 考后松弛色系与 Android 分段契约自测已启用。');
