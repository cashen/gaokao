import { SCHEMA_VERSION, FAMILY_STATUS, YEARS, canonicalAdmissionIdentity, normalizeSimulationChoice, normalizeHistory, buildSimulationChoiceHref, parseSimulationChoiceHref, writeSimulationChoiceHref, projectIdentity } from '../../shared/resources/simulation/simulation-choice-contract.v001.js';
...
function blankChoice(order = 1) {
  return normalizeSimulationChoice({
    id: `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`,
    order,
    state: 'empty',
    familyStatus: '还没决定',
    familyNote: '',
    threeYearHistory: { status: 'idle', years: {} },
    source: { module: 'simulation', entry: 'manual', capturedAt: now() },
...
function normalizeChoice(source, order, fallbackSource = 'simulation') {
  ...
  return {
    ...value,
    order,
    manualCheck: { ...emptyManual(), ...(source?.manualCheck || {}) }
  };
}
...
function load() {
  ...
  return {
    ...
    version: 3,
    schemaVersion: SCHEMA_VERSION,
    scores: { ...base.scores, ...(source.scores || {}) },
    volunteers: source.volunteers.length
      ? source.volunteers.map((item, index) => normalizeChoice(item, index + 1))
      : [blankChoice(1)]
  };
}
...
function save(notify = true) {
  runtime.state = {
    ...
    version: 3,
    schemaVersion: SCHEMA_VERSION,
    volunteers: runtime.state.volunteers.map((item, index) => ({
      ...normalizeChoice(item, index + 1),
      manualCheck: { ...emptyManual(), ...(item.manualCheck || {}) },
      updatedAt: now()
    }))
  };
...
function update(id, mutate, notify = true) {
  ...
  runtime.state = {
    ...runtime.state,
    volunteers: runtime.state.volunteers.map(item => String(item.id) === String(id) ? normalizeChoice(mutate({ ...item }), item.order) : item)
  };
  save(notify);
}
function toast(text) {
...
function historyText(item, year) {
  const value = item?.threeYearHistory?.years?.[year];
  if (!value) return '暂无对应投档记录';
  if (value.status === 'loading') return '正在查询…';
  if (value.status === 'error') return '暂时无法获取，请稍后重试';
...
function delta(item) {
  const admissionRank = Number(item?.threeYearHistory?.years?.[2026]?.rank);
...
function stepsHtml(item) {
  ...
  const majorEntered = Boolean(item.majorQuery);
  const majorDone = Boolean(item.majorRecordId && item.major && item.majorCode2026);
...
function historyHtml(item) {
  return `<section class="history-grid" aria-label="三年参考记录">${YEARS.map(year => `<div class="history-cell"><span>${year}</span><strong>${historyText(item, year)}</strong></div>`).join('')}<div class="history-cell history-delta"><span>相对当前</span><strong>${esc(delta(item))}</strong></div></section>`;
}

function cardHtml(item, index, total) {
  const [kind, label] = rowStatus(item);
  const expanded = runtime.expanded.has(String(item.id));
  const confirmed = item.confirmedSchool && normalizedKey(item.confirmedSchool) === normalizedKey(item.school);
  const majorLabel = item.major ? `${item.major} · 招生代码 ${item.majorCode2026}` : '选学校后，这里只显示该校实际招生专业';
  const identity = canonicalAdmissionIdentity(item);
...
function render() {
  ...
  if (name && document.activeElement !== name) name.value = s.studentName || '';
  if (subject && document.activeElement !== subject) subject.value = s.subjectTrack || '辽宁物理类（物化生）';
  if (score && document.activeElement !== score) score.value = s.totalScore || '';
  if (rank) rank.textContent = rankText(s.rank);
  if (source) source.textContent = s.rank ? '2026辽宁物理类官方成绩统计口径' : '输入总分后自动换算';
  const list = document.querySelector('#wbRows');
  if (list) { list.innerHTML = s.volunteers.map((item, index) => cardHtml(item, index, s.volunteers.length)).join(''); list.querySelectorAll('[data-family-note]').forEach(fitFamilyNote); }
  const empty = document.querySelector('#wbEmpty');
  if (empty) empty.hidden = s.volunteers.length > 0;
  const status = document.querySelector('#wbStatus');
  if (status) status.textContent = s.volunteers.length ? '一条志愿只要确认学校、专业和参考记录，其他核对可以后补。' : '先加入一所已经考虑过的学校即可。';
  const summary = document.querySelector('#wbSummary');
  if (summary) { const count = s.volunteers.reduce((acc, item) => { const [kind] = rowStatus(item); if (kind === 'complete') acc.complete++; else if (kind === 'needs-check') acc.check++; else acc.incomplete++; return acc; }, { complete: 0, check: 0, incomplete: 0 }); summary.innerHTML = `<span>共 ${s.volunteers.length} 条</span><span>✓ ${count.complete} 条已对应</span><span>⚠ ${count.check} 条待确认</span><span>○ ${count.incomplete} 条待填写</span>`; }
}
...
function candidateHtml(kind, items, id) {
  ...
  return `<div class="candidate-title">${title}</div><div class="candidate-list">${items.map(item => `<button type="button" class="candidate" data-candidate-kind="${kind}" data-row-id="${esc(id)}" data-name="${esc(item.name)}" data-record-id="${esc(item.id || '')}" data-code="${esc(item.code || '')}"><span class="candidate-main"><strong>${esc(item.name)}</strong><small>${esc(item.meta || '')}</small></span>${kind === 'major' ? `<span class="candidate-history">${esc(item.historyText || '')}</span>` : ''}<span class="candidate-action">选择</span></button>`).join('')}</div>`;
}
...
function clearChoiceIdentity(id) {
  update(id, item => ({ ...item, confirmedSchool: '', major: '', majorCode2026: '', majorRecordId: '', standardMajorName: '', standardMajorCode: '', admissionProject: { ...item.admissionProject, schoolCode2026: item.schoolCode || '', majorCode2026: '', majorRecordId: '' }, canonicalAdmissionKey: '', threeYearHistory: normalizeHistory({ status: 'idle', years: {} }), state: item.school ? 'school-entered' : 'empty', error: '' }), false);
}

function clearUrl() {
  if (!globalThis.history?.replaceState) return;
  const current = new URL(location.href);
  ['school', 'schoolCode', 'majorName', 'majorCode2026', 'majorCode', 'majorRecordId', 'source', 'entry'].forEach(key => current.searchParams.delete(key));
  history.replaceState({ ...(history.state || {}), simulationChoice: null }, '', `${current.pathname}${current.search}${current.hash}`);
}
...
function onSchool(id, value) {
  ...
  const pending = runtime.pending.get(String(id));
  if (pending) pending.cancelled = true;
  update(id, item => ({ ...item, school: query, confirmedSchool: '', schoolCode: '', majorQuery: '', major: '', majorCode2026: '', majorRecordId: '', standardMajorName: '', standardMajorCode: '', admissionProject: {}, canonicalAdmissionKey: '', threeYearHistory: normalizeHistory({ status: 'idle', years: {} }), state: query ? 'school-entered' : 'empty', error: '' }), false);
  clearUrl();
  if (!query) { clearTimeout(runtime.timers.get(String(id))); slot(id, ''); helper(id, '输入学校名称，不需要等待页面刷新。'); return; }
  helper(id, '正在查找招生学校候选…');
  scheduleSchoolSearch(id, query);
}
...
async function queryMajor(id, query) {
  ...
  try {
    ...
    const codeQuery = queryKey.replace(/\D/g, '') ? queryKey : '';
    const candidates = records
      .filter(record => normalizedKey(record.school) === normalizedKey(latest.confirmedSchool))
      .map(record => {
        const name = norm(record.major);
        const code = norm(record.majorCode2026 || record.majorCode || '');
        const nameKey = normalizedKey(name);
        const codeKey = normalizedKey(code);
        const exactName = nameKey === queryKey;
        const prefixName = nameKey.startsWith(queryKey);
        const containName = nameKey.includes(queryKey);
        const exactCode = Boolean(codeQuery && codeKey === queryKey);
        const prefixCode = Boolean(codeQuery && codeKey.startsWith(queryKey));
        const historyCount = YEARS.filter(year => { const score = Number(record[`score${year}`]); const rank = Number(record[`rank${year}`]); return (Number.isFinite(score) && score > 0) || (Number.isFinite(rank) && rank > 0); }).length;
      ...
      .filter(item => item.name && item.id && item.score > 0)
      .sort((a, b) => b.score - a.score || a.name.length - b.name.length || a.name.localeCompare(b.name, 'zh-CN') || a.id.localeCompare(b.id, 'zh-CN'))
      .filter((item, index, list) => list.findIndex(other => other.id === item.id) === index)
      .slice(0, MAJOR_LIMIT);
    runtime.pending.set(`major-record:${id}`, { query, records: candidates });
    update(id, item => ({ ...item, state: 'major-results', threeYearHistory: normalizeHistory({ status: 'idle', years: {} }) }), false);
    slot(id, candidateHtml('major', candidates, id));
    helper(id, candidates.length ? `找到 ${candidates.length} 个该校实际招生专业，请按名称和招生代码选择。` : '暂未找到与这个输入对应的该校实际招生专业。', candidates.length ? 'neutral' : 'warn');
  } catch (error) {
    if (error?.name !== 'AbortError') {
      update(id, item => ({ ...item, state: 'history-error', error: `暂时无法获取，请稍后重试：${String(error?.message || '查询失败')}`, threeYearHistory: normalizeHistory({ status: 'error', years: {} }) }), false);
      slot(id, '');
      helper(id, '暂时无法获取，请稍后重试。没有把请求失败当成“无记录”。', 'warn');
    }
  } finally {
...
function yearFromRecord(record, year) {
  const score = Number(record?.[`score${year}`]);
  const rank = Number(record?.[`rank${year}`]);
  const hasScore = Number.isFinite(score) && score > 0;
  const hasRank = Number.isFinite(rank) && rank > 0;
  return { status: hasScore || hasRank ? 'history' : 'missing', score: hasScore ? score : null, rank: hasRank ? rank : null, recordId: norm(record?.id) };
}
function historyFromRecord(record) {
  const recordId = norm(record?.id);
  return { status: 'ready', recordId, years: Object.fromEntries(YEARS.map(year => [year, yearFromRecord(record, year)])) };
}
...
function applyMajorRecord(id, record, sourceEntry = 'manual') {
  ...
  const name = norm(record?.major);
  const code = norm(record?.majorCode2026 || record?.majorCode || '');
  if (!current || !recordId || !name || !code) { helper(id, '这条专业缺少招生名称、招生代码或记录编号，不能安全对应历史。', 'warn'); return false; }
  const normalized = normalizeSimulationChoice({
    ...current,
    school: norm(record.school || current.school),
    schoolCode: norm(record.schoolCode2026 || current.schoolCode),
    majorQuery: current.majorQuery || name,
    major: name,
    majorCode2026: code,
    majorRecordId: recordId,
    standardMajorName: norm(record.standardMajorName),
    standardMajorCode: norm(record.standardMajorCode),
    admissionProject: { ...projectIdentity(record), schoolCode2026: norm(record.schoolCode2026 || current.schoolCode), majorCode2026: code, majorRecordId: recordId },
    canonicalAdmissionKey: canonicalAdmissionIdentity({ majorRecordId: recordId, schoolCode: record.schoolCode2026 || current.schoolCode, majorCode2026: code }).value,
    threeYearHistory: historyFromRecord(record),
    state: 'record-confirmed',
    error: '',
    source: { module: current.source?.module || 'simulation', entry: sourceEntry, capturedAt: now() },
    updatedAt: now()
  }, { noteMax: NOTE_MAX });
  runtime.state = { ...runtime.state, volunteers: runtime.state.volunteers.map(item => String(item.id) === String(id) ? { ...normalized, manualCheck: { ...emptyManual(), ...(current.manualCheck || {}) } } : item) };
  save(true);
  slot(id, '');
  writeSimulationChoiceHref(normalized, { replace: true, noteMax: NOTE_MAX });
  const count = YEARS.filter(year => normalized.threeYearHistory.years[year].status === 'history').length;
  helper(id, `✓ 已确认：${normalized.school} · ${name} · 招生代码 ${code}。${count} 年有对应记录。`, 'ok');
  return true;
}

function selectSchoolCandidate(id, name, code = '') {
  const value = norm(name);
  if (!value || !row(id)) return;
  runtime.pending.get(String(id)) && (runtime.pending.get(String(id)).cancelled = true);
  runtime.controllers.get(String(id))?.abort();
  clearTimeout(runtime.timers.get(String(id)));
  update(id, item => ({ ...item, school: value, confirmedSchool: value, schoolCode: norm(code), majorQuery: '', major: '', majorCode2026: '', majorRecordId: '', standardMajorName: '', standardMajorCode: '', admissionProject: {}, canonicalAdmissionKey: '', threeYearHistory: normalizeHistory({ status: 'idle', years: {} }), state: 'school-confirmed', error: '' }));
  clearUrl();
...
async function loadExactInbound(id, link) {
  const current = row(id);
  if (!current || !link.majorRecordId || !link.major) return;
  ...
  try {
    const params = new URLSearchParams({ schoolKeyword: link.school, major: link.major, limit: '120', offset: '0' });
    const response = await fetch(`${HISTORY_API}?${params}`, { headers: { accept: 'application/json' }, signal: controller.signal });
    const data = await response.json();
    if (!response.ok || data?.ok === false) throw new Error(data?.message || `HTTP ${response.status}`);
    const exact = (Array.isArray(data.records) ? data.records : []).find(record => {
      if (norm(record.id) !== link.majorRecordId) return false;
      if (link.majorCode2026 && norm(record.majorCode2026) !== link.majorCode2026) return false;
      if (link.schoolCode && norm(record.schoolCode2026) !== link.schoolCode) return false;
      return normalizedKey(record.school) === normalizedKey(link.school) && norm(record.major) === link.major;
    });
    if (!exact) {
      update(id, item => ({ ...item, state: 'record-missing', error: '没有找到这条招生记录；页面没有用模糊专业替代。', threeYearHistory: normalizeHistory({ status: 'ready', recordId: link.majorRecordId, years: {} }, link.majorRecordId) }));
      helper(id, '没有找到这条具体招生记录；页面没有改用近似专业代替。请回原模块重新确认。', 'warn');
      return;
    }
    applyMajorRecord(id, { ...exact, schoolCode2026: exact.schoolCode2026 || link.schoolCode }, 'deep-link');
    toast('已按招生记录编号恢复这条志愿');
  } catch (error) {
    if (error?.name !== 'AbortError') {
      update(id, item => ({ ...item, state: 'history-error', error: `暂时无法获取，请稍后重试：${String(error?.message || '查询失败')}`, threeYearHistory: normalizeHistory({ status: 'error', years: {} }, link.majorRecordId) }), false);
      helper(id, '暂时无法获取，请稍后重试。深链身份已经保留，没有降级成模糊匹配。', 'warn');
    }
  } finally {
    if (runtime.controllers.get(key) === controller) runtime.controllers.delete(key);
    save(false);
...
function inbound() {
  const link = parseSimulationChoiceHref(location);
  if (!link.school) return;
  let target = link.majorRecordId ? runtime.state.volunteers.find(item => item.majorRecordId === link.majorRecordId) : null;
  if (!target) target = runtime.state.volunteers.find(item => !item.school && !item.majorQuery) || null;
  if (!target) {
    target = blankChoice(runtime.state.volunteers.length + 1);
    runtime.state = { ...runtime.state, volunteers: [...runtime.state.volunteers, target] };
  }
  const id = String(target.id);
  if (link.majorRecordId && link.major) {
    void loadExactInbound(id, link);
    return;
  }
  ...
  if (link.major) {
    slot(id, '');
    helper(id, link.schoolCode ? '已带入学校，请从该校实际招生专业中选择。' : '已带入学校，请先确认学校候选，再选择实际招生专业。');
    if (link.schoolCode) void queryMajor(id, link.major);
  }
}
...
function updateRank() {
  ...
  fetch(`${RANK_API}?score=${score}`, { headers: { accept: 'application/json' }, signal: controller.signal })
    ...
    .then(data => { if (String(runtime.state.totalScore) !== String(score)) return; runtime.state = { ...runtime.state, rank: data.rank ?? null }; save(false); if (rankElement) rankElement.textContent = rankText(runtime.state.rank); if (sourceElement) sourceElement.textContent = '2026辽宁物理类官方成绩统计口径'; })
...
function move(id, direction) { const list = [...runtime.state.volunteers]; const i = list.findIndex(item => String(item.id) === String(id)); const j = i + direction; if (i < 0 || j < 0 || j >= list.length) return; [list[i], list[j]] = [list[j], list[i]]; runtime.state = { ...runtime.state, volunteers: list.map((item, index) => ({ ...item, order: index + 1, updatedAt: now() })) }; save(); }
...
function bind() {
  ...
  document.body.addEventListener('click', event => {
    ...
    if (candidate) {
      ...
      if (candidate.dataset.candidateKind === 'school') selectSchoolCandidate(id, candidate.dataset.name, candidate.dataset.code || '');
      else {
        const cache = runtime.pending.get(`major-record:${id}`);
        const record = cache?.records?.find(item => norm(item.id) === norm(candidate.dataset.recordId));
        if (record) applyMajorRecord(id, record, 'manual'); else helper(id, '这条专业候选已过期，请重新输入专业。', 'warn');
      }
      return;
    }
    ...
    if (action) {
      ...
      if (type === 'detail') { runtime.expanded.has(String(id)) ? runtime.expanded.delete(String(id)) : runtime.expanded.add(String(id)); render(); }
      else if (type === 'delete') { if (confirm('删除这条模拟志愿？只影响这里的整理清单。')) { runtime.state = { ...runtime.state, volunteers: runtime.state.volunteers.length === 1 ? [blankChoice(1)] : runtime.state.volunteers.filter(item => String(item.id) !== String(id)) }; runtime.expanded.delete(String(id)); save(); } }
...
function initWorker() {
  ...
  runtime.worker.addEventListener('message', event => {
    ...
    if (data.type === 'school-candidates') {
      const items = (data.candidates || []).slice(0, 6).map(item => ({ name: norm(item.officialName || item.name), code: norm(item.schoolCode2026 || item.schoolCode || ''), meta: [item.province, item.city, item.level].map(norm).filter(Boolean).join(' · ') }));
      slot(id, candidateHtml('school', items, id));
      helper(id, items.length ? '请按名称、地区和层次选择正确的学校。' : '暂未找到合适的招生学校候选。', items.length ? 'neutral' : 'warn');
    } else if (data.type === 'school-resolved') {
      const official = norm(data.result?.officialName);
      if (!official) return;
      selectSchoolCandidate(id, official, norm(data.result?.schoolCode2026 || data.result?.schoolCode || ''));
