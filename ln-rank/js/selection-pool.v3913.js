...
import { buildTrendSummaryForSelection, renderSelectionTrendBox } from './major-trend-integration.v3912.js';
import { renderParentCoach } from './feature/decision-coach/decision-coach-render.v3963.js';
import { getCampusForRecord } from './campus-display-policy.v3912.js';
  ...
  return `<article class="workspace-item band-${bandClass(item)}" data-id="${escapeHtml(item.id)}" data-index="${index}">
    ...
    <div class="workspace-item-main">
      ...
      <div class="workspace-item-meta">
        <span class="is-band">${band.detail}</span><span>${score}</span><span>${rank}</span><span>${delta}</span>${location}${campusTag}${itemCodeText(item)}
      </div>
