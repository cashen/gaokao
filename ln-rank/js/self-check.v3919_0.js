...
function renderClientChecks(){
  ...
  add('CLIENT-ASSET-001：版本资源 v3919_0', Array.from(document.querySelectorAll('script[src],link[href]')).every(el => (el.getAttribute('src')||el.getAttribute('href')||'').includes('3919_0') || !(el.getAttribute('src')||el.getAttribute('href')||'').includes('3918_')), '当前页面资源引用应统一到 v3919_0。');
  add('ASSET-IMPORT-001：不再主动引用 normalizer wrapper', Array.from(document.querySelectorAll('script[src],link[href]')).every(el => !String(el.getAttribute('src')||el.getAttribute('href')||'').includes('major-bands-response-normalizer')), 'v3.9.19.0 起 app 直接引用 score-band-contract.js，减少新增 wrapper 文件导致的 MIME 风险。');
...
console.info('复核清单生成检查：已纳入 v3.9.19.0；自选专业页与报告会汇总校区、费用、位次、计划、体检、外语和履约复核事项。');

console.info('v3.9.19.0 UI结构检查：自选页按确认分数、整理清单、检查方案、生成报告四步收口，报告状态就近反馈。');

console.info('v3.9.19.0 维护基线检查：自选页已将检查与报告按钮固定在同一操作卡，方案解读结果移到下方输出区，避免第3步诊断后挤走第4步报告入口。');

async function runManifestCheck(){
  try{const res=await fetch('./active-assets.json',{cache:'no-store'});const m=await res.json();const missing=[];for(const list of [m.html,m.jsEntry,m.cssEntry,m.stableModules]){for(const p of list||[]){const r=await fetch('./'+p,{method:'HEAD',cache:'no-store'}).catch(()=>null);if(!r||!r.ok)missing.push(p)}};const root=document.getElementById('clientChecks');if(root)root.insertAdjacentHTML('beforeend',renderUiCheck({name:'ASSET-MANIFEST-001：active-assets 资源真实存在',ok:!missing.length,detail:missing.length?missing.join(' / '):'active manifest 检查通过'}));}catch(e){const root=document.getElementById('clientChecks');if(root)root.insertAdjacentHTML('beforeend',renderUiCheck({name:'ASSET-MANIFEST-001：active-assets 可读取',ok:false,detail:String(e)}));}}
setTimeout(runManifestCheck,600);
