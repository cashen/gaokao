import {
  ...
  savePoolItems
} from './feature/selection-pool/selection-pool-store.v3982.js';
...
function buildCurrentState({ persistScore = true } = {}) {
  ...
  currentComputedState = { rawItems, items: computedItems, stats, candidateContext: { ...candidateContext, bottomLineMode: bottomLineMode() }, orderSignature, signature, score, bottomLineMode: bottomLineMode(), majorTrendSummary };
...
function contextStatusHtml(state) {
  ...
  const text = score
    ? `当前计算口径：2025 辽宁物理类 · 考生 ${score} 分 · 已按当前成绩重算 ${total} 个专业。排序、方案解读和家庭讨论报告都会先使用这个口径。`
...
function statHtml(stats, items, state) {
  ...
  return `${contextStatusHtml(state)}<div class="pool-stats workspace-stats">
    ...
    <div><strong>${stats.rushCount}</strong><span>前段尝试</span></div>
    <div><strong>${stats.stableCount}</strong><span>主要承接</span></div>
    <div><strong>${stats.safeCount}</strong><span>后段补充</span></div>
...
function renderList() {
  ...
  listRoot.innerHTML = items.length
    ...
    : '<div class="pool-empty">自选专业为空。请返回查询页，先在专业卡片中点击“加入自选专业”。</div>';
...
function renderAnalysis() {
  ...
  if (!root) return;
  if (!currentAnalysis) {
    root.innerHTML = '<div class="pool-empty diagnose-empty">点击“检查这套方案”，系统会按位次、控制线锚点和自选专业结构，检查前段、中段、后段是否接得住。</div>';
    return;
  }
  const state = getState();
  ...
  const staleNotice = fresh ? '' : '<div class="analysis-stale">排序或分数已经变化，请重新点击“检查这套方案”后再生成带解读的报告。</div>';
  ...
  const structureText = narrative?.structureDiagnosis
    || (stats.total ? `当前自选专业共 ${fmt(stats.total)} 个：前段尝试 ${fmt(stats.rushCount)} 个，主要承接 ${fmt(stats.stableCount)} 个，后段补充 ${fmt(stats.safeCount)} 个。建议重点看中段是否接得住、后段是否够稳。` : '当前自选专业数量和前中后段结构待核验。');
  ...
  const dataReview = `<details class="analysis-raw-details"><summary>展开查看前中后段快速复核</summary>
    <div class="analysis-mini-grid">
      <div><span>前段尝试</span><b>${escapeHtml(fmt(stats.rushCount))}</b></div>
      <div><span>主要承接</span><b>${escapeHtml(fmt(stats.stableCount))}</b></div>
      <div><span>后段补充</span><b>${escapeHtml(fmt(stats.safeCount))}</b></div>
      <div><span>高一点</span><b>${escapeHtml(fmt(stats.highRushCount))}</b></div>
  ...
  root.innerHTML = `<div class="analysis-box">${staleNotice}${rankZoneCard}${trendBox}${narrativeCard}${dataReview}</div>`;
}
function renderFeishuStatus() {
  ...
  root.innerHTML = `<div class="pool-feishu-status ${feishuStatus.ok ? 'is-ok' : 'is-error'}">${escapeHtml(feishuStatus.message || '')}${links}${detail}</div>`;
...
async function sendFeishu({ withAnalysis = false } = {}) {
  ...
  feishuStatus = { ok: true, message: withAnalysis ? '正在按当前成绩口径重新计算，并生成带解读的家庭讨论报告…' : '正在按当前成绩口径生成家庭讨论报告…' };
  renderFeishuStatus();
  try {
    ...
    const latest = getState();
    if (btn) { btn.disabled = !latest.items.length || !latest.candidateContext.score; btn.textContent = withAnalysis ? '生成带解读的报告' : '生成家庭讨论报告'; }
    renderAll();
  }
}


function buildPlainTextReport(state = getState(), analysis = currentAnalysis) {
  const items = state.items || [];
  const trendSummary = state.majorTrendSummary || buildTrendSummaryForSelection(items, state.score);
  const lines = [];
  lines.push('辽宁物理类专业初选参考报告');
  lines.push('基于 2025 年数据生成，用于家庭讨论和人工复核；正式填报以当年一分一段、招生计划和志愿系统为准。');
  lines.push('');
  lines.push(`考生分数：${state.score || '未填写'} 分`);
  lines.push(`自选专业：${items.length} 个`);
  lines.push('');
  if (analysis?.aiNarrative) {
    const ai = analysis.aiNarrative;
    lines.push('方案解读：');
    if (ai.overall) lines.push(`- 一句话结论：${ai.overall}`);
    if (ai.zoneJudgement || ai.rankZoneExplain) lines.push(`- 当前分数和位次定位：${ai.zoneJudgement || ai.rankZoneExplain}`);
    if (ai.structureDiagnosis) lines.push(`- 自选专业结构：${ai.structureDiagnosis}`);
    if (Array.isArray(ai.riskDiagnosis) && ai.riskDiagnosis.length) lines.push(`- 主要风险：${ai.riskDiagnosis.slice(0, 3).join('；')}`);
    if (Array.isArray(ai.actions) && ai.actions.length) lines.push(`- 下一步调整建议：${ai.actions.slice(0, 3).join('；')}`);
    lines.push('');
  }
  if (Array.isArray(trendSummary?.notes) && trendSummary.notes.length) {
    lines.push('专业热度变化参考：');
    trendSummary.notes.slice(0, 3).forEach((note, idx) => lines.push(`${idx + 1}. ${note}`));
  ...
  items.forEach((item, idx) => {
    ...
    const score = Number.isFinite(Number(item.score2025 ?? item.score)) ? `${fmt(item.score2025 ?? item.score)}分` : '分数待核验';
    const rank = Number.isFinite(Number(item.rank2025 ?? item.rank)) ? `${fmt(item.rank2025 ?? item.rank)}位` : '位次待核验';
    lines.push(`${idx + 1}. ${item.school || '学校待核验'} · ${item.major || '专业待核验'}${code}｜2025最低分 ${score}｜2025最低位次 ${rank}｜相对孩子 ${item.scoreDelta == null ? '待核验' : item.scoreDelta + '分'}｜匹配关系：${item.poolBand?.detail || item.statusLabel || '待判断'}`);
  });
  lines.push('');
  lines.push('需要人工复核：2026 年招生计划、专业备注、选科要求、体检限制、学费、校区、培养模式和转专业政策。');
  return lines.join('\n');
}

async function copyPlainReport() {
  const state = getState();
  if (!state.items.length) { alert('当前还没有自选专业。'); return; }
  const text = buildPlainTextReport(state, currentAnalysis);
  try {
    ...
    box.remove();
    alert(ok ? '文字版报告已复制。' : '复制失败，请手动选择页面内容复制。');
  }
}
...
function bind() {
  ...
  $('copyPlainReport')?.addEventListener('click', copyPlainReport);
  ...
  $('sortByBand')?.addEventListener('click', () => {
    ...
    if (!confirm(`将按当前 ${state.candidateContext.score} 分口径，把当前自选专业按“高一点→冲一冲→接近匹配→相对稳妥→稳妥补充→最后兜底”的顺序重新整理。确认执行吗？`)) return;
