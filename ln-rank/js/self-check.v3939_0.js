/* v3.9.39 active-assets driven self check: version + report contract + scoped CSS gate */
const VERSION = 'v3.9.39';
const ASSET = 'v3939_0';
const ASSET_QUERY = '3939_0';
...
function hasWrongActiveQuery(text){return /\?v=(?!3939_0\b)[0-9A-Za-z_.-]+/.test(text)}
...
async function checkSelfCheckApi(){
  const data = await fetchJson('/api/ln-rank-self-check?v=3939_0');
  const checks=[];
...
async function main(){
 const checks=[];
 try{
  const assets=await fetchJson('/ln-rank/active-assets.json?v=3939_0');
  const meta=await fetchJson('/ln-rank/release-meta.json?v=3939_0');
  const manifest=await fetchJson('/ln-rank/module-manifest.json?v=3939_0');
  ...
  for(const rel of [...assets.jsEntry,...assets.cssEntry]){
    ...
    if(rel.endsWith('.js')) checks.push({name:`JS query 统一：${rel}`,ok:!hasWrongActiveQuery(text),detail:'active 文件内 import/export query 应统一 ?v=3939_0'});
    if(rel.endsWith('.css')){
      checks.push({name:`CSS 不返回 HTML：${rel}`,ok:!text.trimStart().startsWith('<'),detail:'active CSS 不能是 HTML 回退页'});
      checks.push({name:`CSS selector 半径：${rel}`,ok:!hasBlastRadiusSelector(text),detail:'active CSS 禁止 [class*=card/chip/tag] 这类跨页模糊选择器'});
    }
  }
  ...
  const contract=await fetchText('/ln-rank/js/domain/version-contract.js?v=3939_0');
  checks.push({name:'前端 version-contract 同步',ok:contract.includes(VERSION)&&contract.includes('3939_0')&&contract.includes(meta.release),detail:'display / asset / release 应与 release-meta 一致'});
  const intent=await fetchText('/ln-rank/docs/product-intent-map.md?v=3939_0');
