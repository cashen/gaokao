...
function hasOldVersionQuery(text){return /\?v=(?!3933_3\b)[0-9A-Za-z_.-]+/.test(text)}
async function main(){
 const checks=[];
 try{
  const assets=await fetchJson('/ln-rank/active-assets.json?v=3933_3');
  checks.push({name:'active-assets 版本',ok:assets.version==='v3.9.33.3'&&assets.assetVersion==='v3933_3',detail:`${assets.version} / ${assets.assetVersion}`});
  ...
  const activeJs=await fetchText('/ln-rank/js/app.v3933_3.js?v=3933_3') + '\n' + await fetchText('/ln-rank/js/selection-pool.v3933_3.js?v=3933_3');
