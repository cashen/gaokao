...
export function isInitialQuery(value){
  const source=String(value||'').normalize('NFKC').trim();
  const normalized=normalizeInitialQuery(source);
  return normalized.length>=2&&/^[a-z0-9\s._-]+$/i.test(source);
}
...
export function createSchoolInitialCodes(officialName,aliases=[],providedCodes=[]){
  const codes=new Set();
  const collegeStem=String(officialName||'').replace(/学院$/u,'');
  for(const raw of [...providedCodes,officialName,...aliases]){
    if(String(officialName||'').endsWith('学院')&&String(raw||'').trim()===collegeStem)continue;
    const direct=normalizeInitialQuery(raw);
    if(direct&&/^[a-z0-9]+$/.test(String(raw||'').normalize('NFKC').toLowerCase().replace(/[\s._-]+/g,'')))codes.add(direct);
    const code=toPinyinInitials(raw);
    if(code.length>=2)codes.add(code);
    const text=String(raw||'').replace(/[（(].*?[）)]/g,'').trim();
    if(text.endsWith('大学')&&code.endsWith('dx')&&code.length>3)codes.add(code.slice(0,-2));
  }
  return [...codes].filter(code=>code.length>=2).sort((a,b)=>b.length-a.length||a.localeCompare(b));
}
...
function initialForHan(char){
  ...
  while(low<=high){const middle=(low+high)>>1;if(collator.compare(char,PINYIN_BOUNDARIES[middle][1])>=0){answer=PINYIN_BOUNDARIES[middle][0];low=middle+1;}else high=middle-1;}
