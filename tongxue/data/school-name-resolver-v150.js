...
async function loadCatalog(url,fetchImpl){
 ...
 if(payload?.buildId!==TONGXUE_V150_BUILD_ID)throw new Error(`学校地域索引版本不一致：${String(payload?.buildId||'missing')}`);
 const records=extractSchoolRecords(payload),expectedCount=Number(payload?.count||0);
 if(expectedCount&&records.length!==expectedCount)throw new Error(`学校名单数量异常：应为 ${expectedCount}，实际为 ${records.length}`);
 if(records.length<2900||records.some(row=>!row.province||!row.city))throw new Error('学校地域索引不完整。');
 const baseResolver=createSchoolNameResolver(records),resolver=createEntityAwareResolver(baseResolver,baseResolver.metadata);
 return Object.freeze({resolver,metadata:resolver.metadata,count:baseResolver.count,entityCount:resolver.entityCount,asOfDate:String(payload?.asOfDate||''),buildId:payload.buildId,region:Object.freeze({...payload.region})});
}
...
export function extractSchoolRecords(payload){if(Array.isArray(payload))return uniqueRecords(payload.map(readRecord).filter(Boolean));if(!payload||typeof payload!=='object')return[];if(Array.isArray(payload.schools))return uniqueRecords(payload.schools.map(readRecord).filter(Boolean));return[];}
...
export function createSchoolNameResolver(schoolRows){
 ...
 const names=records.map(record=>record.name),nameSet=new Set(names),recordByName=new Map(records.map(record=>[record.name,record]));
 const metadata=new Map(records.map(record=>[record.name,Object.freeze({...record,location:formatLocation(record.province,record.city)})]));
 const entries=records.map(record=>({officialName:record.name,normalized:normalizeSchoolText(record.name),province:record.province,city:record.city,level:record.level,aliases:new Set(),aliasNormalized:[],initialCodes:record.initialCodes||[]}));
 const entryByName=new Map(entries.map(entry=>[entry.officialName,entry]));
 const officialMap=new Map(),aliasMap=new Map(),firstCharIndex=new Map(),bigramIndex=new Map(),genericIndex=new Map(),initialExactMap=new Map(),initialBucketIndex=new Map(),provinceIndex=new Map(),cityIndex=new Map(),provinceAliasMap=new Map(),cityAliasMap=new Map(),searchCache=new Map();
 ...
 for(const entry of entries){
  ...
  indexEntry(firstCharIndex,bigramIndex,entry,entry.normalized);for(const alias of entry.aliasNormalized)indexEntry(firstCharIndex,bigramIndex,entry,alias);
  addToSetMap(provinceIndex,entry.province,entry);addToSetMap(cityIndex,entry.city,entry);
  for(const alias of regionAliases(entry.province,'province'))addToSetMap(provinceAliasMap,alias,entry.province);
  for(const alias of regionAliases(entry.city,'city'))addToSetMap(cityAliasMap,alias,entry.city);
 }
 ...
 const context={entries,entryByName,officialMap,aliasMap,firstCharIndex,bigramIndex,genericIndex,initialExactMap,initialBucketIndex,provinceIndex,cityIndex,provinceAliasMap,cityAliasMap,searchCache};
 return Object.freeze({count:names.length,names:Object.freeze([...names]),metadata,getMetadata(name){return metadata.get(cleanOfficialName(name))||null;},resolve(query,options={}){return resolveSchoolName(query,{...context,...options});},search(query,options={}){return searchSchoolNames(query,context,options);},regionQuery(query,options={}){return resolveRegionQuery(query,context,options);}});
}
...
export function resolveSchoolName(query,context){
 ...
 const explicitRegion=/[\s,，]/.test(input)?resolveRegionQuery(input,context,{limit}):null;
 ...
 const region=resolveRegionQuery(input,context,{limit});if(region)return region;
...
export function searchSchoolNames(query,contextOrEntries,options={}){
 const normalizedInput=normalizeSchoolText(query),limit=Number.isFinite(options.limit)?Math.max(1,options.limit):8;if(!normalizedInput)return[];
 ...
 if(isLatinCodeInput(query)){const code=normalizeInitialQuery(query);if(code.length<2)return cacheSearch(context.searchCache,cacheKey,[]);return cacheSearch(context.searchCache,cacheKey,searchInitialSchoolNames(code,context,limit));}
 const region=resolveRegionQuery(query,context,{limit});if(region)return cacheSearch(context.searchCache,cacheKey,region.candidates);
 ...
 for(const entry of candidateEntries){let best=scoreText(normalizedInput,entry.normalized,'official');for(const alias of entry.aliasNormalized||[]){const aliasScore=scoreText(normalizedInput,alias,'alias');if(aliasScore.score>best.score)best=aliasScore;}if(best.score<0.42)continue;scored.push({officialName:entry.officialName,score:roundScore(best.score),matchType:best.matchType});}
...
export function resolveRegionQuery(query,context,options={}){
 const intent=parseRegionIntent(query,context);if(!intent)return null;
 const limit=Number.isFinite(options.limit)?Math.max(1,options.limit):8;
 let pool=[];
 if(intent.city)pool=[...(context.cityIndex?.get(intent.city)||[])];else if(intent.province)pool=[...(context.provinceIndex?.get(intent.province)||[])];
 if(intent.province&&intent.city)pool=pool.filter(entry=>entry.province===intent.province);
 const rows=[];
 for(const entry of pool){
  let score=0.9,matchType=intent.keyword?'region_keyword':'region_school';
  if(intent.keyword){let best=scoreText(normalizeSchoolText(intent.keyword),entry.normalized,'official');for(const alias of entry.aliasNormalized||[]){const aliasScore=scoreText(normalizeSchoolText(intent.keyword),alias,'alias');if(aliasScore.score>best.score)best=aliasScore;}if(best.score<0.42)continue;score=Math.min(0.98,best.score);matchType='region_keyword';}
  rows.push({officialName:entry.officialName,score:roundScore(score),matchType});
 }
 rows.sort((a,b)=>compareRegionEntries(context,a,b));
 const cityGroups=[];
 if(intent.province&&!intent.city&&!intent.keyword){const counts=new Map();for(const entry of pool)counts.set(entry.city,(counts.get(entry.city)||0)+1);for(const[city,count]of counts)cityGroups.push({city,count,query:`${intent.province} ${city}`});cityGroups.sort((a,b)=>a.city.localeCompare(b.city,'zh-CN'));}
 const label=[intent.province,intent.city].filter(Boolean).join(' · '),type=intent.city?'city':'province';
 return{status:'region',input:cleanOfficialName(query),resolvedName:null,candidates:rows.slice(0,limit),matchType:intent.keyword?'region_keyword':`region_${type}`,confidence:0,region:Object.freeze({type,province:intent.province,city:intent.city,keyword:intent.keyword,label}),total:rows.length,cityGroups};
}
...
function parseRegionIntent(value,context){
 const input=cleanOfficialName(value);if(!input||isLatinCodeInput(input))return null;
 const tokens=input.split(/[\s,，]+/).filter(Boolean);if(!tokens.length)return null;
 if(tokens.length===1){const key=normalizeRegionAlias(tokens[0]),cities=[...(context.cityAliasMap?.get(key)||[])],provinces=[...(context.provinceAliasMap?.get(key)||[])];if(cities.length===1)return{province:'',city:cities[0],keyword:''};if(provinces.length===1)return{province:provinces[0],city:'',keyword:''};return null;}
 let province='',city='';const keyword=[];
 for(const token of tokens){const key=normalizeRegionAlias(token),cities=[...(context.cityAliasMap?.get(key)||[])],provinces=[...(context.provinceAliasMap?.get(key)||[])];if(!city&&cities.length===1){city=cities[0];continue;}if(!province&&provinces.length===1){province=provinces[0];continue;}keyword.push(token);}
 if(!province&&!city)return null;
 return{province,city,keyword:keyword.join('')};
}
function regionAliases(value,type){const full=normalizeRegionAlias(value),aliases=new Set([full]);let short=value;if(type==='province')short=value.replace(/(壮族自治区|回族自治区|维吾尔自治区|自治区|省|市)$/u,'');else short=value.replace(/(自治州|地区|盟|市)$/u,'');const normalizedShort=normalizeRegionAlias(short);if(normalizedShort)aliases.add(normalizedShort);return aliases;}
function normalizeRegionAlias(value){return String(value||'').normalize('NFKC').replace(/[\s·•,，。；;：:'"“”‘’!！?？_—-]+/g,'').trim();}
function compareRegionEntries(context,a,b){const am=context.entryByName?.get(a.officialName),bm=context.entryByName?.get(b.officialName);const al=am?.level==='本科'?0:1,bl=bm?.level==='本科'?0:1;return al-bl||a.officialName.length-b.officialName.length||a.officialName.localeCompare(b.officialName,'zh-CN');}
function searchInitialSchoolNames(code,context,limit){if(code.length<2)return[];const exact=[...(context.initialExactMap?.get(code)||[])];if(exact.length)return exact.sort(compareEntries).slice(0,limit).map(entry=>initialCandidate(entry,0.995,'initial_exact'));const pool=[...(context.initialBucketIndex?.get(code.slice(0,2))||[])],rows=[];for(const entry of pool){const matching=(entry.initialCodes||[]).filter(item=>item.startsWith(code)).sort((a,b)=>a.length-b.length)[0]||'';if(!matching)continue;const coverage=code.length/matching.length;rows.push(initialCandidate(entry,Math.min(0.97,0.76+coverage*0.2),'initial_prefix'));}rows.sort((a,b)=>b.score-a.score||a.officialName.length-b.officialName.length||a.officialName.localeCompare(b.officialName,'zh-CN'));return rows.slice(0,limit);}
...
function buildLegacyContext(entries){const normalizedEntries=entries.map(entry=>({...entry,aliasNormalized:[...(entry.aliases||[])].map(normalizeSchoolText),initialCodes:normalizeInitialCodes(entry.initialCodes||[]),province:entry.province||'',city:entry.city||''}));const firstCharIndex=new Map(),bigramIndex=new Map(),genericIndex=new Map(),initialExactMap=new Map(),initialBucketIndex=new Map(),provinceIndex=new Map(),cityIndex=new Map(),provinceAliasMap=new Map(),cityAliasMap=new Map();for(const entry of normalizedEntries){indexEntry(firstCharIndex,bigramIndex,entry,entry.normalized);for(const alias of entry.aliasNormalized)indexEntry(firstCharIndex,bigramIndex,entry,alias);for(const code of entry.initialCodes)indexInitial(initialExactMap,initialBucketIndex,code,entry);if(entry.province){addToSetMap(provinceIndex,entry.province,entry);for(const alias of regionAliases(entry.province,'province'))addToSetMap(provinceAliasMap,alias,entry.province);}if(entry.city){addToSetMap(cityIndex,entry.city,entry);for(const alias of regionAliases(entry.city,'city'))addToSetMap(cityAliasMap,alias,entry.city);}}for(const[shortcut,suffixes]of GENERIC_SHORTCUTS)genericIndex.set(shortcut,normalizedEntries.filter(entry=>suffixes.some(suffix=>entry.officialName.includes(suffix))).sort(compareEntries));return{entries:normalizedEntries,entryByName:new Map(normalizedEntries.map(entry=>[entry.officialName,entry])),firstCharIndex,bigramIndex,genericIndex,initialExactMap,initialBucketIndex,provinceIndex,cityIndex,provinceAliasMap,cityAliasMap,searchCache:new Map()};}
...
function readRecord(value){if(typeof value==='string'){const name=cleanOfficialName(value);return name?{name,province:'',city:'',level:'',initialCodes:[]}:null;}if(Array.isArray(value)){const name=cleanOfficialName(value[0]);return name?{name,province:cleanOfficialName(value[1]),city:cleanOfficialName(value[2]),level:cleanOfficialName(value[3]),initialCodes:normalizeInitialCodes(value[4])}:null;}if(!value||typeof value!=='object')return null;const name=cleanOfficialName(value.name||value.school||value.schoolName||value.school_name||'');return name?{name,province:cleanOfficialName(value.province||''),city:cleanOfficialName(value.city||value.location||''),level:cleanOfficialName(value.level||''),initialCodes:normalizeInitialCodes(value.initialCodes||value.initials||value.pinyinInitials)}:null;}
...
function formatLocation(province,city){return province&&city&&province!==city?province+' · '+city:(city||province||'');}
