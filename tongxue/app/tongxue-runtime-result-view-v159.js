...
export const TONGXUE_COMMON_NAME_ENTRY_VERSION = 'tongxue-common-name-v005-social-labels';
const STYLE_ID = 'tongxue-common-name-style-v005';
const UI_ID = 'tongxue-common-name-ui-v005';
...
function renderCommonNames(state = {}) {
  ...
  const socialPanel = socialLabels.length
    ? `<section class="tongxue-common-name-panel" data-social-labels-version="${esc(SCHOOL_SOCIAL_LABEL_SOURCE_VERSION)}"><div class="tongxue-common-name-kicker">来源站高校标签</div><ul class="tongxue-social-labels">${socialLabels.map(label => `<li class="tongxue-social-label">${esc(label)}</li>`).join('')}</ul><p class="tongxue-social-label-source">来自 ${esc(SCHOOL_SOCIAL_LABEL_SOURCE_META.sourceName)} 的公开学校标签接口；当前源站共记录 ${Number(SCHOOL_SOCIAL_LABEL_SOURCE_META.sourceLabelCount)} 个标签、${Number(SCHOOL_SOCIAL_LABEL_SOURCE_META.sourceRelationCount)} 条学校关系，其中 ${Number(SCHOOL_SOCIAL_LABEL_SOURCE_META.resolvedRelationCount)} 条已由共享高校档案精确解析；未解析关系不会展示。这个区域只用于帮助理解学校之间的常见分组或称呼，不是官方分类，也不作为录取、排名或专业强弱依据。</p></section>`
    : '';
