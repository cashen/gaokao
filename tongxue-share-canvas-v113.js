...
class Writer{
  ...
  summary(group){const c=this.ctx;c.font=font(31,500);const lists=group.items.map(item=>wrap(c,item,820)),height=86+lists.reduce((sum,lines)=>sum+lines.length*48+10,0);this.ensure(Math.min(height,2500));const top=this.y;c.fillStyle=group.attention?'#fff6ed':'#fff';rounded(c,58,top,964,Math.min(height,2500),28);c.fill();c.strokeStyle=group.attention?'#f0d4bc':'#dce6e2';c.lineWidth=2;c.stroke();c.fillStyle=group.attention?'#8d4b20':'#155e75';c.font=font(31,900);c.fillText(group.title,94,top+58);let y=top+108;c.fillStyle='#17242d';c.font=font(31,500);for(const lines of lists){lines.forEach((line,index)=>{c.fillText((index?'　':'• ')+line,96,y);y+=48;});y+=10;}this.y=top+Math.min(height,2500)+30;}
