...
export function buildZoneCandidates(facts = {}) {
  ...
  if (sp != null && sp < -35) return addPair('undergraduate-quality-zone', 'public-sensitive-zone', 0.64, ['考生已高于本科线但距离特控线较远', '本科质量、学费和专业接受度是主矛盾'], ['若自选池多为公办边缘项，需要按公办竞争敏感区复核']);
  if (sp != null && sp < 0) return addPair('public-sensitive-zone', 'special-edge-zone', 0.64, ['考生低于特控线但距离不远', '公办本科和热门专业竞争敏感'], ['若自选池冲高较多，要按特控线边缘区的保底深度复核']);

  // 特控线上方，用分差、位次差和自选池结构共同给候选。
  ...
  if (sp <= 15 || (rp != null && rp >= -7600)) return addPair('special-edge-zone', 'applied-tech-main-zone', 0.68, ['考生贴近特控线', '稳妥区和深保底仍是重点'], ['如果自选池应用工科承接足，可少量上移到应用技术主体区']);
  ...
  return addPair('top-platform-fine-sort-zone', 'high-platform-zone', 0.64, ['考生处于顶尖平台精细排序区', '主矛盾是强平台强专业和长期发展匹配'], ['仍需设置高质量可接受保底']);
}
