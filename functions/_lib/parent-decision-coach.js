...
function buildHeadline(facts = {}, healthLights = {}, zonePolicy = {}) {
  ...
  if (overall === 'risk') return `当前方案在“${zoneName}”下需要关注偏高，建议先补齐主要参考和稳妥补充，再生成正式报告。`;
  if (overall === 'warn') return `当前方案已有基础，但在“${zoneName}”下仍需先确认底线、稳妥补充和集中度需要关注。`;
...
export function buildRuleBasedParentCoach({ facts = {}, healthLights = {}, rule = {}, candidateZones = [], narrative = null } = {}) {
  ...
  else addUnique(actions, '确认稳妥补充：确认后段不是“分低但不想读”的假稳妥补充。');
