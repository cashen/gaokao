...
function buildBottomLineReview(facts = {}) {
  ...
  if (!b.mode || b.mode === 'all') {
    return '当前未设置特殊公办底线，适合先看全貌；后续进入自选专业体检时，仍需逐条核验民办、高收费、中外合作和费用承受能力。';
  }
  ...
  if (b.mode === 'public_regular_only') {
    ...
    return issues
      ? `当前底线为“${label}”，自选专业中仍有 ${fmt(issues)} 个项目需要复核，重点看公办中外/高收费、民办/独立或性质费用未知项目是否应替换。`
      : `当前底线为“${label}”，自选专业暂未发现明显违反底线的项目，仍建议核验学费、校区和招生章程。`;
  }
