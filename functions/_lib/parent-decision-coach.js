...
function buildBottomLineReview(facts = {}) {
  ...
  if (!b.mode || b.mode === 'all') {
    return '当前未设置特殊公办底线，适合先看全貌；后续进入自选池体检时，仍需逐条核验民办、高收费、中外合作和费用承受能力。';
  }
  ...
  if (b.mode === 'public_regular_only') {
    ...
    return issues
      ? `当前底线为“${label}”，自选池中仍有 ${fmt(issues)} 个项目需要复核，重点看公办中外/高收费、民办/独立或性质费用未知项目是否应替换。`
      : `当前底线为“${label}”，自选池暂未发现明显违反底线的项目，仍建议核验学费、校区和招生章程。`;
  }
  if (b.mode === 'public_include_sino') {
    return `当前底线为“${label}”：公办中外/高收费项目可以保留，但不等同于普通公办，需重点核验学费、培养模式、毕业证书、是否必须出国、校区和家庭承受能力。`;
  }
  return `当前底线为“${label}”，请结合家庭费用承受能力和专业接受度人工复核。`;
}
function buildFamilyQuestions(facts = {}, healthLights = {}, zoneKey = '') {
  const b = facts.bottomLineSummary || {};
  const stats = facts.poolStructure || {};
  const questions = [];
  addUnique(questions, '孩子是否明确接受当前自选池里的主专业方向？');
  addUnique(questions, '保底区里的学校、城市、专业和学费，是否真的愿意读？');
  ...
  if (lightLevel(healthLights, 'locationDiversity') === 'warn' || stats.topCityPct >= 45) addUnique(questions, '地域集中是否是家庭主动选择，还是因为不了解其他城市学校造成的单点风险？');
...
function buildHeadline(facts = {}, healthLights = {}, zonePolicy = {}) {
  ...
  if (!score) return '请先填写考生成绩，再进行自选池体检和报告生成。';
  if (overall === 'risk') return `当前方案在“${zoneName}”下风险偏高，建议先补齐稳妥区和保底区，再生成正式报告。`;
  if (overall === 'warn') return `当前方案已有基础，但在“${zoneName}”下仍需先复核底线、保底和集中度风险。`;
...
export function buildRuleBasedParentCoach({ facts = {}, healthLights = {}, rule = {}, candidateZones = [], narrative = null } = {}) {
  ...
  const actions = [];
  if (!stats.total) {
    addUnique(actions, '先回查询页加入候选专业，至少形成上探、主体、稳妥和保底几个层次后再诊断。');
  }
  if (bottom.mode && bottom.mode !== 'all') {
    const issueCount = Number(bottom.filteredOutCount || 0);
    if (issueCount > 0) addUnique(actions, `当前底线为“${bottom.modeLabel || modeLabel(bottom.mode)}”，自选池中有 ${fmt(issueCount)} 个项目不完全符合，请先人工复核是否保留。`);
    else addUnique(actions, `当前底线为“${bottom.modeLabel || modeLabel(bottom.mode)}”，下一步重点核验费用、校区和招生章程是否一致。`);
  }
  if (lightLevel(healthLights, 'bottomDepth') === 'warn') addUnique(actions, '先补保底区：增加低一层位次、学校城市专业都能接受的保底项。');
  else addUnique(actions, '复核保底区：确认后段不是“分低但不想读”的假保底。');
  ...
  if (stats.rushCount > Math.ceil((stats.total || 0) * 0.4)) addUnique(actions, '冲刺数量偏多，保留高价值目标即可，不要让前段冲刺替代中后段承接。');
