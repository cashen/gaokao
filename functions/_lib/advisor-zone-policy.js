export const ADVISOR_ZONE_POLICIES = {
  'missing-rank-zone': {
    ...
    mainConflict: '没有位次锚点时，冲稳保和保底深度只能做粗略讨论。',
  ...
  'undergraduate-edge-zone': {
    ...
    bottomLineAdvice: '保底必须可接受，不建议为了低分安全堆完全不想读的专业。',
  ...
  'undergraduate-quality-zone': {
    ...
    bottomLineAdvice: '后段保底要真实可读，不能只看比自己低多少分。',
    forbiddenOverstatements: ['本科稳了', '不用保底']
  },
  'public-sensitive-zone': {
    zoneKey: 'public-sensitive-zone',
    zoneName: '公办本科竞争敏感区',
    humanName: '公办本科竞争敏感区',
    mainGoal: '稳住公办本科机会，防止浅保底和热门专业扎堆。',
    mainConflict: '分数看着不低，但公办热门方向竞争集中，浅保底可能失效。',
    preferDirections: ['省内外可接受公办', '应用工科', '专业接受度高的保底项'],
    cautionDirections: ['冲刺过多', '同城同专业扎堆', '民办/高收费未核验'],
    bottomLineAdvice: '保底要按位次深度拉开，且学校、城市、专业、学费都要能接受。',
    forbiddenOverstatements: ['公办稳了', '贴线保底足够']
  },
  'special-edge-zone': {
    zoneKey: 'special-edge-zone',
    zoneName: '特控线边缘区 / 公办质量守门区',
    humanName: '特控线边缘区',
    mainGoal: '稳妥区要厚，保底区要深，避免只用低几分的浅保底。',
    mainConflict: '开始进入更高层次门口，但稳定性还不够强。',
    preferDirections: ['省内公办', '应用工科', '可接受专业', '低收费稳妥项'],
    cautionDirections: ['盲目冲热门', '用低几分当保底', '高收费/中外合作未核验'],
    bottomLineAdvice: '保底要按位次深度下探，后段必须是家庭和孩子都能接受的学校专业。',
  ...
  'applied-tech-main-zone': {
    ...
    cautionDirections: ['新办AI/微电子需核验师资实验室', '普通经管泛化', '把高冲当主体'],
    bottomLineAdvice: '保底仍要守住公办、学费和专业接受度，主体区不能只靠冲刺。',
  ...
  'industry-entry-zone': {
    ...
    bottomLineAdvice: '保底不必过度下沉，但必须保留可接受的稳妥公办项。',
  ...
  'industry-platform-zone': {
    ...
    bottomLineAdvice: '保底以可接受为底线，不要为了“保”填心里不能接受的专业。',
  ...
  'platform-major-balance-zone': {
    ...
    bottomLineAdvice: '高分段保底重在可接受和路径清楚，不是越低越好。',
  ...
  'high-platform-zone': {
    ...
    bottomLineAdvice: '保底不应过度下沉，应选择路径清楚、专业能接受的高质量稳妥项。',
  ...
  'top-platform-fine-sort-zone': {
    ...
    mainConflict: '主要矛盾不是保底深度，而是强平台与强专业的精细取舍。',
    preferDirections: ['顶尖平台优势专业', '强基/拔尖/本研路径', '长期发展匹配方向'],
    cautionDirections: ['盲目冲极限专业', '忽略专业兴趣和培养强度', '把普通保底逻辑套到高分段'],
    bottomLineAdvice: '保底以高质量可接受为底线，重点核验专业组、校区和培养路径。',
    forbiddenOverstatements: ['不用保底', '随便报都好']
  }
};
