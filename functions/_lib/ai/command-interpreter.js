...
function deterministicBase(text,workspace={},resolvedSchoolNames=[],resolvedSchoolAliases=[]){
  const source=clean(text,1200),retryContext=retryContextForText(source,workspace),score=scoreFromText(source),schoolNamesFromInput=schoolNamesFromText(source,resolvedSchoolNames),positive0=positiveMajors(source,schoolNamesFromInput,resolvedSchoolAliases),negative=negativeMajors(source,schoolNamesFromInput,resolvedSchoolAliases),schools0=schoolNamesFromInput,geo=geographyFromText(source),bottomLineMode=bottomLineFromText(source),platformTarget=platformTargetFromText(source),clearMajor=clearMajorLanguage(source),clearSchool=clearSchoolLanguage(source),clearRegion=clearRegionLanguage(source),reference=ordinalReference(source,workspace);
  ...
  return{
    ...
    clearMajor,clearSchool,clearRegion,reference,familyChanges,negativeMajorKeywords:negative,changeSet:patch,mentorProfile,retryRequested:Boolean(retryContext),retryFailedOnly:Boolean(retryContext?.failedOnly),
