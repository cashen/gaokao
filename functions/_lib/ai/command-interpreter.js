...
const SCHOOL_STEM_MAJOR_TERMS=Object.freeze(new Set(['师范']));
...
function majorMentions(text){
  ...
  for(const term of MAJOR_TERMS){let from=0;while(from<source.length){const i=source.indexOf(term,from);if(i<0)break;if(isSchoolStemMajorMention(source,term,i)){from=i+term.length;continue;}const before=source.slice(Math.max(0,i-10),i),after=source.slice(i+term.length,i+term.length+10);const negative=/(不看|不要|不考虑|排除|不接受|别看|去掉|删掉|不是|不选).{0,3}$/.test(before)||/^(不看|不要|不考虑|排除|不接受|别看|去掉|删掉|算了|不要了|不选)/.test(after);out.push({term,index:i,negative});from=i+term.length;}}
...
function likelySchoolMentionTokens(text){const source=clean(text,360);if(!source||/[\u4e00-\u9fa5]{2,18}?(?:大学|学院)/.test(source))return[];const tokens=[],regionOnly=new Set(['沈阳','大连','辽宁','辽宁省','省内','省外','全国','东北','西北','西南','华中']);const boundaries=[...MAJOR_TERMS.filter(term=>!SCHOOL_STEM_MAJOR_TERMS.has(term)),'学校环境','校园环境','校园氛围','学习氛围','人文关怀','管理人性','管理严格','老师负责','辅导员','同学评价','学生评价','学生口碑','真实体验','同学体验','在校体验','所有专业','全部专业','全校专业','招生专业','专业都多少分','各专业多少分','分都多少','都多少分','大概都多少分','大约都多少分','最低录取分','最低投档分','最低分','投档分','录取分','多少分','分数线','位次','排名','去年','往年','历年','学校简介','学校介绍','介绍一下','介绍下','讲讲','说说','聊聊','了解一下','什么学校','什么来头','学校定位','怎么样','如何','咋样','宿舍','住宿','食堂','食宿','章程','录取规则','学费','校区','能不能上','能不能报','够不够','能上吗','能报吗'].sort((a,b)=>b.length-a.length);for(const term of boundaries){let from=0;while(from<source.length){const i=source.indexOf(term,from);if(i<0)break;const prefix=source.slice(0,i),clauses=prefix.split(/[，,。！？!?；;：:]/).map(v=>v.trim()).filter(Boolean);let candidate=clauses.at(-1)||'';candidate=candidate.replace(/(?:^|[^\d])\d{3}\s*分?/g,' ');candidate=cleanSchoolAliasCandidate(candidate);if(candidate.length>=2&&candidate.length<=10&&!regionOnly.has(candidate)&&!/^(?:我|孩子|家长|学校|专业|分数|位次|去年|往年|历年)/.test(candidate)&&!/(?:公办|民办|中外|高收费|本科|就业|预算|候选|志愿|方案|平台|专业质量|培养)/.test(candidate))tokens.push(candidate);from=i+term.length;}}
