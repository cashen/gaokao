...
function cleanSchoolAliasCandidate(value){let result=clean(value,40).replace(/^(?:那|再|还是|然后|顺便|等等|等下|算了|先|换成|换|改成|改看|看看|看下|看一下|帮我看|帮我查|我想看|我想查|我想问|我问你|问你|想看|想查|想问|我想知道|想知道|查下|查一下|问下|问一下|请看|请查)+/,'').replace(/[\s，,。！？!?；;：:]+/g,'').trim();result=result.replace(/(?:的|呢|吗|呀|啊|吧|都|大概|大约|一般|分别|各自)+$/g,'').trim();return result;}
function likelySchoolMentionTokens(text){const source=clean(text,360);if(!source||/[\u4e00-\u9fa5]{2,18}?(?:大学|学院)/.test(source))return[];const tokens=[],regionOnly=new Set(['沈阳','大连','辽宁','辽宁省','省内','省外','全国','东北','西北','西南','华中']);const boundaries=[...MAJOR_TERMS,'所有专业','全部专业','全校专业','招生专业','专业都多少分','各专业多少分','分都多少','都多少分','大概都多少分','大约都多少分','最低录取分','最低投档分','最低分','投档分','录取分','多少分','分数线','位次','排名','去年','往年','历年','学校简介','学校介绍','什么学校','怎么样','如何','咋样','宿舍','住宿','食堂','食宿','章程','录取规则','学费','校区','能不能上','能不能报','够不够','能上吗','能报吗'].sort((a,b)=>b.length-a.length);for(const term of boundaries){let from=0;while(from<source.length){const i=source.indexOf(term,from);if(i<0)break;const prefix=source.slice(0,i),clauses=prefix.split(/[，,。！？!?；;：:]/).map(v=>v.trim()).filter(Boolean);let candidate=clauses.at(-1)||'';candidate=candidate.replace(/(?:^|[^\d])\d{3}\s*分?/g,' ');candidate=cleanSchoolAliasCandidate(candidate);if(candidate.length>=2&&candidate.length<=10&&!regionOnly.has(candidate)&&!/^(?:我|孩子|家长|学校|专业|分数|位次|去年|往年|历年)/.test(candidate)&&!/(?:公办|民办|中外|高收费|本科|就业|预算|候选|志愿|方案|平台|专业质量|培养)/.test(candidate))tokens.push(candidate);from=i+term.length;}}
...
async function loadAiSchoolResolver(request,env={},queries=[],resolverModule=null){const module=resolverModule||await import('../../../tongxue/data/school-name-resolver-v150.js'),payload=await fetchAiSchoolJson(request,env,module.SCHOOL_NAME_DATA_URL),rows=Array.isArray(payload?.schools)?payload.schools:[];if(rows.length<2900)throw new Error('学校简称索引不完整。');const selected=selectAiSchoolResolverRows(rows,queries,220);if(!selected.length)return null;return module.createSchoolNameResolver(selected);}
...
function deterministicBase(text,workspace={},resolvedSchoolNames=[],resolvedSchoolAliases=[]){
  ...
  const previousFocus=priorFocus(workspace),needsSchool=['school_major_history','school_history','school_official_qa','fit_assessment','school_background'].includes(agentTask),needsMajor=['school_major_history','major_region_history','fit_assessment','major_background'].includes(agentTask);
