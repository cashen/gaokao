export const AI_PROVIDER_ROUTER_VERSION = 'ai-provider-router-v3990_0';
...
function errorSummary(error) {
  return [error?.name, error?.message, error?.status, error?.code].filter(Boolean).join(' | ').slice(0, 360);
}

function externalEndpoint(baseValue) {
  const base = clean(baseValue, 500).replace(/\/+$/, '');
  if (!base) return '';
...
async function callWorkersAi(env, messages, options = {}) {
  ...
  const result = await env.AI.run(model, {
    messages,
    temperature: 0,
    max_tokens: Math.max(300, Math.min(1000, Number(options.maxTokens || 700)))
  });
...
async function callOpenAiCompatible(env, messages, options = {}) {
  ...
  if (!endpoint || !apiKey || !model) {
    throw Object.assign(new Error('外部模型需要 AI_EXTERNAL_BASE_URL、AI_EXTERNAL_API_KEY、AI_EXTERNAL_MODEL。'), { code: 'AI_EXTERNAL_CONFIG_MISSING' });
  }
  ...
  try {
    ...
    if (!response.ok) {
      const error = new Error(`外部模型 HTTP ${response.status}`);
      error.status = response.status;
      error.code = 'AI_EXTERNAL_HTTP_ERROR';
      error.detail = bodyText.slice(0, 400);
      throw error;
    }
    let payload;
    try { payload = JSON.parse(bodyText); } catch { throw new Error('外部模型返回非 JSON。'); }
    ...
    return {
      ...
      requestId: clean(response.headers.get('x-request-id') || payload?.id, 180),
      latencyMs: Date.now() - started
    };
  } finally {
    clearTimeout(timer);
...
export function aiProviderConfig(env = {}) {
  ...
  return {
    version: AI_PROVIDER_ROUTER_VERSION,
    primary,
    fallback,
    workersAiBound: Boolean(env?.AI && typeof env.AI.run === 'function'),
    workersModelConfigured: Boolean(clean(env?.AI_WORKSPACE_MODEL || env?.AI_MODEL, 180)),
    externalConfigured: Boolean(clean(env?.AI_EXTERNAL_BASE_URL, 500) && clean(env?.AI_EXTERNAL_API_KEY, 20) && clean(env?.AI_EXTERNAL_MODEL || env?.AI_WORKSPACE_MODEL || env?.AI_MODEL, 180)),
...
export async function runAiProvider(env = {}, messages = [], options = {}) {
  ...
  const attempts = [config.primary];
  if (config.fallback && config.fallback !== config.primary) attempts.push(config.fallback);
  const failures = [];

  for (const provider of attempts.slice(0, 2)) {
    try {
      const result = await callOne(provider, env, messages, options);
      return { ...result, routerVersion: AI_PROVIDER_ROUTER_VERSION, failures };
    } catch (error) {
      failures.push({ provider, error: errorSummary(error), code: clean(error?.code, 80) });
    }
  }
  ...
  return {
    ...
    usage: null,
    latencyMs: 0,
    routerVersion: AI_PROVIDER_ROUTER_VERSION,
    failures,
