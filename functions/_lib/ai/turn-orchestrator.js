...
export const AI_TURN_ORCHESTRATOR_VERSION='ai-turn-orchestrator-v0.03';
...
export async function orchestrateAiTurn(context,payload={}){
  ...
  if(command.agentTask!=='decision_research'&&explicitCompare&&command.schoolNames?.length>=2){command.agentTask='school_comparison';command.taskLocked=true;command.executionPolicy=taskExecutionPolicy(command.agentTask,command.scoreUsage);}
  else if(command.agentTask!=='decision_research'&&explicitCompare&&command.majorKeywords?.length>=2){command.agentTask='major_comparison';command.taskLocked=true;command.executionPolicy=taskExecutionPolicy(command.agentTask,command.scoreUsage);}
  ...
  try{
    switch(command.agentTask){
      ...
      case'major_region_history':
        result.majorHistory=await runMajorRegionHistory(executionContext,{majorKeyword:focus.major||(view.majorKeywords||[])[0]||'',majorKeywords:focus.majors?.length?focus.majors:(view.majorKeywords||[]),regionKeys:command.transientRegionView&&command.regionKeys?.length?command.regionKeys:(regionExecution.exact?regionExecution.includeKeys:(view.regionKeys||['all'])),bottomLineMode:view.bottomLineMode||'all'});result.partial=!result.majorHistory.ok||result.majorHistory.partial===true||result.majorHistory.allFailed===true;break;
