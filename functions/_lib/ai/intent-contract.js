...
export const AI_INTENT_CONTRACT_VERSION='ai-intent-contract-v0.02';
...
export function buildIntentContract(command={},workspace={}){
  const task=clean(command.agentTask,80)||'general_advice',text=clean(command.rawText||command.question,1200),rawTopic=task==='school_experience'?experienceTopicFromText(text):(task==='school_official_qa'&&experienceTopicFromText(text)!=='general'?experienceTopicFromText(text):task.includes('background')?'background':task.includes('history')?'admission':task.includes('comparison')?'comparison':'general'),topic=task==='school_experience'||task==='school_official_qa'?normalizeExperienceTopic(rawTopic):rawTopic;
  return{
