export const AI_KNOWLEDGE_LANGUAGE_VERSION='ai-education-knowledge-language-v0.02';
...
export function looksEducationKnowledgeQuestion(value='',{schools=[],majors=[]}={}){
  const source=text(value),definitionQuestion=DEFINITION_RE.test(source),ruleQuestion=RULE_QUESTION_RE.test(source);
  if(!source||(!definitionQuestion&&!ruleQuestion))return false;
...
export const KNOWLEDGE_LANGUAGE_TESTING=Object.freeze({DEFINITION_RE,RULE_QUESTION_RE,KNOWLEDGE_HINT_RE,SCHOOL_IDENTITY_RE,CURRENT_PREFIX_RE,RULE_SUFFIX_RE});
