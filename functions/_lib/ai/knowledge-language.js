...
export const AI_KNOWLEDGE_LANGUAGE_VERSION='ai-education-knowledge-language-v0.03';
...
const INTRODUCTION_RE=new RegExp(`^(?:(?:你好|请问|请|麻烦|帮我|给我|我想知道|想知道|想问|问下|问一下)[，,：:\\s]*)?${INTRODUCTION_ACTION}[，,：:\\s]*`);
...
export function looksEducationKnowledgeQuestion(value='',{schools=[],majors=[]}={}){
  const source=text(value),definitionQuestion=DEFINITION_RE.test(source),ruleQuestion=RULE_QUESTION_RE.test(source),introductionQuestion=INTRODUCTION_RE.test(source)||INTRODUCTION_SUFFIX_RE.test(source);
...
export const KNOWLEDGE_LANGUAGE_TESTING=Object.freeze({DEFINITION_RE,INTRODUCTION_RE,INTRODUCTION_SUFFIX_RE,RULE_QUESTION_RE,KNOWLEDGE_HINT_RE,SCHOOL_IDENTITY_RE,CURRENT_PREFIX_RE,RULE_SUFFIX_RE});
