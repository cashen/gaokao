...
function delegatedEntry(context,request){
  const key=requestKey(request),entry=context?.aiDeterministicToolResults?.[key];
...
export async function runStudentVoice(context,{scope='school',school='',major='',majorCode='',topic='',question=''}={}){
  ...
  const reviews=normalizedReviews(payload),summary=clean(payload.summary,4000),mode=clean(payload.mode,60),hasContent=Boolean(summary||reviews.length),matched=Number.isFinite(Number(payload?.evidence?.matched))?Number(payload.evidence.matched):(summary?null:reviews.length),sampleCount=matched===null?reviews.length:Math.max(0,matched),resolvedMajor=payload?.major&&typeof payload.major==='object'?{code:clean(payload.major.code,40)||code,name:clean(payload.major.name,160)||majorName}:{code,name:majorName};
  return{
    ok:hasContent,
    code:hasContent?'':(mode||'student_voice_no_content'),
    message:hasContent?'':noContentMessage({scope:normalizedScope,school:schoolName,major:resolvedMajor.name||majorName,topic:normalizedTopic,mode}),
    ...
    boundary:`大学生声音属于UGC，不是学校官方事实、专业客观结论、就业率或录取依据；认证/点赞只作来源说明，不参与排序或推荐。范围=${normalizedScope}，不允许静默扩大或缩小。`,
