...
export function buildEvidencePlan(command={},workspace={},view={}){
  const frame=command.semanticFrame||{},steps=[],add=(kind,reason,params={})=>{if(steps.length<3&&!steps.some(x=>x.kind===kind))steps.push({index:steps.length+1,kind,reason,params});};
  const score=Number(command.score||frame.score||workspace?.examContext?.score||view?.score)||null,schools=unique(frame.schools||command.schoolNames||[],4),majors=unique(frame.majors||command.majorKeywords||[],8),pairs=Array.isArray(frame.pairs)?frame.pairs.slice(0,3):[],needs=new Set(frame.evidenceNeeds||[]);
  ...
  return{version:AI_EVIDENCE_PLAN_VERSION,maxSteps:3,steps,bounded:true,schoolFanoutLimit:2,pairFanoutLimit:3,webFetchLimit:2};
}
