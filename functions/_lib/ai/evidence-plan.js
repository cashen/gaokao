...
export function buildEvidencePlan(command={},workspace={},view={}){
  ...
  const score=scoreAllowed?(Number(command.score||frame.score||workspace?.examContext?.score||view?.score)||null):null,schools=unique(frame.schools||command.schoolNames||[],4),majors=unique(frame.majors||command.majorKeywords||[],8),pairs=Array.isArray(frame.pairs)?frame.pairs.slice(0,3):[];
  if(score&&scoreRequested&&(pairs.length>=2||schools.length>=2||majors.length>=2))add('admissions_compare','先用辽宁2026确定性投档事实确认本轮明确要求的分数可达空间。',{score,schools,majors,pairs});
