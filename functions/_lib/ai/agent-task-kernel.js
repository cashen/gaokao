...
export const AGENT_TASKS=Object.freeze([
  ...
  'school_major_history','school_history','school_official_qa','fit_assessment',
...
export function deterministicAgentTask({text='',schools=[],majors=[],regionKeys=[],score=null,workspace={},candidateIntent=false,compareIntent=false,rankIntent=false}={}){
  ...
  if(explicitScoreUsage(source,workspace)==='suspended'){
    if(['school_major_history','school_history','school_official_qa','school_background','major_background','background_discovery','school_comparison','major_comparison'].includes(priorTask))return priorTask;
...
export function taskExecutionPolicy(task,scoreUsage='remembered'){
  ...
  switch(task){
    ...
    case'school_major_history':case'school_history':case'school_official_qa':case'school_background':case'major_background':case'background_discovery':return{score:score==='suspended'?'suspended':'remembered',region:'remembered',major:'active',school:'active',bottomLine:'remembered',commitView:false};
