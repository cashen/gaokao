...
export const AGENT_TASKS=Object.freeze([
  ...
  'school_major_history','school_history','major_region_history','school_research','school_official_qa','fit_assessment',
...
export function agentFocusSeed(seed={},workspace={}){
  ...
  return {
    ...
    majors:unique(seed.majors||prior.majors||[],6),
...
export function deterministicAgentTask({text='',schools=[],majors=[],regionKeys=[],score=null,workspace={},candidateIntent=false,compareIntent=false,rankIntent=false}={}){
  ...
  const broadSchoolResearch=Boolean(school&&(looksBroadSchoolResearch(source)||resolvedSchoolGeneralQuestion||bareResolvedSchool)&&!looksFit(source)&&!looksBackground(source)&&!looksHistory(source)&&!looksSpecificOfficialTopic(source)&&explicitMajors.length===0&&(schools.length||['school_major_history','school_history','school_research','school_official_qa','fit_assessment','school_background'].includes(priorTask)||/(这个学校|这所学校|那个学校|那所学校|该校)/.test(source)));
  ...
  if(explicitScoreUsage(source,workspace)==='suspended'){
    if(['school_major_history','school_history','school_research','school_official_qa','school_background','major_background','background_discovery','school_comparison','major_comparison'].includes(priorTask))return priorTask;
...
export function taskExecutionPolicy(task,scoreUsage='remembered'){
  ...
  switch(task){
    ...
    case'school_major_history':case'school_history':case'school_research':case'school_official_qa':case'school_background':case'major_background':case'background_discovery':return{score:score==='suspended'?'suspended':'remembered',region:'remembered',major:'active',school:'active',bottomLine:'remembered',commitView:false};
