...
export function buildSelectionReviewChecklist(items = []) {
  ...
  (Array.isArray(items) ? items : []).forEach(item => {
    ...
    if (vol == null) push(by.rank, item, '历史位次证据不完整，不能只按单一年份最低位次判断。', '结合 2026 招生计划、当年位次和同类专业分布人工复核。');
    else if (vol >= 12000) push(by.rank, item, `2024/2025 最低位次波动约 ${vol.toLocaleString('zh-CN')} 位，波动较大。`, '降低位次余量的稳定判断权重，结合 2026 招生计划复核。');
    else if (vol >= 6000) push(by.rank, item, `2024/2025 最低位次波动约 ${vol.toLocaleString('zh-CN')} 位，需结合当年计划复核。`, '不要只按 2025 最低位次判断。');
    if (outProvince(item) || /计划人数待核验|省外/.test(t)) push(by.plan, item, '省外高校在辽宁招生计划人数需要重点确认。', '核验 2026 年辽宁物理类招生计划人数，避免裸读位次余量。');
