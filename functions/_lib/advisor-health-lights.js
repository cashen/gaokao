...
export function buildAdvisorHealthLights(facts = {}, rule = {}) {
  ...
  return {
    ...
    structure: light(total && stats.stableCount >= Math.ceil(total * 0.25) && stats.safeCount >= Math.max(3, Math.ceil(total * 0.18)) ? 'ok' : 'warn', '前中后段结构', total ? `前段尝试${stats.rushCount || 0}个、主要承接${stats.stableCount || 0}个、后段补充${stats.safeCount || 0}个。` : '自选专业为空。'),
    bottomDepth: light(stats.safeCount >= Math.max(3, Math.ceil(total * 0.18)) ? 'info' : 'warn', '后段补充深度', stats.safeCount >= Math.max(3, Math.ceil(total * 0.18)) ? '后段补充数量不算少，但仍需核验接受度和位次深度。' : '后段补充数量或深度不足，需要补充可接受最后兜底项。'),
