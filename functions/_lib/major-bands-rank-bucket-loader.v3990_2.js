...
export const MAJOR_BANDS_RANK_BUCKET_LOADER_VERSION = 'major-bands-rank-bucket-loader-bounded-all-band-v3990_2';
export const MAJOR_BANDS_RANK_BUCKET_CACHE_VERSION = 'major-bands-rank-bucket-cache-v3990_2';
export const MAJOR_BANDS_REQUEST_BAND_SCOPE_VERSION = 'major-bands-request-band-scope-v3990_2';
export const MAJOR_BANDS_RANK_BUCKET_RECORD_OWNERSHIP = 'miss-owned-hit-shallow-cloned-v3990_2';
...
function bucketReadKey(indexBucket, scope = {}, options = {}) {
  ...
  const suffix = range && Number.isFinite(Number(range.minRank)) && Number.isFinite(Number(range.maxRank))
    ...
    : 'all-ranks';
  const projection = options.projection || 'full-record-v3990_2';
...
export async function loadMajorBandsRankWindow(context, selectedBuckets = [], options = {}) {
  ...
  if (!buckets.length) {
    return {
      ...
      stats: {
        ...
        projectionVersion: options.projection || 'full-record-v3990_2',
  ...
  return {
    ...
    stats: {
      ...
      pageIdFilterCount: options.allowedIds instanceof Set ? options.allowedIds.size : 0,
      pageIdFilterVersion: 'major-bands-page-id-predecode-filter-v3990_2',
      projectionVersion: results[0]?.loaded?.projectionVersion || options.projection || 'full-record-v3990_2',
