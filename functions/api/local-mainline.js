...
import { clean, num, loadBackgroundMatchedRecords, groupScoreRecords, buildCandidatePositionContext } from '../_lib/background-position-engine.js';
  ...
  try {
    ...
    if (mode === 'meta') {
      return json({
        ...
        dataYear: 2026,
        audienceYear: 2027,
        index: getLocalMainlineMeta(),
        schools: getSchoolSummaries(),
        majors: getMajorSummaries()
      });
    }
    if (mode === 'school') {
      ...
      return json({
        ...
        boundary: '这些信息只用于家庭讨论和人工复核，不代表录取判断依据。'
      });
    }
    if (mode === 'major') {
      ...
      return json({
        ...
        normalizedCount: result.normalizedCount,
        dataReadOk: result.dataReadOk,
        boundary: '同名专业在不同学校的培养场景可能不同，需继续核验培养方案和招生章程。'
      });
    }
    if (mode === 'score' || mode === 'position') {
      ...
      const result = await loadMatchedRecords(context.request, context.env || {}, {
      ...
      return json({
        ...
        humanMessage: result.records.length ? '' : '这个参考分数附近暂时没有匹配到省内背景记录，可以换一个分数，或按学校 / 专业入口查看。',
    ...
    return json({
      ok: false,
      message: '省内背景数据暂时没有读取成功。',
      hint: '这不影响主页面专业初选，也不影响稍后按学校或专业继续查看。',
