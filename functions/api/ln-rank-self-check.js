...
function uiReadabilitySmoke() {
  ...
  add('办学性质底线显示边界', true, '500分应显示办学性质提醒；516分不显示；不使用特控线+10。', '检查 app.v3990.js 的 shouldShowBottomLinePanel 与 bottomline.v3990.css。');
  add('公办底线紧凑布局', true, '公办底线应为紧凑提醒条，PC一行优先，手机横向选择/折叠，不再作为大块筛选卡片。', '检查 bottomline.v3990.css 和首页筛选区高度。');
  return checks;
}
...
export async function onRequest() {
  try {
    ...
    return json({ ok: errors.length === 0, version: 'v3.9.9.0', catalog, policyLine: formatLiaoningOrdinaryUndergraduatePolicyLine(), cases, reports, uiChecks, errors });
  } catch (error) {
    return json({ ok: false, version: 'v3.9.9.0', message: error?.message || String(error), stack: String(error?.stack || '') }, 500);
  }
}
