...
async function loadMatched211Records(request, env, filters = {}) {
  return loadBackgroundMatchedRecords(request, env || {}, filters, {
    ...
    presentHit: (hit) => {
      if (!hit || !hit.matched) return null;
      return {
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (mode === 'score') {
      ...
      const result = await loadMatched211Records(context.request, context.env || {}, { candidateScore: score, maxScore: score + 10, minScore: score - 25, level, natureMode, max: url.searchParams.get('max') || 300 });
      ...
      return jsonOk({
        ...
        boundary: '这里不是录取判断。当前先按孩子输入分数对照辽宁 2025 物理类历史专业记录；2026 一分一段接入后，会先换算孩子位次，再按同一个参考窗口查看本地属性和 211 背景。'
      });
    }
