...
export async function onRequest(context){
 ...
 const school=String(url.searchParams.get('school')||'').trim(),entityId=String(url.searchParams.get('entity')||'').trim();
 ...
 payload.version=VERSION;payload.requestedSchool=school;payload.scope='school';if(entity){payload.school=entity.displayName;payload.entity=publicSchoolEntity(entity);if(response.status===404){payload.error='entity_source_not_found';payload.message='来源站暂时没有该分校或校区的独立记录；本站不会自动使用母体学校评价替代。';}}
