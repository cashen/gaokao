...
async function edgeRead(cache,key){if(!cache)return null;try{const response=await cache.match(key);if(!response?.ok)return null;const payload=await response.json();return payload?.ok&&payload?.sourceAvailable!==false?payload:null;}catch{return null;}}
