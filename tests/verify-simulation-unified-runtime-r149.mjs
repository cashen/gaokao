...
assert.match(html,/simulation-report\.css\?v=v016\.61-r151/,'unified simulation CSS revision missing');
...
assert.match(runtime,/const RELEASE='v016\.61-r151'/);
...
assert.equal(release.version,'simulation-workspace-v016.61');
assert.equal(release.revision,'r151-browser-runtime-hardening');
assert.equal(release.runtime,'/ln-rank/js/simulation-runtime.js');
assert.equal(release.runtimeRevision,'v016.61-r151');
...
assert.equal(release.pdfRuntimeRevision,'v016.61-r151');
...
console.log('simulation unified architecture contract r151: PASS');
