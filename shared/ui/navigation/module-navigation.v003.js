export const VERSION = 'module-navigation-v0.03';
...
function hasHeaderSurface() {
  return Boolean(document.querySelector('[data-ui-global-header],[data-ui-global-header-mount]'));
}

function ensureMobileNav(visit) {
  if (!hasHeaderSurface()) return null;
  ...
  document.body?.setAttribute('data-ui-navigation-standalone', 'true');
...
function removeStandaloneNav() {
  ...
  document.body?.removeAttribute('data-ui-navigation-standalone');
}
...
function applyNavigation() {
  ...
  const hasSurface = hasHeaderSurface();
