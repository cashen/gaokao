import { CURRENT_RELEASE } from '../../resources/release/current-release.js?v=3960_0';
import {
  readFamilyCandidateScore,
  readFamilySelectionItems,
  countFamilyPendingItems,
  resolveFamilyNextAction
} from '../../../ln-rank/js/domain/family-decision-contract.v3955_0.js?v=3960_0';
import { UI_LANGUAGE } from '../contracts/copy-contract.v3959_0.js?v=3960_0';
import { UI_PAGE_REGISTRY, UI_ORCHESTRATION_VERSION, getUiPage } from '../ui-registry.v3960_0.js?v=3960_0';
...
const STYLE_URLS=Object.freeze([
  '/shared/ui/tokens/foundation.v3959_0.css?v=3960_0',
  '/shared/ui/tokens/semantic.v3959_0.css?v=3960_0',
  '/shared/ui/shell/family-shell.v3960_0.css?v=3960_0'
]);
let mounted=false;
let scheduled=false;
...
function syncMobilePrimary(){
  ...
  const loading=Boolean(document.getElementById('queryButton')?.disabled&&document.body.classList.contains('is-filter-dirty'));
...
function scheduleAfterState(){
  globalThis.setTimeout(scheduleUpdate,0);
}
...
function bind(){
  document.addEventListener('input',event=>{
    if(event.target?.matches?.('#candidateScore,#pathCandidateScore,input,select'))scheduleAfterState();
  });
  document.addEventListener('change',scheduleAfterState);
  ...
  window.addEventListener('hashchange',()=>{syncMobileRoute();scheduleUpdate()});
  document.addEventListener('click',event=>{
    ...
    if(mobilePrimary&&document.body.classList.contains('ui-mobile-update-required')){
      ...
      if(query&&!query.disabled){event.preventDefault();query.click();}
    }
    if(event.target?.closest?.('[data-pool-index],#clearPool,[data-remove-id],button,a'))scheduleAfterState();
  });
