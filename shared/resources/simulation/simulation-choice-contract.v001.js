...
function projectIdentity(record = {}) {
  ...
  return {
    kind: isSino ? 'sino' : isSpecial ? 'special' : 'ordinary',
    label: isSino ? '中外合作/高收费（需核验）' : isSpecial ? '特殊培养/项目（需核验）' : '普通项目'
  };
}
...
function normalizeHistory(value, recordId = '') {
  ...
  return {
    status,
    recordId: text(value?.recordId || recordId),
    years: Object.fromEntries(YEARS.map(year => [year, normalizeYear(source[year], recordId)])),
...
function normalizeSource(value, fallback = 'simulation') {
  ...
  return {
    module,
    entry: text(raw.entry || 'manual') || 'manual',
    href: text(raw.href || ''),
    capturedAt: text(raw.capturedAt) || now()
  };
}
...
function normalizeSimulationChoice(input = {}, options = {}) {
  ...
  return {
    ...
    confirmedSchool,
    schoolCode,
    majorQuery: text(input.majorQuery),
    major,
    majorCode2026,
    majorRecordId: recordId,
    standardMajorName: text(input.standardMajorName),
    standardMajorCode: text(input.standardMajorCode),
    admissionProject,
...
function buildSimulationChoiceHref(choice = {}, options = {}) {
  ...
  href.searchParams.set('source', normalized.source.module);
  href.searchParams.set('entry', normalized.source.entry);
...
function writeSimulationChoiceHref(choice = {}, options = {}) {
  const href = buildSimulationChoiceHref(choice, options);
  const url = new URL(href, 'https://gaokao.powers.org.cn');
  const current = new URL(globalThis.location?.href || 'https://gaokao.powers.org.cn/ln-rank/simulation-report.html');
...
export {
  ...
  projectIdentity,
  isComplete,
  buildSimulationChoiceHref,
  parseSimulationChoiceHref,
  writeSimulationChoiceHref
};
