import { LIAONING_PHYSICS_EXAM_CONFIG, SUPPORTED_LIAONING_PHYSICS_YEARS } from '../exam/liaoning-physics.js';

const EXAM = LIAONING_PHYSICS_EXAM_CONFIG;

export const CURRENT_RELEASE = Object.freeze({
  display: 'v3.9.66.0',
  version: 'v3.9.66.0',
  asset: '3966_0',
  assetVersion: 'v3966_0',
  release: 'v3.9.66.0-three-year-rank-evidence-no-fenxi',
  releaseName: 'v3.9.66.0-three-year-rank-evidence-no-fenxi',
  label: 'three-year-rank-evidence-no-fenxi',
  dataYear: EXAM.dataYear,
  audienceYear: EXAM.audienceYear,
  rankTableYear: EXAM.rankYear,
  rankTableYears: SUPPORTED_LIAONING_PHYSICS_YEARS,
  region: EXAM.region,
  subject: EXAM.subject,
  noFenxiIncluded: true,
  resourceOwnershipVersion: 'resource-ownership-v3966_0',
  uiOrchestrationVersion: 'ui-orchestration-v3966_0',
  algorithmOrchestrationVersion: 'algorithm-orchestration-v3963',
  selectionWorkspaceVersion: 'selection-workspace-orchestration-v3966_0',
  searchIntentVersion: 'score-school-search-v3966_0',
  schoolAllModeVersion: 'school-all-mode-v3966_0',
  schoolUiGovernanceVersion: 'school-ui-governance-v3966_0',
  schoolModeMountVersion: 'school-mode-static-mount-v3966_0',
  runtimeCacheVersion: 'runtime-cache-coherence-v3966_0',
  staticContentTypeOwnershipVersion: 'static-content-type-ownership-v3966_0',
  cacheRecoveryVersion: 'cached-runtime-recovery-v3966_0',
  reportFrontendVersion: 'feishu-browser-v3966_0',
  historyEvidenceVersion: 'ln-physics-history-evidence-v3966_0',
  tongxueRuntimeVersion: 'tongxue-runtime-v159-r3966',
  resourceOwners: Object.freeze({
    release: '/shared/resources/release/current-release.js',
    releasePresenter: '/shared/resources/release/release-presenter.v3966_0.js',
    exam: '/shared/resources/exam/liaoning-physics.js',
    rankTables: '/functions/_lib/rank-table-provider.js',
    historyScoreRankEvidence: '/functions/_lib/historical-score-rank-evidence.js',
    historyEvidenceContract: '/shared/resources/exam/historical-score-rank-contract.js',
    historyEvidencePresenter: '/ln-rank/js/feature/major-pool/history-score-render.v3966_0.js',
    regions: '/shared/resources/geo/china-region-catalog.js',
    schools: '/shared/resources/schools/school-profile-center.js',
    schoolIdentity: '/shared/resources/schools/school-identity-center.js',
    majors: '/shared/resources/majors/major-catalog-contract.js',
    reports: '/shared/resources/reports/feishu-report-contract.v3966_0.js',
    reportFrontend: '/ln-rank/js/feature/feishu/index.v3966_0.js',
    reportFeedbackState: '/ln-rank/js/feature/feishu/report-controller.v3966_0.js',
    ui: '/shared/ui/ui-registry.v3965_0.js',
    uiActions: '/shared/ui/contracts/action-contract.v3959_0.js',
    uiSemantic: '/shared/ui/tokens/semantic.v3959_0.css',
    uiModeSwitch: '/shared/ui/components/mode-switch.v3963_0.css',
    algorithms: '/shared/algorithms/algorithm-registry.js',
    searchIntentStructure: '/ln-rank/index.html',
    runtimeCache: '/shared/resources/release/runtime-cache-contract.v3966_0.js',
    runtimeBootstrap: '/ln-rank/js/app.v3966_0.js',
    searchIntentState: '/ln-rank/js/workspace/selection-workspace-orchestrator.v3966_0.js',
    schoolResults: '/ln-rank/js/feature/school-majors/school-all-mode.v3966_0.js',
    mainPageStyles: '/ln-rank/css/ln-rank-workspace.v3964_0.css',
    historyEvidenceStyles: '/ln-rank/css/history-evidence.v3966_0.css',
    selectionPageStyles: '/ln-rank/css/selection-pool.v3964_0.css',
    selectionRuntime: '/ln-rank/js/selection-pool-runtime.v3966_0.js',
    selectionQuickEntryStructure: '/ln-rank/index.html#selectionPoolShell',
    tongxueRuntime: '/tongxue/app/tongxue-runtime-v159-r3966.js',
    tongxueState: '/tongxue/app/tongxue-runtime-controller-v159.js',
    reportHistoryPlacement: 'appendix-only'
  })
});

export function getCurrentRelease() {
  return CURRENT_RELEASE;
}

export function currentAssetQuery() {
  return CURRENT_RELEASE.assetVersion;
}
