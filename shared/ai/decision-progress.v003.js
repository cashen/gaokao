...
export function deriveDecisionProgress(workspace={}){
  ...
  const pairConfirmed=decisionCount(workspace,'school_major','keep')>0;
  rows.push(stage('school_major',pairConfirmed?'confirmed':pairs.length?'exploring':'not_started',pairConfirmed?'已有明确保留的学校×专业':pairs.length?`正在研究 ${pairs.slice(0,2).map(item=>item.label).join('、')}`:'还没有落到具体学校×专业',pairs.length?[]:['具体学校×专业']));

  const tradeoffConfirmed=decisionCount(workspace,'family_tradeoff','keep')>0;
  rows.push(stage('family_tradeoff',tradeoffConfirmed?'confirmed':tradeoffs.length>=2?'ready':tradeoffs.length?'exploring':'not_started',tradeoffConfirmed?'关键家庭取舍已经确认':tradeoffs.length>=2?'已经有足够的明确家庭条件参与比较':tradeoffs.length?'已有部分家庭条件，还不够完整':'就业、读研、地域、预算等取舍还没说清',tradeoffs.length>=2?[]:['至少两个明确取舍维度']));

  const planConfirmed=decisionCount(workspace,'plan','keep')>0;
  rows.push(stage('plan',planConfirmed?'confirmed':planCount>0?'ready':'not_started',planCount>0?`当前方案 ${planCount} 项`:'还没有形成可检查的家庭方案',planCount>0?[]:['导入或形成家庭方案']));
  ...
  return{version:AI_DECISION_PROGRESS_VERSION,stages:rows,recommendedStage:firstOpen?.key||'audit',completedCount,total:rows.length,majors,pairs,tradeoffDimensions:tradeoffs,selectionCount:planCount};
}
