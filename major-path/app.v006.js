...
import { mountMajorPathBackgroundContext, MAJOR_PATH_BACKGROUND_CONTEXT_VERSION } from './background-context.v002.js';
...
export const MAJOR_PATH_HUMAN_VERSION = 'major-path-human-v0.06';
export const MAJOR_PATH_VIEWPORT_VERSION = 'major-path-viewport-v0.06';
...
function contextForMajor(major) {
  const originalCode = String(sourceContext.majorCode || '').trim().toUpperCase();
  if (!major?.code || major.code === originalCode) return sourceContext;
...
await import('./app-core.v006.js?v=006_0');
...
function directContextForMajor(major, context = sourceContext) {
  const originalCode = String(sourceContext.majorCode || '').trim().toUpperCase();
  if (major.code === originalCode) {
    const summary = summarizeDecisionContext(context.decisionContext);
    const base = conciseSourceContext(context, major);
    if (context.contextState === 'omitted') {
      return {
        title: base.title,
        body: `${base.body} 原查询条件没有随链接带入，请返回来源页面核对分数和学校。`
      };
    }
    if (summary.lines.length) {
      return {
        title: base.title,
        body: `${base.body} 当前只读条件：${summary.lines.join(' · ')}。不会自动修改家庭方案；${context.returnState === 'available' ? '返回可恢复原查询。' : '可从返回按钮回到原入口。'}`
      };
    }
    return base;
  }
  ...
  action.textContent = context.sourceSurface === 'academic-background'
    ...
    : ['tongxue', 'tongxue-major'].includes(context.sourceSurface)
      ...
      : context.context === 'school' && context.school
        ...
        : '← 返回刚才的专业列表';
  action.dataset.majorPathReturn = context.sourceSurface || context.sourceType || 'independent';
  action.dataset.majorPathReturnState = context.returnState || 'missing';
}

function installDecisionActions(focus, major, source = sourceContext) {
...
function makePathwayFocus(shell, major, undergradSection, graduateSection, source = sourceContext) {
  ...
  if (renderState.directBoot) {
    const sourceView = directContextForMajor(major, source);
    ...
    note.dataset.majorPathSourceBoundary = major.code === String(sourceContext.majorCode || '').trim().toUpperCase()
      ...
      : 'continuation';
    note.innerHTML = `<strong>${sourceView.title}</strong><span>${sourceView.body}</span>`;
    focus.append(note);
  }
  ...
  installDecisionActions(focus, major, source);
...
function installChangeMajorAction(focus) {
  ...
  button.addEventListener('click', () => {
    ...
    removeQueryKeys(['majorCode', 'major', 'from', 'context', 'sourceKey', 'sourceMajor', 'sourceSurface', 'school', 'returnTo', 'returnState', 'contextState', 'decisionContext']);
...
function humanizeMajorResult(shell) {
  ...
  const viewContext = contextForMajor(major);
  const focus = makePathwayFocus(shell, major, undergrad, graduate, viewContext);
  const background = mountMajorPathBackgroundContext({ shell, major, focus, sourceContext: viewContext, direct: renderState.directBoot });
  const studentVoice = mountMajorPathStudentVoice({ shell, major, anchor: background || focus, sourceContext: viewContext });
...
window.__MAJOR_PATH_HUMAN_META__ = Object.freeze({
  ...
  coreVersion: window.__MAJOR_PATH_META__?.version || 'major-path-core-v0.06',
