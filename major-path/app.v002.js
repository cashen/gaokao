...
function findMajor(code) {
  return MAJOR_CATALOG_2026.find(item => item.code === code) || null;
}
...
function svgNode({ x, y, width = 230, height = 54, title, meta = '', kind = 'plain', majorCode = '', extraClass = '' }) {
  ...
  return `<g class="graph-node graph-node-${escapeHtml(kind)} ${escapeHtml(extraClass)}"${interactive}>
    <rect x="${x}" y="${y}" width="${width}" height="${height}" rx="14"></rect>
    <text x="${x + 14}" y="${y + 22}"><tspan class="graph-node-title">${escapeHtml(shortLabel(title))}</tspan>${meta ? `<tspan class="graph-node-meta" x="${x + 14}" dy="18">${escapeHtml(shortLabel(meta, 20))}</tspan>` : ''}</text>
  </g>`;
}

function graphPath(x1, y1, x2, y2, kind = 'catalog') {
  const bend = Math.max(40, Math.abs(x2 - x1) * .42);
  return `<path class="graph-edge graph-edge-${escapeHtml(kind)}" d="M ${x1} ${y1} C ${x1 + bend} ${y1}, ${x2 - bend} ${y2}, ${x2} ${y2}"></path>`;
}
...
function graduateRouteText(relation) {
  const routes = [...(relation.shared?.academic || []), ...(relation.shared?.professional || [])];
  return routes.map(item => `${item.code} ${item.name}`).join('、');
}

function renderDirectorySvg(graph) {
  const shown = graph.siblings.slice(0, 8);
  const summary = Math.max(0, graph.siblings.length - shown.length);
  const height = Math.max(340, 170 + (shown.length + (summary ? 1 : 0)) * 66);
  let edges = graphPath(260, 67, 330, 67, 'catalog') + graphPath(570, 67, 650, 67, 'catalog');
  let nodes = svgNode({x:40,y:40,width:220,title:graph.hierarchy.discipline.name,meta:`本科门类 ${graph.hierarchy.discipline.code || ''}`,kind:'discipline'})
    + svgNode({x:330,y:40,width:240,title:graph.hierarchy.majorClass.name,meta:`本科专业类 ${graph.hierarchy.majorClass.code || ''}`,kind:'class'})
    + svgNode({x:650,y:40,width:280,title:graph.focus.name,meta:`当前专业 ${graph.focus.code}`,kind:'focus',majorCode:graph.focus.code});
  shown.forEach((item, index) => {
    const y = 135 + index * 66;
    edges += graphPath(570, 67, 650, y + 27, 'catalog');
    nodes += svgNode({x:650,y,width:280,title:item.name,meta:`同类专业 ${item.code}`,kind:'sibling',majorCode:item.code});
  });
  if (summary) {
    const y = 135 + shown.length * 66;
    edges += graphPath(570,67,650,y + 27,'catalog');
    nodes += svgNode({x:650,y,width:280,title:`还有 ${summary} 个同类专业`,meta:'下方列表可全部查看',kind:'summary'});
  }
...
function renderNeighborSvg(graph) {
  const same = [...graph.siblingRelations].sort((a,b) => b.sharedRouteCount - a.sharedRouteCount || a.target.code.localeCompare(b.target.code)).slice(0, 5);
  const cross = graph.crossNeighbors.slice(0, 5);
  const routes = [
    ...(graph.pathway.academic || []).map(item => ({...item,kind:'academic'})),
    ...(graph.pathway.professional || []).map(item => ({...item,kind:'professional'}))
  ].slice(0, 6);
  const rows = Math.max(same.length, cross.length, routes.length, 1);
  const height = 190 + rows * 66;
  let edges = '';
  let nodes = svgNode({x:360,y:40,width:260,title:graph.focus.name,meta:`当前专业 ${graph.focus.code}`,kind:'focus',majorCode:graph.focus.code});
  nodes += `<text class="graph-group-title" x="40" y="150">同专业类</text><text class="graph-group-title" x="370" y="150">共享的读研导航</text><text class="graph-group-title" x="700" y="150">跨专业类升学交叉</text>`;
  same.forEach((relation,index) => {
    const y = 170 + index * 66;
    edges += graphPath(360,67,270,y + 27,'catalog');
    nodes += svgNode({x:40,y,width:230,title:relation.target.name,meta:relation.sharedRouteCount ? `共享 ${relation.sharedRouteCount} 个读研方向` : '同一本科专业类',kind:'sibling',majorCode:relation.target.code});
  });
  routes.forEach((route,index) => {
    const y = 170 + index * 66;
    edges += graphPath(490,94,490,y,'graduate');
    nodes += svgNode({x:370,y,width:240,title:route.name,meta:`${route.code} · ${route.kind === 'academic' ? '一级学科' : '专业学位类别'}`,kind:'graduate'});
  });
  cross.forEach((relation,index) => {
    const y = 170 + index * 66;
    edges += graphPath(620,67,700,y + 27,'cross');
    nodes += svgNode({x:700,y,width:240,title:relation.target.name,meta:`共享 ${relation.sharedRouteCount} 个读研方向`,kind:'cross',majorCode:relation.target.code});
  });
  if (!same.length) nodes += `<text class="graph-empty" x="40" y="192">当前专业类没有其他本科专业</text>`;
...
function renderClassSvg(classGraph) {
  const shown = classGraph.majors.slice(0, 12);
  const summary = Math.max(0, classGraph.majors.length - shown.length);
  const height = Math.max(330, 120 + (shown.length + (summary ? 1 : 0)) * 62);
  let edges = graphPath(250,67,330,67,'catalog');
  let nodes = svgNode({x:40,y:40,width:210,title:classGraph.discipline.name,meta:`本科门类 ${classGraph.discipline.code || ''}`,kind:'discipline'})
    + svgNode({x:330,y:40,width:250,title:classGraph.majorClass.name,meta:`本科专业类 ${classGraph.majorClass.code || ''}`,kind:'class'});
  shown.forEach((major,index) => {
    const y = 25 + index * 62;
    edges += graphPath(580,67,660,y + 27,'catalog');
    nodes += svgNode({x:660,y,width:270,title:major.name,meta:major.code,kind:'sibling',majorCode:major.code});
  });
  if (summary) {
    const y = 25 + shown.length * 62;
    edges += graphPath(580,67,660,y + 27,'catalog');
    nodes += svgNode({x:660,y,width:270,title:`还有 ${summary} 个专业`,meta:'下方候选列表完整保留',kind:'summary'});
  }
...
function renderCandidateSvg(candidateGraph, query = '') {
  const groups = candidateGraph.groups.slice(0, 6);
  let y = 34;
  let edges = '';
  let nodes = svgNode({x:35,y:40,width:210,title:`“${query}”`,meta:'家长输入',kind:'query'});
  for (const group of groups) {
    const groupY = y;
    nodes += svgNode({x:330,y:groupY,width:250,title:group.majorClass,meta:`${group.majorClassCode} · ${group.discipline}`,kind:'class'});
    edges += graphPath(245,67,330,groupY + 27,'catalog');
    for (const major of group.majors.slice(0, 5)) {
      nodes += svgNode({x:660,y,width:270,title:major.name,meta:major.code,kind:'sibling',majorCode:major.code});
      edges += graphPath(580,groupY + 27,660,y + 27,'catalog');
      y += 62;
    }
    y += 24;
  }
  const height = Math.max(310, y + 30);
  return `<svg class="relationship-svg" viewBox="0 0 980 ${height}" role="img" aria-label="${escapeHtml(query)}候选专业归类关系图"><g>${edges}</g><g>${nodes}</g></svg>`;
}

function graphLegend() {
  return `<div class="graph-legend" aria-label="图例"><span><i class="legend-line catalog"></i>本科目录硬关系</span><span><i class="legend-line graduate"></i>研究生升学导航</span><span><i class="legend-line cross"></i>跨专业类升学交叉</span></div>`;
}

function warmMajorCopy(graph) {
  const sharedSiblingCount = graph.siblingRelations.filter(item => item.sharedRouteCount > 0).length;
  const siblingText = graph.siblings.length
    ? `它在“${graph.focus.majorClass}”这个专业家族里还有 ${graph.siblings.length} 个兄弟专业${sharedSiblingCount ? `，其中 ${sharedSiblingCount} 个与它共享当前页面已验证的研究生导航方向` : ''}。`
    : `它所在的“${graph.focus.majorClass}”当前没有其他本科专业。`;
  const crossText = graph.crossNeighborTotal
    ? `另外还能找到 ${graph.crossNeighborTotal} 个有结构证据的跨专业类升学交叉点，图中只展示证据更集中的一部分。`
    : '目前没有必要为了“扩展选择”硬凑跨专业类关系。';
  return `${siblingText}${crossText} 这些节点适合一起比较，但不是“课程一样”或“可以无条件平替”的结论；具体课程仍要看学校培养方案。`;
}

function relationshipList(graph) {
  const same = graph.siblingRelations.slice(0, 12);
  const cross = graph.crossNeighbors.slice(0, 8);
  return `<div class="relationship-fallback"><div><h4>同专业类可以一起看</h4>${same.length ? `<div class="neighbor-list">${same.map(item => `<button type="button" class="neighbor-chip" data-major-code="${escapeHtml(item.target.code)}"><strong>${escapeHtml(item.target.name)}</strong><span>${escapeHtml(item.label)}${item.sharedRouteCount ? ` · ${escapeHtml(graduateRouteText(item))}` : ''}</span></button>`).join('')}</div>` : '<p>当前专业类没有其他本科专业。</p>'}</div><div><h4>跨专业类的升学交叉</h4>${cross.length ? `<div class="neighbor-list">${cross.map(item => `<button type="button" class="neighbor-chip cross" data-major-code="${escapeHtml(item.target.code)}"><strong>${escapeHtml(item.target.name)}</strong><span>${escapeHtml(item.label)} · ${escapeHtml(graduateRouteText(item))}</span></button>`).join('')}</div>` : '<p>当前没有证据足够强的跨专业类节点，不为了“看起来丰富”强行补关系。</p>'}</div></div>`;
}

function renderMajorRelationshipSection(graph) {
  return `<section class="relationship-section" data-major-relationship-graph="${escapeHtml(graph.focus.code)}" data-graph-mode="directory">
    <div class="relationship-head"><div><p class="eyebrow">把专业放回关系里看</p><h3>专业关系图谱</h3></div><span class="graph-truth-chip">13门类 · 92专业类 · 883专业</span></div>
    <div class="relationship-warm"><strong>家长先这样理解</strong><p>${escapeHtml(warmMajorCopy(graph))}</p></div>
    <div class="graph-tabs" role="tablist" aria-label="专业关系图视角"><button type="button" role="tab" aria-selected="true" data-graph-mode="directory">按本科目录看</button><button type="button" role="tab" aria-selected="false" data-graph-mode="neighbors">看相邻选择与读研交叉</button></div>
    <p class="graph-mobile-hint">关系图可以左右滑动；点专业节点可继续查看。</p>
    <div class="graph-viewport" data-graph-panel>${renderDirectorySvg(graph)}</div>
    ${graphLegend()}
    ${relationshipList(graph)}
    <div class="graph-boundary"><strong>“相邻”不等于“平替”。</strong> ${escapeHtml(MAJOR_RELATIONSHIP_GRAPH_META.neighborBoundary)}</div>
  </section>`;
}

function renderGraphMode(section, mode) {
  const major = findMajor(section?.dataset?.majorRelationshipGraph || '');
  const graph = major ? RELATIONSHIPS.buildMajorGraph(major) : null;
  if (!graph || !section) return;
  section.dataset.graphMode = mode;
  for (const button of section.querySelectorAll('[data-graph-mode]')) button.setAttribute('aria-selected', String(button.dataset.graphMode === mode));
  const panel = section.querySelector('[data-graph-panel]');
  if (panel) panel.innerHTML = mode === 'neighbors' ? renderNeighborSvg(graph) : renderDirectorySvg(graph);
}

function graduateSecondLevelNote() {
  return `<div class="second-level-note"><strong>为什么这里没有硬画“全国统一二级学科”？</strong><p>国家《研究生教育学科专业目录》统一到学科门类、一级学科和专业学位类别。二级学科与专业领域由学位授予单位在授权权限内按规定自主设置与调整，所以真正查某校读研方向时，要继续看该校当年招生专业目录，而不是由本页替国家编一棵不存在的统一二级树。</p></div>`;
}
...
function renderMajor(major) {
  ...
  const graph = RELATIONSHIPS.buildMajorGraph(major);
  ...
  els.result.innerHTML = `<article class="result-shell" data-result-major="${escapeHtml(major.code)}">
    ...
    <div class="divider"></div>
    ${graph ? renderMajorRelationshipSection(graph) : ''}
    <div class="divider"></div>
    <section><h3 class="section-heading">继续读研，可以先看哪些国家目录方向</h3>${graduateSecondLevelNote()}<div class="degree-grid"><div class="degree-card"><h4>学术学位</h4><p>研究生目录里的一级学科。更偏学科研究与学术训练，但不同学校培养方案差异很大。</p><div class="degree-list">${academicHtml}</div></div><div class="degree-card"><h4>专业学位</h4><p>面向职业与应用场景的专业学位类别。代码属于研究生目录，不是本科专业代码的延伸。</p><div class="degree-list">${professionalHtml}</div></div></div>${renderFields(pathway.professionalFields)}<div class="relation-note"><strong>关系边界：</strong>${escapeHtml(pathway.note)}<br>${escapeHtml(pathway.boundary)}</div></section>
    <div class="divider"></div>
...
function renderAmbiguityGraph(intent, candidates) {
  if (['major_class','class_stem'].includes(intent.semanticType) && intent.label) {
    const classGraph = RELATIONSHIPS.buildClassGraph(intent.label);
    if (classGraph) {
      const routeCount = classGraph.commonGraduateRoutes.length;
      return `<section class="class-insight" data-class-relationship-graph="${escapeHtml(intent.label)}"><div class="relationship-warm"><strong>先把“${escapeHtml(intent.label)}”当成一个专业家族</strong><p>这是教育部本科目录里的专业类，下面有 ${classGraph.majors.length} 个具体本科专业。它们属于同一目录家族，但不等于课程完全一样。${routeCount ? `这一专业类还能从现有升学导航中看到 ${routeCount} 个研究生一级学科/专业学位类别交叉点。` : ''}</p></div><p class="graph-mobile-hint">关系图可以左右滑动；点专业节点可直接进入。</p><div class="graph-viewport">${renderClassSvg(classGraph)}</div>${graphLegend()}</section>`;
    }
  }
  const candidateGraph = RELATIONSHIPS.buildCandidateGraph(candidates);
  if (!candidateGraph?.candidateCount) return '';
  return `<section class="class-insight"><div class="relationship-warm"><strong>先看这些候选分别属于哪里</strong><p>名字里都带着你输入的词，不代表它们是同一个专业。图先把候选放回各自专业类，再由你选择，避免只看名字误判。</p></div><p class="graph-mobile-hint">关系图可以左右滑动；点专业节点可直接进入。</p><div class="graph-viewport">${renderCandidateSvg(candidateGraph, intent.query)}</div>${graphLegend()}</section>`;
}
...
function renderDisambiguation(intent, { expanded = false } = {}) {
  ...
  els.result.innerHTML = `<section class="result-shell disambiguation-shell" data-disambiguation-query="${escapeHtml(intent.query)}">
    ...
    ${renderAmbiguityGraph(intent, candidates)}
    ...
    <div class="relation-note"><strong>这里做的是语义消歧：</strong>家长简称、专业类简称和关键词只用来找候选，不会静默改成某一个正式本科专业。你点中具体专业后，页面才进入专业家族关系和本科→研究生路径。</div>
  </section>`;
  els.suggestions.hidden = true;
  els.result.scrollIntoView({ behavior: 'smooth', block: 'start' });
}
...
function submitQuery(query) {
  ...
  els.result.innerHTML = '<section class="result-shell"><div class="answer-first"><strong>没有匹配到规范本科专业</strong><p>请尝试输入教育部本科专业全名、六位本科专业代码、专业类名称或家长常用简称。模糊说法只会进入候选消歧，不会被强行当成一个具体专业。</p></div></section>';
}

function renderBrowseClass(className, target) {
  const graph = RELATIONSHIPS.buildClassGraph(className);
...
function renderBrowse() {
  ...
  els.disciplineChips.addEventListener('click', event => {
    ...
    els.classBrowser.dataset.discipline = discipline;
    els.classBrowser.innerHTML = `<h3 class="class-title">${escapeHtml(discipline)} · 先选专业类</h3><div class="class-list">${[...classMap.entries()].map(([name, majors]) => `<button type="button" class="class-chip" data-major-class="${escapeHtml(name)}">${escapeHtml(name)} · ${majors.length}</button>`).join('')}</div><div id="browseMajors" class="browse-class-panel" hidden></div>`;
  });
  els.classBrowser.addEventListener('click', event => {
    const classButton = event.target.closest('[data-major-class]');
    if (classButton) {
      renderBrowseClass(classButton.dataset.majorClass, els.classBrowser.querySelector('#browseMajors'));
      return;
    }
    const majorButton = event.target.closest('[data-major-code]');
    if (!majorButton) return;
    const major = findMajor(majorButton.dataset.majorCode);
    if (major) { els.input.value = major.name; renderMajor(major); }
  });
}

function activateMajorFromElement(element) {
  const major = findMajor(element?.dataset?.majorCode || '');
...
els.suggestions.addEventListener('click', event => {
  ...
  const major = findMajor(button.dataset.majorCode);
  if (major) {
    els.input.value = major.name;
    els.suggestions.hidden = true;
    renderMajor(major);
...
els.result.addEventListener('click', event => {
  const tab = event.target.closest('.graph-tabs [data-graph-mode]');
  if (tab) {
    renderGraphMode(tab.closest('[data-major-relationship-graph]'), tab.dataset.graphMode);
    return;
  }
  const majorButton = event.target.closest('[data-major-code]');
  if (majorButton && activateMajorFromElement(majorButton)) return;
...
els.result.addEventListener('keydown', event => {
  ...
  const node = event.target.closest('.graph-node[data-major-code]');
  if (!node) return;
  event.preventDefault();
  activateMajorFromElement(node);
});
...
window.__MAJOR_PATH_META__ = Object.freeze({
  ...
  relationshipGraphVersion: MAJOR_RELATIONSHIP_GRAPH_META.version,
