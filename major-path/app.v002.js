...
function suggestionItem(item,index=0) {
  return `<button type="button" class="suggestion${index===0?' active':''}" role="option" data-major-code="${escapeHtml(item.code)}"><span class="suggestion-main"><span class="suggestion-name">${escapeHtml(item.name)}</span><span class="suggestion-meta">${escapeHtml(majorContext(item))}</span></span><span class="suggestion-type">${escapeHtml(item.code)}</span></button>`;
}
function renderSuggestions(query) {
  const intent=SEARCH.resolve(query,{limit:8});
  if(intent.kind==='empty'){ els.suggestions.hidden=true; els.suggestions.innerHTML=''; return; }
...
function renderMajor(major) {
  ...
  els.result.innerHTML=`<article class="result-shell" data-result-major="${escapeHtml(major.code)}"><header class="result-head"><div><p class="eyebrow">你高考报的是</p><h2>${escapeHtml(major.name)}</h2><div class="meta"><span class="meta-chip">本科门类：${escapeHtml(focus.discipline)}</span><span class="meta-chip">${classMeta}</span></div></div><span class="code-badge">本科专业代码 ${escapeHtml(major.code)}</span></header><section class="answer-first"><strong>一句话先看懂</strong><p>${escapeHtml(oneLine)}</p></section><div class="divider"></div><section><h3 class="section-heading">本科目录里的位置</h3><div class="undergrad-line">${undergradPath}</div>${hierarchy?.majorClass?.isUnlisted?'<div class="relation-note"><strong>目录提醒：</strong>交叉学科的这类专业在2026标准目录中专业类未单列，所以这里直接从门类连到具体专业，不补造“1400专业类”。</div>':''}</section><div class="divider"></div>${graph?renderMajorRelationshipSection(graph):''}<div class="divider"></div><section><h3 class="section-heading">继续读研，可以先看哪些国家目录方向</h3>${graduateSecondLevelNote()}<div class="degree-grid"><div class="degree-card"><h4>学术学位</h4><p>研究生目录里的一级学科。更偏学科研究与学术训练，但不同学校培养方案差异很大。</p><div class="degree-list">${academicHtml}</div></div><div class="degree-card"><h4>专业学位</h4><p>面向职业与应用场景的专业学位类别。代码属于研究生目录，不是本科专业代码的延伸。</p><div class="degree-list">${professionalHtml}</div></div></div>${renderFields(pathway.professionalFields)}<div class="relation-note"><strong>关系边界：</strong>${escapeHtml(pathway.note)}<br>${escapeHtml(pathway.boundary)}</div></section><div class="divider"></div><section><h3 class="section-heading">权威依据</h3><div class="source-list">${sourceItem(UNDERGRAD_SOURCE,`2026本科目录 · 当前库 ${MAJOR_CATALOG_2026_META.total||883} 个专业`)}${sourceItem(GRADUATE_CATALOG_SOURCES.catalog2022,'研究生一级学科与专业学位类别，自2023年起实施；二级学科/专业领域由学位授予单位按规定自主设置调整')}${pathway.professionalFields.length?sourceItem(GRADUATE_CATALOG_SOURCES.admissions2026,'仅用于页面中明确点名的2026报考条件与2027已公布变化'):''}</div></section></article>`;
  const url=new URL(location.href);url.searchParams.set('major',major.name);history.replaceState(null,'',`${url.pathname}?${url.searchParams.toString()}`);document.title=`${major.name}：专业关系与读研方向 - 专业升学地图`;els.result.scrollIntoView({behavior:'smooth',block:'start'});
}
...
els.input.addEventListener('keydown',event=>{if(event.key==='Escape')els.suggestions.hidden=true;if(event.key==='Enter'){event.preventDefault();submitQuery(els.input.value);}});
els.suggestions.addEventListener('click',event=>{const button=event.target.closest('[data-major-code]');if(!button)return;const intent=SEARCH.resolve(els.input.value,{limit:8}),major=findMajor(button.dataset.majorCode);if(major){els.input.value=major.name;els.suggestions.hidden=true;renderMajor(major);if(intent.kind==='direct'&&intent.major?.code===major.code)renderRecognition(intent);}});
...
window.__MAJOR_PATH_META__=Object.freeze({version:'major-path-v0.02',undergraduateCount:MAJOR_CATALOG_2026.length,relationVersion:UNDERGRAD_GRADUATE_PATHWAY_META.version,searchVersion:MAJOR_SEARCH_INTENT_META.version,relationshipGraphVersion:MAJOR_RELATIONSHIP_GRAPH_META.version,relationshipStats:RELATIONSHIPS.stats()});
