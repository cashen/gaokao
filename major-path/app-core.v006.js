...
const CORE_VERSION = 'major-path-core-v0.06';
...
function majorHref(major) {
  if (!major?.code) return '/major-path/';
  const params = new URLSearchParams({ majorCode: major.code, major: major.name });
  return `/major-path/?${params.toString()}`;
}
...
function renderMajor(major) {
  ...
  const url=new URL(location.href);const previousCode=url.searchParams.get('majorCode');url.searchParams.set('major',major.name);url.searchParams.set('majorCode',major.code);const next=`${url.pathname}?${url.searchParams.toString()}`;if(previousCode&&previousCode!==major.code)history.pushState({majorCode:major.code},'',next);else history.replaceState({...(history.state||{}),majorCode:major.code},'',next);document.title=`${major.name}：专业关系与读研方向 - 专业升学地图`;els.result;
}
...
els.result.addEventListener('click',event=>{const target=event.target instanceof Element?event.target:null;const tab=target?.closest('.graph-tabs [data-graph-mode]');if(tab){renderGraphMode(tab.closest('[data-major-relationship-graph]'),tab.dataset.graphMode);return;}const majorButton=target?.closest('[data-major-code]');if(majorButton&&activateMajorFromElement(majorButton)){if(majorButton instanceof HTMLAnchorElement)event.preventDefault();return;}const expand=target?.closest('[data-expand-disambiguation]');if(expand){const intent=SEARCH.resolve(expand.dataset.expandDisambiguation,{limit:100});if(intent.kind==='ambiguous')renderDisambiguation(intent,{expanded:true});}});
...
document.addEventListener('click',event=>{const target=event.target instanceof Element?event.target:null;if(target&&!target.closest('.input-wrap'))els.suggestions.hidden=true;});

renderBrowse();
const initialUrl=new URL(location.href);const initialCode=initialUrl.searchParams.get('majorCode');const initial=initialUrl.searchParams.get('major');if(initialCode){const initialMajor=findMajor(initialCode);if(initialMajor){els.input.value=initialMajor.name;renderMajor(initialMajor);}else if(initial){submitQuery(initial);}}else if(initial)submitQuery(initial);
window.__MAJOR_PATH_META__=Object.freeze({version:CORE_VERSION,undergraduateCount:MAJOR_CATALOG_2026.length,relationVersion:UNDERGRAD_GRADUATE_PATHWAY_META.version,searchVersion:MAJOR_SEARCH_INTENT_META.version,relationshipGraphVersion:MAJOR_RELATIONSHIP_GRAPH_META.version,relationshipStats:RELATIONSHIPS.stats()});
