...
try {
  for (const device of devices) {
    ...
    assert.equal(state.bodyRelease, 'v3.9.90.2', `${device.name}: body release`);
    assert.equal(state.htmlRelease, 'v3.9.90.2', `${device.name}: html release`);
    ...
    assert.equal(state.shell?.generation, 'v3990_2', `${device.name}: shell generation`);
    assert.equal(state.shell?.release, 'v3.9.90.2', `${device.name}: shell release`);
    assert.equal(state.scripts.length, 1, `${device.name}: one bootstrap script`);
    assert.ok(state.scripts[0].includes('/ln-rank/js/ux/family-home.v3990_2.js?v=3990_2'), `${device.name}: current home script`);
