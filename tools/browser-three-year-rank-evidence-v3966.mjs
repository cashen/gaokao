...
const baseUrl = process.env.V3966_BASE_URL || 'http://127.0.0.1:8765';
const artifactDir = process.env.V3966_ARTIFACT_DIR || '/tmp/v3966-three-year-browser';
...
const historyEvidence = {
  version: 'ln-physics-history-evidence-v3966_0',
  region: 'ln',
  subject: 'physics',
  primaryYear: 2026,
  years: {
...
try {
  for (const testCase of cases) {
    ...
    try {
      ...
      assert.equal(runtime.release, 'v3.9.66.0');
      assert.equal(runtime.workspace, 'selection-workspace-orchestration-v3966_0');
      assert.equal(runtime.school, 'school-all-mode-v3966_0');
      ...
      const history = page.locator('.history-score--appendix').first();
      ...
      assert.equal(reportPosts[0].selectedRecords[0].historyEvidence.version, 'ln-physics-history-evidence-v3966_0');
      ...
      const detail = page.locator('.history-evidence-detail').first();
...
console.log(JSON.stringify({ ok: true, contract: 'three-year-rank-evidence-browser-v3966_0', cases: results }, null, 2));
