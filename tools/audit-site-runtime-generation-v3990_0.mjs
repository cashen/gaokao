...
assert.equal(SITE_RUNTIME_CONTRACT.queryVersion, '3990_0');
assert.equal(SITE_RUNTIME_CONTRACT.releaseVersion, 'v3.9.90.0');
...
for (const key of currentKeys) {
  ...
  if (!pageRouteKeys.has(key)) assert.ok(String(value).includes('3990_0'), `current entrypoint is not v3990_0: ${key} -> ${value}`);
}
...
assert.equal(SITE_RUNTIME_CONTRACT.owners.interaction, '/shared/ui/interaction/interaction-transaction.v3990_0.js');
...
const interaction = fs.readFileSync('shared/ui/interaction/interaction-transaction.v3990_0.js', 'utf8');
for (const marker of [
...
const interactionCss = fs.readFileSync('shared/ui/interaction/interaction-transaction.v3990_0.css', 'utf8');
