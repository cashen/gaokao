...
const moduleUrl = file => `${pathToFileURL(path.join(root, file)).href}?audit=v3964_1`;

const main = read('ln-rank/index.html');
const selection = read('ln-rank/selection-pool.html');
const app = read('ln-rank/js/app.v3964_1.js');
const appRuntime = read('ln-rank/js/app-runtime.v3964_1.js');
...
const selectionEntry = read('ln-rank/js/selection-pool.v3964_1.js');
const selectionRuntime = read('ln-rank/js/selection-pool-runtime.v3964_1.js');
const shell = read('shared/ui/shell/family-shell.v3964_1.js');
const registry = read('shared/ui/ui-registry.v3964_1.js');
...
assert.equal(CURRENT_RELEASE.assetVersion, 'v3964_1');
...
assert.equal(CURRENT_RELEASE.runtimeCacheVersion, 'runtime-cache-coherence-v3964_1');
assert.equal(CURRENT_RELEASE.resourceOwnershipVersion, 'resource-ownership-v3964_1');
assert.equal(CURRENT_RELEASE.staticContentTypeOwnershipVersion, 'static-content-type-ownership-v3964_1');
assert.equal(CURRENT_RELEASE.cacheRecoveryVersion, 'cached-runtime-recovery-v3964_1');
...
assert.ok(app.includes("import('./app-runtime.v3964_1.js?v=3964_1')"));
...
assert.ok(shell.includes('ui-registry.v3964_1.js?v=3964_1'));
assert.ok(appRuntime.includes('runtime-cache-contract.v3964_1.js?v=3964_1'));
...
console.log(JSON.stringify({
  ok: true,
  contract: 'human-task-ui-v3964_1',
