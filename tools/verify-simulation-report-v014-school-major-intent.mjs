...
for(const expected of ['loadSchoolNameResolver','createMajorCatalogResolver','/api/ai/major-history','schoolKeyword','actual','实际招生记录','不会自动','网络失败','轻微错字'])if(!js.includes(expected))throw new Error(`runtime missing ${expected}`);
...
for(const expected of ['所有学校','实际招生记录','机械','网络','计算机','测空技术与仪器','不幻觉'])if(!plan.includes(expected)&&!(expected==='所有学校'&&plan.includes('任意学校')))throw new Error(`plan missing ${expected}`);
