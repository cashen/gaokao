...
for(const expected of ['loadSchoolNameResolver','createMajorCatalogResolver','/api/ai/major-history','schoolKeyword','实际招生记录','不会自动替你的决定','暂时无法在线核对','相近输入','不会把全国专业目录候选冒充成该校招生事实'])if(!js.includes(expected))throw new Error(`runtime missing ${expected}`);
