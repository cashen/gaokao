...
const OUTPUT_DIR = path.join(ROOT, 'ln-rank/data/ai-major-history-v3990_2');
const VERSION = 'ai-major-history-index-v3990_2';
...
for (let shard = 0; shard < SHARD_COUNT; shard += 1) {
  ...
  shardEntries.push({ shard, file: `/ln-rank/data/ai-major-history-v3990_2/${relative}`, bytes: fs.statSync(file).size, sha256: sha256(file), majorCount: Object.keys(majorMap).length, recordCount: Object.values(majorMap).reduce((sum, rows) => sum + rows.length, 0) });
}
const manifest = {
  version: VERSION,
