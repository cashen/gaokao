...
const schoolAll = read('ln-rank/js/feature/school-majors/school-all-mode.v3963_0.js');
...
const modeSwitchCss = read('shared/ui/components/mode-switch.v3963_0.css');
...
includesAll(root, ['辽宁高考家庭决策工作台','先圈出一批可以讨论的专业','近期公开评论','时间只帮助安排节奏','family-shell.v3961_0.js?v=3963_0','data-current-release'], 'homepage');
...
includesAll(main, [
  ...
  'app.v3963_0.js?v=3963_0','selection-workspace.v3963_0.css?v=3963_0',
  'school-all-mode.v3963_0.css?v=3963_0','mode-switch.v3963_0.css?v=3963_0',
  'semantic.v3959_0.css?v=3962_2','family-decision-workspace.v3955_0.css',
  'id="schoolViewModeMount"','id="schoolAllResultsPanel"','class="ui-mode-switch search-intent-switch"',
...
assert.ok(filterStart >= 0 && modeIndex >= 0 && modeIndex < filterStart, 'search intent switch must live at the top of the family console');

includesAll(selection, ['id="selected-list"','id="family-review"','检查已选专业','检查真实承接与待确认事项','生成家庭复核报告','其他保存方式','selection-pool.v3960_0.js?v=3963_0','同一算法快照',`data-release="${CURRENT_RELEASE.display}"`,'data-current-release'], 'selection page');
...
includesAll(schoolAll, ['createSelectionPoolAdapter','buildTongxueSchoolHref',"version: 'school-all-mode-v3963_0'","mountPolicy: 'static-result-owner'",'assertStaticStructure',"sharedControlOwner: 'selection-workspace-orchestration-v3963'",'普通招生记录','需要单独核验的特殊项目','UI_ACTION_COPY','data-school-detail-toggle','expandedRecordKey'], 'school-all family flow');
...
includesAll(modeSwitchCss, ['.ui-mode-switch','.search-intent-switch','writing-mode:horizontal-tb','container-name:ui-mode-switch','@container ui-mode-switch (max-width:280px)'], 'shared mode-switch CSS');
...
includesAll(zyPage, ['zy2026.v3959_0.js?v=3963_0','data-current-release'], 'ZY2026 page');
...
assert.equal(release.schoolAllModeVersion, 'school-all-mode-v3963_0');
assert.equal(active.schoolAllModeVersion, 'school-all-mode-v3963_0');
assert.equal(release.schoolUiGovernanceVersion, 'school-ui-governance-v3963_0');
assert.equal(active.schoolUiGovernanceVersion, 'school-ui-governance-v3963_0');
assert.equal(release.schoolModeMountVersion, 'school-mode-static-mount-v3963_0');
assert.equal(active.schoolModeMountVersion, 'school-mode-static-mount-v3963_0');
assert.equal(active.structure2026.js, '../zy2026/assets/zy2026.v3959_0.js');
for (const entry of ['js/app.v3963_0.js','js/workspace/selection-workspace-orchestrator.v3963_0.js','js/workspace/family-card-presenter.v3961_0.js','js/feature/school-majors/school-all-mode.v3963_0.js','../shared/ui/shell/family-shell.v3961_0.js','../shared/algorithms/algorithm-registry.js']) assert.ok(active.jsEntry.includes(entry), `active entry missing ${entry}`);
...
assert.ok(active.cssEntry.includes('../shared/ui/components/mode-switch.v3963_0.css'));
assert.ok(active.cssEntry.includes('../shared/ui/shell/family-shell.v3960_0.css'));
assert.ok(active.cssEntry.includes('css/selection-workspace.v3963_0.css'));
assert.ok(active.cssEntry.includes('css/school-all-mode.v3963_0.css'));
