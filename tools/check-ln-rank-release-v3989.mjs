...
for (const p of [
  ...
  'ln-rank/js/app.v3989.js','ln-rank/js/selection-pool.v3987.js','ln-rank/js/self-check.v3989.js',
  'ln-rank/js/keyword-token-classifier.v3989.js','ln-rank/js/review-point-builder.v3989.js',
...
for (const html of ['ln-rank/index.html','ln-rank/selection-pool.html']) {
  const text = read(html);
  assert(text.includes('版本：v3.9.8.9'), `${html} visible footer version not v3.9.8.9`);
  assert(!/版本：v3\.9\.8\.6/.test(text), `${html} still shows v3.9.8.6`);
}
assert(read('ln-rank/js/major-trend-render.v3989.js').includes('版本：v3.9.8.9'), 'major trend visible version not v3.9.8.9');
assert(!/major-trend-rules\.v3986/.test(read('ln-rank/js/major-trend-render.v3989.js')), 'major trend render still imports v3986 rules');
...
assert(/快速判断带|band-tab/.test(read('ln-rank/css/layout-shell.v3989.css')), 'score band quick strip css missing');
assert(/控制面板|search-workbench/.test(read('ln-rank/css/search-workbench.v3989.css')), 'compact search workbench css missing');
assert(/UI readability/.test(read('ln-rank/css/major-search.v3989.css')), 'UI readability css missing');
assert(read('ln-rank/index.html').includes('bottomline.v3989.css'), 'index does not use bottomline v3989');
assert(read('ln-rank/index.html').includes('search-workbench.v3989.css'), 'index does not use compact search workbench v3989');
assert(/办学性质提醒|公办底线紧凑|v3\.9\.8\.9/.test(read('ln-rank/css/bottomline.v3989.css') + read('ln-rank/js/app.v3989.js')), 'bottomline compact assets missing');
...
console.log('check-ln-rank-release-v3989 ok');
