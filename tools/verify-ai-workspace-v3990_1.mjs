...
async function testFactAnswerUsesTurnScoreWithoutPollutingCurrentScope(){
  let workspace=createAiWorkspace({examContext:{score:580,rank:25000},decisionStage:'narrow_region',activeView:{score:580,regionKeys:['shenyang'],majorKeywords:['机械']}});
  const result=await orchestrateAiTurn({request:new Request('https://local.invalid/api/ai/turn'),env:{}},{workspace:compactAiWorkspaceForServer(workspace),input:'600分位次是多少'});
  assert.equal(result.ok,true);assert.equal(result.commitView,false);assert.equal(result.result.rank.rankEnd,14235);assert.equal(result.resolvedView.score,600);assert.equal(result.resolvedView.majorKeywords[0],'机械');assert.deepEqual(result.resolvedView.regionKeys,['shenyang']);assert.equal(result.result.decisionStage,'narrow_region');assert.ok(result.turnRecord.changeSummary.includes('不改变你正在看的条件'));
  workspace=applyAiWorkspaceEvent(workspace,result.event);workspace=applyAiWorkspaceEvent(workspace,{type:'result_committed',payload:{result:result.result,turn:result.turnRecord}});
  assert.equal(workspace.activeView.score,580);assert.deepEqual(workspace.activeView.regionKeys,['shenyang']);assert.deepEqual(workspace.activeView.majorKeywords,['机械']);assert.equal(workspace.examContext.score,580);assert.equal(workspace.examContext.rank,25000);
}
...
testPatchSemantics();testNoImplicitDimensionClearing();testGeoSingleSource();testPersistentDecisionProfileAndTurns();testDeltaExplainsScope();testPrivacyBudget();await testModelCannotInventFilters();await testFactAnswerUsesTurnScoreWithoutPollutingCurrentScope();testFactsAndSkillAttribution();testHumanUiSource();
console.log(JSON.stringify({ok:true,version:'v3991_0',checks:['patch-inherit-set-add-remove-clear','580-mechanical-liaoning-shenyang','city-shared-region','no-implicit-clearing','persistent-decision-profile','conversation-turn-history','causal-delta','privacy-budget','model-filter-isolation','fact-turn-score-no-scope-pollution','rank-fact','skill-attribution','human-ui-source']},null,2));
