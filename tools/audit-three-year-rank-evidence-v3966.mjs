...
assert.equal(CURRENT_RELEASE.display, 'v3.9.66.0');
assert.equal(CURRENT_RELEASE.assetVersion, 'v3966_0');
...
const anchors = {
  ...
  2025: [[707,11],[700,29],[693,77],[600,14365],[367,118109],[150,143368]],
...
const rows = Object.fromEntries([2024,2025,2026].map(year => [year, lookupScoreRank({ year, score: year === 2024 ? 600 : 600, region:'ln', subject:'physics' })]));
const sample = {
...
const sourceChecks = {
  ...
  school: read('ln-rank/js/feature/school-majors/school-all-mode.v3966_0.js'),
  mainCard: read('ln-rank/js/feature/major-pool/render.v3966_0.js'),
  selection: read('ln-rank/js/selection-pool-runtime.v3966_0.js'),
...
assert.match(sourceChecks.mainCard, /history-score-render\.v3966_0/);
assert.match(sourceChecks.selection, /history-score-render\.v3966_0/);
...
fs.writeFileSync(path.join(root,'analysis/2026/three-year-rank-evidence-audit-v3966.json'),JSON.stringify(result,null,2)+'\n');
