...
for(const viewport of viewports){
  ...
  await page.route('**/api/ai/major-history**',async route=>{
    ...
    school==='沈阳工业大学' ? [
      {school:'沈阳工业大学',standardMajorName:'自动化',majorCode2026:'080801'},
      {school:'沈阳工业大学',standardMajorName:'机械设计制造及其自动化',majorCode2026:'080202'},
    ].filter(r=>!majorInputs.length||majorInputs.some(x=>r.standardMajorName.includes(x)||r.majorCode2026.startsWith(x))) : [];
...
// Reproduce the concrete PC failure reported from the preview: similar school candidates
// must be visibly distinguishable and the real "选这所" and "选这个" text controls must work.
{
  const browser=await chromium.launch({headless:true});
  const context=await browser.newContext({viewport:{width:1280,height:900},locale:'zh-CN'});
  await context.addInitScript(()=>{localStorage.clear();localStorage.setItem('gaokao:simulation-report:v002',JSON.stringify({version:2,studentName:'',subjectTrack:'辽宁物理类（物化生）',totalScore:'',volunteers:[{id:'v017-shenyang-pc',order:1,school:'',majorCode:'',majorName:'',confirmedSchool:'',familyStatus:'待讨论',history:{years:{}}}],selectionPool:[]}))});
  const page=await context.newPage();
  ...
  await card.locator('[data-field="majorCode"]').fill('自动化');
  const majorChoice=card.locator('[data-v017-major-code]').filter({hasText:'自动化'}).first();
  await majorChoice.waitFor({state:'visible',timeout:5000});
  await majorChoice.getByText('选这个',{exact:true}).click();
  await page.waitForFunction(el=>/已确认：沈阳工业大学 · 自动化 · 080801/.test(el?.textContent||''),await helper.elementHandle(),{timeout:5000});
  const persisted=await page.evaluate(()=>JSON.parse(localStorage.getItem('gaokao:simulation-report:v002')).volunteers[0]);
  assert.equal(persisted.school,'沈阳工业大学');assert.equal(persisted.confirmedSchool,'沈阳工业大学');assert.equal(persisted.majorCode,'080801');assert.equal(persisted.majorName,'自动化');
