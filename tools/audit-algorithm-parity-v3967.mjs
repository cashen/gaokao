...
].map(record => ({ ...record, canonicalPosition:resolveCanonicalPosition({candidateScore:candidate.score,candidateRank:candidate.rank,recordScore:record.score2026,recordRank:record.rank2026,rangePreset:'standard'}) }));
const score = rankResultRecords(records,{intent:'score-search',sortMode:'canonical-staged',diversify:false});
const school = rankResultRecords(records,{intent:'school-search',sortMode:'position-near',diversify:false});
...
const schoolAgain=rankResultRecords(records,{intent:'school-search',sortMode:'position-near',diversify:false});
...
for (const rel of ['functions/api/major-bands.js','functions/api/school-majors.js']) assert.match(fs.readFileSync(rel,'utf8'),/result-ranking\.v3967_0/);
console.log(JSON.stringify({ok:true,ranking:RESULT_RANKING_VERSION,trend:TREND_INTERPRETATION_VERSION,historicalRankSelection:HISTORICAL_RANK_SELECTION_VERSION,scoreOrder:score.map(x=>x.id),schoolOrder:school.map(x=>x.id)},null,2));
