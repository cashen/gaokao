...
assert(index.includes('ln-v3-alpha1-fix3'), 'index body class missing fix3');
assert(debug.includes('ln-v3-alpha1-fix3'), 'debug body class missing fix3');
assert(version.includes("name: 'V3.0.0.alpha1.fix3｜Debug残留状态清理修正版'"), 'version name missing fix3');
assert(version.includes('v300alpha1fix3-20260512'), 'version stamp missing fix3');
...
const stateStore = read('fenxi/v3/assets/js/state-store.v3.js');
assert(stateStore.includes('normalizeState'), 'state-store normalization missing');
assert(stateStore.includes('out.childPreference.weights = {}'), 'childPreference empty weights cleanup missing');
assert(stateStore.includes('V3骨架已启动'), 'ui lastMessage clean default missing');
...
assert(childDebug.includes('finally') && childDebug.includes('childPreference: before.childPreference') && childDebug.includes('ui: before.ui'), 'child debug selftest must restore and clean state');
assert(childDebug.includes('自测状态回滚干净'), 'child debug selftest clean rollback check missing');
assert(!fs.existsSync(path.join(root, 'index.html')), 'root index.html should not exist in fenxi-only package');
console.log('All V3.0.0.alpha1.fix3 checks passed.');
