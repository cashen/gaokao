...
const baseUrl = process.env.V3990_0_BASE || 'http://127.0.0.1:8765';
const artifactDir = process.env.V3990_0_ARTIFACT_DIR || '/tmp/v3990-0-native-chooser';
...
try {
  for (const profile of profiles) {
    ...
    try {
      ...
      assert.equal(initial.release, 'v3.9.90.0');
      assert.equal(initial.generation, 'v3990_0');
      assert.equal(initial.interactionVersion, 'interaction-transaction-v3990_0');
      ...
      assert.equal(requestUrl.searchParams.get('interactionVersion'), 'interaction-transaction-v3990_0');
      assert.equal(requestUrl.searchParams.get('siteRuntimeGeneration'), 'v3990_0');
...
console.log(JSON.stringify({
  ...
  release: 'v3.9.90.0',
  generation: 'v3990_0',
  interaction: 'interaction-transaction-v3990_0',
