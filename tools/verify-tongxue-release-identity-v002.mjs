...
for (const source of [page, changelog]) {
  assert.ok(source.includes('data-release="v3.9.90.3"'), 'Tongxue page release marker drift');
  assert.ok(source.includes('data-site-runtime-generation="v3990_3"'), 'Tongxue runtime generation marker drift');
...
assert.ok(page.includes('能力版本 v1.5.9 · 全站发布 v3.9.90.3'), 'Tongxue index footer must expose capability and site release');
assert.ok(changelog.includes('<span class="badge">当前全站发布</span><h2>v3.9.90.3</h2>'), 'Tongxue changelog must lead with current site release');
...
assert.ok(changelog.includes('当前全站发布：<span data-current-release>v3.9.90.3</span>'), 'Tongxue changelog footer release drift');
assert.ok(release.includes("version: 'v3.9.90.3'"), 'canonical release drift');
assert.ok(release.includes("siteRuntimeGeneration: 'v3990_3'"), 'canonical generation drift');
