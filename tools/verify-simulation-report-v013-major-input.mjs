...
for (const expected of [
  'simulation-report-v014-school-major-intent.css?v=014-school-major-intent',
...
for (const expected of [
  ...
  '/api/ai/major-history',
  'schoolKeyword',
  'schoolGroundedCandidates',
  'typoSchoolCandidates',
  'broadTerms',
  '不会把全国专业目录候选冒充成该校招生事实',
  'event.stopImmediatePropagation',
  'persistCard'
]) {
  if (!js.includes(expected)) throw new Error(`runtime missing ${expected}`);
}
for (const expected of ['.major-input-suggestions', '.major-suggestion', '.school-input-suggestions-v014', 'min-height:46px']) {
  if (!css.includes(expected)) throw new Error(`css missing ${expected}`);
}
for (const expected of ['任意学校', '实际招生记录', '测空技术与仪器', '计算机', '不幻觉']) {
