...
try {
  for (const device of devices) {
    ...
    await page.waitForFunction(() =>
      globalThis.__GAOKAO_HOME_RUNTIME__?.version === 'family-home-runtime-v3990_0'
      && globalThis.__GAOKAO_UI__?.version === 'family-shell-v3990_0'
    );
    ...
    assert.equal(state.bodyRelease, 'v3.9.90.0', `${device.name}: body release`);
    assert.equal(state.htmlRelease, 'v3.9.90.0', `${device.name}: html release`);
    assert.equal(state.bodyGeneration, 'v3990_0', `${device.name}: body generation`);
    assert.equal(state.htmlGeneration, 'v3990_0', `${device.name}: html generation`);
    assert.equal(state.visibleRelease, 'v3.9.90.0', `${device.name}: visible release`);
    assert.equal(state.runtime?.version, 'family-home-runtime-v3990_0', `${device.name}: runtime`);
    assert.equal(state.runtime?.generation, 'v3990_0', `${device.name}: runtime generation`);
    assert.equal(state.runtime?.release, 'v3.9.90.0', `${device.name}: runtime release`);
    assert.match(state.runtime?.shellOwner || '', /family-shell\.v3990_0\.js$/);
    assert.match(state.runtime?.stateOwner || '', /family-decision-contract\.v3970_0\.js$/);
    assert.equal(state.shell?.version, 'family-shell-v3990_0', `${device.name}: shell owner`);
    assert.equal(state.shell?.generation, 'v3990_0', `${device.name}: shell generation`);
    assert.equal(state.shell?.release, 'v3.9.90.0', `${device.name}: shell release`);
    assert.equal(state.scripts.length, 1, `${device.name}: one bootstrap script`);
    assert.ok(state.scripts[0].includes('/ln-rank/js/ux/family-home.v3990_0.js?v=3990_0'), `${device.name}: current home script`);
...
console.log(JSON.stringify({
  ...
  release: 'v3.9.90.0',
  generation: 'v3990_0',
  runtime: 'family-home-runtime-v3990_0',
  shell: 'family-shell-v3990_0',
