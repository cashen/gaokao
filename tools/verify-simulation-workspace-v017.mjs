...
assert.equal(manifest.version,'simulation-workspace-v016.46');
assert.equal(manifest.revision,'r136-school-search-debounce');
assert.equal(manifest.runtimeRevision,'v016.46-r136');
for(const src of [
  '/ln-rank/js/simulation-report-v016-pdf.js?v=v016.46-r136',
  '/ln-rank/js/simulation-report-v016-history-reference-only.js?v=v016.46-r136',
  '/ln-rank/js/simulation-report-v016-legacy-render-guard.js?v=v016.46-r136',
  '/ln-rank/js/simulation-report-v017-responsive-input.js?v=v016.46-r136',
  '/ln-rank/css/simulation-report-v017-responsive-input.css?v=v016.46-r136'
...
for(const expected of ['getMetadata','province','city','level','candidateView','loadSchoolCatalog','resolveUnifiedSchoolQuery','SEARCH_DEBOUNCE_MS','searchTimers','searchSequences','scheduleSearch']) assert.ok(worker.includes(expected),`worker missing ${expected}`);
...
console.log('simulation-workspace-v016.46 contract: PASS');
