...
assert.equal(manifest.version,'simulation-workspace-v016.42');
assert.equal(manifest.revision,'r132-candidate-lifecycle-and-history-hydration');
for(const src of [
  '/ln-rank/js/simulation-report-v016-legacy-render-guard.js?v=v016.42-r132',
  '/ln-rank/js/simulation-report-v016-pdf.js?v=v016.42-r132',
  '/ln-rank/js/simulation-report-v017-responsive-input.js?v=v016.42-r132',
  '/ln-rank/css/simulation-report-v017-responsive-input.css?v=v016.42-r132'
...
for(const expected of ['390','768','1280','选这所','选这个','沈阳工业大学','电气','电气工程及其自动化','confirmedSchool','majorName','history.years','getByText']) assert.ok(browser.includes(expected),`browser gate missing ${expected}`);
...
console.log('simulation-workspace-v016.42 contract: PASS');