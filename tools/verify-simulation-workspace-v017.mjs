...
for(const expected of ['A4','splitPages','createPageShell','two short volunteer entries share a page','break-inside:avoid','pdf-page','第 ${pageNo} / ${totalPages} 页']) {
  if(expected==='two short volunteer entries share a page') continue;
  assert.ok(pdf.includes(expected),`pdf runtime missing ${expected}`);
}
assert.ok(pdf.includes('rawPages'));
assert.ok(pdf.includes('page.offsetHeight>usableBottom'));
assert.ok(pdf.includes('历史分数/位次仅作参考'));
assert.ok(!pdf.includes('· 需核验'));
assert.ok(!pdf.includes('有历史记录需要核对'));
assert.ok(pdf.includes('pdf-line'));
