...
for(const count of [1,2,6,10,12,14]){
  const pages=await page.evaluate(async state=>window.__GAOKAO_SIMULATION_PDF_V01650__.measurePages(state),makeState(count));
  assert.equal(pages,1,`expected ${count} compact volunteers to fit on one A4 page, got ${pages}`);
}
const thirty=await page.evaluate(async state=>window.__GAOKAO_SIMULATION_PDF_V01650__.measurePages(state),makeState(30));
assert.ok(thirty>=2,`30 volunteers should paginate, got ${thirty}`);
assert.equal(errors.length,0,errors.join('\n'));
await browser.close();
console.log('simulation-workspace-v016.51 PDF pagination: PASS');
