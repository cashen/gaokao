...
import { PRODUCTION_RESOURCE_VERIFICATION_CONTRACT as CONTRACT } from '../shared/governance/production-resource-verification-contract.v3990_1.js';

const read = path => fs.readFileSync(path, 'utf8');
const exists = value => fs.existsSync(String(value).split('?')[0].replace(/^\//, ''));
const manifest = JSON.parse(read('ln-rank/site-active-generation.v3990_1.json'));
...
const PAGINATION_GUARD = '/ln-rank/js/feature/major-pool/pagination-snapshot-guard.v3990_1.js';
const QUERY_CACHE_VERSION = 'major-bands-query-execution-cache-single-heavy-v3990_1';
const EXECUTION_GATE_VERSION = 'major-bands-query-execution-gate-v3990_1';
const QUERY_MEMORY_MODE = 'requested-band-lightweight-order-current-page-v3990_1';
const REQUEST_BAND_LOADER_VERSION = 'major-bands-rank-bucket-loader-bounded-all-band-v3990_1';
const RESULT_ORDER_VERSION = 'major-bands-result-order-ephemeral-v3990_1';

assert.equal(CONTRACT.version, 'production-resource-graph-verification-v3990_1');
...
assert.equal(manifest.resourceGraph.productionVerificationOwner, '/shared/governance/production-resource-verification-contract.v3990_1.js');
...
assert.equal(manifest.currentGenerationInternalModules.majorBandsPaginationSnapshotGuard, `${PAGINATION_GUARD}?v=3990_1`);
...
containsAll(productionWorkflow, [
  ...
  'verify-production-resource-graph-v3990_1.mjs',
  'verify-production-pagination-snapshot-guard-v3990_1.mjs',
  'pagination-snapshot-guard.v3990_1.js', 'PRODUCTION_PAGINATION_SNAPSHOT_EVIDENCE',
...
excludesAll(productionWorkflow, [
  '\"executionGateVersion\":\"major-bands-query-execution-gate-v3990_1\"',
...
containsAll(deployWorkflow, [
  ...
  "display: 'v3.9.90.1'", "siteRuntimeGeneration: 'v3990_1'",
  'audit-canonical-release-version-v3990_1.mjs',
  'audit-production-resource-verification-v3990_1.mjs',
  'audit-site-runtime-generation-v3990_1.mjs',
  'wrangler@4.28.1 pages deploy .', '--commit-hash="$GITHUB_SHA"',
  'verify-production-resource-graph-v3990_1.mjs', 'verify-production-baseline-v3971.mjs',
  'EXPECTED_RELEASE: v3.9.90.1', 'RELEASE_SHA: ${{ github.sha }}',
...
// v3990_1 intentionally keeps one maintained verifier algorithm per production
...
const adapterPairs = [
  ['tools/verify-production-resource-graph-v3990_1.mjs', 'verify-production-resource-graph-v3990_0.mjs', '.verify-production-resource-graph-v3990_1.generated.mjs'],
  ['tools/verify-production-pagination-snapshot-guard-v3990_1.mjs', 'verify-production-pagination-snapshot-guard-v3990_0.mjs', '.verify-production-pagination-snapshot-guard-v3990_1.generated.mjs']
];
for (const [path, sourceName, generatedName] of adapterPairs) {
  const adapter = read(path);
  containsAll(adapter, [
    sourceName, generatedName,
    ".replaceAll('v3.9.90.0', 'v3.9.90.1')",
    ".replaceAll('v3990_0', 'v3990_1')",
    ".replaceAll('3990_0', '3990_1')",
...
containsAll(baselineVerifier, [
  "PREVIOUS_V3990_RELEASE = 'v3.9.90.0'", "CURRENT_V3990_RELEASE = 'v3.9.90.1'",
...
const executionCache = read('functions/_lib/major-bands-query-execution-cache.v3990_1.js');
...
const queryKernel = read('functions/_lib/major-bands-rank-query-kernel.v3990_1.js');
...
const bucketLoader = read('functions/_lib/major-bands-rank-bucket-loader.v3990_1.js');
...
const resultOrder = read('functions/_lib/major-bands-result-order.v3990_1.js');
