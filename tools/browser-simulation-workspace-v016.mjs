...
for(const viewport of viewports){
  ...
  const majorStarted=performance.now();await major.fill('');await major.pressSequentially('机械',{delay:15});await page.locator('[data-v017-major]').first().waitFor({state:'visible',timeout:350});assert.ok(performance.now()-majorStarted<350,`${viewport.name}: major local candidates must render without network`);
  await major.press('Backspace');assert.equal(await major.inputValue(),'机',`${viewport.name}: Backspace must update the active major input synchronously`);await major.type('械');await page.locator('[data-v017-major]').first().waitFor({state:'visible',timeout:350});
  ...
  await page.waitForFunction(el=>el?.value==='机械',await major.elementHandle());
  await page.evaluate(()=>{const el=document.querySelector('[data-field="majorCode"]');const dt=new DataTransfer();dt.setData('text/plain','080801');el.focus();el.dispatchEvent(new ClipboardEvent('paste',{bubbles:true,clipboardData:dt}));el.value='080801';el.dispatchEvent(new InputEvent('input',{bubbles:true,inputType:'insertFromPaste',data:'080801'}));});
  await page.waitForFunction(el=>el?.value==='080801',await major.elementHandle());
