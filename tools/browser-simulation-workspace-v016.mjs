...
for(const viewport of viewports){
  ...
  await major.fill('');
  await page.evaluate(()=>{const el=document.querySelector('[data-field="majorCode"]');const dt=new DataTransfer();dt.setData('text/plain','机械');el.focus();el.dispatchEvent(new ClipboardEvent('paste',{bubbles:true,clipboardData:dt}));el.value='机械';el.dispatchEvent(new InputEvent('input',{bubbles:true,inputType:'insertFromPaste',data:'机械'}));});
  await page.waitForFunction(el=>el?.value==='机械',await major.elementHandle());
  await page.waitForTimeout(600);assert.equal(await major.inputValue(),'机械');
