...
for (const p of [
  ...
  'ln-rank/js/app.v3988.js','ln-rank/js/selection-pool.v3987.js','ln-rank/js/self-check.v3988.js',
  'ln-rank/js/keyword-token-classifier.v3988.js','ln-rank/js/review-point-builder.v3988.js',
...
for (const html of ['ln-rank/index.html','ln-rank/selection-pool.html']) {
  const text = read(html);
  assert(text.includes('版本：v3.9.8.8'), `${html} visible footer version not v3.9.8.8`);
  assert(!/版本：v3\.9\.8\.6/.test(text), `${html} still shows v3.9.8.6`);
}
assert(read('ln-rank/js/major-trend-render.v3987.js').includes('版本：v3.9.8.8'), 'major trend visible version not v3.9.8.8');
...
assert(/UI readability/.test(read('ln-rank/css/major-search.v3988.css')), 'UI readability css missing');
assert(read('ln-rank/index.html').includes('bottomline.v3988.css'), 'index does not use bottomline v3988');
assert(read('ln-rank/index.html').includes('search-workbench.v3988.css'), 'index does not use compact search workbench v3988');
assert(/办学性质提醒|公办底线紧凑|v3\.9\.8\.8/.test(read('ln-rank/css/bottomline.v3988.css') + read('ln-rank/js/app.v3988.js')), 'bottomline compact assets missing');
...
console.log('check-ln-rank-release-v3988 ok');
