...
// Main score-search renderer and orchestration.
copy('ln-rank/js/feature/major-pool/render.v3964_0.js', 'ln-rank/js/feature/major-pool/render.v3966_0.js', source =>
  exactReplace(source,
    "./history-score-render.v3964_0.js?v=3964_0",
    "./history-score-render.v3966_0.js?v=3966_0",
    'major-pool history presenter'));
copy('ln-rank/js/feature/major-pool/index.v3964_0.js', 'ln-rank/js/feature/major-pool/index.v3966_0.js', source =>
  exactReplace(source,
    "./render.v3964_0.js?v=3964_0",
    "./render.v3966_0.js?v=3966_0",
    'major-pool render owner'));
...
// School-first mode must use the same current and historical presenter.
...
// Selection workspace keeps the same evidence object and renderer.
...
// Main runtime wrappers remain thin, but point only at the v3966 consumers.
...
copy('ln-rank/js/app.v3965_0.js', 'ln-rank/js/app.v3966_0.js', source => {
  ...
  next = exactReplace(next,
    ...
    'app runtime entry');
  next = next.replace('[ln-rank-runtime-v3965_0]', '[ln-rank-runtime-v3966_0]');
  return next;
});

// Server report builders consume the shared semantic formatter.
...
// Browser report payload and AI payload carry evidence, not only naked numbers.
update('ln-rank/js/feature/feishu/report-controller.v3965_0.js', source =>
  exactReplace(source,
...
// Activate immutable entries and shared evidence CSS.
update('ln-rank/index.html', source => {
  let next = exactReplace(source,
    '/ln-rank/js/app.v3965_0.js?v=3965_0',
    '/ln-rank/js/app.v3966_0.js?v=3966_0',
    'main app entry');
  if (!next.includes('/ln-rank/css/history-evidence.v3966_0.css?v=3966_0')) {
    next = exactReplace(next,
      '</head>',
      '  <link rel="stylesheet" href="/ln-rank/css/history-evidence.v3966_0.css?v=3966_0" />\n</head>',
      'main history CSS');
  }
  return next;
});
update('ln-rank/selection-pool.html', source => {
  let next = exactReplace(source,
    '/ln-rank/js/selection-pool.v3965_0.js?v=3965_0',
...
console.log(JSON.stringify({
  ok: true,
  version: 'v3966_0',
  policy: 'single-three-year-rank-provider-and-evidence-presenter'
}, null, 2));
