...
check(manifest.get('historyMatch',{}).get('exact')==8929,'manifest exact history count');check(manifest.get('historyMatch',{}).get('unmatched')==2699,'manifest unmatched history count')
count=0
for chunk in manifest.get('chunks',[]):
    path=ROOT/'fenxi'/chunk['file'];check(path.exists(),f'missing chunk {path}')
    if path.exists():
        payload=json.loads(path.read_text(encoding='utf-8'));rows=payload if isinstance(payload,list) else payload.get('records',[]);check(len(rows)==chunk['recordCount'],f'chunk count mismatch {chunk["id"]}');count+=len(rows)
...
check('data-current-release' in unified and '当前发布：' in unified,'unified page shared release presentation');check('2024—2026' in unified,'unified page three-year copy');check('相比多数专业，更难报了' in unified and '相比多数专业，更容易报了' in unified,'human difficulty copy');check('不承诺录取结果' in unified,'result boundary');check('/ln2026.html#score-band' in redirect,'legacy score-band redirect')
check('major-difficulty-2026.v3965_0.js' in unified,'difficulty current UI orchestration wrapper')
check('major-difficulty-2026.v3964_1.js' not in unified,'difficulty legacy wrapper must not remain active')
difficulty_wrapper=(ROOT/'ln-rank/js/major-difficulty-2026.v3965_0.js').read_text(encoding='utf-8')
check('major-difficulty-2026.v3953_0.js' in difficulty_wrapper,'ascending score-band core behind current UI wrapper')
check('shared/resources/release/release-presenter.v3965_0.js?v=3965_0' in difficulty_wrapper,'difficulty shared release presenter')
check('shared/ui/shell/family-shell.v3965_0.js?v=3965_0' in difficulty_wrapper,'difficulty current shared UI shell')
release_presenter=(ROOT/'shared/resources/release/release-presenter.v3965_0.js').read_text(encoding='utf-8')
check('current-release.js?v=3965_0' in release_presenter,'difficulty current shared release owner')
check((ROOT/'ln-rank/js/major-difficulty-2026.v3964_1.js').exists(),'difficulty previous immutable wrapper preserved')
...
check(zy.get('productVersion')=='v3.9.53.0','zy2026 data product version');check(zy.get('records2026')==11628,'zy2026 record count');check(10000<=int(zy.get('records2025',0))<=12000,'zy2026 2025 count');check(zy.get('recordDelta')==zy.get('records2026')-zy.get('records2025'),'zy2026 delta')
...
print(json.dumps({'ok':True,'records':count,'strictCompleteCount':analysis['strictCompleteCount'],'analysisVersion':analysis['version'],'releasePresentation':'shared-current-release-v3965_0','zyRelations':zy.get('relations')},ensure_ascii=False))
