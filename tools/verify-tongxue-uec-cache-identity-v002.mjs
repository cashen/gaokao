...
const resultCore = fs.readFileSync('tongxue/app/tongxue-runtime-result-view-core-v159.js', 'utf8');
...
const RESULT_VIEW_IDENTITY = '/tongxue/app/tongxue-runtime-result-view-v159.js?v=159-flow006&r=r051-major-scope-clarity';
...
// v002: result-view is now a compatibility entry and the single business owner is the core.
assert.ok(resultView.includes('createBaseResultView'), 'compat result-view wrapper missing');
for (const marker of [
  'data-student-voice-scope="major"',
  "PAGE_VERSION = 'v1.5.9-uec01-evidence02'",
  '先看这两件事',
  '这些概括从哪来？'
]) assert.ok(resultCore.includes(marker), `result core lost major voice marker: ${marker}`);
assert.ok(!resultCore.includes('AI总结'), 'result core must not expose AI implementation label');
assert.ok(!resultCore.includes('provenance'), 'result core must not expose provenance jargon');
...
console.log(JSON.stringify({
  ...
  resultViewBusinessOwner: 'tongxue-runtime-result-view-core-v159',
  humanCopy: true,
  ownerCount: 1
}, null, 2));
