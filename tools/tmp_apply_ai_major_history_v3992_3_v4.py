...
replacements = [
    ...
    (
        ...
        '''call_anchor="await schoolHistoryAliasJourney(page,device.name);"'''
    )
]
for old, new in replacements:
    if source.count(old) != 1:
        raise SystemExit(f'v4 patch-source anchor mismatch: {source.count(old)} for {old[:100]}')
