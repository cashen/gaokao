...
for (const p of [
  ...
  'ln-rank/js/app.v3910rc.js','ln-rank/js/selection-pool.v3987.js','ln-rank/js/self-check.v3910rc.js',
  'ln-rank/js/keyword-token-classifier.v3910rc.js','ln-rank/js/review-point-builder.v3910rc.js',
...
for (const html of ['ln-rank/index.html','ln-rank/selection-pool.html']) {
  const text = read(html);
  assert(text.includes('版本：v3.9.10rc'), `${html} visible footer version not v3.9.10rc`);
  assert(!/版本：v3\.9\.8\.6/.test(text), `${html} still shows v3.9.8.6`);
}
assert(read('ln-rank/js/major-trend-render.v3910rc.js').includes('版本：v3.9.10rc'), 'major trend visible version not v3.9.10rc');
assert(!/major-trend-rules\.v3986/.test(read('ln-rank/js/major-trend-render.v3910rc.js')), 'major trend render still imports v3986 rules');
...
assert(/快速判断带|band-tab/.test(read('ln-rank/css/layout-shell.v3910rc.css')), 'score band quick strip css missing');
assert(/控制面板|search-workbench/.test(read('ln-rank/css/search-workbench.v3910rc.css')), 'compact search workbench css missing');
assert(/UI readability/.test(read('ln-rank/css/major-search.v3910rc.css')), 'UI readability css missing');
assert(read('ln-rank/index.html').includes('bottomline.v3910rc.css'), 'index does not use bottomline v3910rc');
assert(read('ln-rank/index.html').includes('search-workbench.v3910rc.css'), 'index does not use compact search workbench v3910rc');

assert(read('ln-rank/index.html').includes('ui-density.v3910rc.css'), 'index does not use ui-density v3910rc');
assert(read('ln-rank/index.html').includes('responsive-control-panel.v3910rc.css'), 'index does not use responsive control panel v3910rc');
assert(/--ui-control-max|control panel|搜索控制台/.test(read('ln-rank/css/ui-density.v3910rc.css')), 'ui density rc css missing control panel rules');
assert(/Android|任务流|keyword/.test(read('ln-rank/css/responsive-control-panel.v3910rc.css')), 'responsive control panel rc css missing mobile rules');
assert(/办学性质提醒|公办底线|bottomline/.test(read('ln-rank/css/bottomline.v3910rc.css') + read('ln-rank/js/app.v3910rc.js')), 'bottomline compact assets missing');
...
console.log('check-ln-rank-release-v3910rc ok');
