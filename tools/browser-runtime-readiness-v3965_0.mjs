...
const artifactDir = process.env.RUNTIME_CACHE_ARTIFACT_DIR || '/tmp/runtime-cache-v3965_0';
...
try {
...

  {
    ...
    page.on('pageerror', error => pageErrors.push(String(error?.stack || error)));
    await page.route('**/ln-rank/js/app-runtime.v3965_0.js*', route => {
...

  {
    ...
    page.on('pageerror', error => pageErrors.push(String(error?.stack || error)));
    await page.route('**/ln-rank/js/selection-pool-runtime.v3965_0.js*', route => {
...
console.log(JSON.stringify({
  ok: true,
  contract: 'runtime-cache-browser-regression-v3965_0',
  cases: results
}, null, 2));
