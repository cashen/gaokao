...
try {
  for(const device of DEVICES){
    ...
    const directory=await page.locator('[data-major-relationship-graph="080901"]').innerText();
    ...
    const neighbors=await page.locator('[data-major-relationship-graph="080901"]').innerText();
    ...
    const classView=await page.locator('#result').innerText();
    ...
    const ambiguous=await page.locator('#result').innerText();
    ...
    await noOverflow(page,device.name,'最终语义旅程');
