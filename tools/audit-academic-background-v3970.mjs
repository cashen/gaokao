...
for (const marker of [
  ...
  '按分数位置看',
  '按学校查询'
]) assert.ok(localHtml.includes(marker), `LocalStrength page missing ${marker}`);
assert.ok(!/local-mainline-app\.v3967_0|academic-background-app\.v3968_0\.js/.test(localHtml), 'LocalStrength page still loads legacy runtime');
...
const localStrengthService = read('functions/_lib/local-strength-api.js');
for (const marker of [
...
]) assert.ok(localStrengthService.includes(marker), `LocalStrength service missing ${marker}`);
