...
async function runCase({ name, school, expectLabels = [], expectNoSocialLabels = false }) {
  ...
  try {
    ...
    const details = page.locator('#tongxue-common-name-ui-v005');
...
await runCase({ name:'campus label set follows the source campus relation', school:'哈尔滨工业大学（深圳）', expectLabels:['101计划','985','C9','E9','中坚九校','双一流','国防七子','建筑老八校','强基','机械五虎'] });
...
await browser.close();
console.log(JSON.stringify({ ok:true, version:'tongxue-common-name-v005-social-labels', baseUrl, cases:results }, null, 2));
