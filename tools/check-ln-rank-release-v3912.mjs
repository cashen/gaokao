...
for (const p of [
  ...
  'ln-rank/js/app.v3912.js','ln-rank/js/selection-pool.v3912.js','ln-rank/js/self-check.v3912.js','ln-rank/js/major-trend-integration.v3912.js',
  'ln-rank/js/campus-display-policy.v3912.js','ln-rank/js/action-state-policy.v3912.js','ln-rank/css/action-hierarchy.v3912.css',
  'ln-rank/js/keyword-token-classifier.v3912.js','ln-rank/js/review-point-builder.v3912.js',
...
for (const html of ['ln-rank/index.html','ln-rank/selection-pool.html']) {
  const text = read(html);
  assert(text.includes('版本：v3.9.12'), `${html} visible footer version not v3.9.12`);
  assert(!/版本：v3\.9\.8\.6/.test(text), `${html} still shows v3.9.8.6`);
}
assert(read('ln-rank/js/major-trend-render.v3912.js').includes('版本：v3.9.12'), 'major trend visible version not v3.9.12');
assert(!/major-trend-rules\.v3986/.test(read('ln-rank/js/major-trend-render.v3912.js')), 'major trend render still imports v3986 rules');
...
assert(/快速判断带|band-tab/.test(read('ln-rank/css/layout-shell.v3912.css')), 'score band quick strip css missing');
assert(/控制面板|search-workbench/.test(read('ln-rank/css/search-workbench.v3912.css')), 'compact search workbench css missing');
assert(/UI readability/.test(read('ln-rank/css/major-search.v3912.css')), 'UI readability css missing');
assert(read('ln-rank/index.html').includes('bottomline.v3912.css'), 'index does not use bottomline v3912');
assert(read('ln-rank/index.html').includes('search-workbench.v3912.css'), 'index does not use compact search workbench v3912');
assert(read('ln-rank/index.html').includes('floating-pool-entry.v3912.css'), 'index does not use floating entry v3912');

assert(read('ln-rank/index.html').includes('ui-density.v3912.css'), 'index does not use ui-density v3912');
assert(read('ln-rank/index.html').includes('responsive-control-panel.v3912.css'), 'index does not use responsive control panel v3912');
assert(/--ui-control-max|control panel|搜索控制台/.test(read('ln-rank/css/ui-density.v3912.css')), 'ui density rc css missing control panel rules');
assert(/Android|任务流|keyword/.test(read('ln-rank/css/responsive-control-panel.v3912.css')), 'responsive control panel rc css missing mobile rules');
assert(/办学性质提醒|公办底线|bottomline/.test(read('ln-rank/css/bottomline.v3912.css') + read('ln-rank/js/app.v3912.js')), 'bottomline compact assets missing');

assert(/is-empty/.test(read('ln-rank/js/major-trend-integration.v3912.js')), 'trend empty-state fix missing');
assert(/自选专业入口避让搜索控制台|bottom:max\(22px/.test(read('ln-rank/css/floating-pool-entry.v3912.css')), 'floating pool safe-zone css missing');
assert(read('ln-rank/release-meta.json').includes('v3.9.12') && read('ln-rank/release-meta.json').includes('v3912'), 'release meta not v3.9.12/v3912');

assert(read('functions/_lib/kb/liaoning-campus-major-kb.generated.js').includes('沈阳药科大学'), 'campus KB missing Shenyang Pharmaceutical');
assert(read('functions/_lib/kb/campus-accessor.js').includes('getCampusForItem'), 'campus accessor missing');
assert(read('functions/_lib/kb/action-hierarchy-policy.js').includes('getQueryActionLabel'), 'action hierarchy policy missing');
assert(read('ln-rank/js/campus-display-policy.v3912.js').includes('校区：本溪'), 'front campus display policy missing');
assert(read('ln-rank/css/action-hierarchy.v3912.css').includes('action-primary'), 'action hierarchy css missing');
assert(read('ln-rank/js/action-state-policy.v3912.js').includes('更新条件'), 'query action label update missing');

assert(read('functions/_lib/fenxi-session.js').includes('verifyFenxiRequest') || read('functions/_lib/fenxi-session.js').includes('createFenxiSetCookie'), 'fenxi-session dependency incomplete');
...
console.log('check-ln-rank-release-v3912 ok');
