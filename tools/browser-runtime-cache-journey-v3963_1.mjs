...
const artifactDir = process.env.RUNTIME_CACHE_ARTIFACT_DIR || '/tmp/runtime-cache-v3963_1';
...
try {
...

  {
    ...
    page.on('pageerror', error => pageErrors.push(String(error?.stack || error)));
    await page.route('**/ln-rank/js/app-runtime.v3963_1.js*', route => {
    ...
    try {
      ...
      assert.match(await page.locator('#runtimeStatusMessage').textContent(), /没有接管地区、学校和查询按钮/);
...
console.log(JSON.stringify({
  ok: true,
  contract: 'runtime-cache-browser-regression-v3963_1',
  cases: results
}, null, 2));
