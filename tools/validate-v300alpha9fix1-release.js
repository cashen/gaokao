...
assert(version.includes('V3.0.0.alpha9.fix1｜进度闭环与导出状态校准版'), 'version name missing');
assert(version.includes('v300alpha9fix1-20260512'), 'version stamp missing');
assert(version.includes('ln-v3-alpha9-fix1'), 'body class missing');

['fenxi/v3/index.html','fenxi/v3/debug.html','fenxi/v3/index.htm','fenxi/v3/debug.htm','fenxi/v3/debug/index.html'].forEach(file => {
  const html = read(file);
  assert(html.includes('v300alpha9fix1-20260512'), `${file} cache stamp missing`);
...
console.log('All V3.0.0.alpha9.fix1 checks passed.');
