...
try {
  for (const device of devices) {
    ...
    assert.equal(state.bodyRelease, 'v3.9.71.0', `${device.name}: body release`);
    assert.equal(state.htmlRelease, 'v3.9.71.0', `${device.name}: html release`);
    assert.equal(state.visibleRelease, 'v3.9.71.0', `${device.name}: visible release`);
    assert.equal(state.runtime?.version, 'family-home-runtime-v3970_0', `${device.name}: runtime`);
    assert.equal(state.runtime?.release, 'v3.9.71.0', `${device.name}: runtime release`);
...
console.log(JSON.stringify({ ok: true, release: 'v3.9.71.0', runtime: 'family-home-runtime-v3970_0', devices: results }, null, 2));
