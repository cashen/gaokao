...
const [page,entry,copy,headers,primary,source]=await Promise.all([
...
check('页面版本',page.includes('同学你好 v1.5.2')&&page.includes('./app/tongxue-performance-v152.js?v=152'));
check('品牌构建标识',count(page,'<meta name="tongxue-build" content="tongxue-v152-copy-20260721">')===1);
...
check('运行时构建校验',entry.includes("EXPECTED_BUILD='tongxue-v152-copy-20260721'")&&entry.includes("tongxue-performance-v112.js?v=152"));
check('运行时版本',entry.includes("installShareMetadataStabilizer('v1.5.2')")&&entry.includes("pageVersion:'v1.5.2'"));
...
check('品牌缓存',headers.includes('/tongxue/assets/brand/*')&&headers.includes('/tongxue/app/tongxue-performance-v152.js')&&headers.includes('/tongxue/app/tongxue-copy-v152.js')&&headers.includes('immutable'));
...
check('无过度承诺',!page.includes('轻微错别字')&&!page.includes('都可以搜索')&&!page.includes('看看学长学姐怎么说'));
check('不恢复 v1.6.0',!page.includes('v1.6.0')&&!page.includes('高频讨论信号')&&!page.includes('报考前核验清单')&&!page.includes('共识与分歧'));
console.log('TONGXUE_BRAND_UI_V152_RESULTS '+JSON.stringify({primaryBytes:primaryStat.size,sourceBytes:sourceStat.size,failures}));
