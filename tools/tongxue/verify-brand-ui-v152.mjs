...
const [page,entry,copy,portrait,headers,primary,source]=await Promise.all([
 ...
 readFile('tongxue/app/tongxue-performance-v153.js','utf8'),
...
check('页面版本',page.includes('同学你好 v1.5.3')&&page.includes('./app/tongxue-performance-v153.js?v=153'));
check('品牌构建标识',count(page,'<meta name="tongxue-build" content="tongxue-v153-copyfix-20260721">')===1);
...
check('运行时构建校验',entry.includes("EXPECTED_BUILD='tongxue-v153-copyfix-20260721'")&&entry.includes("tongxue-performance-v112.js?v=153"));
check('运行时版本',entry.includes("installShareMetadataStabilizer('v1.5.3')")&&entry.includes("pageVersion:'v1.5.3'"));
check('无副作用画像入口',entry.includes("tongxue-school-portrait-v121.js?v=153")&&!entry.includes('tongxue-school-portrait-v120.js'));
...
check('品牌缓存',headers.includes('/tongxue/assets/brand/*')&&headers.includes('/tongxue/app/tongxue-performance-v153.js')&&headers.includes('/tongxue/portrait/tongxue-school-portrait-v121.js')&&headers.includes('immutable'));
...
console.log('TONGXUE_BRAND_UI_V153_RESULTS '+JSON.stringify({primaryBytes:primaryStat.size,sourceBytes:sourceStat.size,failures}));
