...
assert.ok(resolved.members.every(item => ['identity-center','education-ministry-directory'].includes(item.resolutionKind)));
assert.ok(resolved.members.some(item => item.resolutionKind === 'education-ministry-directory'), 'directory fallback must be exercised');
...
assert.match(wrapper, /TONGXUE_COMMON_NAME_ENTRY_VERSION = 'tongxue-common-name-v004-social-labels'/);
