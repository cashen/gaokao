...
const [page,changelog,legacy,home,share,resolver,wrapper,rootFiles]=await Promise.all([
  ...
  readFile('tongxue/app/tongxue-performance-v140.js','utf8'),
  readdir('.')
]);
requireText(page,'<title>同学你好 - 看看学长学姐怎么说</title>','页面标题');
requireText(page,'./app/tongxue-performance-v140.js?v=140','页面入口');
requireText(page,'同学你好 v1.4.0 · 更新于 2026-07-20','页面版本');
...
if(!(changelog.indexOf('v1.4.0')<changelog.indexOf('v1.3.0')&&changelog.indexOf('v1.3.0')<changelog.indexOf('v1.2.0')))failures.push('更新记录未按倒序排列');
requireText(wrapper,"installShareMetadataStabilizer('v1.4.0')",'运行时版本');
requireText(wrapper,"tongxue-performance-v112.js?v=140",'核心运行时缓存版本');
...
if(!resolver.includes("school-pinyin-initials-v140.js"))failures.push('首字母索引未接入解析器');
