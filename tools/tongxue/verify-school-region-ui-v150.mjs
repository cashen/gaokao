...
const [page,entry,regionUi,entityUi,resolver,headers]=await Promise.all([
 readFile('tongxue/index.html','utf8'),readFile('tongxue/app/tongxue-performance-v155.js','utf8'),readFile('tongxue/app/tongxue-region-ui-v152.js','utf8'),readFile('tongxue/app/tongxue-school-entity-ui-v152.js','utf8'),readFile('tongxue/data/school-name-resolver-v150.js','utf8'),readFile('_headers','utf8')
]);
const failures=[],check=(label,passed)=>{if(!passed)failures.push(label);},count=(source,value)=>source.split(value).length-1;
const build='<meta name="tongxue-build" content="tongxue-v155-direct-handoff-20260723">';
...
check('页面版本',page.includes('同学你好 v1.5.5')&&page.includes('tongxue-performance-v155.js?v=155'));
...
check('入口先安装地域层',entry.indexOf('installRegionUi();')<entry.indexOf("await import('./tongxue-performance-v112.js?v=155')"));
...
check('版本资源长期缓存',headers.includes('/tongxue/data/school-search-index.20260617-v150.json')&&headers.includes('/tongxue/app/tongxue-performance-v155.js')&&headers.includes('immutable'));
