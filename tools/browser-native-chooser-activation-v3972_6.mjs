...
const baseUrl = process.env.V3972_6_BASE || 'http://127.0.0.1:8765';
const artifactDir = process.env.V3972_6_ARTIFACT_DIR || '/tmp/v3972-6-native-chooser';
fs.mkdirSync(artifactDir, { recursive: true });

const record = {
  id: 'native-chooser|001', school: '东北大学', major: '自动化类', schoolCode2026: '0141', majorCode2026: '003',
...
function scorePayload(url) {
  ...
  return {
    ok: true,
    meta: {
      audienceYear: 2027, activeDataYear: 2026, candidateScore: 579,
      candidateReferenceRank2026: 21051, candidateReferenceRankStart2026: 20760,
      candidateReferenceRankEnd2026: 21051, candidateSameCount2026: 292,
    ...
    bands: {
      upper: { key: 'upper', title: '稍高目标', rangeText: '稍高目标', count: 0, records: [], pagination: { offset: 0, limit: 40, returned: 0, hasMore: false } },
      near: { key: 'near', title: '主要参考', rangeText: '主要参考', count: 1, records: [record], pagination: { offset: 0, limit: 40, returned: 1, hasMore: false } },
      steady: { key: 'steady', title: '低分侧补充', rangeText: '低分侧补充', count: 0, records: [], pagination: { offset: 0, limit: 40, returned: 0, hasMore: false } }
    }
  };
}
...
try {
  for (const profile of profiles) {
    ...
    await page.route('**/api/school-majors**', route => route.fulfill({
      status: 200,
      contentType: 'application/json; charset=utf-8',
      body: JSON.stringify({ ok: true, meta: { mode: 'school-all', school: '东北大学', pagination: { hasMore: false } }, summary: {}, keywordQuery: { rawKeywords: [] }, records: [record] })
    }));
    ...
    try {
      ...
      assert.equal(initial.release, 'v3.9.72.6');
      assert.equal(initial.generation, 'v3972_6');
      assert.equal(initial.interactionVersion, 'interaction-transaction-v3972_6');
      ...
      assert.equal(requestUrl.searchParams.get('interactionVersion'), 'interaction-transaction-v3972_6');
      assert.equal(requestUrl.searchParams.get('siteRuntimeGeneration'), 'v3972_6');
...
console.log(JSON.stringify({
  ...
  release: 'v3.9.72.6',
  generation: 'v3972_6',
  interaction: 'interaction-transaction-v3972_6',
