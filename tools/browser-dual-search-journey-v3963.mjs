...
const artifactDir = process.env.DUAL_SEARCH_ARTIFACT_DIR || '/tmp/dual-search-v3963_0';
...
try {
  for (const testCase of cases) {
    ...
    try {
      ...
      assert.equal(finalMetrics.release, 'v3.9.63.0');
      assert.equal(finalMetrics.workspaceVersion, 'selection-workspace-orchestration-v3963');
      assert.equal(finalMetrics.schoolVersion, 'school-all-mode-v3963_0');
...
console.log(JSON.stringify({
  ok: true,
  contract: 'dual-search-real-journey-v3963_0',
  cases: results
}, null, 2));
