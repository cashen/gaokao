...
if(meta.version!=='v3.9.15'||meta.assetVersion!=='v3915') fail('release-meta version mismatch');
const manifest=JSON.parse(fs.readFileSync(path.join(root,'ln-rank/module-manifest.json'),'utf8'));
if(manifest.version!=='v3.9.15'||manifest.assetVersion!=='v3915') fail('module-manifest version mismatch');
for(const p of ['ln-rank/index.html','ln-rank/selection-pool.html','ln-rank/major-trend-2025.html','ln-rank/self-check.html','ln-rank/module-health.html']){
  ...
  if(!t.includes('3915')) fail(`${p} does not reference 3915`);
...
const r=spawnSync('node',['tools/analyze-ln-rank-assets-v3915.mjs','--fail-on-warn'],{cwd:root,stdio:'inherit'}); if(r.status!==0) fail('asset analysis failed');
for(const p of ['ln-rank/js/app.v3915.js','ln-rank/js/selection-pool.v3915.js','ln-rank/js/major-trend-render.v3915.js','ln-rank/js/self-check.v3915.js']){ const r2=spawnSync('node',['--check',p],{cwd:root,stdio:'inherit'}); if(r2.status!==0) fail(`syntax ${p}`); }
...
if(data.version!=='v3.9.15') fail('self-check api version mismatch');
console.log('OK v3915 release check passed');
