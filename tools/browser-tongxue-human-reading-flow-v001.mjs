...
async function finish(opened, name) {
  ...
  assert.equal(resultModule?.searchParams.get('r'), 'r052-tongxue-school-social-labels', `${name}: UI cache revision drift`);
  await context.close();
...
try {
  for (const testCase of viewports) {
    ...
    try {
      await major.page.waitForFunction(() => document.body.dataset.majorPathDirect === 'major-path-human-v0.06', null, { timeout:20000 });
    } catch (error) {
