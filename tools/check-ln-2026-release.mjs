...
ok(VERSION==='v3.9.63.0'&&ASSET==='v3963_0','current release version');
ok(CURRENT_RELEASE.uiOrchestrationVersion==='ui-orchestration-v3963','current UI version');
ok(CURRENT_RELEASE.algorithmOrchestrationVersion==='algorithm-orchestration-v3963','algorithm version preserved');
ok(CURRENT_RELEASE.selectionWorkspaceVersion==='selection-workspace-orchestration-v3963','workspace version');
ok(CURRENT_RELEASE.schoolAllModeVersion==='school-all-mode-v3963_0','school-all mode version');
ok(CURRENT_RELEASE.schoolUiGovernanceVersion==='school-ui-governance-v3963_0','school UI governance version');
ok(CURRENT_RELEASE.schoolModeMountVersion==='school-mode-static-mount-v3963_0','school static mount version');
ok(CURRENT_RELEASE.resourceOwners.uiModeSwitch==='/shared/ui/components/mode-switch.v3963_0.css','shared mode switch owner');

const releasePresenter=t('shared/resources/release/release-presenter.js');
ok(releasePresenter.includes("from './current-release.js?v=3963_0'")&&releasePresenter.includes('data-current-release')&&releasePresenter.includes('dataset.release')&&releasePresenter.includes('dataset.uiRelease'),'shared release presenter');
...
ok(main.includes('app.v3963_0.js?v=3963_0'),'v3963 main wrapper loaded');
ok(main.includes('selection-workspace.v3963_0.css?v=3963_0'),'workspace CSS loaded');
ok(main.includes('school-all-mode.v3963_0.css?v=3963_0'),'school-all CSS loaded');
ok(main.includes('semantic.v3959_0.css?v=3962_2'),'governed semantic CSS loaded');
ok(main.includes('mode-switch.v3963_0.css?v=3963_0'),'shared mode switch CSS loaded');
ok(main.includes('id="schoolViewModeMount"')&&main.includes('id="schoolAllResultsPanel"'),'static school mounts');
ok(main.includes('class="ui-mode-switch search-intent-switch"')&&main.includes('class="ui-segmented ui-mode-switch__actions"'),'shared school mode controls');
...
ok(filterStart>=0&&modeIndex>=0&&modeIndex<filterStart,'search intent mount is not at the top of the shared console');
...
ok(selection.includes('selection-pool.v3960_0.js?v=3963_0'),'selection preserved runtime');
...
ok(difficultyPage.includes('major-difficulty-2026.v3959_0.js?v=3963_0'),'difficulty UI adapter preserved');
...
ok(root.includes('foundation.v3959_0.css?v=3959_0')&&root.includes('family-shell.v3961_0.js?v=3963_0'),'root shared UI shell');
...
ok(zyPage.includes('zy2026.v3959_0.js?v=3963_0')&&zyPage.includes('zy2026.v3954_0.css?v=3955_0'),'zy2026 UI wrapper preserved');
...
ok(t('zy2026/assets/zy2026.v3959_0.js').includes('shared/ui/shell/family-shell.v3961_0.js'),'zy shared UI adapter');
...
const appWrapper=t('ln-rank/js/app.v3963_0.js');
...
ok(t('ln-rank/js/feature/major-pool/render.v3963_0.js').includes('双非（非985/211）'),'school profile card tags');
...
ok(appWrapper.includes('release-presenter.js?v=3963_0'),'main shared release presenter');
...
ok(appWrapper.includes('school-all-mode.v3963_0.js?v=3963_0'),'main unified school-all adapter');
...
const modeSwitch=t('shared/ui/components/mode-switch.v3963_0.css');
const actionContract=t('shared/ui/contracts/action-contract.v3959_0.js');
const schoolAll=t('ln-rank/js/feature/school-majors/school-all-mode.v3963_0.js');
const schoolAllCss=t('ln-rank/css/school-all-mode.v3963_0.css');
...
ok(modeSwitch.includes('.ui-mode-switch')&&modeSwitch.includes('.search-intent-switch')&&modeSwitch.includes('container-name:ui-mode-switch')&&modeSwitch.includes('writing-mode:horizontal-tb'),'shared mode switch component');
...
ok(schoolAll.includes('buildTongxueSchoolHref')&&schoolAll.includes('createSelectionPoolAdapter')&&schoolAll.includes('UI_ACTION_COPY')&&schoolAll.includes("const API_PATH = '/api/school-majors'"),'school-all shared frontend resources');
ok(schoolAll.includes('data-school-detail-toggle')&&schoolAll.includes('expandedRecordKey')&&schoolAll.includes("mountPolicy: 'static-result-owner'")&&schoolAll.includes('assertStaticStructure'),'school-all static mount and governed details');
...
ok(schoolApi.includes('../_lib/ln-rank-manifest.js')&&schoolApi.includes('school-identity-center.js')&&schoolApi.includes('canonical-position.v3963_0.js'),'school-all shared API resources');
ok(schoolApi.includes("mode: 'shared-records-school-exact'"),'school-all exact shared record mode');

const uiRegistry=t('shared/ui/ui-registry.v3963_0.js');
...
ok(uiRegistry.includes("modeSwitch: '/shared/ui/components/mode-switch.v3963_0.css'")&&uiRegistry.includes("schoolModeMountOwner: 'static-selection-console'")&&uiRegistry.includes("schoolModeControlOwner: 'shared-ui-mode-switch'"),'shared school mode ownership registry');
...
const orchestrator=t('ln-rank/js/workspace/selection-workspace-orchestrator.v3963_0.js');
...
const position=t('shared/algorithms/position/canonical-position.v3963_0.js');
const ranking=t('shared/algorithms/ranking/staged-ranking.v3960_0.js');
ok(algorithmRegistry.includes('algorithm-orchestration-v3963'),'algorithm registry');
...
ok(t('functions/api/major-bands.js').includes('canonical_rank_primary_2026_position'),'major bands canonical mode');
...
ok(active.schoolAllModeVersion==='school-all-mode-v3963_0','active school-all version');
ok(active.schoolUiGovernanceVersion==='school-ui-governance-v3963_0','active governance version');
ok(active.schoolModeMountVersion==='school-mode-static-mount-v3963_0','active static mount version');
ok(active.mainJs==='js/app.v3963_0.js'&&active.jsEntry.includes('js/app.v3963_0.js'),'active v3961 main');
...
ok(active.cssEntry.includes('../shared/ui/components/mode-switch.v3963_0.css'),'shared mode switch css active');
ok(active.cssEntry.includes('css/selection-workspace.v3963_0.css'),'workspace css active');
ok(active.cssEntry.includes('css/school-all-mode.v3963_0.css'),'school-all css active');
...
ok(release.schoolAllModeVersion==='school-all-mode-v3963_0','release school-all version');
ok(release.schoolUiGovernanceVersion==='school-ui-governance-v3963_0','release governance version');
ok(release.schoolModeMountVersion==='school-mode-static-mount-v3963_0','release static mount version');
...
for(const path of ['.bootstrap','.github/workflows/bootstrap-zy2026-v3953.yml','.github/workflows/materialize-zy2026-v3953.yml','.github/workflows/agent-algorithm-orchestration-v3963.yml','.github/workflows/sync-v3962-1-school-ui.yml','tools/release/sync-v3962-1-school-ui.py','.github/workflows/sync-v3962-2-once.yml','tools/release/sync-v3962_2.mjs'])ok(!fs.existsSync(path),`temporary path remains ${path}`);
