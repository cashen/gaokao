...
ok(main.includes('app.v3961_0.js?v=3962_2'),'v3962.1 cached main wrapper loaded');
...
ok(!main.includes('school-all-mode.v3962_0.css?v=3962_0'),'old school CSS not active');
...
ok(appWrapper.includes('school-all-mode.v3962_2.js?v=3962_2'),'main governed school-all adapter');
...
ok(semantic.includes('.ui-button--compact')&&semantic.includes('.ui-chip--compact')&&semantic.includes('@media(pointer:coarse)'),'shared compact UI components');
ok(actionContract.includes('addSelectedMajor')&&actionContract.includes('removeSelectedMajor')&&actionContract.includes('inspectDetails'),'shared school action copy');
...
ok(schoolAll.includes('data-school-detail-toggle')&&schoolAll.includes('expandedRecordKey'),'school-all governed full-row details');
ok(!schoolAll.includes('MutationObserver')&&!schoolAll.includes('setTimeout(')&&!schoolAll.includes('<details>'),'school-all no observer, timer, or nested details');
...
ok(!schoolAllCss.includes('@media (max-width:')&&!schoolAllCss.includes('min-width: min(620px'),'school-all no device breakpoint or forced detail width');
...
for(const inactive of ['js/app.v3960_0.js','js/app.v3951_0.js','js/ux/family-presentation.v3955_0.js','js/ux/multi-terminal.v3949_4.js','js/ux/compare-workspace.v3953_0.js','js/feature/school-majors/school-all-mode.v3962_0.js'])ok(!active.jsEntry.includes(inactive),`legacy active ${inactive}`);
...
ok(!active.cssEntry.includes('css/school-all-mode.v3962_0.css'),'old school-all css inactive');
...
ok(release.schoolAllSharedUiGovernanceContract===true&&release.schoolAllFullRowDetailsContract===true&&release.schoolAllContainerResponsiveContract===true&&release.schoolAllBrowserMultiTerminalContract===true,'v3962.1 school UI governance contracts');
...
for(const path of ['.bootstrap','.github/workflows/bootstrap-zy2026-v3953.yml','.github/workflows/materialize-zy2026-v3953.yml','.github/workflows/agent-algorithm-orchestration-v3960.yml','.github/workflows/sync-v3962-1-school-ui.yml','tools/release/sync-v3962-1-school-ui.py'])ok(!fs.existsSync(path),`temporary path remains ${path}`);
ok(!t('ln-rank/js/core/score-guard.js').includes('<400'),'no 400 guard');
console.log(`LN 2026 ${VERSION} governed school-all, selection workspace, resource, UI and algorithm orchestration checks passed`);
