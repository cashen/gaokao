...
import { UI_COMPONENT_REGISTRY, UI_RESOURCE_REGISTRY_VERSION } from '../shared/ui/ui-resource-registry.v3990_0.js';
...
import { LN_RANK_RUNTIME_CACHE_CONTRACT } from '../shared/resources/release/runtime-cache-contract.v3990_0.js';
import { RESOURCE_EXECUTION_VERSION, RESOURCE_EXECUTION_REGISTRY } from '../shared/governance/resource-execution-contract.v3990_0.js';

const read = rel => fs.readFileSync(rel, 'utf8');
const main = read('ln-rank/index.html');
const selection = read('ln-rank/selection-pool.html');
const shell = read('shared/ui/shell/family-shell.v3990_0.js');
const component = read('shared/ui/components/family-plan-entry.v3990_0.js');
...
const interaction = read('shared/ui/interaction/interaction-transaction.v3990_0.js');
const runtime = read('ln-rank/js/app-runtime.v3990_0.js');
const workspace = read('ln-rank/js/workspace/selection-workspace-orchestrator.v3990_0.js');
...
assert.equal(CURRENT_RELEASE.familyActionVersion, 'family-action-v3990_0');
...
assert.equal(RESOURCE_EXECUTION_REGISTRY.familyAction.owner, '/shared/ui/components/family-plan-entry.v3990_0.js');
assert.equal(RESOURCE_EXECUTION_REGISTRY.familyAction.classification, 'current-generation');
assert.equal(RESOURCE_EXECUTION_REGISTRY.familyAction.schemaVersion, 'family-action-v3990_0');
...
assert.ok(LN_RANK_RUNTIME_CACHE_CONTRACT.activeGenerationModules.includes('/shared/ui/components/family-plan-entry.v3990_0.js'));
...
for (const marker of ['生成家庭方案报告', '知道链接的人可以查看', 'selection-pool.v3990_0.js?v=3990_0']) {
...
assert.ok(component.includes("export const FAMILY_PLAN_ENTRY_VERSION = 'family-plan-entry-v3990_0'"));
...
assert.ok(shell.includes("export const FAMILY_SHELL_VERSION = 'family-shell-v3990_0'"));
assert.ok(shell.includes("family-plan-entry.v3990_0.js?v=3990_0"));
...
for (const marker of [
  "selection-workspace-orchestration-v3990_0",
  "selection-workspace-orchestrator.v3969_0.js?v=3969_0",
  'delegateVersion',
  'navigationOwner'
]) assert.ok(workspace.includes(marker), `workspace wrapper missing ${marker}`);

assert.equal(manifest.releaseVersion, 'v3.9.90.0');
assert.equal(manifest.generation, 'v3990_0');
assert.equal(manifest.resourceGraph.uiRegistry, CURRENT_RELEASE.resourceOwners.uiResourceRegistry);
assert.equal(manifest.currentGenerationEntrypoints.familyPlanEntry, '/shared/ui/components/family-plan-entry.v3990_0.js?v=3990_0');
