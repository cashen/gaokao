...
for (const needle of [
  ...
  'volunteer-card',
  '核对与提醒'
]) assert.ok(page.includes(needle), `current page missing ${needle}`);
...
for (const needle of [
  '.compact-check-sheet', '.compact-risk-item', 'max-width: 24%',
  'display: none !important', 'break-inside: avoid', '.col-check {\n    width: 28%'
]) assert.ok(printStyles.includes(needle), `print styles missing ${needle}`);

for (const needle of [
  '固定核对只保留“代码/专业组”和“特殊限制”', '风险提醒最多 2 条',
