...
const allowed=new Set(['3932','3933','3933_1','3933_2',q]);const oldEntries=fs.readdirSync(path.join(lr,'js')).filter(n=>/^(app|selection-pool|major-trend-render|self-check)\.v/.test(n)).filter(n=>{const v=n.match(/\.v([^.]*)\.js$/)?.[1];return v&&!allowed.has(v);});
