...
const stamp='v300rc1-20260512';
const version=read('fenxi/v3/assets/js/version.v3.js');
assert(version.includes('V3.0.0.rc1｜受控试用候选版'),'version name');
assert(version.includes(stamp),'version stamp');
assert(version.includes('ln-v3-rc1'),'body class token');
...
assert(adapter.includes('rc1-controlled-trial-candidate'),'release adapter stage');
...
assert(debug.includes('V3.0.0.rc1'),'debug version check updated');
...
assert(versionTxt.includes('V3.0.0.rc1｜受控试用候选版'),'VERSION updated');
console.log('All V3.0.0.rc1 checks passed.');
