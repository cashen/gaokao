...
for (const src of [
  '/ln-rank/js/simulation-report-v015-human-workbench.js?v=v016.19-r109',
  '/ln-rank/js/simulation-report-v016-legacy-render-guard.js?v=v016.19-r109',
  '/ln-rank/js/simulation-report-v016-pdf.js?v=v016.19-r109',
  '/ln-rank/js/simulation-report-v017-responsive-input.js?v=v016.19-r109'
]) assert.ok(html.includes(src), `html missing ${src}`);
assert.ok(html.includes('simulation-school-search-worker-v001.js?v=v016.19-r109'));
...
assert.equal(manifest.version,'simulation-workspace-v016.19');
assert.equal(manifest.revision,'r109-responsive-input-school-major-cache');
...
assert.ok(responsive.includes("new Worker('/ln-rank/js/simulation-school-search-worker-v001.js?v=v016.19-r109',{type:'module'})"));
...
assert.ok(responsive.includes('rowContext'));
assert.ok(responsive.includes('schoolFactCache'));
assert.ok(worker.includes('typeof r.search===\'function\''));
assert.ok(api.includes("url.searchParams.getAll('major')"),'API must accept batched major values');
assert.ok(api.includes("url.searchParams.getAll('majors')"));
...
for(const expected of ['html2canvas','jsPDF','pdf-v016-title','报考信息（待核实）','第二页及后续页面重复顶部考生信息','pdf.save']) assert.ok(pdf.includes(expected),`pdf runtime missing ${expected}`);
...
console.log('simulation-workspace-v016.19 contract: PASS');
