...
for(const src of ['/ln-rank/js/simulation-report-v016-legacy-render-guard.js?v=v016.31-r121','/ln-rank/js/simulation-report-v016-pdf.js?v=v016.31-r121','/ln-rank/js/simulation-report-v017-responsive-input.js?v=v016.31-r121']) assert.ok(html.includes(src),`html missing ${src}`);
...
assert.equal(manifest.version,'simulation-workspace-v016.31');assert.equal(manifest.revision,'r121-responsive-input-fastpath-gate');
...
for(const expected of ['390','768','1280','pressSequentially','compositionstart','insertFromPaste','Backspace','duplicate','refresh','学校候选','大工','大连理工大学','东北大学','confirmedSchool','major']) assert.ok(browser.includes(expected),`browser regression missing ${expected}`);
...
console.log('simulation-workspace-v016.31 contract: PASS');
