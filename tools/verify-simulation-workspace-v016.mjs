...
for(const src of ['/ln-rank/js/simulation-report-v016-legacy-render-guard.js?v=v016.34-r124','/ln-rank/js/simulation-report-v016-pdf.js?v=v016.34-r124','/ln-rank/js/simulation-report-v017-responsive-input.js?v=v016.34-r124']) assert.ok(html.includes(src),`html missing ${src}`);
...
assert.equal(manifest.version,'simulation-workspace-v016.34');assert.equal(manifest.revision,'r124-exact-head-ci-gate');
...
for(const expected of ['390','768','1280','pressSequentially','Backspace','compositionstart','insertFromPaste','duplicate','refresh','学校候选','大工','大连理工大学','东北大学','confirmedSchool','major']) assert.ok(browser.includes(expected),`browser regression missing ${expected}`);
...
console.log('simulation-workspace-v016.34 contract: PASS');
