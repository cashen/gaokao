...
const report={generatedAt:new Date().toISOString(),version:'v3.9.15',assetVersion:'v3915',entries,totals:{allJs:allJs.length,activeJs:activeJs.size,orphanJs:orphanJs.length,allCss:allCss.length,activeCss:activeCss.size,orphanCss:orphanCss.length,missing:missing.length,versionedInternal:versionedInternal.length,cssLayerMissing:cssLayerMissing.length},activeJs:[...activeJs].sort(),activeCss:[...activeCss].sort(),orphanJs,orphanCss,missing,versionedInternal,cssLayerMissing};
fs.mkdirSync(file('reports'),{recursive:true});
fs.writeFileSync(file('reports/ln-rank-asset-analysis-v3915.json'),JSON.stringify(report,null,2));
fs.writeFileSync(file('reports/ln-rank-dependency-graph-v3915.json'),JSON.stringify({entries,activeJs:report.activeJs,activeCss:report.activeCss},null,2));
fs.writeFileSync(file('reports/ln-rank-asset-analysis-v3915.md'),`# ln-rank asset analysis v3915\n\n- active JS: ${activeJs.size}/${allJs.length}\n- orphan JS: ${orphanJs.length}\n- active CSS: ${activeCss.size}/${allCss.length}\n- orphan CSS: ${orphanCss.length}\n- missing: ${missing.length}\n- versioned internal active: ${versionedInternal.length}\n- missing CSS layers: ${cssLayerMissing.length}\n`);
fs.writeFileSync(file('reports/ln-rank-dependency-graph-v3915.md'),`# ln-rank dependency graph v3915\n\n## Entrypoints\n${entries.map(x=>'- '+x).join('\n')}\n\n## Active JS\n${report.activeJs.map(x=>'- '+x).join('\n')}\n\n## Active CSS\n${report.activeCss.map(x=>'- '+x).join('\n')}\n`);
