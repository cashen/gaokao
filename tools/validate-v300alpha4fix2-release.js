...
const requiredFiles = [
  ...
  'fenxi/v3/docs/V3_alpha4fix1_Step2继续跳转与页面滚动修复说明.md',
  'fenxi/v3/docs/V3_alpha4fix2_Debug空数据自测降级与输入提示修复说明.md',
...
assert(index.includes('ln-v3-alpha4-fix2'), 'index body class missing alpha4.fix2');
assert(debug.includes('ln-v3-alpha4-fix2'), 'debug body class missing alpha4.fix2');
assert(version.includes("V3.0.0.alpha4.fix2｜Debug空数据自测降级与输入提示修正版"), 'version name missing alpha4.fix2');
assert(version.includes('v300alpha4fix2-20260512'), 'version stamp missing alpha4.fix2');
...
assert(familyStep.includes("LN_V3_ROUTER.go('child', 'family:save-next')"), 'Step2 save-next must route to child');
assert(familyStep.includes('进入第 3 步：孩子专业偏好'), 'Step2 save-next status message missing');
assert(router.includes('scrollToStepTop'), 'router must scroll to next step after route');
assert(wizard.includes('scrollToStepTop') && wizard.includes('window.scrollTo'), 'wizard scroll helper missing');
...
assert(familyDebug.includes('Step2 保存继续可进入 Step3'), 'Step2 debug route-next check missing');
...
assert(childDebug.includes('manualOnly 等待 Step1/Step2 数据'), 'Step3 debug must downgrade manualOnly check when no records');
...
console.log('All V3.0.0.alpha4.fix2 checks passed.');
