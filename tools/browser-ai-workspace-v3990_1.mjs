...
async function parentEntryUiJourney(page,name){
  ...
  const assetVersion=await page.locator('body').getAttribute('data-ai-plus-assets');assert(assetVersion==='aiplus-assets-v3992_10',`AIPLuS asset version drift: ${assetVersion}`);const assetHrefs=await page.locator('link[rel="stylesheet"],script[type="module"]').evaluateAll(nodes=>nodes.map(node=>node.getAttribute('href')||node.getAttribute('src')||''));assert(assetHrefs.every(href=>href.includes('3992_10')||href.includes('3992_9')||href.includes('3992_1')||href.includes('3992_4')||href.includes('3992_0')),`stale AIPLuS asset query remains: ${JSON.stringify(assetHrefs)}`);
