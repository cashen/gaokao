...
async function parentEntryUiJourney(page,name){
  ...
  const assetVersion=await page.locator('body').getAttribute('data-ai-plus-assets');assert(assetVersion==='aiplus-assets-v002_1',`AIPLuS asset version drift: ${assetVersion}`);const assetHrefs=await page.locator('link[rel="stylesheet"],script[type="module"]').evaluateAll(nodes=>nodes.map(node=>node.getAttribute('href')||node.getAttribute('src')||''));assert(assetHrefs.every(href=>href.includes('002_1')),`stale AIPLuS asset query remains: ${JSON.stringify(assetHrefs)}`);
