...
async function waitForFinalTurnResponse(page,text,timeout=150000){return page.waitForResponse(async response=>{if(!responseMatchesInput(response,text))return false;try{const data=await response.json();return data?.pendingDeterministicTool!==true;}catch{return true;}},{timeout});}
...
try{
  for(const device of devices){const context=await browser.newContext({viewport:device.viewport,isMobile:Boolean(device.isMobile),hasTouch:Boolean(device.hasTouch),locale:'zh-CN'});const page=await context.newPage();const errors=[];page.__expectedSyntheticMajorBands503Count=0;page.__aiPlusNetworkTrace=[];page.on('request',request=>{const url=request.url();if(url.includes('/api/ai/turn')||url.includes('/api/major-bands'))page.__aiPlusNetworkTrace.push({kind:'request',method:request.method(),url,postData:request.postData()?.slice(0,2000)||''});});page.on('response',response=>{const url=response.url();if(url.includes('/api/ai/turn')||url.includes('/api/major-bands'))page.__aiPlusNetworkTrace.push({kind:'response',status:response.status(),url});});page.on('pageerror',e=>errors.push(`pageerror:${e.message}`));page.on('console',m=>{if(m.type()!=='error')return;const text=m.text();if(page.__expectedSyntheticMajorBands503Count>0&&/Failed to load resource:.*status of 503/.test(text)){page.__expectedSyntheticMajorBands503Count-=1;return;}errors.push(`console:${text}`);});try{const response=await page.goto(`${BASE}/aiplus/?browser=${encodeURIComponent(EXPECTED_SHA||'preview')}-${device.name}`,{waitUntil:'networkidle',timeout:60000});assert(response?.ok(),`${device.name}: /aiplus HTTP ${response?.status()}`);await waitForHealth(page);assert(await page.locator('#decisionContextDetails').evaluate(el=>!el.open),`${device.name}: engineering/support panel should default collapsed`);await coreHumanJourney(page,device.name);await profilePersistence(page,device.name);await responsiveHistoryDrawerJourney(page,device.name);await transientMajorBandsRecovery(page,device.name);await parentEntryUiJourney(page,device.name);await viewportStabilityJourney(page,device.name);await singleSchoolPromptJourney(page,device.name);await schoolResearchAndHistoryJourney(page,device.name);await schoolHistoryAliasJourney(page,device.name);await majorRegionHistoryJourney(page,device.name);await scoreBandParentJourneys(page,device.name);await latestWins(page,device.name);await selectionAndModel(page,device.name);await checkGeometry(page,`${device.name}:final`);await page.screenshot({path:path.join(ARTIFACT_DIR,`${device.name}.png`),fullPage:true});assert(errors.length===0,`${device.name}: browser errors ${errors.join(' | ')}`);}catch(error){fs.writeFileSync(path.join(ARTIFACT_DIR,`${device.name}-failure.txt`),`${error.stack||error}\n${errors.join('\n')}\nnetworkTrace=${JSON.stringify(page.__aiPlusNetworkTrace,null,2)}\nui=${await page.locator('body').innerText().catch(()=>'' )}`);await page.screenshot({path:path.join(ARTIFACT_DIR,`${device.name}-failure.png`),fullPage:true}).catch(()=>{});throw error;}finally{await context.close();}}
