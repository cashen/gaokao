...
const PAGINATION_SNAPSHOT_GUARD_PATH = '/ln-rank/js/feature/major-pool/pagination-snapshot-guard.v3990_3.js';

assert.equal(SITE_RUNTIME_CONTRACT.version, 'site-runtime-coherence-v3990_3');
assert.equal(SITE_RUNTIME_CONTRACT.generation, 'v3990_3');
assert.equal(SITE_RUNTIME_CONTRACT.queryVersion, '3990_3');
assert.equal(SITE_RUNTIME_CONTRACT.releaseVersion, 'v3.9.90.3');
...
for (const modulePath of LN_RANK_RUNTIME_CACHE_CONTRACT.activeGenerationModules) {
  ...
  assert.ok(modulePath.includes('3990_3'), `runtime cache current module has stale generation: ${modulePath}`);
}
...
for (const key of currentKeys) {
  ...
  if (!pageRouteKeys.has(key)) assert.ok(String(value).includes('3990_3'), `current entrypoint is not v3990_3: ${key} -> ${value}`);
}
...
assert.equal(SITE_RUNTIME_CONTRACT.owners.interaction, '/shared/ui/interaction/interaction-transaction.v3990_3.js');
...
const interaction = fs.readFileSync('shared/ui/interaction/interaction-transaction.v3990_3.js', 'utf8');
for (const marker of [
...
const interactionCss = fs.readFileSync('shared/ui/interaction/interaction-transaction.v3990_3.css', 'utf8');
...
const runtime = fs.readFileSync('ln-rank/js/app-runtime.v3990_3.js', 'utf8');
const workspace = fs.readFileSync('ln-rank/js/workspace/selection-workspace-orchestrator.v3990_3.js', 'utf8');
const paginationSnapshotGuard = fs.readFileSync('ln-rank/js/feature/major-pool/pagination-snapshot-guard.v3990_3.js', 'utf8');
for (const [label, source] of [['app', app], ['runtime', runtime], ['workspace', workspace], ['paginationSnapshotGuard', paginationSnapshotGuard]]) {
  assert.ok(source.includes('v3990_3'), `${label} does not use current generation`);
}
assert.ok(runtime.includes("const INTERACTION_VERSION = 'interaction-transaction-v3990_3'"));
assert.ok(workspace.includes("const INTERACTION_VERSION = 'interaction-transaction-v3990_3'"));
for (const marker of [
  'pagination-snapshot-guard.v3990_3.js?v=3990_3',
  ...
  "owner: '/ln-rank/js/app-runtime.v3990_3.js'",
  "implementation: '/ln-rank/js/feature/major-pool/pagination-snapshot-guard.v3990_3.js'"
]) assert.ok(runtime.includes(marker), `selection runtime missing snapshot ownership: ${marker}`);
for (const marker of [
  "MAJOR_BANDS_PAGINATION_SNAPSHOT_GUARD_VERSION = 'major-bands-pagination-snapshot-guard-v3990_3'",
...
const manifest = JSON.parse(fs.readFileSync('ln-rank/site-active-generation.v3990_3.json', 'utf8'));
...
assert.deepEqual(manifest.currentGenerationInternalModules, {
  majorBandsPaginationSnapshotGuard: `${PAGINATION_SNAPSHOT_GUARD_PATH}?v=3990_3`
});
...
console.log(JSON.stringify({
  ...
  paginationSnapshotGuard: 'major-bands-pagination-snapshot-guard-v3990_3',
