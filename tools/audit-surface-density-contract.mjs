...
const projectRoot=process.argv[2]?path.resolve(process.argv[2]):path.resolve(process.cwd(),'ln-rank');const lr=projectRoot.endsWith('ln-rank')?projectRoot:path.join(projectRoot,'ln-rank');const read=rel=>fs.readFileSync(path.join(lr,rel),'utf8');const checks=[];const check=(name,ok,detail='')=>checks.push({name,ok:Boolean(ok),detail});
const render=read('js/feature/major-pool/render.js');const selection=read('js/selection-pool.v3933_1.js');const knowledge=read('js/knowledge/major-knowledge-contract.js');const css=read('css/components/local-context-contract.css');const distMain=read('css/dist/ln-rank-main.v3933_1.css');const distSelection=read('css/dist/ln-rank-selection.v3933_1.css');
...
const failed=checks.filter(r=>!r.ok);const report={version:'v3933_1',generatedAt:new Date().toISOString(),status:failed.length?'fail':'pass',checks};fs.writeFileSync(path.join(lr,'surface-density-audit.v3933_1.json'),JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));if(failed.length)process.exit(1);
