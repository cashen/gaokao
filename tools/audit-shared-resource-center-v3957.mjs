...
import { REGION_OPTIONS, REGION_GROUPS, deriveRegionGroups, matchRegionRule, normalizeProvinceName } from '../shared/resources/geo/china-region-catalog.js';
...
assert.equal(SHARED_RESOURCE_CENTER_VERSION, 'v3957_0');
...
const appWrapper = read('ln-rank/js/app.v3957_0.js');
const schoolLinkCenter = read('shared/resources/schools/school-resource-center.js');
assert.ok(mainPage.includes('/ln-rank/js/app.v3957_0.js?v=3957_0'));
assert.ok(!mainPage.includes('/ln-rank/js/app.v3951_0.js?v=3957_0'));
assert.ok(appWrapper.includes("await import('./app.v3951_0.js?v=3957_0')"));
...
for (const meta of [json('ln-rank/release-meta.json'), json('ln-rank/active-assets.json')]) {
  ...
  assert.equal(meta.sharedResourceCenterVersion, 'v3957_0');
}
console.log('SHARED_RESOURCE_CENTER_V3957_OK');
