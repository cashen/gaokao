...
try {
  for (const viewport of [{ width: 390, height: 844, name: 'android' }, { width: 768, height: 1024, name: 'pad' }, { width: 1280, height: 900, name: 'desktop' }]) {
    ...
    assert.equal(initialMetrics.scrollWidth, initialMetrics.clientWidth, `${viewport.name}: page horizontally overflows on load`);
    ...
    assert.equal(finalMetrics.scrollWidth, finalMetrics.clientWidth, `${viewport.name}: page horizontally overflows after selection`);
    assert.ok(finalMetrics.cardWidth <= finalMetrics.viewport, `${viewport.name}: volunteer card is wider than viewport`);
    assert.equal(pageErrors.length, 0, `${viewport.name}: page errors ${pageErrors.map(e => e.message).join('; ')}`);
    await context.close();
  }
  console.log('simulation p0 r156 school-major-mobile browser contract: PASS');
} finally {
  await browser.close();
}
