...
const runtime = read('ln-rank/js/feature/school-majors/school-all-mode.v3963_0.js');
const orchestrator = read('ln-rank/js/workspace/selection-workspace-orchestrator.v3963_0.js');
const resultCss = read('ln-rank/css/school-all-mode.v3963_0.css');
...
const registry = read('shared/ui/ui-registry.v3963_0.js');
...
for (const id of ['schoolAllResultsPanel','schoolAllTitle','schoolAllMeta','schoolAllContent','schoolAllSort','schoolAllBack']) {
  assert.ok(runtime.includes(`'${id}'`), `school result owner does not consume static ${id}`);
}
for (const id of ['schoolViewModeMount','schoolResolveStatus','schoolAllBack']) {
  assert.ok(orchestrator.includes(`'${id}'`), `shared search owner does not consume static ${id}`);
}
...
assert.ok(filterStart >= 0 && modeIndex >= 0 && modeIndex < filterStart, 'search intent switch must live at the top of the shared console');
assert.ok(page.includes('class="ui-mode-switch search-intent-switch"'), 'static mode switch must use shared UI component');
...
assert.ok(runtime.includes("mountPolicy: 'static-result-owner'"), 'runtime must declare static result ownership');
assert.ok(runtime.includes('assertStaticStructure'), 'runtime must fail when the unified mount is absent');
assert.ok(runtime.includes("sharedControlOwner: 'selection-workspace-orchestration-v3963'"), 'runtime must declare the shared control owner');
assert.ok(!runtime.includes("getElementById('queryButton')"), 'result owner may not bind the shared query button');

for (const marker of ['.ui-mode-switch{','.search-intent-switch','writing-mode:horizontal-tb','container-name:ui-mode-switch','@container ui-mode-switch (max-width:280px)']) {
...
assert.ok(registry.includes("modeSwitch: '/shared/ui/components/mode-switch.v3963_0.css'"), 'UI registry must own the mode switch resource');
assert.ok(registry.includes("schoolModeMountOwner: 'static-selection-console'"), 'workspace contract must own the static mount');
...
assert.ok(app.includes('school-all-mode.v3963_0.js?v=3963_0'), 'app must activate the unified runtime');
assert.ok(page.includes('mode-switch.v3963_0.css?v=3963_0'), 'page must load shared mode component');
assert.ok(page.includes('school-all-mode.v3963_0.css?v=3963_0'), 'page must load current school result CSS');
...
console.log(JSON.stringify({
  ok: true,
  contract: 'school-mode-static-shared-ui-v3963_0',
