...
assert(version.includes('V3.0.0.alpha6.fix2｜家庭路径优先级与就业画像校准版'), 'version name not updated');
assert(version.includes('v300alpha6fix2-20260512'), 'version stamp not updated');
assert(version.includes('ln-v3-alpha6-fix2'), 'body class token missing');
['fenxi/v3/index.html','fenxi/v3/index.htm','fenxi/v3/debug.html','fenxi/v3/debug.htm','fenxi/v3/debug/index.html'].forEach(p=>{
  ...
  const s=read(p);
  assert(s.includes('ln-v3-alpha6-fix2'), p+' body class not alpha6fix2');
  assert(s.includes('v300alpha6fix2-20260512'), p+' stamp not alpha6fix2');
  assert(s.includes('?v=v300alpha6fix2-20260512'), p+' cache bust query missing');
  assert(s.includes('/fenxi/v3/assets/js/adapters/plans-adapter.v3.js?v=v300alpha6fix2-20260512'), p+' missing versioned plans adapter script');
  assert(s.includes('/fenxi/v3/assets/js/adapters/scenario-adapter.v3.js?v=v300alpha6fix2-20260512'), p+' missing versioned scenario adapter');
});
[
 ...
 'fenxi/v3/docs/V3_alpha6fix2_家庭路径优先级与就业画像校准说明.md'
...
assert(scenario.includes('省内公办稳妥应压住普通就业叙事'), 'scenario adapter missing family-path priority calibration');
assert(scenario.includes('但不盖过当前分数段和家庭地域底线'), 'scenario adapter missing work-first soft profile calibration');
...
console.log('All V3.0.0.alpha6.fix2 checks passed.');
