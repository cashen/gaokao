...
if (!css.includes('.local-chain-inline') || !css.includes('.workspace-local-chain-chip') || !css.includes('.local-chain-summary-card')) errors.push('CSS 合同类缺失。');
const selectionPath = fs.existsSync(path.join(projectRoot, 'js/selection-pool.v3931.js')) ? 'js/selection-pool.v3931.js' : 'js/selection-pool.v3930.js';
const selection = fs.readFileSync(path.join(projectRoot, selectionPath), 'utf8');
if (!selection.includes('renderLocalStrongChainSummaryPanel') || !selection.includes('itemLocalStrongChainChip')) errors.push('自选页未接入强链短提示/汇总。');
const render = fs.readFileSync(path.join(projectRoot, 'js/feature/major-pool/render.js'), 'utf8');
if (!render.includes('renderLocalStrongChainInline')) errors.push('专业卡片未接入强链短标签。');
const surface = fs.readFileSync(path.join(projectRoot, 'js/knowledge/local-chain-surface-contract.js'), 'utf8');
if (!surface.includes('getLocalChainPresentation') || !surface.includes("'card'") || !surface.includes("'report'")) errors.push('展示调度层缺失。');
const reportBuilderPath = path.join(repoRoot, 'functions/_lib/feishu-selection-pool-report-builder.js');
if (fs.existsSync(reportBuilderPath)) {
  const reportBuilder = fs.readFileSync(reportBuilderPath, 'utf8');
  if (!reportBuilder.includes('localChainMarkdownLines') || !reportBuilder.includes('辽宁属地强链')) errors.push('飞书 Markdown 报告未接入强链诊断。');
} else {
  warnings.push('未找到飞书报告 builder，跳过服务端报告检查。');
}
