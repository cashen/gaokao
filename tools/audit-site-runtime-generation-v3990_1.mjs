...
const PAGINATION_SNAPSHOT_GUARD_PATH = '/ln-rank/js/feature/major-pool/pagination-snapshot-guard.v3990_1.js';

assert.equal(SITE_RUNTIME_CONTRACT.version, 'site-runtime-coherence-v3990_1');
assert.equal(SITE_RUNTIME_CONTRACT.generation, 'v3990_1');
assert.equal(SITE_RUNTIME_CONTRACT.queryVersion, '3990_1');
assert.equal(SITE_RUNTIME_CONTRACT.releaseVersion, 'v3.9.90.1');
...
for (const modulePath of LN_RANK_RUNTIME_CACHE_CONTRACT.activeGenerationModules) {
  ...
  assert.ok(modulePath.includes('3990_1'), `runtime cache current module has stale generation: ${modulePath}`);
}
...
for (const key of currentKeys) {
  ...
  if (!pageRouteKeys.has(key)) assert.ok(String(value).includes('3990_1'), `current entrypoint is not v3990_1: ${key} -> ${value}`);
}
...
assert.equal(SITE_RUNTIME_CONTRACT.owners.interaction, '/shared/ui/interaction/interaction-transaction.v3990_1.js');
...
const interaction = fs.readFileSync('shared/ui/interaction/interaction-transaction.v3990_1.js', 'utf8');
for (const marker of [
...
const interactionCss = fs.readFileSync('shared/ui/interaction/interaction-transaction.v3990_1.css', 'utf8');
...
const app = fs.readFileSync('ln-rank/js/app.v3990_1.js', 'utf8');
const runtime = fs.readFileSync('ln-rank/js/app-runtime.v3990_1.js', 'utf8');
const workspace = fs.readFileSync('ln-rank/js/workspace/selection-workspace-orchestrator.v3990_1.js', 'utf8');
const paginationSnapshotGuard = fs.readFileSync('ln-rank/js/feature/major-pool/pagination-snapshot-guard.v3990_1.js', 'utf8');
for (const [label, source] of [['app', app], ['runtime', runtime], ['workspace', workspace], ['paginationSnapshotGuard', paginationSnapshotGuard]]) {
  assert.ok(source.includes('v3990_1'), `${label} does not use current generation`);
}
assert.ok(runtime.includes("const INTERACTION_VERSION = 'interaction-transaction-v3990_1'"));
assert.ok(workspace.includes("const INTERACTION_VERSION = 'interaction-transaction-v3990_1'"));
for (const marker of [
  'pagination-snapshot-guard.v3990_1.js?v=3990_1',
  ...
  "owner: '/ln-rank/js/app-runtime.v3990_1.js'",
  "implementation: '/ln-rank/js/feature/major-pool/pagination-snapshot-guard.v3990_1.js'"
]) assert.ok(runtime.includes(marker), `selection runtime missing snapshot ownership: ${marker}`);
for (const marker of [
  "MAJOR_BANDS_PAGINATION_SNAPSHOT_GUARD_VERSION = 'major-bands-pagination-snapshot-guard-v3990_1'",
...
const manifest = JSON.parse(fs.readFileSync('ln-rank/site-active-generation.v3990_1.json', 'utf8'));
...
assert.deepEqual(manifest.currentGenerationInternalModules, {
  majorBandsPaginationSnapshotGuard: `${PAGINATION_SNAPSHOT_GUARD_PATH}?v=3990_1`
});
...
console.log(JSON.stringify({
  ...
  paginationSnapshotGuard: 'major-bands-pagination-snapshot-guard-v3990_1',
