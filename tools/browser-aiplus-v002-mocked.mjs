...
async function waitReady(page){await page.waitForFunction(()=>{const text=document.querySelector('#healthBar')?.textContent||'';return text.includes('确定性查询仍可用')||text.includes('顾问可用');},null,{timeout:15000});}
async function waitTurn(page,before,state,finalBefore){
  await page.waitForFunction(count=>document.querySelectorAll('#conversationStream .turn[data-turn-id]').length>count&&document.querySelector('#sendButton')?.textContent?.trim()==='发送',before,{timeout:30000});
...
async function submit(page,state,text,{inspectLoading=false}={}){
  const before=await page.locator('#conversationStream .turn[data-turn-id]').count(),finalBefore=state.turnResponses.length;
  ...
  return waitTurn(page,before,state,finalBefore);
}
async function reset(page){const count=await page.locator('#conversationStream .turn[data-turn-id]').count();if(count){page.once('dialog',dialog=>dialog.accept());await page.locator('#newWorkspace').click();await page.waitForFunction(()=>document.querySelectorAll('#conversationStream .turn[data-turn-id]').length===0,null,{timeout:10000});}}
...
async function multiMajorJourney(page,state,name){
  ...
  if(state.failElectricOnce){
    ...
    const y=await page.evaluate(()=>scrollY),retryStart=state.schoolRequests.length,before=await page.locator('#conversationStream .turn[data-turn-id]').count(),finalBefore=state.turnResponses.length;await page.getByRole('button',{name:/重试没有完成的部分/}).click();const retry=await waitTurn(page,before,state,finalBefore);assert(retry.command?.retryFailedOnly===true,'retry command did not preserve failed-only intent');assert(JSON.stringify(retry.command?.majorKeywords)===JSON.stringify(['电气']),'retry queried successful majors again');const retried=state.schoolRequests.slice(retryStart);assert(retried.length===1&&retried[0].major==='电气',`retry fan-out drift ${JSON.stringify(retried)}`);const stability=await page.evaluate(expected=>({same:document.querySelector('#conversationStream .turn[data-turn-id]')===window.__stableFirstTurn,y:scrollY,trace:window.__scrollTrace,notice:!document.querySelector('#newAnswerNotice')?.hidden}),y);assert(stability.same,'existing answer card was rerendered after a new turn');assert(Math.abs(stability.y-y)<=2,`viewport jumped after answer ${JSON.stringify(stability)}`);assert(!stability.trace.some(item=>item.kind==='element'||JSON.stringify(item.args).includes('smooth')),`smooth/element scrolling leaked ${JSON.stringify(stability.trace)}`);assert(stability.trace.filter(item=>item.kind==='window').length<=1,`multiple viewport owners ${JSON.stringify(stability.trace)}`);assert(stability.notice,'new-answer notice should replace forced scrolling');
  }
}
