...
assert(idx.includes('v300beta8fix1-20260512'), 'index cache stamp beta8.fix1');
assert(idx2.includes('v300beta8fix1-20260512'), 'index.htm cache stamp beta8.fix1');
assert(dbg.includes('v300beta8fix1-20260512'), 'debug cache stamp beta8.fix1');
assert(ver.includes('V3.0.0.beta8.fix1｜导出报告双模式自测校准版'), 'version name beta8.fix1');
assert(ver.includes("stamp: 'v300beta8fix1-20260512'"), 'version stamp beta8.fix1');
assert(ver.includes("bodyClass: 'ln-v3-beta8-fix1'"), 'body class beta8.fix1');
...
assert(self.includes('V3.0.0.beta8.fix1'), 'debug selftest expects beta8');
...
assert(self.includes('Step7 精简版用于家庭快速沟通'), 'mainflow checks compact report role');
assert(self.includes('Step7 完整版包含家庭处境与决策日志'), 'mainflow checks full report decision log');
assert(self.includes('Step7 完整版包含复核任务清单'), 'mainflow checks full report review tasks');
assert(versionTxt.includes('V3.0.0.beta8.fix1'), 'VERSION.txt beta8.fix1');
assert(exists('fenxi/v3/docs/V3_beta8_导出报告增强与精简完整版说明.md'), 'beta8 docs exist');
if (process.exitCode) process.exit(process.exitCode);
console.log('All V3.0.0.beta8.fix1 checks passed.');
