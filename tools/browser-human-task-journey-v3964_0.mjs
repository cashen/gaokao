...
const artifactDir = process.env.DUAL_SEARCH_ARTIFACT_DIR || '/tmp/dual-search-v3964_0';
...
const yearCaliberVersion = 'ln-physics-report-years-v3964_0';
...
try {
  for (const testCase of cases) {
    ...
    try {
      ...
      assert.equal(await page.locator('#resultsTitle').textContent(), '可讨论专业 · 主要参考');
      const cardDetails = page.locator('.major-card-details').first();
      assert.equal(await cardDetails.getAttribute('open'), null, `${testCase.name}: secondary evidence opened before user intent`);
      assert.equal(await page.locator('.history-score--appendix').first().isVisible(), false);
      await cardDetails.locator('summary').click();
      assert.equal(await page.locator('.history-score--appendix').first().isVisible(), true);
      assert.match(await page.locator('.history-score__label').first().textContent(), /不参与2026当前分组/);
      ...
      assert.equal(await page.locator('.major-card .school-all-entry-button').first().textContent(), '看这所学校的在辽专业');
      ...
      assert.match(await page.locator('[data-ui-mobile-selection] span').textContent(), /已选|还没选专业/);
      ...
      assert.equal(await page.locator('#familyConditionsDetails').getAttribute('open'), '');
      ...
      assert.equal(finalMetrics.release, 'v3.9.64.0');
      assert.equal(finalMetrics.workspaceVersion, 'selection-workspace-orchestration-v3964_0');
      assert.equal(finalMetrics.schoolVersion, 'school-all-mode-v3964_0');
      ...
      assert.equal(await page.locator('#selectionRuntimeStatus').isHidden(), true);
      assert.equal(await page.locator('.analysis-not-run').isVisible(), true);
      assert.equal(await page.locator('[data-drag-id]').count(), 0);
      assert.equal(await page.locator('[data-up],[data-down],[data-move-toggle]').count(), 0);
...
console.log(JSON.stringify({
  ok: true,
  contract: 'dual-search-real-journey-v3964_0',
  cases: results
}, null, 2));
