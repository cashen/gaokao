// V2.9.8.1 export-engine: CSV/PNG export helpers + decision evidence fields
...
function csvEscape(v){if(v==null)v='';v=String(v);return/[",\n]/.test(v)?'"'+v.replace(/"/g,'""')+'"':v}function rowsToCsv(rows){
  const head=['学校','省份','城市','区域','学校地域来源','地域置信度','学校性质','院校层级','专业','标准专业','学科门类','专业类代码','本科专业类','专业代码','学硕一级/跨门类参考','专硕类别/领域参考','二级学科示例','目录可信度','招生名复核','易混主题','孩子关注点','激活方向','兴趣匹配等级','匹配依据','匹配专业代码','匹配专业名','匹配专业类代码','匹配专业类','兴趣复核说明','录取安全标签','投档证据2025','投档证据2024','位次变化说明','孩子学习特点摘要','提醒标签','换来的价值','需要接受','填报前复核','2025分','2025位次','2024分','2024位次','层级','画像分','风险','地域说明','闭环路径','闭环层级','闭环角色','风险层','取舍点','就业环境','毕业出口','复核动作','闭环标签','闭环冲突','闭环说明'];
  const body=(rows||[]).map(r=>{
    r=enrich(r);
    (()=>{const ev=(window.LN_PATH_EXPLAIN_ENGINE_V298||window.LN_PATH_EXPLAIN_ENGINE_V2976||window.LN_PATH_EXPLAIN_ENGINE_V2975)?.exportEvidence?.(r)||{}; const dec=window.LN_EXPORT_DECISION_FIELDS_V2981?.fields?.(r)||{}; return [r.school,r.schoolProvince||'',r.schoolCity||'',r.schoolRegion||'',r.schoolGeoSourceMethod||'',confidenceLabel(r.schoolGeoConfidence||'low'),r.schoolNature?.label||'',r.schoolTier?.label||'',r.major,r.cleanMajor,r.undergradDisciplineName,r.officialCategoryCode,r.undergradCategoryName,r.officialMajorCode,r.gradAcademicText,r.gradProfessionalText,r.gradSecondaryText,confidenceLabel(r.gradReferenceConfidence||r.taxonomyConfidence),r.admissionReviewTags||'', confusablePairsForRecordV2946(r).map(p=>p.group_name).join('|'), ev.childInterest||'', ev.activeInterest||'', ev.matchLevel||'', ev.evidence||'', ev.catalogMajorCode||'', ev.catalogMajorName||'', ev.catalogCategoryCode||'', ev.catalogCategoryName||'', ev.reviewMessage||'', dec.safety||'', dec.evidence2025||'', dec.evidence2024||'', dec.evidenceTrend||'', dec.profileSummary||'', dec.decisionTags||'', dec.gain||'', dec.accept||'', dec.review||'', r.score2025,r.rank2025,r.score2024,r.rank2024,r._level||'',Math.round(r._profile||0),(r.riskFlags||[]).join('|'),CITY_GEO_NOTE_V29472, ...(function(){const cf=window.LN_RULES_CLOSURE_V291?.closureCsvFields?.(r,'B')||{}; return [cf.path||'',cf.layer||'',cf.role||'',cf.riskLayer||'',cf.tradeoff||'',cf.environment||'',cf.exit||'',cf.review||'',cf.tags||'',cf.conflicts||'',cf.notes||''];})()];})()
      .map(x=>`"${String(x??'').replace(/"/g,'""')}"`).join(',');
  });
  return [head.join(','),...body].join('\n');
}function download(name,text){const b=new Blob([text],{type:'text/csv;charset=utf-8'});const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=name;a.click();URL.revokeObjectURL(a.href)}function exportFiltered(){download('辽宁物理类_V2.9.8.1_筛选结果_证据卡增强版.csv',rowsToCsv(filtered))}function exportCandidates(){download('辽宁物理类_V2.9.8.1_候选清单_证据卡增强版.csv',rowsToCsv(candidates.map(enrich)))}
...
function exportSummaryPng(kind='filtered'){
  ...
  ctx.fillText(`版本：V2.9.8.1｜生成时间：${new Date().toLocaleString('zh-CN')}`,M+38,y+100);
  ...
  topRows.forEach((r,i)=>{
    ...
    const _cf=window.LN_RULES_CLOSURE_V291?.closureCsvFields?.(r,'B')||{};
    const _judge=(typeof judge==='function'?judge(r):'')||'';
    wrapCanvasText(ctx,`判断：${_judge||_cf.path||'—'}${_cf.role?'｜'+_cf.role:''}${_cf.riskLayer?'｜'+_cf.riskLayer:''}${_cf.tradeoff?'｜'+_cf.tradeoff:''}${_cf.conflicts?'｜冲突：'+_cf.conflicts:''}`,M+900,yy+92,contentW-980,23,'#6b7f93','19px sans-serif',2);
    yy += rowH;
  });
  y += listH + 24;
  ...
  const name=kind==='candidates'?'辽宁物理类_V2.9.8.1_候选清单摘要.png':'辽宁物理类_V2.9.8.1_筛选摘要.png';
...
rowsToCsv = function(rows){
  const head=['来源方案','方案角色','路径标签','路径提醒','孩子想法提醒','分段解释','孩子关注点','激活方向','兴趣匹配等级','匹配依据','匹配专业代码','匹配专业名','匹配专业类代码','匹配专业类','学校','省份','城市','区域','学校地域来源','地域置信度','学校性质','院校层级','专业','主专业名','办学类型','标准专业','学科门类','专业类代码','本科专业类','专业代码','学硕一级/跨门类参考','专硕类别/领域参考','二级学科示例','目录可信度','招生名复核','易混主题','2025分','2025位次','2024分','2024位次','层级','画像分','风险','复核项','地域说明','闭环路径','闭环角色','风险层','取舍点','就业环境','毕业出口','复核动作','闭环冲突'];
