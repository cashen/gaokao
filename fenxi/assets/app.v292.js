...
function initTaxonomy(taxObj, aliasObj, groupObj){
  ...
  if(el){
    el.textContent=`专业学科映射：已加载 ${fmt(TAXONOMY_MAP.size)} 个标准专业/专业类；原始专业别名 ${fmt(Object.keys(RAW_MAJOR_ALIAS).length)} 个。`;
  }
}
...
function attachTaxonomy(r){
  ...
  r.taxonomyConfidence=t.item.overallConfidence||'low';
  r.undergradDisciplineName=t.item.undergradDiscipline?.name||'';
  r.undergradCategoryName=t.item.undergradCategory?.name||'';
  ...
  r.isMajorGroup=t.item.majorKind==='本科专业类';
...
function renderTaxonomyLine(r){
  ...
  if(!t)return '<div class="taxonomy-line"><span class="review">学科归属：需复核</span></div>';
  const path=[r.undergradDisciplineName,r.undergradCategoryName,r.cleanMajor].filter(Boolean).join(' / ');
  const primary=r.primaryDisciplineNames||'一级学科需复核';
  const cls=r.taxonomyConfidence==='low'?'review':'primary';
  return `<div class="taxonomy-line">
    <span class="path">本科口径：${path||'需复核'}</span>
    <span class="${cls}">一级学科参考：${primary}</span>
...
function renderTaxonomyDetail(r){
  ...
  return `<details class="taxonomy-detail">
    <summary>查看专业归属与一级学科参考</summary>
    <div class="tax-grid">
      <div><span>原始招生专业</span><b>${r.major||'-'}</b></div>
      <div><span>标准专业/专业类</span><b>${r.cleanMajor||'-'}</b></div>
      <div><span>本科目录口径</span><b>${[r.undergradDisciplineName,r.undergradCategoryName].filter(Boolean).join(' / ')||'需复核'}</b></div>
      <div><span>一级学科参考</span><b>${r.primaryDisciplineNames||'需复核'}</b></div>
      <div><span>学科群</span><b>${r.subjectGroup||'其他/需复核'}</b></div>
      <div><span>置信度</span><b>${confidenceLabel(r.taxonomyConfidence)}｜${t.matchMethod||''}</b></div>
    </div>
    ${aliasNote?`<div class="tax-warn">${aliasNote}</div>`:''}
    ${warns}
    <div class="tax-warn">说明：本科专业与研究生一级学科不是官方一一对应，本工具给出的是高报理解口径参考。</div>
  </details>`;
}
...
async function ensureDataForCurrentRank(){
  ...
  if(!rank){
    DATA=[]; filtered=[];
    if(metaEl)metaEl.textContent=`已就绪｜总数据 ${fmt(MANIFEST.totalRecords)} 条｜专业学科映射已加载｜输入位次后加载对应分段`;
    return;
  }
...
function applyFilters(){
  ...
  arr=arr.filter(r=>{
    ...
    if(fPrimary&&!((r.primaryDisciplineNames||'').includes(fPrimary)||(r.primaryDisciplineCodes||'').includes(fPrimary)||(r.cleanMajor||'').includes(fPrimary)||(r.undergradCategoryName||'').includes(fPrimary)))return false;
...
function csvEscape(v){if(v==null)v='';v=String(v);return/[",\n]/.test(v)?'"'+v.replace(/"/g,'""')+'"':v}function rowsToCsv(rows){
  const head=['学校','专业','标准专业','学科群','本科专业类','一级学科参考','学科置信度','2025分','2025位次','2024分','2024位次','层级','画像分','风险'];
  const body=(rows||[]).map(r=>{
    r=enrich(r);
    return [r.school,r.major,r.cleanMajor,r.subjectGroup,r.undergradCategoryName,r.primaryDisciplineNames,confidenceLabel(r.taxonomyConfidence),r.score2025,r.rank2025,r.score2024,r.rank2024,r._level||'',Math.round(r._profile||0),(r.riskFlags||[]).join('|')]
      .map(x=>`"${String(x??'').replace(/"/g,'""')}"`).join(',');
  });
  return [head.join(','),...body].join('\n');
}function download(name,text){const b=new Blob([text],{type:'text/csv;charset=utf-8'});const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=name;a.click();URL.revokeObjectURL(a.href)}function exportFiltered(){download('辽宁物理类_V292筛选结果_含学科.csv',rowsToCsv(filtered))}function exportCandidates(){download('辽宁物理类_V292候选清单_含学科.csv',rowsToCsv(candidates.map(enrich)))}
...
function exportSummaryPng(kind='filtered'){
  ...
  if(!rows.length){ alert(kind==='candidates'?'候选清单为空，无法导出 PNG。':'当前筛选结果为空，无法导出 PNG。'); return; }
  const info=pickedSummary();
  const plan=pickPlanRows();
  const topRows=(kind==='candidates' ? rows : rows.slice(0,12));
  const W=1600, M=56, contentW=W-M*2;
  const rowH=122;
  const H=280 + 220 + 260 + topRows.length*rowH + 180 + (kind==='filtered'?110:60);
  const canvas=document.createElement('canvas'); canvas.width=W; canvas.height=H;
  const ctx=canvas.getContext('2d');
  // bg
  const grad=ctx.createLinearGradient(0,0,W,H); grad.addColorStop(0,'#f7fbff'); grad.addColorStop(1,'#eef5ff'); ctx.fillStyle=grad; ctx.fillRect(0,0,W,H);
  // header
  roundRect(ctx,M,34,contentW,154,28,'#183153',null);
  ctx.fillStyle='#fff'; ctx.font='bold 42px sans-serif'; ctx.fillText(kind==='candidates'?'辽宁物理类志愿工具｜候选清单 PNG 摘要':'辽宁物理类志愿工具｜当前筛选 PNG 摘要',M+36,92);
  ctx.font='24px sans-serif'; ctx.fillStyle='rgba(255,255,255,.92)';
  ctx.fillText(`版本：V2.9.2｜生成时间：${new Date().toLocaleString('zh-CN')}`,M+36,132);
  ctx.fillText(`说明：PNG 为当前页面摘要图，便于转发沟通；正式填报仍需复核招生计划、校区、体检、学费与专业组。`,M+36,168);
  // summary cards
  roundRect(ctx,M,212,contentW,186,24,'#ffffff','rgba(47,91,176,.12)');
  ctx.fillStyle='#17324d'; ctx.font='bold 28px sans-serif'; ctx.fillText('一、筛选条件摘要',M+24,250);
  let tx=M+24, ty=276;
  const tags=[
    `成绩 ${info.score} 分`, `位次 ${info.rank}`, `策略 ${info.strategy}`, `优先级 ${info.priority}`, `预算 ${info.budget}`,
    `区域 ${info.regionMode}`, `目标省份 ${info.provinces}`, `规则 ${info.mentorMode}`, `拒绝项 ${info.rejects}`
  ];
  let lineX=tx, lineY=292;
  tags.forEach((t,i)=>{ const step=drawTag(ctx,lineX,lineY,t,'#edf4ff','#24425f'); if(lineX+step>W-M-80){ lineX=tx; lineY+=48; drawTag(ctx,lineX,lineY,t,'#edf4ff','#24425f'); lineX += step; } else lineX += step; });
  ctx.fillStyle='#59718c'; ctx.font='22px sans-serif';
  wrapCanvasText(ctx,`关键词：学校「${info.schoolKeyword}」｜专业「${info.majorKeyword}」｜学科群「${info.subjectGroup}」｜一级学科「${info.primary}」｜学科可信度「${info.taxConfidence}」｜层级「${info.level}」`,M+24,370,contentW-48,30,'#59718c','22px sans-serif',2);
  // plan ABC
  roundRect(ctx,M,420,contentW,226,24,'#ffffff','rgba(47,91,176,.12)');
  ctx.fillStyle='#17324d'; ctx.font='bold 28px sans-serif'; ctx.fillText('二、A / B / C 方案概览',M+24,458);
  const cols=[['A：冲',plan.A,'#f4e9ff','#7b1fa2'],['B：稳',plan.B,'#eaf3ff','#1565c0'],['C：保',plan.C,'#fff4e6','#ef6c00']];
  cols.forEach((it,idx)=>{
    const x=M+24+idx*((contentW-48)/3), y=478, w=(contentW-96)/3;
    roundRect(ctx,x,y,w,148,20,it[2],'rgba(0,0,0,.03)');
    ctx.fillStyle=it[3]; ctx.font='bold 26px sans-serif'; ctx.fillText(it[0],x+18,y+32);
    const r=it[1];
    ctx.fillStyle='#183153'; ctx.font='bold 24px sans-serif'; ctx.fillText(r?String(r.school).slice(0,17):'暂无合适候选',x+18,y+66);
    ctx.font='22px sans-serif'; wrapCanvasText(ctx,r?`${r.major}`:'可放宽区域或条件再筛选。',x+18,y+98,w-36,28,'#334b68','22px sans-serif',2);
    ctx.fillStyle='#627b97'; ctx.font='20px sans-serif'; wrapCanvasText(ctx,formatLevelText(r),x+18,y+136,w-36,24,'#627b97','20px sans-serif',1);
  });
  // stats
  const yStats=666;
  roundRect(ctx,M,yStats,contentW,88,24,'#ffffff','rgba(47,91,176,.12)');
  ctx.fillStyle='#17324d'; ctx.font='bold 28px sans-serif'; ctx.fillText('三、结果概况',M+24,yStats+37);
  let sx=M+200; ['当前结果 '+fmt((filtered||[]).length)+' 条','可冲 '+fmt((filtered||[]).filter(r=>r._level==='可冲').length),'匹配 '+fmt((filtered||[]).filter(r=>r._level==='匹配').length),'稳妥 '+fmt((filtered||[]).filter(r=>r._level==='稳妥').length),'保底 '+fmt((filtered||[]).filter(r=>r._level==='保底').length)].forEach((t,i)=>{sx += drawTag(ctx,sx,yStats+18,t,'#f5f8fd','#38506a');});
  // list
  const listY=yStats+110;
  roundRect(ctx,M,listY,contentW,topRows.length*rowH+84,24,'#ffffff','rgba(47,91,176,.12)');
  ctx.fillStyle='#17324d'; ctx.font='bold 28px sans-serif'; ctx.fillText(kind==='candidates'?'四、候选清单':'四、前排结果摘要',M+24,listY+40);
  ctx.fillStyle='#7187a1'; ctx.font='21px sans-serif'; ctx.fillText(kind==='candidates'?'展示当前候选清单全部条目（若过多建议先清理）。':'为保证易读性，PNG 默认展示当前排序下前 12 条。',M+24,listY+70);
  let yy=listY+92;
  topRows.forEach((r,i)=>{
    const color=levelColor(r._level);
    roundRect(ctx,M+18,yy,contentW-36,rowH-12,18,'#fbfdff','rgba(23,49,83,.08)');
    roundRect(ctx,M+34,yy+16,96,32,16,color,null); ctx.fillStyle='#fff'; ctx.font='bold 20px sans-serif'; ctx.fillText(r._level||'-',M+54,yy+38);
    ctx.fillStyle='#17324d'; ctx.font='bold 25px sans-serif'; ctx.fillText(`${i+1}. ${String(r.school).slice(0,26)}`,M+150,yy+34);
    ctx.font='23px sans-serif'; wrapCanvasText(ctx,`${r.major}`,M+150,yy+66,620,28,'#334b68','23px sans-serif',1);
    ctx.fillStyle='#5f7691'; ctx.font='20px sans-serif';
    wrapCanvasText(ctx,`2025：${fmt(r.score2025)}分 / ${fmt(r.rank2025)}位｜2024：${fmt(r.score2024)}分 / ${fmt(r.rank2024)}位｜画像 ${Math.round(r._profile||0)}分`,M+150,yy+95,720,24,'#5f7691','20px sans-serif',1);
    ctx.fillStyle='#24507b'; ctx.font='20px sans-serif'; wrapCanvasText(ctx,`学科：${r.undergradCategoryName||'待复核'}｜一级学科：${r.primaryDisciplineNames||'待复核'}`,M+900,yy+38,contentW-970,24,'#24507b','20px sans-serif',2);
    ctx.fillStyle='#6b7f93'; ctx.font='19px sans-serif'; wrapCanvasText(ctx,`判断：${judge(r)||'—'}`,M+900,yy+88,contentW-970,23,'#6b7f93','19px sans-serif',2);
    yy += rowH;
  });
  const footY=H-80;
  ctx.fillStyle='#627b97'; ctx.font='20px sans-serif';
  wrapCanvasText(ctx,'注：本摘要图用于初选沟通，不替代正式志愿表。中外合作、高收费、专业类分流、一级学科映射置信度低等情况，请务必再做人工核验。',M,footY,contentW,26,'#627b97','20px sans-serif',2);
  const name=kind==='candidates'?'辽宁物理类_V2.9.2_候选清单摘要.png':'辽宁物理类_V2.9.2_筛选摘要.png';
...
function renderDebugPanel(){
  ...
  panel.innerHTML=`<b>V2.9.2 Debug</b><br/>
...
async function boot(){
  ...
  try{
    ...
    initTaxonomy(taxonomyObj, aliasObj, groupObj);
