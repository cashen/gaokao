...
// V2.9.8.fix1: detailed candidate list also follows the same child-interest closed-loop.
// This is a soft sort only: no candidate is excluded unless the user turns on "只看真实命中兴趣方向".
function interestSortScoreV298Fix1(r){
  try{
    const rt=window.LN_CHILD_INTEREST_RUNTIME_V296;
    if(!rt) return 0;
    const st=rt.readState?.()||{};
    const activeIds=rt.effectiveGroupIds?.(st)||[];
    if(!activeIds.length) return 0;
    const m=rt.matchRecord?.(r);
    if(!m || !m.active) return 0;
    const levelScore={core:300, related:160, review:60, none:0, no:0};
    const bucketBoost=Number(m.boosts?.B||0);
    return (levelScore[m.level]||0) + bucketBoost;
  }catch(e){return 0;}
}
function hasActiveChildInterestV298Fix1(){
  try{
    const rt=window.LN_CHILD_INTEREST_RUNTIME_V296;
    if(!rt) return false;
    return (rt.effectiveGroupIds?.(rt.readState?.()||{})||[]).length>0;
