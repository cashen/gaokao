...
/* ===== v2.8.1 新增：导出结果摘要图 ===== */
function getSelectedRegionNamesForExport(){
  try{
    const activeGroups=[...document.querySelectorAll('#regionGroupChips .chip.active')].map(x=>x.dataset.regionGroup||x.textContent.trim()).filter(Boolean);
    const activeProvinces=[...document.querySelectorAll('#provinceChips .chip.active')].map(x=>x.dataset.province||x.textContent.trim()).filter(Boolean);
    if(activeProvinces.length)return activeProvinces.join(' / ');
    if(activeGroups.length)return activeGroups.join(' / ');
  }catch(e){}
  return '未设置';
}
function getActiveStrategyLabelForExport(){
  try{
    const btn=document.querySelector('#strategyCards .strategy-card.active');
    if(btn){
      const title=btn.querySelector('strong')?.textContent?.trim();
      if(title)return title;
    }
  }catch(e){}
  const mp={employment:'普通家庭｜稳就业',grid:'电网能源｜目标就业',medical:'医学方向｜长周期投入',exam:'考公体制｜保留入口',city:'城市优先｜大城市',shenyang:'沈阳优先｜省会资源',dalian:'大连优先｜城市资源',publicLow:'低分公办本科｜先保身份',school:'学校层级｜平台优先',broad:'先不限制｜全量观察'};
  return mp[currentStrategy]||'未设置';
}
function getExportAvoidList(){
  const arr=[];
  const budget=document.getElementById('budget')?.value;
  const strategy=currentStrategy;
  if(budget==='normal')arr.push('高收费/中外合作谨慎');
  if(strategy!=='medical')arr.push('医学方向谨慎');
  const strict=document.getElementById('strictProfile');
  if(strict&&strict.checked)arr.push('严格画像缩水');
  return arr.length?arr:['未设置'];
}
function buildCurrentFormStateForExport(){
  const score=document.getElementById('myScore')?.value || '';
  const rank=currentRank || document.getElementById('myRank')?.value || '';
  const priorityEl=document.getElementById('priority');
  const budgetEl=document.getElementById('budget');
  const regionMode=document.getElementById('regionMode')?.value;
  const regionModeText={hard:'硬筛：只看所选区域',soft:'软筛：所选区域优先',none:'不限制区域'}[regionMode]||'未设置';
  const strategyLabel=getActiveStrategyLabelForExport();
  const regionText=getSelectedRegionNamesForExport();
  return {
    score: score || '未填写',
    rank: rank || '未填写',
    subjectType: '物理类',
    subjects: '物化生',
    gender: '未填写',
    regionPreference: `${regionModeText}｜${regionText}`,
    majorPreference: strategyLabel,
    avoidList: getExportAvoidList(),
    schoolNature: currentStrategy==='publicLow' || budgetEl?.value==='normal' ? '公办优先' : '不限制',
    schoolLevel: '本科优先',
    enableBackgroundCheck: true,
    enableZxfQuestions: true,
    keepCivilServicePath: currentStrategy==='exam',
    employmentFirst: priorityEl?.value==='employment' || currentStrategy==='employment',
    excludeChineseForeign: budgetEl?.value==='normal'
  };
}
function buildRowsForExportImage(){
  return (filtered||[]).map(r=>{
    const er=typeof enrichRecord==='function'?enrichRecord(r):r;
    return {
      ...er,
      schoolName: er.school,
      majorName: er.major,
      city: er.schoolCity || er.lnArea || er.schoolProvince || '',
      schoolNature: er.schoolNature?.label || er.schoolNature || '',
      level: er._level,
      matchScore: er._profile || 0,
      hasRisk: (er.riskFlags||[]).length>0 || er.isHighFee,
      reason: (er._reasons||[]).slice(0,2).join('；') || (typeof judge==='function'?judge(er):''),
      note: `2025：${fmt(er.score2025)}分 / ${fmt(er.rank2025)}位${er.rankChangeLabel?'｜'+er.rankChangeLabel:''}`
    };
  });
}
async function exportSummaryImage(){
  if(!window.ZYExportSummary){
    alert('导出组件未加载，请刷新页面后重试。');
    return;
  }
  const rows=buildRowsForExportImage();
  if(!rows.length){
    alert('当前没有可导出的结果，请先完成筛选。');
    return;
  }
  const formState=buildCurrentFormStateForExport();
  await window.ZYExportSummary.exportSummaryAsPng(formState, rows, {
    version:'v2.8.1',
    dataScope:'2025 辽宁物理类投档数据｜分块数据引擎版',
    maxItems:8,
    footer:'生成工具：辽宁物理类高考志愿初选工具 v2.8.1｜完整明细请导出 CSV 查看'
  });
}
/* ===== /v2.8.1 新增：导出结果摘要图 ===== */
