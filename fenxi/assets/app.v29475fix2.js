...
const APP_VERSION_V29472 = 'V2.9.4.7.5.fix2｜A/B/C 首选推荐与一键加入自选版';
...
}function download(name,text){const b=new Blob([text],{type:'text/csv;charset=utf-8'});const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=name;a.click();URL.revokeObjectURL(a.href)}function exportFiltered(){download('辽宁物理类_V2.9.4.7.5.fix2_筛选结果_高报师方案.csv',rowsToCsv(filtered))}function exportCandidates(){download('辽宁物理类_V2.9.4.7.5.fix2_候选清单_高报师方案.csv',rowsToCsv(candidates.map(enrich)))}
...
function exportSummaryPng(kind='filtered'){
  ...
  const name=kind==='candidates'?'辽宁物理类_V2.9.4.7.5.fix2_候选清单摘要.png':'辽宁物理类_V2.9.4.7.5.fix2_筛选摘要.png';
...
/* V2.9.4.7.5.fix2: A/B/C 首选推荐 + 备选小卡 + 一键加入自选
   目标：A/B/C 不再只是三列候选，而是三条路径；每条路径有“首选推荐”大卡 + 2-3 张备选小卡。
   同时支持加入本方案候选、加入全部 A/B/C 候选，并自动去重。 */
...
function primaryPlanItemV29475Fix2(r,type){
  ...
  return `<div class="plan-primary-v29475fix2 ${type.toLowerCase()}">
    <div class="primary-kicker-v29475fix2"><span>首选推荐</span><em>${htmlSafeV2945(levelKindV29475(r,type)||'观察')}</em></div>
...
function backupPlanItemV29475Fix2(r,type,idx){
  const tags=planReviewTagsV29473(r,type).slice(0,4).map(x=>`<span>${htmlSafeV2945(x)}</span>`).join('');
  return `<div class="plan-backup-v29475fix2 ${type.toLowerCase()}">
    ...
    <strong>${htmlSafeV2945(r.school)}</strong>
    <em>${htmlSafeV2945(r.major)}</em>
    <small>${htmlSafeV2945(compactMetaV29475Fix2(r))}</small>
    <p><b>为什么：</b>${strongWhyV29475Fix2(r,type,idx)}</p>
    <p><b>风险：</b>${htmlSafeV2945(planRiskTextV29475(r,type))}</p>
    <div class="review-tags-v29473"><strong>复核</strong>${tags}</div>
...
function renderPlanABC(){
  const box=document.getElementById('planABC'); if(!box)return;
  box.classList.add('abc-board-v29475fix','abc-board-v29475fix2');
  if(!currentRank){box.innerHTML=''; const d=document.getElementById('conflictDiagnosisV29473'); if(d)d.innerHTML=''; return;}
  const diag=diagnoseV29473();
  renderDiagnosisV29473(diag);
  const buckets=pickSchemeBucketsV29475();
  latestPlanBucketsV29475Fix2=buckets;
  const total=(buckets.A.length+buckets.B.length+buckets.C.length);
  box.setAttribute('data-total-cards', String(total));
  const toolbar=`<div class="abc-toolbar-v29475fix2"><div><b>A/B/C 方案盘</b><span>A看底线，B看专业，C看上限；冲稳保是每条路径里的安全等级。</span></div><button class="execute-secondary" onclick="addAllPlansV29475Fix2()">加入全部 A/B/C 候选</button></div>`;
  box.innerHTML=toolbar+planColumnV29475('A',buckets.A)+planColumnV29475('B',buckets.B)+planColumnV29475('C',buckets.C);
}
function pickPlanRows(){
