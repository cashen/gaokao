...
function csvEscape(v){if(v==null)v='';v=String(v);return/[",\n]/.test(v)?'"'+v.replace(/"/g,'""')+'"':v}function rowsToCsv(rows){
  const head=['学校','学校性质','院校层级','专业','标准专业','学科群','本科专业类','一级学科参考','学科置信度','2025分','2025位次','2024分','2024位次','层级','画像分','风险'];
  const body=(rows||[]).map(r=>{
    r=enrich(r);
    return [r.school,r.schoolNature?.label||'',r.schoolTier?.label||'',r.major,r.cleanMajor,r.subjectGroup,r.undergradCategoryName,r.primaryDisciplineNames,confidenceLabel(r.taxonomyConfidence),r.score2025,r.rank2025,r.score2024,r.rank2024,r._level||'',Math.round(r._profile||0),(r.riskFlags||[]).join('|')]
...
function exportSummaryPng(kind='filtered'){
  ...
  ctx.fillText(`版本：V2.9.4｜生成时间：${new Date().toLocaleString('zh-CN')}`,M+38,y+100);
  ...
  topRows.forEach((r,i)=>{
    ...
    wrapCanvasText(ctx,`学科：${r.undergradCategoryName||'待复核'}｜一级学科：${r.primaryDisciplineNames||'待复核'}`,M+900,yy+40,contentW-980,24,'#24507b','20px sans-serif',2);
  ...
  const name=kind==='candidates'?'辽宁物理类_V2.9.4_候选清单摘要.png':'辽宁物理类_V2.9.4_筛选摘要.png';
...
function renderDebugPanel(){
  ...
  panel.innerHTML=`<b>V2.9.4 Debug</b><br/>
