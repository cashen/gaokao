...
(function(){
  const VERSION = 'V2.92RC2.interact-dedupe';
  ...
  function runScheduled(reason, fp){
    ...
    // V2.92RC2: 不再绕过数据分块加载直接调用 applyFilters。
    ...
    if (typeof window.__LN_AUTO_REFRESH_DIRECT__ === 'function') {
      Promise.resolve(window.__LN_AUTO_REFRESH_DIRECT__(finalReason)).catch(function(e){console.warn('[V2.92RC2 interact-dedupe] auto refresh failed', e);});
