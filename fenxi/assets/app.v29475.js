...
const APP_VERSION_V29472 = 'V2.9.4.7.5｜预算宽路径与紧凑方案盘版';
...
function pickBucketV29475(type,limit=4,chosen=new Set()){
  const usable=(filtered||[]).filter(r=>!(r._excludes||[]).length);
  const scored=usable.slice().sort((a,b)=>planScoreV29475(b,type,chosen)-planScoreV29475(a,type,chosen));
  const out=[];
  for(const r of scored){
    if(out.length>=limit)break;
    if(chosen.has(r.id))continue;
    if(type==='A' && (r.isHighFee||r.isCoopV29475||r.isPrivateV29475) && out.length>=2)continue;
    out.push(r); chosen.add(r.id);
  }
  return out;
}
function pickSchemeBucketsV29475(){
  const chosen=new Set();
  return {A:pickBucketV29475('A',4,chosen),B:pickBucketV29475('B',4,chosen),C:pickBucketV29475('C',4,chosen)};
}
