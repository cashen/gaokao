...
(function(){
  ...
  var VERSION='v291rc0closure3';
  var STAMP='291rc0closure3-20260514';
  ...
  function patchPlanScore(){
    if(typeof window.planScoreV29475!=='function' || window.planScoreV29475.__closure3) return;
    ...
    wrapped.__closure3=true; wrapped.__original=old; window.planScoreV29475=wrapped; try{planScoreV29475=wrapped;}catch(e){} if(window.LN_PLAN_ENGINE){window.LN_PLAN_ENGINE.planScoreV29475=wrapped;}
  }
  ...
  function pick(type,avoid){
    ...
    var sample=ranked.slice(0,720).map(function(x){return x[0];});
    var scored=ranked.slice(0,720);
  ...
  function exportClosureSummary(rows){
    var list=(rows||[]).slice(0,12).map(function(r,i){var e=evalRecord(r,'B'), p=e.path; return [i+1,r.school||'',r.major||'',p.key,p.layer,(e.tags||[]).join('|'),(e.notes||[]).join('；'),(e.conflicts||[]).join('|')];});
  ...
  function closureCsvFields(r,type){
    ...
    return {path:p.key, layer:p.layer, tags:(e.tags||[]).join('|'), notes:(e.notes||[]).join('；'), conflicts:(e.conflicts||[]).join('|')};
  }
  ...
  function renderABC(viewOnly){
    ...
    var toolbar='<div class="abc-toolbar-v296"><div><b>A/B/C 方案视角</b><span>A 守底线，B 重点讨论，C 看机会；已接入孩子情况、现场/夜班/长周期、考公/就业路径、成本风险、路径集中提示和当前优先考虑。</span></div><button class="execute-secondary" onclick="addAllPlansV29475Fix2()">加入全部 A/B/C 候选</button></div>';
