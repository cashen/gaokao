// V2.9.6.fix6 qualification gate UI: compact summary card + drawer manager.
(function(){
  ...
  function row(g,state){
    ...
    return `<div class="qgate-row-v296fix6"><div><div class="qgate-row-title-v296fix6"><b>${esc(g.name)}</b><em>${esc(badge)}</em></div><p>${esc((g.reviewTips||[]).slice(0,2).join('；'))}</p></div><select data-action="qualification-gate-change" data-gate-id="${esc(g.id)}">${opts.map(o=>`<option value="${o[0]}" ${val===o[0]?'selected':''}>${esc(o[1])}</option>`).join('')}</select></div>`;
  }
  function drawerHtml(){
    const st=window.LN_QUALIFICATION_GATE_V296?.readState?.()||{};
    const s=summaryData();
    const groups=grouped();
    return `<div class="qgate-drawer-v296fix6"><div class="qgate-drawer-head-v296fix6"><b>统一管理资格型入口</b><span>${esc(s.enabled?.length?`已纳入 ${s.enabled.length} 类；其余继续默认隐藏`:`默认隐藏 ${s.hidden||0} 类需资格入口`)}</span></div><p class="small">未确认资格前，系统默认隐藏高校专项、预科/民族班、定向培养等特殊入口，避免误当普通本科志愿。系统不替用户判断资格，只根据你确认的状态纳入比较。</p>${Object.keys(groups).map(k=>`<section><h4>${esc(k)}</h4>${groups[k].map(g=>row(g,st)).join('')}</section>`).join('')}<div class="qgate-later-v296fix6"><b>后续仅作为提醒扩展：</b>${esc((window.LN_QUALIFICATION_GATE_RULES_V296?.later||[]).join('、'))}</div></div>`;
  }
