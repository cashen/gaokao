...
function addCandidate(id){const r=DATA.find(x=>x.id===id);if(r&&!candidates.find(x=>x.id===id))candidates.push(r);localStorage.setItem('ln_candidates_v29',JSON.stringify(candidates));renderCandidates();renderStructure()}
...
}function download(name,text){const b=new Blob([text],{type:'text/csv;charset=utf-8'});const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=name;a.click();URL.revokeObjectURL(a.href)}function exportFiltered(){download('辽宁物理类_V29筛选结果_含学科.csv',rowsToCsv(filtered))}function exportCandidates(){download('辽宁物理类_V29候选清单_含学科.csv',rowsToCsv(candidates.map(enrich)))}
async function boot(){
  bootChips();
  try{
    ...
    const metaEl=document.getElementById('metaRecords');
    if(metaEl)metaEl.textContent=`已就绪｜总数据 ${fmt(MANIFEST.totalRecords)} 条｜专业学科映射已加载｜输入位次后加载对应分段`;
    candidates=JSON.parse(localStorage.getItem('ln_candidates_v29')||'[]');
    renderCandidates();
