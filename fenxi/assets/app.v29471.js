...
function renderDebugPanel(){
  ...
  panel.innerHTML=`<b>V2.9.4.7.1.fix Debug</b><br/>
  ...
  易混模型：<code>${CONFUSABLE_MODEL_2946?((CONFUSABLE_MODEL_2946.detectedPairs?.count||0)+' 对 / '+(CONFUSABLE_MODEL_2946.recordIndex?.count||0)+' 条索引'):'未加载'}</code><br/>学校地域：<code>${SCHOOL_GEO_MODEL_29471?(SCHOOL_GEO_MODEL_29471.items.length+' 所'):'未加载'}</code>｜学生画像：<code>${STUDENT_PROFILE_MODEL_29471?((STUDENT_PROFILE_MODEL_29471.rules||[]).length+' 条规则'):'未加载'}</code><br/>app.js：<code>app.v29471.js</code>｜app.css：<code>app.v29471.css</code>`;
}
function initV292UX(){
  applyCardViewModeClassV29461();
  ensureCardViewModeToolbarV29461();
