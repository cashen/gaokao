...
(function(){
  ...
  function text(r){return String([r&&r.school,r&&r.major,r&&r.cleanMajor,r&&r.majorText,r&&r.admissionMajor,r&&r.remark,r&&r.feeTypeLabelV29475,r&&r.schoolNature&&r.schoolNature.label].filter(Boolean).join(' '));}
  function score(){const v=num(el('myScore')&&el('myScore').value);return v>0?v:0}
  function rank(){let r=num(el('myRank')&&el('myRank').value);if(r>0)return r;try{r=num(typeof currentRank!=='undefined'?currentRank:0);if(r>0)return r;}catch(e){}try{r=num(typeof resolveRank==='function'?resolveRank():0);if(r>0)return r;}catch(e){}return 0}
  function band(){const r=rank(),s=score();if(r>0){if(r<=12000)return'top';if(r<=25000)return'high';if(r<=60000)return'middle';if(r<=90000)return'low';return'edge'}if(s>=650)return'top';if(s>=610)return'high';if(s>=530)return'middle';if(s>=480)return'low';if(s>0)return'edge';return'unknown'}
  function bp(){return (GOV.bands&&GOV.bands[band()])||(GOV.bands&&GOV.bands.unknown)||{aTooLow:-40,bTooLow:-30,cFantasy:25,bSafeMax:3,cNeedOpportunity:false}}
  function dScore(r){const s=score(),cs=num(r&&r.score2025);return s&&cs?cs-s:null}
  function dRank(r){const ur=rank(),cr=num(r&&r.rank2025);return ur&&cr?cr-ur:null}
  function isPublic(r){return ['public','publicSoft'].includes(r&&r.schoolTier&&r.schoolTier.level)||(r&&r.schoolNature&&r.schoolNature.label==='公办倾向')}
  function isHighCost(r){const t=text(r);return !!(r&&(r.isHighFee||r.isCoopV29475||r.isPrivateV29475||/高收费|中外合作|民办|独立学院/.test(t)))}
  function selectedRejectsSafe(){try{return typeof selectedRejects==='function'?selectedRejects():[]}catch(e){return []}}
  function snapshot(){try{return typeof conditionSnapshotV29473==='function'?conditionSnapshotV29473():{}}catch(e){return {}}}
  function pathInfo(r){try{return typeof majorPathInfoV29476==='function'?majorPathInfoV29476(r):{}}catch(e){return {}}}
  function liftInfo(r){try{return typeof liftExchangeInfoV29476==='function'?liftExchangeInfoV29476(r):{}}catch(e){return {score:0,got:[],paid:[],label:''}}}
  function hardReject(r){
    const rejects=selectedRejectsSafe().join(' '),s=snapshot(),t=text(r);const out=[];
    const noFee=s.noHighFee||s.budget==='normal'||s.budget==='low'||/高收费/.test(rejects);
    if(noFee&&isHighCost(r))out.push('高收费/民办/中外与预算底线冲突');
    if(/工地|现场|设备|艰苦/.test(rejects)&&/土木|建筑|采矿|地质|测绘|石油|油气|过程装备|工程管理|安全工程/.test(t))out.push('现场/设备环境与拒绝项冲突');
    if(/夜班|长学制|长周期/.test(rejects)&&/临床|口腔|麻醉|儿科|护理|急诊|长学制/.test(t))out.push('医学夜班/长周期与拒绝项冲突');
    return {hard:out.length>0,reasons:out};
  }
  function tooLow(r,type){const d=dScore(r);if(d===null)return false;const p=bp();const lim=type==='A'?p.aTooLow:type==='B'?p.bTooLow:-9999;return d<lim}
  function fantasyC(r){const d=dScore(r);if(d===null)return false;return d>bp().cFantasy}
  function safeBand(r){return ['保底','过低'].includes(r&&r._level)}
  function realOpportunity(r){const p=pathInfo(r),l=liftInfo(r),s=snapshot();const got=(l&&l.got)||[];return ['可冲','匹配'].includes(r&&r._level)||['985','211'].includes(r&&r.schoolTier&&r.schoolTier.level)||got.length>0||(s.cities&&s.cities.length&&typeof cityMatchesV29472==='function'&&cityMatchesV29472(r,s.cities))||((s.budgetWide||s.coopIntent)&&isHighCost(r))||!!(p.isCore||p.isRelated);}
  function interestIds(){try{const st=window.LN_CHILD_INTEREST_RUNTIME_V296?.readState?.()||{};return Array.isArray(st.selectedGroups)?st.selectedGroups:[]}catch(e){return []}}
  function conflictAdjust(r,type){const ids=interestIds(),rej=selectedRejectsSafe().join(' '),t=text(r);let sc=0;
    if(ids.includes('electric_energy')&&/工地|现场|设备|艰苦/.test(rej)){if(/智能电网|电气工程|电力|能源动力/.test(t))sc+=type==='B'?-18:-8;if(/电子信息|自动化|通信|计算机|软件|人工智能|仪器|测控|数据/.test(t))sc+=type==='B'?20:8;}
    if(ids.includes('medical_health')&&/夜班|长学制|长周期/.test(rej)){if(/临床|口腔|麻醉|儿科|长学制/.test(t))sc+=type==='B'?-22:-10;if(/医学检验|医学影像技术|康复|药学|公共卫生|护理|生物医学工程/.test(t))sc+=type==='B'?18:8;}
    return sc;
  }
  function pathFit(r,type){const p=pathInfo(r);let sc=0;if(p.isCore)sc+=type==='B'?44:24;else if(p.isRelated)sc+=type==='B'?30:16;else if(p.key&&p.key!=='unknown')sc+=type==='B'?14:6;const q=(el('qMajor')&&el('qMajor').value||'').trim();try{if(q&&typeof majorMatchesV29475==='function'&&majorMatchesV29475(r,q))sc+=type==='B'?24:10}catch(e){}return sc;}
  function rankFit(r,type){const lv=r&&r._level,d=dScore(r);let sc=0;if(type==='A'){if(['匹配','稳妥'].includes(lv))sc+=34;if(lv==='保底')sc+=20;if(lv==='过低')sc-=22;if(lv==='可冲')sc-=10;if(tooLow(r,'A'))sc-=36;}else if(type==='B'){if(['匹配','稳妥','可冲'].includes(lv))sc+=30;if(lv==='保底')sc-=14;if(lv==='过低')sc-=36;if(tooLow(r,'B'))sc-=38;}else{if(['可冲','匹配'].includes(lv))sc+=30;if(lv==='稳妥')sc+=8;if(['保底','过低'].includes(lv))sc-=32;if(fantasyC(r))sc-=48;} if(d!==null&&type==='C'&&d>0&&d<=bp().cFantasy)sc+=10; return sc;}
  function familyFit(r,type){const s=snapshot();let sc=0;if(isPublic(r))sc+=type==='A'?28:type==='B'?10:4;if(!isHighCost(r))sc+=type==='A'?24:type==='B'?14:4;if(['辽宁','吉林','黑龙江'].includes(r&&r.schoolProvince))sc+=(s.regionMode==='hard'||s.strongProvince)?14:5;if(isHighCost(r)){if(s.budgetWide||s.coopIntent)sc+=type==='C'?24:4;else sc+=type==='A'?-80:type==='B'?-64:-30;}return sc;}
  function roleScore(r,type,chosen){if(!r)return HARD;if(chosen&&chosen.has&&chosen.has(rowKey(r)))return HARD;let base=0;try{base=OLD_SCORE(r,type,null)||0}catch(e){}let sc=base*0.68;const hard=hardReject(r);if((type==='A'||type==='B')&&hard.hard)return HARD+base;
    if(type==='A'){sc+=rankFit(r,'A')*1.05+familyFit(r,'A')*1.2+pathFit(r,'A')*.28+(r._profile||0)*.16;if(tooLow(r,'A'))sc-=32;}
    else if(type==='B'){sc+=pathFit(r,'B')*1.25+rankFit(r,'B')*1.0+familyFit(r,'B')*.7+(r._profile||0)*.30;if(safeBand(r))sc-=18;if(tooLow(r,'B'))sc-=34;}
    else{const opp=realOpportunity(r);sc+=rankFit(r,'C')*.95+pathFit(r,'C')*.55+familyFit(r,'C')*.35+(liftInfo(r).score||0)*.9;if(opp)sc+=28;else if(bp().cNeedOpportunity)sc-=36;if(hard.hard)sc-=45;if(fantasyC(r))sc-=50;if(safeBand(r)&&!opp)sc-=30;}
    sc+=conflictAdjust(r,type);return Math.round(sc*10)/10;}
  function rowKey(r){try{return typeof planKeyV29475Fix2==='function'?planKeyV29475Fix2(r):[r.school,r.major,r.rank2025,r.score2025].join('§')}catch(e){return [r&&r.school,r&&r.major,r&&r.rank2025,r&&r.score2025].join('§')}}
  function cluster(r){try{return typeof planClusterV29475Fix2==='function'?planClusterV29475Fix2(r):(pathInfo(r).key||'unknown')}catch(e){return pathInfo(r).key||'unknown'}}
  function canUse(r,type,chosen,clusters,out,phase){if(!r)return false;const k=rowKey(r);if(chosen.has(k))return false;const hard=hardReject(r);if((type==='A'||type==='B')&&hard.hard)return false;if(type==='A'&&phase<2){if(isHighCost(r))return false;if(tooLow(r,'A')&&out.length<2)return false;}if(type==='B'&&phase<2){const safe=out.filter(safeBand).length;if(tooLow(r,'B')&&out.length<2)return false;if(safeBand(r)&&safe>=bp().bSafeMax)return false;}if(type==='C'&&phase<2){if(bp().cNeedOpportunity&&!realOpportunity(r)&&out.length<2)return false;if(fantasyC(r)&&out.length<2)return false;}if((type==='B'||type==='C')&&phase<2&&clusters.has(cluster(r))&&out.length<3)return false;return true;}
  function fill(type,limit,globalChosen){const pool=(typeof filtered!=='undefined'&&Array.isArray(filtered)?filtered:(Array.isArray(window.filtered)?window.filtered:[])).filter(Boolean);const scored=pool.slice().sort((a,b)=>roleScore(b,type,globalChosen)-roleScore(a,type,globalChosen));const out=[],local=new Set(),clusters=new Set();for(let phase=0;phase<3;phase++){for(const r of scored){const k=rowKey(r);if(local.has(k)||globalChosen.has(k))continue;if(!canUse(r,type,globalChosen,clusters,out,phase))continue;out.push(r);local.add(k);globalChosen.add(k);clusters.add(cluster(r));if(out.length>=limit)return out;}}return out;}
  function pick(){const before=(typeof filtered!=='undefined'&&Array.isArray(filtered)?filtered.length:(Array.isArray(window.filtered)?window.filtered.length:0));if(!before&&OLD_PICK){const old=OLD_PICK();return Object.assign(old||{A:[],B:[],C:[]},{meta:{version:VERSION,fallback:'no-filtered-pool'}});}const chosen=new Set();let A=fill('A',4,chosen),B=fill('B',4,chosen),C=fill('C',4,chosen);if((!A.length||!B.length||!C.length)&&OLD_PICK){try{const old=OLD_PICK()||{};if(!A.length)A=old.A||[];if(!B.length)B=old.B||[];if(!C.length)C=old.C||[];}catch(e){}}
    const diag={version:VERSION,band:band(),policy:bp(),pool:before,A:{n:A.length,tooLow:A.filter(x=>tooLow(x,'A')).length,hard:A.filter(x=>hardReject(x).hard).length},B:{n:B.length,safe:B.filter(safeBand).length,tooLow:B.filter(x=>tooLow(x,'B')).length,hard:B.filter(x=>hardReject(x).hard).length},C:{n:C.length,fantasy:C.filter(fantasyC).length,opportunity:C.filter(realOpportunity).length,hard:C.filter(x=>hardReject(x).hard).length}};window.__LN_ABC_FORMULA_DIAG_LAST__=diag;return {A,B,C,meta:diag};}
  window.planScoreV29475=function(r,type,chosen){return roleScore(r,type,chosen);};
  window.pickSchemeBucketsV29475=pick;try{planScoreV29475=window.planScoreV29475;pickSchemeBucketsV29475=window.pickSchemeBucketsV29475;}catch(e){}
  const oldWhy=typeof primaryWhyV2950==='function'?primaryWhyV2950:null;
  window.primaryWhyV2950=function(r,type){const d=dScore(r),hard=hardReject(r),p=pathInfo(r);let prefix='';if(type==='A')prefix='为什么放在A：这是底线方案，优先安全、成本、地域和可落地，不是无意义低分保底';else if(type==='B')prefix='为什么放在B：这是主线方案，优先专业/就业/家庭目标的平衡，避免变成第二个A';else prefix='为什么放在C：这是机会方案，必须有真实上探价值，不能把幻想包装成机会';prefix+=(d!==null?'；分差'+d+'分':'');if(hard.hard)prefix+='；注意：'+hard.reasons.join('、');if(p.label)prefix+='；路径：'+p.label;try{return prefix+'。'+String(oldWhy?oldWhy(r,type):'');}catch(e){return prefix+'。'}};try{primaryWhyV2950=window.primaryWhyV2950;}catch(e){}
  window.LN_PLAN_ENGINE=Object.assign(window.LN_PLAN_ENGINE||{},{planScoreV29475:window.planScoreV29475,pickSchemeBucketsV29475:window.pickSchemeBucketsV29475,formulaGovernanceV293RC1:GOV,formulaVersion:VERSION});
})();
