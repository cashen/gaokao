...
async function boot(){
  ...
  try{
    ...
    try{
      ...
      console.warn('[V2.9.6.fix4] 招生名/本科目录模型加载失败，不影响主筛选：', majorErr);
    }
    try{
...
function autoRefreshDirectV296(reason){
  return autoRefreshAsync().then(()=>{
    ...
    window.LN_QUALIFICATION_GATE_UI_V296?.renderSummary?.();
...
setTimeout(()=>{try{initSimpleModeV2950();renderBaselineSummaryV2950();window.LN_QUALIFICATION_GATE_UI_V296?.renderSummary?.();}catch(e){console.warn('[V2.9.6.fix4] 简洁模式初始化失败',e)}},0);
