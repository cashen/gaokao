// V2.91RC0.rules-closure5.export-md5fix1｜Markdown DOM 安全插入修正版
// 只改 Markdown 导出；不改候选池、不改基础筛选、不改兴趣真实命中。
(function(){
  if(window.LN_EXPORT_MD4_OPT===false) return;
  var VERSION='v291rc0md5fix1';
  var STAMP='291rc0md5fix1-20260515';
  var FULL_VERSION='V2.91RC0.closure5fix1.frontfix6.export-md5fix1.errorfix2';
  var timer=null;
    ...
    try{
      ...
      if(area){
        ...
        if(!area.querySelector('[data-export-md4="full"]')){
          ...
          btn.type='button';
          btn.className='export-md-action-v291';
      ...
      if(cand&&!cand.querySelector('[data-export-md4="selected"]')){
        ...
        bar.className='export-md-self-v291';
        ...
        var h=cand.querySelector(':scope > h2');
        if(h&&h.parentNode===cand){safeInsert(cand,bar,h.nextSibling&&h.nextSibling.parentNode===cand?h.nextSibling:null);}else{safeInsert(cand,bar,cand.firstChild&&cand.firstChild.parentNode===cand?cand.firstChild:null);}
      }
  ...
  function refreshDebug(){try{window.LN_DEBUG_V2983?.setFlags?.({exportMd:'v291rc0md5fix1',exportMdStamp:STAMP,markdownCopy:true,markdownDownload:false,markdownRankDetail:true,markdownVolunteerOrder:true,mdSortPolicy:'gap-based-md5-0-500-1500-5000-12000',doesModifyFormula:false,doesChangeCandidatePool:false,doesChangeSorting:false}); window.__LN_EXPORT_MD_STATE__={version:VERSION,stamp:STAMP,ready:true,md5fix1:true};}catch(e){}}
  function apply(){inject(); refreshDebug();}
  function boot(){bind(); apply(); try{new MutationObserver(function(){clearTimeout(timer);timer=setTimeout(apply,120);}).observe(document.body,{childList:true,subtree:true});}catch(e){} setInterval(apply,1500);} window.LN_EXPORT_MD_V291={ready:true,version:VERSION,stamp:STAMP,fullMarkdown:fullMarkdown,selectedMarkdown:selectedMarkdown,md5:true,md5fix1:true}; if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot); else boot();
})();
