...
}function download(name,text){const b=new Blob([text],{type:'text/csv;charset=utf-8'});const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=name;a.click();URL.revokeObjectURL(a.href)}function exportFiltered(){download('辽宁物理类_V2.9.4.4筛选结果_研究生参考校准.csv',rowsToCsv(filtered))}function exportCandidates(){download('辽宁物理类_V2.9.4.4候选清单_研究生参考校准.csv',rowsToCsv(candidates.map(enrich)))}
...
function exportSummaryPng(kind='filtered'){
  ...
  ctx.fillText(`版本：V2.9.4.4｜生成时间：${new Date().toLocaleString('zh-CN')}`,M+38,y+100);
  ...
  const name=kind==='candidates'?'辽宁物理类_V2.9.4.4_候选清单摘要.png':'辽宁物理类_V2.9.4.4_筛选摘要.png';
...
function renderDebugPanel(){
  ...
  panel.innerHTML=`<b>V2.9.4.4 Debug</b><br/>
  ...
  app.js：<code>app.v2944.js</code>｜app.css：<code>app.v2944.css</code>`;
}
function initV292UX(){
