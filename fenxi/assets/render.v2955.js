...
function riskBadgesV29461(r){
  ...
  const interestBadges=window.LN_CHILD_INTEREST_RUNTIME_V2955?.badgesForRecord?.(r)||[];
...
function renderStrategyCardsV2951(){
  ...
  box.innerHTML=rules.map(rule=>{
    ...
    const interestFit=window.LN_CHILD_INTEREST_RUNTIME_V2955?.scenarioFit?.(rule)||{state:'none',label:''};
    if(interestFit.state==='high') cls.push('interest-match-high-v2955');
    else if(interestFit.state==='near') cls.push('interest-match-near-v2955');
    return `<button class="${cls.join(' ')}" data-strategy="${v2950Text(rule.id)}" data-score-fit="${v2950Text(fit.state)}">
      ...
      ${interestFit.label?`<small class="scenario-interest-v2955 ${v2950Text(interestFit.state)}">${v2950Text(interestFit.label)}</small>`:''}
      <em>${risk}</em>
    </button>`;
  }).join('');
  const intro=document.getElementById('scenarioRuleHintV2951');
  if(intro){
    const child=window.LN_CHILD_INTEREST_RUNTIME_V2955?.summary?.();
