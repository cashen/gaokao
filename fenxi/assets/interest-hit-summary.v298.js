...
(function(){
  ...
  function countFor(interestId, rs){const out={core:0,related:0,review:0,none:0,total:0}; const me=window.LN_CATALOG_MATCH_ENGINE_V298; (rs||rows()).forEach(r=>{const m=me?.matchInterest?.(r,interestId)||{level:'none'}; out[m.level] = (out[m.level]||0)+1; out.total++;}); window.__LN_INTEREST_HIT_CACHE_V298=window.__LN_INTEREST_HIT_CACHE_V298||{}; window.__LN_INTEREST_HIT_CACHE_V298[interestId]=out; return out;}
  function aggregate(rs){const ids=activeInterestIds(); const total={core:0,related:0,review:0,none:0,total:0,byInterest:[]}; window.__LN_INTEREST_HIT_CACHE_V298=window.__LN_INTEREST_HIT_CACHE_V298||{}; ids.forEach(id=>{const c=countFor(id,rs); window.__LN_INTEREST_HIT_CACHE_V298[id]=c; total.core+=c.core; total.related+=c.related; total.review+=c.review; total.none+=c.none; total.total=Math.max(total.total,c.total); const rule=window.LN_CATALOG_INTEREST_BINDING_V298?.get?.(id); total.byInterest.push({interestId:id,title:rule?.title||id,count:c});}); return total;}
