...
/* V2.9.4.5 家长关心点表达增强层：不新增外部保研率数据，只把 2.9.4.4 现有字段翻译成家长可读判断。 */
...
function csvEscape(v){if(v==null)v='';v=String(v);return/[",\n]/.test(v)?'"'+v.replace(/"/g,'""')+'"':v}function rowsToCsv(rows){
  ...
  const body=(rows||[]).map(r=>{
    r=enrich(r);
    return [r.school,r.schoolNature?.label||'',r.schoolTier?.label||'',r.major,r.cleanMajor,r.undergradDisciplineName,r.officialCategoryCode,r.undergradCategoryName,r.officialMajorCode,r.gradAcademicText,r.gradProfessionalText,r.gradSecondaryText,confidenceLabel(r.gradReferenceConfidence||r.taxonomyConfidence),r.admissionReviewTags||'',r.score2025,r.rank2025,r.score2024,r.rank2024,r._level||'',Math.round(r._profile||0),(r.riskFlags||[]).join('|')]
      .map(x=>`"${String(x??'').replace(/"/g,'""')}"`).join(',');
  });
  return [head.join(','),...body].join('\n');
}function download(name,text){const b=new Blob([text],{type:'text/csv;charset=utf-8'});const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=name;a.click();URL.revokeObjectURL(a.href)}function exportFiltered(){download('辽宁物理类_V2.9.4.5筛选结果_研究生参考校准.csv',rowsToCsv(filtered))}function exportCandidates(){download('辽宁物理类_V2.9.4.5候选清单_研究生参考校准.csv',rowsToCsv(candidates.map(enrich)))}
...
function exportSummaryPng(kind='filtered'){
  ...
  ctx.fillText(`版本：V2.9.4.5｜生成时间：${new Date().toLocaleString('zh-CN')}`,M+38,y+100);
  ...
  const name=kind==='candidates'?'辽宁物理类_V2.9.4.5_候选清单摘要.png':'辽宁物理类_V2.9.4.5_筛选摘要.png';
...
function renderDebugPanel(){
  ...
  panel.innerHTML=`<b>V2.9.4.5 Debug</b><br/>
  ...
  app.js：<code>app.v2945.js</code>｜app.css：<code>app.v2945.css</code>`;
}
function initV292UX(){
