// V2.9.5.5 app: entry gate, child interest, state orchestration, event binding and app boot
// V2.9.5.5: keep Pages Functions auth and add child interest rule runtime.
...
async function boot(){
  ...
  if(window.LN_CHILD_INTEREST_RUNTIME_V2955?.render) window.LN_CHILD_INTEREST_RUNTIME_V2955.render();
...
function updateGuideState(){
  ...
  const childSelected = window.LN_CHILD_INTEREST_RUNTIME_V2955?.readState?.().mode === 'selected';
...
function handleActionV2953(action, el){
  switch(action){
    ...
    case 'child-interest-remove': return window.LN_CHILD_INTEREST_RUNTIME_V2955?.handle?.(action, el);
    default: return null;
  }
}
function bindGlobalEventsV2953(){
  document.addEventListener('click', (event)=>{
    ...
    if(childGroupEl){
      ...
      const res=window.LN_CHILD_INTEREST_RUNTIME_V2955?.toggleGroup?.(childGroupEl.dataset.childInterestGroup);
      if(res && res.ok===false && res.reason==='max'){
        const box=document.getElementById('childInterestBoxV2955');
        if(box){ const tip=document.createElement('div'); tip.className='child-interest-toast-v2955'; tip.textContent='建议先选 1—3 个最有兴趣的方向，想换方向可以先删除一个。'; box.prepend(tip); setTimeout(()=>tip.remove(),2600); }
      }
      return;
    }
