...
(function(){
  ...
  function hitLine(sum){if(!sum?.hit)return ''; const h=sum.hit; return `<div class="interest-hit-line-v298">当前能选的专业命中：<b>正主 ${Number(h.core||0)}</b><b>相近 ${Number(h.related||0)}</b><b>需复核 ${Number(h.review||0)}</b></div>`;}
  ...
  function renderDrawerBody(){
    ...
    const body=`<div class="child-interest-drawer-v296 child-interest-drawer-v298"><p class="drawer-help-v296">孩子说的是兴趣，不是专业名。点选会先在抽屉内轻量记录；关闭抽屉后再统一刷新 A/B/C 和详细候选，避免卡顿。</p>${window.LN_CHILD_INTENT_UI_V298?.renderInline?.()||window.LN_CHILD_INTENT_UI_V2976?.renderInline?.()||''}<div class="interest-drawer-live-v298"><b>孩子兴趣</b><span>${esc(sum.title)}</span><p>${esc(sum.text)}</p></div><div class="child-interest-selected-line-v296">${selectedLineHtml(s)}</div><div class="child-interest-search-v296"><input id="childInterestSearchV296" placeholder="搜索专业方向，例如：动物医学、法学、电气、仪器" value="${esc(search)}"/><button class="secondary slim" data-action="child-interest-undecided">清空</button></div><div class="child-interest-grid-v296">${rt().groups().map(g=>groupCard(g,s,q)).join('')||'<div class="notice">没有匹配方向，可以换一个关键词。</div>'}</div><div class="child-interest-cycle-v296"><b>怎么用：</b>兴趣只激活规则；A/B/C 和导出只使用当前能选的专业中的正主、相近或需复核命中项。</div></div>`;
