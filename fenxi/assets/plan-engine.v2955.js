...
function planBadgesV29475Fix2(r,type){
  ...
  const im=window.LN_CHILD_INTEREST_RUNTIME_V2955?.matchRecord?.(r);
...
planScoreV29475 = function(r,type,chosen){
  ...
  if(window.LN_CHILD_INTEREST_RUNTIME_V2955?.planAdjustment){
    sc += window.LN_CHILD_INTEREST_RUNTIME_V2955.planAdjustment(r,type);
  }
  return sc;
};
...
function childInterestLineV2955(r){
  const im=window.LN_CHILD_INTEREST_RUNTIME_V2955?.matchRecord?.(r);
  if(!im?.active || im.level==='no')return '';
  const note=im.level==='review' ? '需复核方向' : (im.level==='core'?'正主方向':'相近方向');
  return `<p class="interest-line-v2955 ${htmlSafeV2945(im.level)}"><b>专业匹配</b>${htmlSafeV2945(im.group?.name||'孩子兴趣')}｜${htmlSafeV2945(im.label)}｜${htmlSafeV2945(note)}</p>`;
}
...
function renderPlanABC(){
  ...
  const interestSummary=window.LN_CHILD_INTEREST_RUNTIME_V2955?.summary?.();
  const interestLine=interestSummary?`<p class="abc-interest-note-v2955">${v2950Text(interestSummary.title)}｜${v2950Text(interestSummary.text)}</p>`:'';
  const toolbar=`<div class="abc-toolbar-v2950"><div><b>A/B/C 方案盘</b><span>A 守底线，B 看专业，C 争上限；冲稳保是每条路径里的安全等级。</span>${interestLine}</div><button class="execute-secondary" onclick="addAllPlansV29475Fix2()">加入全部 A/B/C 候选</button></div>`;
