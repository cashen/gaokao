...
const APP_VERSION_V29472 = 'V2.9.4.7.2｜城市精细化与地域偏好提醒版';
...
}function download(name,text){const b=new Blob([text],{type:'text/csv;charset=utf-8'});const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=name;a.click();URL.revokeObjectURL(a.href)}function exportFiltered(){download('辽宁物理类_V2.9.4.7.2_筛选结果_城市精细化.csv',rowsToCsv(filtered))}function exportCandidates(){download('辽宁物理类_V2.9.4.7.2_候选清单_城市精细化.csv',rowsToCsv(candidates.map(enrich)))}
...
function exportSummaryPng(kind='filtered'){
  ...
  ctx.fillText(`版本：V2.9.4.7.2｜生成时间：${new Date().toLocaleString('zh-CN')}`,M+38,y+100);
  ...
  const name=kind==='candidates'?'辽宁物理类_V2.9.4.7.2_候选清单摘要.png':'辽宁物理类_V2.9.4.7.2_筛选摘要.png';
...
function renderDebugPanel(){
  ...
  panel.innerHTML=`<b>V2.9.4.7.2 Debug</b><br/>
