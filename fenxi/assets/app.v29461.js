...
/* V2.9.4.6.1 卡片折叠与信息密度优化：只改展示层，不改 4.6 数据模型。 */
...
function primarySummaryV29461(r){
  ...
  if(pairs.length){
    ...
    if(current && current.catalog_major_code){
      return `易混提醒：${current.catalog_major_code}｜${current.discipline_category_name||'门类待复核'}｜${current.major_class_name||'专业大类待复核'}。${p.parent_warning||'名字相近，但方向可能不同。'}`;
    }
...
function renderConfusableMajorPanelV2946(r){
  ...
  const summaryLine = current.catalog_major_code
    ? `${current.catalog_major_code}｜${current.discipline_category_name||'门类待复核'}｜${current.major_class_name||'专业大类待复核'}；易混对象：${peer.admission_major_name_raw||'同校/同主题相近专业'}`
    : (p.parent_warning||'该专业容易与同校或同主题专业混淆，建议对比本科代码和专业大类。');
  return `<details class="confusable-v2946 confusable-v29461"${open}>
    ...
    <div class="confusable-body">
      <div class="confusable-warning">${escapeV2946(p.parent_warning||'该专业容易与同校或同主题专业混淆，建议对比本科代码和专业大类。')}</div>
      ...
      <div class="confusable-note">说明：这是“易混提醒”，不是专业好坏排序；具体课程、分流、转专业、校区、学费仍需查学校招生章程和培养方案。</div>
    </div>
  </details>`;
}
...
function renderCards(){
  ...
  el.innerHTML=rows.map(r=>{
    ...
    return`<div class="card card-v29461 level-${r._level}">
      <div class="cardTop">
        <div><div class="school">${r.school} <span class="pill">${r.schoolProvince||'省份待识别'}</span> <span class="pill nature-pill ${r.schoolNature.cls||'unknown'}">${r.schoolNature.label}</span> <span class="pill tier-pill ${r.schoolTier?.cls||'tier-unknown'}">${r.schoolTier?.label||'层级待核验'}</span></div><div class="major">${r.major}</div><div class="official-mini-v29461">${htmlSafeV2945(official)}</div>${renderTaxonomyLine(r)}</div>
...
function csvEscape(v){if(v==null)v='';v=String(v);return/[",\n]/.test(v)?'"'+v.replace(/"/g,'""')+'"':v}function rowsToCsv(rows){
  const head=['学校','学校性质','院校层级','专业','标准专业','学科门类','专业类代码','本科专业类','专业代码','学硕一级/跨门类参考','专硕类别/领域参考','二级学科示例','目录可信度','招生名复核','2025分','2025位次','2024分','2024位次','层级','画像分','风险'];
  const body=(rows||[]).map(r=>{
    r=enrich(r);
    return [r.school,r.schoolNature?.label||'',r.schoolTier?.label||'',r.major,r.cleanMajor,r.undergradDisciplineName,r.officialCategoryCode,r.undergradCategoryName,r.officialMajorCode,r.gradAcademicText,r.gradProfessionalText,r.gradSecondaryText,confidenceLabel(r.gradReferenceConfidence||r.taxonomyConfidence),r.admissionReviewTags||'', confusablePairsForRecordV2946(r).map(p=>p.group_name).join('|'), r.score2025,r.rank2025,r.score2024,r.rank2024,r._level||'',Math.round(r._profile||0),(r.riskFlags||[]).join('|')]
      .map(x=>`"${String(x??'').replace(/"/g,'""')}"`).join(',');
  });
  return [head.join(','),...body].join('\n');
}function download(name,text){const b=new Blob([text],{type:'text/csv;charset=utf-8'});const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=name;a.click();URL.revokeObjectURL(a.href)}function exportFiltered(){download('辽宁物理类_V2.9.4.6筛选结果_研究生参考校准.csv',rowsToCsv(filtered))}function exportCandidates(){download('辽宁物理类_V2.9.4.6候选清单_研究生参考校准.csv',rowsToCsv(candidates.map(enrich)))}
...
function exportSummaryPng(kind='filtered'){
  ...
  ctx.fillText(`版本：V2.9.4.6｜生成时间：${new Date().toLocaleString('zh-CN')}`,M+38,y+100);
  ...
  const name=kind==='candidates'?'辽宁物理类_V2.9.4.6_候选清单摘要.png':'辽宁物理类_V2.9.4.6_筛选摘要.png';
...
function renderDebugPanel(){
  ...
  panel.innerHTML=`<b>V2.9.4.6 Debug</b><br/>
  ...
  易混模型：<code>${CONFUSABLE_MODEL_2946?((CONFUSABLE_MODEL_2946.detectedPairs?.count||0)+' 对 / '+(CONFUSABLE_MODEL_2946.recordIndex?.count||0)+' 条索引'):'未加载'}</code><br/>app.js：<code>app.v29461.js</code>｜app.css：<code>app.v29461.css</code>`;
}
function initV292UX(){
  applyCardViewModeClassV29461();
  ensureCardViewModeToolbarV29461();
