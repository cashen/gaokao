// V2.9.6.fix4 candidate evidence card view model.
...
(function(){
  ...
  function evidenceHtml(r,type,opts){
    ...
    if(compact){
      return `<div class="evidence-mini-v296fix4"><b>${esc(m.line2025)}</b><span>${esc(m.line2024)}</span><em>${esc(m.change)}</em></div>`;
    }
    return `<div class="evidence-card-v296fix4">
      <div class="evidence-tags-v296fix4">${tagHtml}</div>
      <div class="evidence-grid-v296fix4"><div><b>${esc(m.line2025)}</b><span>历史投档证据</span></div><div><b>${esc(m.line2024)}</b><span>上一年对照</span></div></div>
      <p class="evidence-change-v296fix4">${esc(m.change)}</p>
      <p class="evidence-distance-v296fix4">${esc(m.distance)}</p>
    </div>`;
  }
