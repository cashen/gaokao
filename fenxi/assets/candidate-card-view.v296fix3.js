// V2.9.6.fix3 candidate evidence card view model.
...
(function(){
  ...
  function changeText(r){
    ...
    let diff=d;
    if(diff===null && r25!==null && r24!==null) diff=r25-r24;
  ...
  function vm(r,type,idx){
    ...
    return {
      ...
      risk:(typeof planRiskTextV29475==='function' ? planRiskTextV29475(r,type) : '') || '招生章程、学费、校区和专业归属建议复核'
    };
  }
  function evidenceHtml(r,type,opts){
    ...
    if(compact){
      return `<div class="evidence-mini-v296fix3"><b>${esc(m.line2025)}</b><span>${esc(m.line2024)}</span><em>${esc(m.change)}</em></div>`;
    }
    return `<div class="evidence-card-v296fix3">
      <div class="evidence-tags-v296fix3">${tagHtml}</div>
      <div class="evidence-grid-v296fix3"><div><b>${esc(m.line2025)}</b><span>历史投档证据</span></div><div><b>${esc(m.line2024)}</b><span>上一年对照</span></div></div>
      <p class="evidence-change-v296fix3">${esc(m.change)}</p>
      <p class="evidence-distance-v296fix3">${esc(m.distance)}</p>
    </div>`;
  }
