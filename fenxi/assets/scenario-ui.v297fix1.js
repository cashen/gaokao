...
(function(){
  ...
  function render(box){
    ...
    const pref=rule.preference?.priority||''; const risk=rule.riskLevel==='aggressive'?'冲刺':rule.riskLevel==='conservative'?'稳妥':'均衡';
    ...
    box.innerHTML=`<div class="scenario-compact-v296">
      ...
      <div class="scenario-tags-v296"><span>${esc(risk)}</span>${pref?`<span>${esc(pref)}</span>`:''}${interest.label?`<span class="interest">${esc(interest.label)}</span>`:''}</div>
    ...
    if(intro){ const child=window.LN_CHILD_INTEREST_RUNTIME_V296?.summary?.()||window.LN_CHILD_INTEREST_RUNTIME_V2955?.summary?.(); intro.innerHTML=`<b>场景规则中心</b><span>主页面只显示当前场景摘要；完整场景进入抽屉选择。${child?'｜'+esc(child.title):''}</span>`; }
  }
  function card(rule){
  ...
  function openDrawer(){
    ...
    window.LN_DRAWER_V296?.open?.('选择 / 确认场景', `<div class="scenario-drawer-v296"><p class="drawer-help-v296">场景只提供建议和排序倾向；用户已经手动设置的家庭底线优先。</p><div class="scenario-grid-v296">${list.map(card).join('')}</div></div>`);
  }
