...
const plan=fs.readFileSync(path.join(root,'assets/plan-engine.v2954fix2.js'),'utf8');
...
const rules=fs.readFileSync(path.join(root,'assets/rules.v2954fix2.js'),'utf8');
if(!rules.includes("preference:{priority:'medical'")) throw new Error('medical priority not fixed');
const app=fs.readFileSync(path.join(root,'assets/app.v2954fix2.js'),'utf8');
if(!app.includes('highFeeRejectTouchedV2954Fix2')) throw new Error('high fee reject protection missing');
console.log('V2.9.5.4.fix2 validation passed');
