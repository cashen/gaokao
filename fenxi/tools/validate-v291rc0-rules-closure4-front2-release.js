...
ok('version title',index.includes('V2.91RC0.rules-closure4.front2'));
ok('version stamp',index.includes('291rc0front2-20260514'));
ok('frontend css exists',exists('assets/frontend-trust.v291rc0front2.css'));
ok('frontend js exists',exists('assets/frontend-trust.v291rc0front2.js'));
ok('index loads frontend css',index.includes('assets/frontend-trust.v291rc0front2.css'));
ok('index loads frontend js',index.includes('assets/frontend-trust.v291rc0front2.js'));
...
ok('no front1 css loaded',!index.includes('assets/frontend-trust.v291rc0front1.css'));
ok('no front1 js loaded',!index.includes('assets/frontend-trust.v291rc0front1.js'));
for(const [p,h] of Object.entries(requiredHashes)){ ok('hash unchanged '+p,sha(p)===h); }
const css=read('assets/frontend-trust.v291rc0front2.css');
ok('css warm low key palette',css.includes('--ft2-bg')&&css.includes('--ft2-warm')&&css.includes('--ft2-bluegray'));
ok('css removes black buttons intent',css.includes('去黑')&&css.includes('box-shadow:none'));
ok('css top unified',css.includes('顶部')&&css.includes('暖白'));
ok('css risk separate',css.includes('--ft2-risk')&&css.includes('风险'));
ok('css no deep black variables',!css.includes('--ft-dark'));
const js=read('assets/frontend-trust.v291rc0front2.js');
ok('js debug flags',js.includes('frontendTrust')&&js.includes('doesModifyFormula:false'));
ok('js black removed flag',js.includes('blackButtonsRemoved'));
ok('js humanizes risk labels',js.includes('位次风险高')&&js.includes('分数利用偏低')&&js.includes('需重点复核'));
ok('docs update exists',exists('docs/V2.91RC0.rules-closure4.front2_更新说明.md'));
ok('docs checklist exists',exists('docs/V2.91RC0.rules-closure4.front2_验证清单.md'));
