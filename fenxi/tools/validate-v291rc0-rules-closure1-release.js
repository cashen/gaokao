...
ok('版本号正确',index.includes('V2.91RC0.rules-closure1'));
ok('版本戳正确',index.includes('291rc0closure1-20260514'));
ok('未回到 V3',!index.includes('V3.0.0'));
ok('parent-trust2 仍保留',index.includes('parent-trust.v291rc0parenttrust2.js'));
ok('rules-closure JS 已加载',index.includes('assets/rules-closure.v291rc0closure1.js'));
ok('rules-closure CSS 已加载',index.includes('assets/rules-closure.v291rc0closure1.css'));
ok('rulesClosure 线路记录存在',index.includes("rulesClosure:'291rc0closure1-20260514'"));
const js='assets/rules-closure.v291rc0closure1.js';
const css='assets/rules-closure.v291rc0closure1.css';
ok('rules-closure JS 存在',exists(js));
ok('rules-closure CSS 存在',exists(css));
const code=read(js);
['studentGenderEffective','fieldRejectEffective','scenarioPriorityConflict','abcPathDistribution','siteRisk','pathInfo','evaluate','scoreAdjustment','diversifyBuckets'].forEach(s=>ok('closure 包含 '+s,code.includes(s)));
...
ok('priority 冲突处理存在',code.includes("scenario==='exam'")&&code.includes("priority==='grid'"));
...
ok('selftest 包含 closure 加载检查',self.includes('rules-closure1 家庭条件闭环加载检查'));
ok('selftest 包含女生考公拒绝现场样例',self.includes('女生考公拒绝现场压住电气样例'));
const expected={
...
['docs/V2.91RC0.rules-closure1_更新说明.md','docs/V2.91RC0.rules-closure1_验证清单.md'].forEach(f=>ok('文档存在 '+f,exists(f)));
