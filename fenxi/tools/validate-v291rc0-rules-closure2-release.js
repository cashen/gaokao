...
ok('版本号正确',index.includes('V2.91RC0.rules-closure2'));
ok('版本戳正确',index.includes('291rc0closure2-20260514'));
ok('未回到 V3',!index.includes('V3.0.0'));
ok('parent-trust2 仍保留',index.includes('parent-trust.v291rc0parenttrust2.js'));
ok('rules-closure2 JS 已加载',index.includes('assets/rules-closure.v291rc0closure2.js'));
ok('rules-closure2 CSS 已加载',index.includes('assets/rules-closure.v291rc0closure2.css'));
ok('家庭回归清单已加载',index.includes('assets/family-scenario-regression.v291rc0closure2.js'));
ok('rulesClosure 线路记录存在',index.includes("rulesClosure:'291rc0closure2-20260514'"));
const js='assets/rules-closure.v291rc0closure2.js';
const css='assets/rules-closure.v291rc0closure2.css';
const reg='assets/family-scenario-regression.v291rc0closure2.js';
ok('rules-closure2 JS 存在',exists(js));
ok('rules-closure2 CSS 存在',exists(css));
...
].forEach(s=>ok('closure2 包含 '+s,code.includes(s)));
...
ok('回归清单不少于关键用例',regression.includes('female_540_employment_site_reject')&&regression.includes('female_540_grid_site_reject')&&regression.includes('female_600_medical_reject_long')&&regression.includes('basic_science_not_teacher'));
const self=read('assets/debug-selftest.v2983fix12.js');
ok('selftest 包含 closure2 加载检查',self.includes('rules-closure2 家庭决策闭环加载检查'));
ok('selftest 包含女生考公拒绝现场样例',self.includes('rules-closure2 女生考公拒绝现场压住电气样例'));
ok('selftest 包含回归清单检查',self.includes('rules-closure2 家庭场景回归清单加载检查'));
const expected={
...
['docs/V2.91RC0.rules-closure2_更新说明.md','docs/V2.91RC0.rules-closure2_验证清单.md'].forEach(f=>ok('文档存在 '+f,exists(f)));
