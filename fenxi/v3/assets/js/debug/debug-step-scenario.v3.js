(function () {
  ...
  function run() {
    ...
    results.push(check('家庭路径数量不少于 8', (adapter.scenarios || []).length >= 8, 'count=' + ((adapter.scenarios || []).length)));
    ...
    matrix.forEach(function (item) {
      results.push(check('家庭路径矩阵：' + item.name, item.preview && item.preview.recommended === item.expected, JSON.stringify({ expected: item.expected, recommended: item.preview && item.preview.recommended, scores: item.preview && item.preview.scores })));
    });
    var current = store.getState();
    var p = adapter.recommend(current);
    results.push(check('当前状态可生成家庭路径推荐', !!(p && p.recommended), JSON.stringify({ recommended: p && p.recommended, familyRows: p && p.familyFilteredRows, matchedRows: p && p.matchedRows })));
    ...
    results.push(check('家庭路径 preview 写入 store', !!(afterApply.scenario && afterApply.scenario.preview && afterApply.scenario.preview.reason), JSON.stringify(afterApply.scenario && afterApply.scenario.preview || {})));
