...
async function checkHealth(){
  try{const response=await fetch('/api/ai/health',{headers:{accept:'application/json'},cache:'no-store'});const data=await response.json();if(!response.ok||!data?.ok)throw new Error('health failed');const ready=data.provider?.workersModelConfigured||data.provider?.externalConfigured;els.health.className=`health-bar ${ready?'is-ok':'is-warn'}`;els.health.textContent=ready?`工作台已就绪：确定性数据执行 + ${data.provider.primary} 语义解析；模型失败自动退回本地规则。`:'工作台已就绪：未配置专用AI模型，使用确定性语义规则；位次和候选查询不受影响。';const model=data.provider?.primaryModel||'未配置';els.modelConfig.textContent=`${data.provider?.primary||'deterministic'} · ${model}`;}
...
async function probeModel(){
  ...
  try{const response=await fetch('/api/ai/model-probe',{method:'POST',headers:{accept:'application/json'}});const data=await response.json();const actual=data.actual||{};if(!response.ok||!data.ok)throw new Error(data.note||'模型探针失败');els.modelProbeResult.textContent=`本次实测：${actual.provider} · ${actual.model} · ${actual.latencyMs}ms${actual.fallbackUsed?' · 使用备用模型':''}`;}
